# AI Explanation Prompt Template v6
...
本模板是 v5 生产评测后的窄范围修订版，只负责面向测评参与者解释量表类、区间决策类标准报告。它针对 Run 635977982451659310 的父子维度混合问题补充逐条层级检查，并保留原始等级措辞，不扩大 selector。它不是通用聊天 Prompt，也不适用于 clinician、typology、behavioral rating、cognitive 或其他 decision kind。扩大 selector 必须发布新的 Prompt/Profile 版本并建立独立评测证据。
...
任何失败都不得尝试从不合法文本中“尽量提取”结果。允许的自动重试必须创建新 attempt，使用相同 Prompt/Profile/Input 版本，并重新执行完整校验；修复性 Prompt 属于未来独立版本，不得改写已冻结的任何版本证据。
