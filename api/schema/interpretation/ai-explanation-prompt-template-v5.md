# AI Explanation Prompt Template v5
...
本模板是 v4 生产评测后的窄范围修订版，只负责面向测评参与者解释量表类、区间决策类标准报告。它仅针对 Run 635960720508334638 的提示注入用例中综合洞察被清空的问题补充可信事实使用规则，不扩大 selector。它不是通用聊天 Prompt，也不适用于 clinician、typology、behavioral rating、cognitive 或其他 decision kind。扩大 selector 必须发布新的 Prompt/Profile 版本并建立独立评测证据。
