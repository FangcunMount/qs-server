# AI Explanation Prompt Template v4
...
本模板是 v3 生产评测后的窄范围修订版，只负责面向测评参与者解释量表类、区间决策类标准报告。它针对 Run 635938145489138222 暴露的必填来源字段缺失、稳定结果模式误选、信息不足时整合深度不足和关注点未落实问题修订执行约束，不扩大 selector。它不是通用聊天 Prompt，也不适用于 clinician、typology、behavioral rating、cognitive 或其他 decision kind。扩大 selector 必须发布新的 Prompt/Profile 版本并建立独立评测证据。
