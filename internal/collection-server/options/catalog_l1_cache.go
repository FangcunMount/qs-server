...
// PublishedModelCacheOptions 已发布测评模型目录 BFF 进程内 L1 缓存。
type PublishedModelCacheOptions struct {
	CatalogL1CacheOptions `mapstructure:",squash"`
}
...
// NewPublishedModelCacheOptions 创建默认关闭的已发布测评模型 L1 配置。
func NewPublishedModelCacheOptions() *PublishedModelCacheOptions {
...
func validateCatalogL1CacheOptions(opts *CatalogL1CacheOptions, name string) []error {
	...
	if opts.TTLSeconds <= 0 {
		errs = append(errs, fmt.Errorf("%s.ttl_seconds must be greater than 0", name))
	}
	if opts.MaxEntries <= 0 {
		errs = append(errs, fmt.Errorf("%s.max_entries must be greater than 0", name))
	}
...
func validatePublishedModelCacheOptions(opts *PublishedModelCacheOptions) []error {
	if opts == nil {
		return nil
	}
