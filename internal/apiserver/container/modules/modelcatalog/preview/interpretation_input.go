...
func previewInterpretationInput(req modelpreview.Request, outcome *domainoutcome.Execution) (interpinput.InterpretationInput, error) {
	...
	model := report.ModelIdentity{
		Kind: string(outcome.ModelRef.Kind()), Algorithm: string(outcome.ModelRef.Algorithm()),
	...
	in := interpinput.InterpretationInput{
		Association: report.Association{OrgID: 1, AssessmentID: meta.ID(1), TesteeID: 1},
		Model:       model,
	...
	if detail, ok := codec.TraitProfileDetailFromPayload(outcome.Detail.Payload); ok {
		...
		in.TraitProfile = &interpinput.TraitProfileFacts{Detail: reporttypology.TraitProfileReportDetail{
			Traits: traits,
...
func personalityDetail(d codec.PersonalityTypeDetail) reporttypology.PersonalityTypeReportDetail {
	...
	for _, v := range d.Dimensions {
		dims = append(dims, reporttypology.PersonalityTypeDimensionReport{
			Code: v.Code, Name: v.Name, LeftPole: v.LeftPole, RightPole: v.RightPole,
			RawScore: v.RawScore, Preference: v.Preference, Strength: v.Strength, Model: v.Model, Level: v.Level,
		})
	}
	summary := d.Summary
	if summary == "" {
	...
	return reporttypology.PersonalityTypeReportDetail{
		TypeCode: d.TypeCode, TypeName: d.TypeName, OneLiner: d.OneLiner, MatchPercent: d.MatchPercent,
		ImageURL: d.ImageURL, IsSpecial: d.IsSpecial, SpecialTrigger: d.SpecialTrigger, Commentary: d.Commentary,
		Profile: reporttypology.PersonalityTypeProfileReport{
			Summary: summary, Strengths: append([]string(nil), d.Strengths...),
			Weaknesses: append([]string(nil), d.Weaknesses...), Suggestions: append([]string(nil), d.Suggestions...),
		},
		Rarity:              reporttypology.PersonalityTypeRarityReport{Percent: d.Rarity.Percent, Label: d.Rarity.Label, OneInX: d.Rarity.OneInX},
		Dimensions:          dims,
		SourceAttribution:   d.Source.Attribution,
		SourceLicense:       d.Source.License,
		SourceNonCommercial: d.Source.NonCommercial,
	}
}
