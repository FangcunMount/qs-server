...
import (
	...
	governancekeyspace "github.com/FangcunMount/qs-server/internal/pkg/cachegovernance/keyspace"
...
type FamilyObserver = cacheobserve.FamilyObserver
...
type RedisStore struct {
	...
	keys     governancekeyspace.Keyspace
	opts     Options
...
func NewRedisStore(client redis.UniversalClient, builder *keyspace.Builder, opts Options) cachetarget.HotsetRecorder {
	return NewRedisStoreWithObserver(client, builder, opts, nil)
}

func NewRedisStoreWithObserver(client redis.UniversalClient, builder *keyspace.Builder, opts Options, observer FamilyObserver) cachetarget.HotsetRecorder {
	...
	return &RedisStore{
		client:   client,
		keys:     governancekeyspace.FromBuilder(builder),
		opts:     opts,
		observer: observer,
...
func (s *RedisStore) Record(ctx context.Context, target cachetarget.WarmupTarget) error {
	...
	key := s.keys.WarmupHotset(string(target.Family), string(target.Kind))
	if err := s.client.ZIncrBy(ctx, key, 1, target.Scope).Err(); err != nil {
		...
		s.observeFamilyFailure(string(redisruntime.FamilyMeta), err)
		return err
	}
	hotsetRecordTotal.WithLabelValues(string(target.Family), string(target.Kind), "ok").Inc()
	s.observeFamilySuccess(string(redisruntime.FamilyMeta))
...
func (s *RedisStore) Top(ctx context.Context, family cachemodel.Family, kind cachetarget.WarmupKind, limit int64) ([]cachetarget.WarmupTarget, error) {
...
func (s *RedisStore) TopWithScores(ctx context.Context, family cachemodel.Family, kind cachetarget.WarmupKind, limit int64) ([]cachetarget.HotsetItem, error) {
	...
	key := s.keys.WarmupHotset(string(family), string(kind))
...
func (s *RedisStore) observeSize(ctx context.Context, key string, family cachemodel.Family, kind cachetarget.WarmupKind) {
	if s == nil || s.client == nil {
		return
	}
	...
	s.observeFamilySuccess(string(redisruntime.FamilyMeta))
	cacheobserve.SetHotsetSize(string(family), string(kind), card)
}
...
func (s *RedisStore) observeAndTrim(ctx context.Context, key string, family cachemodel.Family, kind cachetarget.WarmupKind) {
	if s == nil || s.client == nil {
		return
	}
	card, err := s.client.ZCard(ctx, key).Result()
