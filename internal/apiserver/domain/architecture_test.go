...
func TestTopLevelDomainChainPackagesDoNotDependOnOuterLayers(t *testing.T) {
	...
	root := repoRoot(t)
	scanRoots := []string{
		filepath.Join(root, "internal", "apiserver", "domain", "modelcatalog"),
...
func TestSurveyScaleDomainDoesNotDependOnOuterLayers(t *testing.T) {
	...
	scanRoots := []string{
		...
		filepath.Join(root, "internal", "apiserver", "domain", "modelcatalog", "scale", "definition"),
...
func TestScaleDomainDoesNotExposePersistencePayloadMappers(t *testing.T) {
	...
	root := repoRoot(t)
	dir := filepath.Join(root, "internal", "apiserver", "domain", "modelcatalog", "scale", "definition")
...
func TestScaleDefinitionLivesUnderAssessmentModelScale(t *testing.T) {
	t.Parallel()

	root := repoRoot(t)
	oldPath := filepath.Join(root, "internal", "apiserver", "domain", "authoring", "scale")
	if _, err := os.Stat(oldPath); err == nil {
		t.Fatalf("%s exists; scale definition belongs under domain/modelcatalog/scale/definition", filepath.ToSlash(mustRel(t, root, oldPath)))
...
func TestSurveyScaleDomainRepositoriesStayCommandSide(t *testing.T) {
	...
	for _, rel := range []string{
		...
		"internal/apiserver/domain/modelcatalog/scale/definition/repository.go",
	} {
...
func TestEvaluationDomainRepositoriesStayCommandSide(t *testing.T) {
	...
	for _, target := range []struct {
		...
		{
			rel:        "internal/apiserver/domain/interpretation/repository.go",
...
func TestEvaluationDomainDoesNotDependOnSurveyScaleOrOuterLayers(t *testing.T) {
	...
	forbiddenImports := map[string]string{
		...
		"github.com/FangcunMount/qs-server/internal/apiserver/transport/":                           "transport",
		"github.com/FangcunMount/qs-server/internal/apiserver/port/":                                "port",
		"github.com/FangcunMount/qs-server/internal/apiserver/domain/modelcatalog/scale/definition": "scale definition domain",
...
func TestEvaluationDomainDoesNotBuildReports(t *testing.T) {
	...
	forbiddenTokens := []string{
		...
		"reporttypology.MBTIReportInput",
		"reporttypology.SBTIReportInput",
		"reportscore.ScaleReportInput",
		"reporttypology.MBTIReportDetail",
		"reporttypology.SBTIReportDetail",
		"NewInterpretReport(",
		"GenerateReportInput",
		"type ReportBuilder ",
	}
	err := filepath.WalkDir(dir, func(path string, entry os.DirEntry, err error) error {
		...
		if strings.Contains(filepath.Base(path), "report_builder") {
			t.Fatalf("%s is a report builder file; report construction belongs in domain/interpretation or application orchestration", filepath.ToSlash(mustRel(t, root, path)))
		}
		data, err := os.ReadFile(path)
		if err != nil {
			return err
...
func TestEvaluationDomainDoesNotImportReportPackage(t *testing.T) {
	...
	root := repoRoot(t)
	const reportImport = "github.com/FangcunMount/qs-server/internal/apiserver/domain/interpretation"
...
func TestAssessmentModelReplacesLegacyRulesetPackages(t *testing.T) {
	t.Parallel()

	root := repoRoot(t)
	for _, rel := range []string{
		"internal/apiserver/domain/ruleset",
		"internal/apiserver/port/ruleset",
	} {
...
func TestReportBuilderEntryPointsLiveInReportDomainOnly(t *testing.T) {
	...
	root := repoRoot(t)
	reportDomain := filepath.Join(root, "internal", "apiserver", "domain", "interpretation")
	entryPoints := []string{
		"BuildReport",
		"ResolveReportType",
	}
	...
	err := filepath.WalkDir(scanRoot, func(path string, entry os.DirEntry, err error) error {
		...
		for _, name := range entryPoints {
			...
			if relErr != nil || strings.HasPrefix(rel, "..") {
				t.Fatalf("%s defines %s(); report builder entry points must live in domain/interpretation", filepath.ToSlash(mustRel(t, root, path)), name)
			}
		}
		return nil
	})
...
func TestReportDomainDoesNotImportRulesetPayloads(t *testing.T) {
	...
	allowedModelCatalogImportFiles := map[string]struct{}{
		...
		"internal/apiserver/domain/interpretation/typology/doc.go":    {},
		"internal/apiserver/domain/interpretation/scoring/doc.go":     {},
	}
...
func TestScaleExecutionDomainDoesNotReintroduceLegacyEngine(t *testing.T) {
	t.Parallel()

	root := repoRoot(t)
	dir := filepath.Join(root, "internal", "apiserver", "domain", "evaluation", "scoring")
	forbiddenTokens := []string{
		"RegisterStrategy",
		"GetStrategy(",
	...
	err := filepath.WalkDir(dir, func(path string, entry os.DirEntry, err error) error {
		...
		for _, token := range forbiddenTokens {
			if strings.Contains(text, token) {
				t.Fatalf("%s contains %q; scale execution domain must not reintroduce legacy interpretation engine registries", filepath.ToSlash(mustRel(t, root, path)), token)
			}
		}
		return nil
	})
