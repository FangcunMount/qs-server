...
func CollectConfiguredSuggestions(overall string, factorScores []report.FactorScoreInput) []Suggestion {
	if len(factorScores) == 0 {
		return nil
	}
	var suggestions []Suggestion
	if overall != "" {
		suggestions = append(suggestions, Suggestion{Category: SuggestionCategoryGeneral, Content: overall})
	}
	for _, score := range factorScores {
		...
		if score.IsTotalScore {
			if score.Suggestion != overall {
				suggestions = append(suggestions, Suggestion{Category: SuggestionCategoryGeneral, Content: score.Suggestion})
			}
			continue
		}
