...
func (*Evaluator) interpret(model ScaleInterpretationModel, factorScores []ScaleFactorScore, totalScore float64, riskLevel RiskLevel) ([]ScaleFactorScore, string, string) {
...
func interpretFactor(model ScaleInterpretationModel, fs ScaleFactorScore) (string, string) {
	if factor, found := findFactor(model, fs.FactorCode); found {
		if rule := findInterpretRuleWithRangeFallback(factor, fs.RawScore); rule != nil && rule.Conclusion != "" {
...
func interpretOverall(model ScaleInterpretationModel, factorScores []ScaleFactorScore, totalScore float64, riskLevel RiskLevel) (string, string) {
	for _, fs := range factorScores {
		if !fs.IsTotalScore {
			continue
		}
		if factor, found := findFactor(model, fs.FactorCode); found {
			if rule := findInterpretRule(factor, fs.RawScore); rule != nil && rule.Conclusion != "" {
				return rule.Conclusion, rule.Suggestion
			}
		}
	}
...
func defaultOverallInterpretation(_ float64, riskLevel RiskLevel) (string, string) {
	switch riskLevel {
	case RiskLevelSevere:
