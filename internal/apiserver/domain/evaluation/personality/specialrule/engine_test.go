...
func TestEngineApplyBeforeScoreMatchesDrinkTrigger(t *testing.T) {
	...
	got, ok := (Engine{}).ApplyBeforeScore(spec.SpecialRules, payload, []evaluationinput.Answer{
...
func TestEngineApplyAfterDecisionUsesFallbackOutcome(t *testing.T) {
	...
	got, ok := (Engine{}).ApplyAfterDecision(spec.SpecialRules, spec.Decision, payload, 0.5)
	if !ok {
