...
func TestExecutePackageDoesNotReferenceConcreteTypologyScorers(t *testing.T) {
	...
	forbidden := []string{
		...
		"mbtiAlgorithmRunner",
		"sbtiAlgorithmRunner",
		"domain/evaluation/typology/patterns",
	}
...
func TestTypologyExecutorStaysAlgorithmAgnostic(t *testing.T) {
	...
	root := repoRoot(t)
	path := filepath.Join(root, "internal", "apiserver", "application", "evaluation", "registry", "mechanisms", "typology", "executor.go")
...
func TestConfiguredEvaluatorDoesNotSpecialCaseLegacyAdapters(t *testing.T) {
	...
	root := repoRoot(t)
	path := filepath.Join(root, "internal", "apiserver", "domain", "evaluation", "typology", "configured", "evaluator.go")
	data, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
...
func TestOutcomeAssemblerUsesAdapterRegistry(t *testing.T) {
	...
	root := repoRoot(t)
	path := filepath.Join(root, "internal", "apiserver", "application", "evaluation", "registry", "mechanisms", "typology", "outcome_mapper.go")
	data, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
...
func TestAlgorithmRunnerStaysModuleRegistryDriven(t *testing.T) {
	...
	root := repoRoot(t)
	path := filepath.Join(root, "internal", "apiserver", "application", "evaluation", "registry", "mechanisms", "typology", "algorithm_runner.go")
	data, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
...
func TestReportRegistryDispatchesThroughTypologyReportBuilder(t *testing.T) {
	...
	root := repoRoot(t)
	path := filepath.Join(root, "internal", "apiserver", "application", "evaluation", "registry", "mechanisms", "typology", "report_registry.go")
	data, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
...
func TestReportBuilderStaysAlgorithmAgnostic(t *testing.T) {
	...
	root := repoRoot(t)
	path := filepath.Join(root, "internal", "apiserver", "application", "evaluation", "registry", "mechanisms", "typology", "report_builder.go")
	data, err := os.ReadFile(path)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTypologyApplicationMainPathDoesNotReferenceLegacyModules(t *testing.T) {
	...
	mainPathFiles := []string{
		"internal/apiserver/application/evaluation/registry/mechanisms/typology/executor.go",
		"internal/apiserver/application/evaluation/registry/mechanisms/typology/module_registry.go",
		"internal/apiserver/application/evaluation/registry/mechanisms/typology/runtime_registry.go",
		"internal/apiserver/application/evaluation/registry/mechanisms/typology/algorithm_runner.go",
		"internal/apiserver/application/evaluation/registry/mechanisms/typology/outcome_mapper.go",
		"internal/apiserver/application/evaluation/registry/mechanisms/typology/report_registry.go",
...
func TestTypologyApplicationLayerDoesNotImportLegacyAdapters(t *testing.T) {
	...
	forbiddenImports := []string{
		"typology/adapter/mbti",
		"typology/adapter/sbti",
		"typology/adapter/bigfive",
	}
...
func TestTypologyLegacyDerivationStaysInLegacyPackages(t *testing.T) {
	...
	mainRoots := []string{
		filepath.Join(root, "internal", "apiserver", "application", "evaluation", "registry", "mechanisms", "typology"),
