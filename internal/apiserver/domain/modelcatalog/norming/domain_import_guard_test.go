...
func TestNormingDoesNotDependOnOuterLayers(t *testing.T) {
	t.Parallel()
	...
	for _, file := range matches {
		...
		for _, imp := range parsed.Imports {
			...
			if path == "github.com/FangcunMount/qs-server/internal/apiserver/domain/modelcatalog" {
				t.Fatalf("domain/modelcatalog/norming must not import modelcatalog root facade in %s", file)
			}
			if strings.Contains(path, "/internal/apiserver/application/") ||
				...
				t.Fatalf("domain/modelcatalog/norming must not import %s in %s", path, file)
			}
		}
	}
}
