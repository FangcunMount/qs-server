...
func TestNewAssessmentModelSetsDefaultProductChannel(t *testing.T) {
	...
	if model.ProductChannel != domain.ProductChannelCognitive {
...
func TestNewAssessmentModelRejectsInvalidProductChannel(t *testing.T) {
	_, err := domain.NewAssessmentModel(domain.NewAssessmentModelInput{
		Code:           "bad",
		Kind:           domain.KindCognitive,
		Algorithm:      domain.AlgorithmSPM,
		ProductChannel: domain.ProductChannel("invalid"),
		Title:          "bad",
	})
	if err == nil {
