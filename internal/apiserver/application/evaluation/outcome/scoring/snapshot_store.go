...
// SnapshotStore 持久化规范计分结果 between 异步 phases。
...
// MemorySnapshotStore 是 in-进程快照 store 用于 tests。
...
func NewMemorySnapshotStore() *MemorySnapshotStore {
	return &MemorySnapshotStore{items: make(map[uint64][]byte)}
}

func (s *MemorySnapshotStore) Save(_ context.Context, assessmentID uint64, outcome *assessment.AssessmentOutcome) error {
	if assessmentID == 0 {
...
func (s *MemorySnapshotStore) Load(_ context.Context, assessmentID uint64) (*assessment.AssessmentOutcome, error) {
	if assessmentID == 0 {
...
func (s *MemorySnapshotStore) Delete(_ context.Context, assessmentID uint64) error {
	if assessmentID == 0 {
		return fmt.Errorf("assessment id is required")
	}
	s.mu.Lock()
