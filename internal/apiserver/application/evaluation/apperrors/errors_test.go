...
func TestApplicationErrorMapperPreservesAPICodes(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "medical scale not found", err: MedicalScaleNotFound(errors.New("missing"), "scale missing"), code: errorCode.ErrMedicalScaleNotFound},
		{name: "answer sheet not found", err: AnswerSheetNotFound(errors.New("missing"), "answer sheet missing"), code: errorCode.ErrAnswerSheetNotFound},
