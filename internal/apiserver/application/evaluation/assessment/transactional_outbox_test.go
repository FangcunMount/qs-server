...
func TestSaveAssessmentAndStageEventsFiltersDisabledFootprintEvents(t *testing.T) {
...
func TestSaveAssessmentAndStageEventsKeepsAllowedFootprintEvents(t *testing.T) {
	...
	if stager.events[0].EventType() != eventcatalog.FootprintAssessmentCreated {
		t.Fatalf("staged event type = %s, want %s", stager.events[0].EventType(), eventcatalog.FootprintAssessmentCreated)
	}
}
