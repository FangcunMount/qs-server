...
func (s *queryService) ListPublished(ctx context.Context, dto shared.ListScalesDTO) (*shared.ScaleSummaryListResult, error) {
	...
	filter.Status = scalereadmodel.ScaleStatusPublished
...
func (s *queryService) listScaleSummaryRows(ctx context.Context, filter scalereadmodel.ScaleFilter, page, pageSize int) (*shared.ScaleSummaryListResult, error) {
	...
	return shared.ToSummaryRowsResult(ctx, items, total, s.identitySvc), nil
}
