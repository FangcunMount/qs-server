...
func (s Resolver) ResolveLatestByCode(ctx context.Context, actor ActorContext, kind domain.Kind, code string) (*modelcatalogport.PublishedModel, error) {
	if kind == "" || code == "" {
	...
	if err := s.Authorizer.Authorize(ctx, actor, ActionResolvePublished, Resource{Code: code, Kind: kind}); err != nil {
		return nil, err
	}
...
func (s Resolver) ListPublished(ctx context.Context, actor ActorContext, filter modelcatalogport.ListPublishedFilter) ([]*modelcatalogport.PublishedModel, int64, error) {
	if s.Lister == nil || s.Authorizer == nil {
		return nil, 0, errors.WithCode(codepkg.ErrInternalServerError, "published model resolver is not configured")
	}
	if err := s.Authorizer.Authorize(ctx, actor, ActionResolvePublished, Resource{Kind: filter.Kind}); err != nil {
		return nil, 0, err
	}
...
type TrustedRuntimeResolver struct {
	Resolver PublishedModelResolver // 已发布模型解析器
	Actor    ActorContext           // 授权上下文
}
...
// Authorize 授权
func (trustedRuntimeAuthorizer) Authorize(_ context.Context, actor ActorContext, action Action, _ Resource) error {
