...
type Coordinator struct {
	...
	dailyEngine  *domainv2.ProjectionEngine
	globalEngine *domainv2.ProjectionEngine
	store        RunStore
...
func (c *Coordinator) Run(ctx context.Context, request RunRequest) (resultRun *Run, resultErr error) {
	...
	now := c.now()
	latestCompleteDay := domainv2.BusinessDate(now).AddDate(0, 0, -1)
	if request.FromDate.IsZero() || request.ToDate.IsZero() {
		return nil, invalidRunRequest("from_date and to_date are required")
	}
	fromDate := domainv2.BusinessDate(request.FromDate)
	toDate := domainv2.BusinessDate(request.ToDate)
	...
	window := domainv2.InstantRange{From: fromDate, To: toDate.AddDate(0, 0, 1)}
	...
	run, err := c.store.Create(ctx, Run{
		...
		Status:      domainv2.RunStatusRunning,
		Stage:       "created",
	...
	lockKey := fmt.Sprintf("statistics:v2:%d", request.OrgID)
	...
	if runErr == nil && !result.Acquired {
		runErr = executionError("lock", "lock_busy", fmt.Errorf("statistics v2 lock busy"))
	}
	if runErr != nil {
...
func normalizeRunMode(mode domainv2.RunMode, validateOnly bool) (domainv2.RunMode, error) {
	if mode == "" {
		if validateOnly {
			return domainv2.RunModeValidate, nil
		}
		// Compatibility for the scheduler and the original internal endpoint.
		return domainv2.RunModePublish, nil
	}
	if validateOnly && mode != domainv2.RunModeValidate {
...
func (c *Coordinator) execute(ctx context.Context, run *Run) error {
	...
	for _, collector := range c.collectors.Ordered() {
		...
		item, err := collector.Collect(ctx, domainv2.CollectRequest{
	...
	snapshotAt := c.now()
	cutoffAt := domainv2.BusinessDate(snapshotAt)
	err := c.tx.WithinTransaction(ctx, func(txCtx context.Context) error {
		request := domainv2.ProjectionRequest{
		...
		if run.Mode == domainv2.RunModeRepair {
...
func mergeCollectorCounts(sources, facts map[string]int64, item domainv2.CollectResult) {
	if item.Collector == "" {
		return
	}
...
func (c *Coordinator) project(ctx context.Context, runID uint64, engine *domainv2.ProjectionEngine, request domainv2.ProjectionRequest, counts map[string]int64) error {
