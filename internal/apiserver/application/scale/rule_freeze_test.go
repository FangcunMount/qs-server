...
func TestPublishedScaleFreezesRuleMutationsButAllowsDisplayUpdate(t *testing.T) {
	...
	factorSvc := NewFactorService(repo, nil, &scaleEventPublisherStub{})
	if _, err := factorSvc.AddFactor(context.Background(), AddFactorDTO{
	...
	if _, err := lifecycleSvc.UpdateQuestionnaire(context.Background(), UpdateScaleQuestionnaireDTO{
	...
	if _, err := lifecycleSvc.UpdateBasicInfo(context.Background(), UpdateScaleBasicInfoDTO{
		Code:  published.GetCode().String(),
		Title: "Updated Scale",
	}); err != nil {
...
func TestFactorMutationPublishesCollectedDomainEventOnce(t *testing.T) {
	...
	item := newApplicationScaleForFreezeTest(t, domainScale.StatusDraft)
	repo := &ruleFreezeScaleRepoStub{item: item}
	publisher := &scaleEventPublisherStub{}
	factorSvc := NewFactorService(repo, nil, publisher)
