...
// fetchAndValidateQuestionnaire 获取并验证问卷，返回问卷对象和提交规格。
func (s *submissionService) fetchAndValidateQuestionnaire(
	ctx context.Context,
	l *logger.RequestLogger,
	dto *SubmitAnswerSheetDTO,
) (*questionnaire.Questionnaire, questionnaire.SubmissionSpec, error) {
	...
	if err != nil {
		return nil, questionnaire.SubmissionSpec{}, err
	}
	...
	if err != nil {
		return nil, questionnaire.SubmissionSpec{}, errors.WrapC(err, errorCode.ErrAnswerSheetInvalid, "问卷不可提交")
	}
	l.Debugw("问卷验证通过", "questionnaire_code", dto.QuestionnaireCode, "version", dto.QuestionnaireVer, "question_count", len(qnr.GetQuestions()))
	return qnr, spec, nil
}
...
func (s *submissionService) resolveSubmittableQuestionnaire(
	...
	if dto.QuestionnaireVer == "" {
		...
		if err := qnr.EnsureSubmittable(); err != nil {
			return nil, errors.WrapC(err, errorCode.ErrAnswerSheetInvalid, "当前没有可提交的已发布问卷版本")
		}
