...
func (*executorBuilder) MechanismKey() registry.MechanismReportBuilderKey {
	return registry.MechanismReportBuilderKey{AlgorithmFamily: modelcatalog.AlgorithmFamilyFactorScoring, DecisionKind: modelcatalog.DecisionKindScoreRange, ReportType: policy.ReportTypeStandard}
}
...
func newExecutorFixture(t *testing.T, builder *executorBuilder) (*executor, *memoryGenerationRepo, *memoryRunRepo, *memoryArtifactRepo, *eventStagerStub, *starterTx) {
	...
	registry, err := registry.NewReportBuilderRegistry(builder)
	if err != nil {
		t.Fatal(err)
	}
