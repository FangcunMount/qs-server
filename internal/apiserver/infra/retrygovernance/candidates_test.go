...
func TestDeliveryCandidatesExposeClaimedReplayForReconciliation(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("retry_disposition='automatic' AND replay_request_id IS NOT NULL")).
		WithArgs(int64(7), 10).
		WillReturnRows(sqlmock.NewRows([]string{"id", "event_id", "message_id", "transport_message_id", "topic_name", "channel_name", "delivery_attempts", "last_error", "retry_disposition", "replay_request_id", "updated_at"}).
			AddRow(21, "event-21", "message-21", "nsq-physical-21", "qs.evaluation.lifecycle", "qs-worker", 8, "publish outcome unknown", "automatic", "batch-7", now).
	...
	if items[0].EventID != "event-21" || items[0].MessageID != "message-21" || items[0].TransportMessageID != "nsq-physical-21" || items[0].TopicName != "qs.evaluation.lifecycle" || items[0].ChannelName != "qs-worker" || items[1].EventID != "" || items[1].MessageID != "message-22" || items[1].TransportMessageID != "" {
