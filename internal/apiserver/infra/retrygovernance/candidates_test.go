...
func TestDeliveryCandidatesExposeClaimedReplayForReconciliation(t *testing.T) {
	...
	mock.ExpectQuery(regexp.QuoteMeta("retry_disposition='automatic' AND replay_request_id IS NOT NULL")).
		WithArgs(int64(7), 10).
		WillReturnRows(sqlmock.NewRows([]string{"id", "delivery_attempts", "last_error", "retry_disposition", "replay_request_id", "updated_at"}).
			AddRow(21, 8, "publish outcome unknown", "automatic", "batch-7", now).
			AddRow(22, 8, "delivery failed", "manual_required", nil, now))
	reader := &Reader{mysql: gormDB}
	var items []app.RetryCandidate
