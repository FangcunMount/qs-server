...
func DialGovernanceManagement(address, caFile, certFile, keyFile string) (*EvaluationClient, *PublicationClient, io.Closer, error) {
	...
	if err != nil {
		return nil, nil, nil, fmt.Errorf("cannot load QS evaluation client certificate")
	}
	roots, err := os.ReadFile(caFile)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("cannot load QS evaluation CA")
	}
	pool := x509.NewCertPool()
	if !pool.AppendCertsFromPEM(roots) {
		return nil, nil, nil, fmt.Errorf("invalid QS evaluation CA")
	}
	...
	return NewEvaluationClient(conn), NewPublicationClient(conn), conn, nil
}
