...
func (s *RunStore) Create(ctx context.Context, in statisticsApp.Run) (*statisticsApp.Run, error) {
...
func (s *RunStore) AssertPublishable(ctx context.Context, orgID int64, target time.Time) error {
	...
	err := s.dbFor(ctx).Table("statistics_org_snapshot").
	...
	current := statisticsDomain.BusinessDate(row.AsOfDate)
	target = statisticsDomain.BusinessDate(target)
...
func (s *RunStore) MarkDataCommitted(ctx context.Context, id uint64, at time.Time) error {
	return s.dbFor(ctx).Table("statistics_sync_run").Where("id=?", id).Updates(map[string]any{"status": statisticsDomain.RunStatusDataCommitted, "stage": "data_committed", "data_committed_at": at}).Error
}
...
func (s *RunStore) MarkCachePublishFailed(ctx context.Context, id uint64, generation int64, message string, at time.Time) error {
	values := map[string]any{
		"status": statisticsDomain.RunStatusDataCommitted, "stage": "publishing_cache",
...
func (s *RunStore) MarkSucceeded(ctx context.Context, id uint64, at time.Time) error {
	return s.dbFor(ctx).Table("statistics_sync_run").Where("id=?", id).Updates(map[string]any{"status": statisticsDomain.RunStatusSucceeded, "stage": "completed", "finished_at": at}).Error
}
func (s *RunStore) MarkFailed(ctx context.Context, id uint64, stage, code, message string, at time.Time) error {
	return s.dbFor(ctx).Table("statistics_sync_run").Where("id=?", id).Updates(map[string]any{"status": statisticsDomain.RunStatusFailed, "stage": stage, "error_code": code, "error_message": truncateRunText(message, 1000), "finished_at": at}).Error
}
...
func (s *RunStore) Get(ctx context.Context, id uint64) (*statisticsApp.Run, error) {
	var po runPO
...
func (s *RunStore) List(ctx context.Context, orgID int64, limit int) ([]statisticsApp.Run, error) {
	...
	out := make([]statisticsApp.Run, 0, len(rows))
	for _, row := range rows {
		out = append(out, *fromRunPO(row))
	}
	return out, nil
}
func fromRunPO(po runPO) *statisticsApp.Run {
	mode := statisticsDomain.RunMode(po.RunMode)
	if mode == "" {
		mode = statisticsDomain.RunModePublish
	}
	r := &statisticsApp.Run{ID: po.ID, OrgID: po.OrgID, BatchKey: po.BatchKey, Attempt: po.Attempt, TriggerType: po.TriggerType, Mode: mode, Window: statisticsDomain.InstantRange{From: po.WindowStart, To: po.WindowEnd}, AsOfDate: po.AsOfDate, CacheGeneration: po.CacheGeneration, CachePublishedAt: po.CachePublishedAt, Status: statisticsDomain.RunStatus(po.Status), Stage: po.Stage, Reason: po.Reason, StartedAt: po.StartedAt, DataCommittedAt: po.DataCommittedAt, FinishedAt: po.FinishedAt, ErrorCode: po.ErrorCode, ErrorMessage: po.ErrorMessage}
