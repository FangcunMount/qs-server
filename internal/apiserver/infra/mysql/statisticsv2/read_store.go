...
func (s *ReadStore) Overview(ctx context.Context, orgID int64, from, to time.Time) (appv2.OverviewMetrics, error) {
	ctx, release, err := s.acquire(ctx)
	if err != nil {
		return appv2.OverviewMetrics{}, err
	}
	defer release()
	var value appv2.OverviewMetrics
	err = s.db.WithContext(ctx).Raw(`
...
func (s *ReadStore) OverviewTrends(ctx context.Context, orgID int64, from, to time.Time) (appv2.OverviewTrends, error) {
	ctx, release, err := s.acquire(ctx)
	if err != nil {
		return appv2.OverviewTrends{}, err
	}
	defer release()
	...
	result := appv2.OverviewTrends{EnrolledTestees: enrolled}
...
func (s *ReadStore) ListClinicians(ctx context.Context, orgID int64, clinicianID *uint64, operatorUserID *int64, from, to time.Time, page, size int) ([]appv2.ClinicianItem, int64, error) {
	ctx, release, err := s.acquire(ctx)
	if err != nil {
		return nil, 0, err
	}
...
func (s *ReadStore) ListEntries(ctx context.Context, orgID int64, entryID, clinicianID *uint64, active *bool, from, to time.Time, page, size int) ([]appv2.EntryItem, int64, error) {
	ctx, release, err := s.acquire(ctx)
	if err != nil {
		return nil, 0, err
	}
...
func (s *ReadStore) CurrentClinicianTesteeSummary(ctx context.Context, orgID int64, clinicianID uint64, from, to time.Time) (appv2.TesteeSummary, error) {
	ctx, release, err := s.acquire(ctx)
	if err != nil {
		return appv2.TesteeSummary{}, err
	}
	defer release()
	var value appv2.TesteeSummary
...
func (s *ReadStore) ContentBatch(ctx context.Context, orgID int64, asOfDate time.Time, refs []appv2.ContentRef) ([]appv2.ContentItem, error) {
	...
	defer release()
	items := make([]appv2.ContentItem, 0, len(refs))
