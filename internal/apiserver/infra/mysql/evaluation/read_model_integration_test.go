...
func TestAssessmentReadModelListAssessmentsFiltersAgainstDatabase(t *testing.T) {
	...
	rows := []AssessmentPO{
		{
			...
			EvaluationModelKind:    &scaleKind,
			EvaluationModelCode:    &scaleCode,
			EvaluationModelVersion: &scaleVersion,
	...
	if got[0].RiskLevel == nil || *got[0].RiskLevel != "high" || got[0].EvaluationModelCode == nil || *got[0].EvaluationModelCode != scaleCode {
