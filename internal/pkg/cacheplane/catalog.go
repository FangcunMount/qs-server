...
func (c *Catalog) Namespace(family Family) string {
	...
	return keyspace.ComposeNamespace(c.root, c.Route(family).NamespaceSuffix)
}

// Builder returns a family-scoped key builder.
func (c *Catalog) Builder(family Family) *keyspace.Builder {
	return keyspace.NewBuilderWithNamespace(c.Namespace(family))
}
