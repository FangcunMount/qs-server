...
func NewRuntime(component string, resolver Resolver, catalog *Catalog, status *observability.FamilyStatusRegistry) *Runtime {
	return &Runtime{
		component: component,
		resolver:  resolver,
		catalog:   catalog,
...
func (r *Runtime) resolve(_ context.Context, family Family) *Handle {
	...
	if r.resolver == nil {
		...
		handle.Mode = observability.FamilyModeDegraded
	...
	if route.RedisProfile == "" {
		...
		if handle.Available {
			handle.Mode = observability.FamilyModeDefault
		} else {
			handle.Mode = observability.FamilyModeDegraded
	...
	case cbdatabase.RedisProfileStateMissing:
		...
		if route.AllowFallbackDefault {
			...
			handle.Degraded = !handle.Available
			if handle.Available {
				handle.Mode = observability.FamilyModeFallbackDefault
			} else {
				handle.Mode = observability.FamilyModeDegraded
				handle.LastError = err
			}
	...
	default:
		...
		handle.Degraded = !handle.Available
		if handle.Available {
			handle.Mode = observability.FamilyModeNamedProfile
		} else {
			handle.Mode = observability.FamilyModeDegraded
			handle.LastError = err
		}
...
func (r *Runtime) updateStatus(handle *Handle) {
	...
	r.status.Update(observability.FamilyStatus{
