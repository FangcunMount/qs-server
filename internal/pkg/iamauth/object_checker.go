...
func (c *ObjectChecker) CheckObject(ctx context.Context, request appauthz.ObjectCheckRequest) (appauthz.ObjectDecision, error) {
	...
	response, err := c.client.SDK().Authz().CheckObject(ctx, request.Subject, request.Resource, request.Action, request.ObjectID, attributes)
	if err != nil {
		switch {
...
func objectAttributes(input map[string]appauthz.ObjectAttribute) ([]*authzv4.ObjectAttribute, error) {
	keys := make([]string, 0, len(input))
	for key := range input {
		keys = append(keys, key)
	}
	sort.Strings(keys)
	result := make([]*authzv4.ObjectAttribute, 0, len(keys))
	for _, key := range keys {
		...
		attribute := &authzv4.ObjectAttribute{Key: key}
		switch {
		case value.String != nil:
			attribute.Value = &authzv4.ObjectAttribute_StringValue{StringValue: *value.String}
		case value.Int64 != nil:
			attribute.Value = &authzv4.ObjectAttribute_Int64Value{Int64Value: *value.Int64}
		case value.Bool != nil:
			attribute.Value = &authzv4.ObjectAttribute_BoolValue{BoolValue: *value.Bool}
		}
