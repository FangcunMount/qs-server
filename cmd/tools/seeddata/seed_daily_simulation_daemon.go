...
func runDailySimulationBatch(
	...
	progress := newSeedProgressBar(progressLabel+" users", count)
			...
			for idx := range jobs {
				...
				if simErr != nil {
					deps.Logger.Warnw("Daily simulation user failed",
						...
						"clinician_id", scenario.Clinician.ID,
					...
					failureMu.Lock()
					if len(failures) < 8 {
						failures = append(failures, fmt.Sprintf("idx=%d guardian=%s child=%s clinician=%s err=%v", profile.Index, profile.GuardianEmail, profile.ChildName, scenario.Clinician.ID, simErr))
					}
					failureMu.Unlock()
				} else {
	...
	for _, scenario := range scenarios {
		selectedClinicians = append(selectedClinicians, fmt.Sprintf("%s:%s", strings.TrimSpace(scenario.Clinician.ID), strings.TrimSpace(scenario.Clinician.Name)))
	}
...
func resolveDailySimulationScenariosForRun(
	...
	scope := seedClinicianScopeSpec{
		refs:        append(append([]string(nil), cfg.ClinicianRefs...), strings.TrimSpace(cfg.ClinicianRef)),
		keyPrefixes: append([]string(nil), cfg.ClinicianKeyPrefixes...),
		ids:         append(append([]FlexibleID(nil), cfg.ClinicianIDs...), cfg.ClinicianID),
	}
	clinicians, err := resolveSeedClinicianScope(ctx, deps, scope)
	if err != nil {
		return nil, err
	}
	selectedClinicians, err := selectDailySimulationCliniciansForRun(clinicians, cfg, runDate)
	if err != nil {
		return nil, err
	}

	scenarios := make([]dailySimulationScenario, 0, len(selectedClinicians))
	for _, clinicianItem := range selectedClinicians {
		scenarioCfg := cfg
		scenarioCfg.EntryID = FlexibleID("")
		scenarioCfg.ClinicianRef = ""
		scenarioCfg.ClinicianRefs = nil
		scenarioCfg.ClinicianKeyPrefixes = nil
		scenarioCfg.ClinicianIDs = nil
		scenarioCfg.ClinicianID = FlexibleID(strings.TrimSpace(clinicianItem.ID))

		entry, target, resolvedClinician, err := ensureDailySimulationEntryAndTarget(ctx, deps, scenarioCfg)
		if err != nil {
			return nil, err
		}
...
func selectDailySimulationCliniciansForRun(
	...
	if minCount <= 0 && maxCount <= 0 {
		minCount = len(clinicians)
		maxCount = len(clinicians)
	}
	if minCount <= 0 {
		minCount = 1
	}
	...
	if minCount > len(clinicians) {
		minCount = len(clinicians)
	}
	if maxCount > len(clinicians) {
		maxCount = len(clinicians)
	}
	...
	order := rng.Perm(len(clinicians))
	selected := make([]*ClinicianResponse, 0, selectedCount)
	for _, idx := range order[:selectedCount] {
		selected = append(selected, clinicians[idx])
	}
	return selected, nil
}
