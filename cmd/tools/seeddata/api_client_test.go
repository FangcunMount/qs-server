...
func TestDecodeResponseDataClinicianRelationListSupportsFormattedTime(t *testing.T) {
	resp := &Response{
		Data: map[string]interface{}{
			"items": []interface{}{
				map[string]interface{}{
					"testee": map[string]interface{}{
						"id":         "testee-1",
						"name":       "Alice",
	...
	var result ClinicianRelationListResponse
	if err := decodeResponseData(resp, &result); err != nil {
		t.Fatalf("decodeResponseData returned error: %v", err)
	}
	if len(result.Items) != 1 {
	...
	if got, want := result.Items[0].Testee.CreatedAt.Format("2006-01-02 15:04:05"), "2026-04-01 09:30:00"; got != want {
		t.Fatalf("unexpected testee created_at: got=%s want=%s", got, want)
	}
	if got, want := result.Items[0].Relation.BoundAt.Format("2006-01-02 15:04:05"), "2026-04-01 11:00:00"; got != want {
		t.Fatalf("unexpected relation bound_at: got=%s want=%s", got, want)
	}
}
