...
| apiserver | Plan、Statistics、Behavior、Evaluation 等 scheduler | 推进周期任务、读侧统计和一致性修复 |
...
StatisticsSyncRunner 按每日时钟执行，在 repair window 内同步：

- 日统计；
- 组织快照统计；
- Plan 统计。

同步完成后可协调缓存预热。repair window 允许任务中断后在后续运行中修复近期数据，而不是只算当天一次。

### 4.3 BehaviorPendingReconcileRunner

该 runner 周期调用 `ReconcilePendingBehaviorEvents`，按 batch limit 补偿尚未完成的行为事件归因。它处理的是事件投影未闭环，不应重新定义行为测评业务规则。

### 4.4 EvaluationConsistencyReconcileRunner
...
### 4.5 BehaviorJourneyScanRunner

该 runner 按组织、source、lookback 和 batch size 从事实表扫描 due 数据，投影行为旅程统计，并可触发窗口重算。`dry_run` 用于在不写入的情况下验证扫描范围。

## 5. 为什么每个 scheduler 都需要 leader lock

apiserver 可以运行多个实例。若每个实例同时执行同一周期任务，会产生重复扫描和写入竞争。五个 runner 都通过 Redis lock lease 争夺 workload 对应的 leader lock：
...
- Statistics/Behavior：`internal/apiserver/application/statistics`；
