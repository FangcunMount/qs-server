...
| apiserver | Plan、Statistics、Evaluation/Interpretation recovery、Report Catalog audit、Mongo consistency audit | 推进周期任务、T+1 统计、短周期租约恢复和只读一致性巡检 |
...
当前 composition root 可装配 Plan、Statistics、Evaluation consistency audit、Evaluation lease recovery、Interpretation lease recovery、Report Catalog audit 和 Mongo consistency audit 共七个 runner。是否实际注册仍取决于有效配置和依赖；“代码中存在 runner”不等于生产已启用。
