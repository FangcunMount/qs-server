...
`mongo-domain-events` 在 claim pending 事件时会优先处理 `answersheet.submitted`、`assessment.interpreted`、`report.generated`，剩余容量再处理 `footprint.*` 等事件，避免行为投影事件阻塞测评生成主链路。
...
6. Mongo outbox 是否由 `footprint.*` 大量占据队头；
7. 事件对应 topic 是否在 `events.yaml` 中配置；
8. worker 是否订阅了该 topic。
