...
| apiserver → MQ | Publish / Relay | [server.go `PrepareRun`](../../internal/apiserver/server.go)：`MessagingOptions.NewPublisher()` → 注入 [container `NewContainerWithOptions`](../../internal/apiserver/container/container.go)（`MQPublisher`）→ [`initEventPublisher` / `NewRoutingPublisher`](../../internal/apiserver/container/container.go)；direct publish 和 relay 最终都经过这里 |
