...
| 业务发布调用 | 各应用服务 / outbox relay | 例如 survey 的 Mongo outbox relay（`answersheet.submitted`）、evaluation 的 MySQL / Mongo outbox（`assessment.submitted` / `assessment.failed` / `assessment.interpreted` / `report.generated`）、以及 scale / plan 等仍保留的 direct publish 路径；**细表**见 [01-apiserver §5](./01-apiserver.md) |
