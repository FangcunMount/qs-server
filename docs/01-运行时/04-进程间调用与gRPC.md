...
该 interceptor 的主体边界是用户 Access Token，固定使用 IAM `v3.2.0` Token Profile（`RS256` + issuer + audience + token type）。Service Token 是独立的出站服务凭据，不能通过该用户 interceptor。
...
collection 可选附加出站 Service Token，但当 server auth 未开启时它不是当前主要边界；worker client 当前主要依赖 mTLS。这些只是仓库配置事实，不能单独证明已部署环境正在使用同一配置和证书。
