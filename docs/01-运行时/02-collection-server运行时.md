...
flowchart TD
    ...
    C --> D["本地静态 envelope 校验"]
    D --> E["SubmitCoalescer<br/>lease + bounded wait"]
    E --> F["apiserver durable lookup"]
    F -->|"hit"| J["原 AnswerSheet ID"]
    F -->|"miss"| G["问卷、ProfileLink、testee 完整校验"]
    G --> H["apiserver SaveAnswerSheet gRPC"]
    H --> I["AnswerSheet + 幂等事实 + Outbox 可靠提交"]
    I --> J
    J --> K["202 Accepted"]
```
...
- 已 commit 的重放不再依赖当前问卷和 ProfileLink 状态；
- readback 基础设施错误不能当作 miss，只有旧 apiserver 的 `Unimplemented` 可在滚动期回退完整 durable path；
...
| 新提交时当前用户无权为受试者填写 | 拒绝访问，不进入持久化 |
| 已 commit 的同 writer/key/内容重放 | 不重新检查历史 ProfileLink，返回原 AnswerSheet ID |
