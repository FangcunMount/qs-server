...
当前 lifecycle 测试冻结了上述依赖顺序，生产 Compose 为 apiserver 与 worker 配置 8 分 30 秒 stop grace，避免 Docker 默认短超时提前发送 SIGKILL。
剩余风险是 readiness 尚未在 drain 前形成显式的摘流阶段，且各 hook 暂无独立耗时指标。
