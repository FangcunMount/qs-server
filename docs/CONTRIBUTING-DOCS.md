# qs-server 文档维护约定

**本文回答**：这份文档只规定 `docs/` 的维护规则，包括事实优先级、目录分工、长文结构、archive 政策和提交流程；它不承担项目介绍，也不替代各目录 README。

---

## 适用范围

- 本文适用于 `docs/00-总览` 到 `docs/06-宣讲` 的现行文档。
- `docs/_archive` 不适用本文的结构要求；archive 只受它自己的归档政策约束。

---

## 事实来源优先级

维护文档时，必须按下面的顺序判断真值：

1. **源码**
   - `internal/`
   - `cmd/`
   - `pkg/`
2. **机器契约与配置**
   - `api/rest/`
   - `internal/apiserver/interface/grpc/proto/`
   - `configs/events.yaml`
   - `configs/*.yaml`
3. **现行正文**
   - `docs/00-05`
4. **宣讲层**
   - `docs/06-宣讲`
5. **历史层**
   - `docs/_archive`

文档与代码、契约冲突时，以代码和契约为准。

---

## 目录边界

| 目录 | 负责什么 | 不负责什么 |
| ---- | -------- | ---------- |
| `00-总览` | 系统地图、代码边界、主链路、本地开发入口 | 模块细节和横切机制细节 |
| `01-运行时` | 三进程职责、进程间调用、运行时链路 | 模块内对象模型 |
| `02-业务模块` | 模块边界、对象、服务分层、模块内关键设计 | 跨进程时序和横切基础设施全量说明 |
| `03-基础设施` | 事件、存储、Redis、缓存限流、IAM、配置等横切能力 | 模块业务语义的完整展开 |
| `04-接口与运维` | REST / gRPC 契约入口、端口部署、调度任务、事故复盘 | 模块对象和内部业务规则 |
| `05-专题分析` | 系统级设计判断与跨层分析 | 第二份模块文或第二份基础设施真值文 |
| `06-宣讲` | 对外介绍、技术宣讲、答辩材料 | 机器契约和实现真值 |
| `_archive` | 历史设计稿、阶段记录、旧文档 | 现行阅读主路径和现行事实来源 |

---

## 长文默认结构

现行长文默认按下面的顺序组织：

1. `本文回答`
2. `30 秒结论`
3. 主图或主表
4. 详细展开
5. 代码锚点、配置锚点或回链

额外规则：

- 目录级 `README.md` 可以更短，但至少要先给阅读入口和边界。
- 如果需要目录，目录应放在 `30 秒结论` 之后。
- 先结论，再展开；不要把核心结论埋在长背景后面。

---

## 表达规则

### 先结论，后细节

- 每个长文开头必须让读者在一屏内知道“这篇文档解决什么问题”。
- 每个重要三级标题下，优先给 1～2 句结论，再展开解释。

### 优先用图和表

以下内容优先使用图或表表达：

- 进程关系
- 模块边界
- 对比关系
- 流程步骤
- 配置项映射

正文负责解释图表不能直接说明的部分，不要让整页只剩长段落。

### 一处讲透，其它地方回链

- 同一事实不要在引言、图注、正文和 FAQ 里完整重复。
- 某处已经讲透后，其它文档只保留摘要和链接。

### 少抄代码，多给锚点

- 优先写“路径 + 类型 / 函数 / 配置项名”
- 不要在文档里复制大段源码
- 必要示例只保留最小关键片段

---

## 状态标签

涉及能力成熟度、证据完整度或未来规划时，统一使用下面的状态：

| 标签 | 含义 |
| ---- | ---- |
| `已实现` | 代码和配置都能证明，能直接写成当前能力 |
| `待补证据` | 方向存在，但仓内证据还不够完整，不能讲成标准能力 |
| `规划改造` | 当前仓库未完成，只能作为后续方案或目标 |
| `历史资料` | 只存在于 `_archive` 或阶段记录中，不属于现行真值层 |

不要把 `规划改造` 写成现状，也不要把 `历史资料` 写成当前设计。

---

## archive 政策

`docs/_archive` 是长期保留的历史层，规则如下：

- 不进入现行阅读主路径
- 不作为现行真值来源
- 默认不参与 `make docs-hygiene`
- 从 archive 回迁内容前，必须重新核对源码和契约

archive 目录本身的说明见 [docs/_archive/README.md](./_archive/README.md)。

---

## 链接与锚点规则

- 文档内优先使用相对路径链接到仓内其他文档或代码。
- 能直接指向代码文件时，不写模糊描述。
- 跨仓库路径可以写成普通代码路径说明，但不要把它们当成仓内可校验链接。
- 历史文档不要作为现行主路径的必经链接。

---

## 提交前检查

修改现行文档后，至少执行：

```bash
make docs-hygiene
git diff --check
```

如果改动涉及代码事实，额外确认对应锚点和契约没有漂移。

---

## Resilience Plane 维护门禁

新增或修改高并发治理能力时，不能只改代码或局部测试。凡是涉及限流、队列、背压、Lock lease、幂等、重复抑制或降级策略，必须同步核对：

1. **模型锚点**：是否已有或需要新增 `ratelimit`、`resilienceplane`、`backpressure`、`locklease`、`SubmitQueue` 等模型 / adapter。
2. **契约测试**：是否覆盖外部语义，例如 HTTP status、`Retry-After`、队列状态、Redis fail-open、锁竞争、降级继续。
3. **观测 outcome**：是否通过 `resilienceplane.Observer` 上报 bounded outcome，且 label 不包含 user ID、request ID、lock key 或 error message。
4. **文档真值层**：是否更新 [resilience/06-新增高并发治理能力SOP.md](./03-基础设施/resilience/06-新增高并发治理能力SOP.md) 或 [resilience/07-能力矩阵.md](./03-基础设施/resilience/07-能力矩阵.md)。
5. **边界声明**：是否明确该能力不改变已有 HTTP 响应、Redis key、SubmitQueue 生命周期、LockSpec 语义或 component-base primitive 归属，除非本次变更明确要求。

默认流程是：`模型 / adapter -> contract tests -> 文档更新 -> docs hygiene`。如果新增保护点不能放入现有能力矩阵，先补设计说明，不要把它混写成已有能力。

---

## Security Control Plane 维护门禁

新增或修改 JWT、IAM、authz snapshot、operator role projection、service auth、mTLS、ACL 或 capability 时，不能只改中间件或 handler。安全控制面的维护默认按 [03-基础设施/security/README.md](./03-基础设施/security/README.md) 的 truth layer 处理，并同步核对：

1. **模型归属**：变更必须能归到 `Principal`、`TenantScope`、`AuthzSnapshot`、`CapabilityDecision`、`ServiceIdentity`、`OperatorRoleProjection` 或 `mTLS/ACL` 中的一类。
2. **契约测试**：涉及身份、租户范围、metadata、context key、status code、capability、service identity 或 ACL 默认策略时，必须补 contract / seam tests。
3. **权限真值**：业务 capability 必须以 IAM authorization snapshot 为准，不允许新增基于 JWT roles 的 handler 级授权。
4. **文档真值层**：同步更新 [security/00-整体架构.md](./03-基础设施/security/00-整体架构.md) 或对应深讲页；旧 [04-IAM与认证.md](./03-基础设施/04-IAM与认证.md) 只保留摘要和回链。
5. **否定边界**：如果能力只是 seam 或规划，例如 ACL 文件加载、强制 service auth transport security、mTLS 身份一致性收口，必须明确写成当前不支持或后续阶段，不得写成已实现。

默认流程是：`模型 -> contract tests -> runtime projection / adapter -> security docs -> docs hygiene`。`securityprojection` 只能做 primitive input 到 `securityplane` 值对象的映射，不能依赖 Gin、gRPC、IAM SDK 或 apiserver 应用包。如果变更会改变 HTTP/gRPC status、JWT claims schema、IAM SDK 配置或 mTLS/ACL 行为，必须作为单独安全策略变更处理。

---

## Runtime Composition / Config Options 维护门禁

新增或修改三进程 container、process stage、runtime client 或配置项时，不能只在局部代码里补一个 setter 或字段。Runtime Composition 的当前 truth layer 见 [03-基础设施/runtime/README.md](./03-基础设施/runtime/README.md)，维护时必须同步核对：

1. **Composition Graph**：apiserver 跨模块依赖优先进入 constructor deps；确需后置注入时只能进入 `ModuleGraph/PostWire`，并更新 architecture allowlist。
2. **ClientBundle**：collection-server / worker 新增 gRPC runtime client 时，必须更新 `ClientBundle`、registry bundle 构造和 container 注入测试；不要新增 per-client setter。
3. **Config contract**：新增配置项必须能从 `configs/*.yaml -> options.Options -> config.Config -> process/container deps` 追踪，并补 dev/prod contract tests。
4. **默认值稳定**：配置默认值、Redis family、Event catalog path、Resilience/IAM/metrics 关键路径变更时，必须写明行为边界和测试锚点。
5. **文档同步**：更新 [runtime/00-CompositionGraph与ConfigOptions.md](./03-基础设施/runtime/00-CompositionGraph与ConfigOptions.md) 或 [05-配置体系.md](./03-基础设施/05-配置体系.md)，不要把规划中的重排写成当前启动事实。

默认流程是：`contract tests -> composition/config code -> docs -> docs hygiene`。如果变更会改变启动顺序、外部配置字段名、环境变量名、HTTP/gRPC/MQ 行为或 Redis key，必须拆成单独行为变更处理。

---

## Data Access Plane 维护门禁

新增或修改 MySQL、Mongo、migration、outbox 存储、read model 或 statistics projection 时，不能只改 repository。Data Access 的当前 truth layer 见 [03-基础设施/data-access/README.md](./03-基础设施/data-access/README.md)，维护时必须同步核对：

1. **边界归属**：确认变更是主写模型、文档模型、read model、outbox/idempotency 还是 migration；不要把一次性修复脚本混入常规 repository。
2. **依赖方向**：domain 不得 import infra、GORM、Mongo driver；repository 不得 import transport/handler。
3. **迁移契约**：新增表、集合、索引、状态字段时必须补 MySQL/Mongo migration，并保留 down 文件或明确不可逆理由。
4. **测试保护**：补 mapper、repository、backpressure、transaction、outbox/read model contract tests；必要时更新 [data_access_architecture_test.go](../internal/pkg/architecture/data_access_architecture_test.go)。
5. **文档同步**：更新 [data-access](./03-基础设施/data-access/README.md) 对应深讲和相关业务模块文档。

默认流程是：`模型/port -> migration -> mapper/repository -> contract tests -> data-access docs -> docs hygiene`。

---

## External Integration Adapter 维护门禁

新增或修改 WeChat、OSS、通知、Webhook、第三方 SDK 或外部 API 适配时，不能让业务层直接理解 SDK 类型或外部错误。External Integration 的当前 truth layer 见 [03-基础设施/integrations/README.md](./03-基础设施/integrations/README.md)，维护时必须同步核对：

1. **窄 port**：application 只依赖业务需要的接口，不直接依赖 SDK client。
2. **adapter 边界**：credential、timeout、endpoint、SDK cache、错误包装和外部字段转换都留在 infra adapter。
3. **测试保护**：常规单测覆盖 validation、cache key、请求组装、模板字段、错误映射；真实网络调用不进入单元测试。
4. **观测边界**：metrics/status label 不得包含 token、secret、openid、手机号、error message 等高基数字段或敏感字段。
5. **文档同步**：更新 [integrations](./03-基础设施/integrations/README.md) 对应深讲；宣讲层只回链，不维护第二份事实。

默认流程是：`业务用例 -> port -> adapter -> local contract tests -> integrations docs -> docs hygiene`。

---

## 业务模块深讲维护门禁

新增或修改业务聚合、状态、事件、读模型、校验规则或 SOP 时，不能只更新旧的单篇模块文。`02-业务模块` 的长期 truth layer 已收口到子目录深讲，默认按下面的流程维护：

1. **模型锚点**：确认变更属于 `survey`、`scale`、`evaluation`、`plan`、`actor`、`statistics` 中哪个限界上下文；跨模块判断放到 `05-专题分析`。
2. **深讲文档**：优先更新对应子目录的 `00-整体模型`、状态机、链路、边界或 SOP；旧 `01-*.md` 到 `06-*.md` 只作为兼容入口同步摘要。
3. **契约证据**：涉及 REST、gRPC、事件或缓存治理时，同步核对 `api/rest`、proto、`configs/events.yaml` 和对应基础设施 truth layer。
4. **测试锚点**：新增或改变领域不变量时，必须在深讲文档中回链到领域测试、应用服务测试或 contract tests。
5. **宣讲边界**：`06-宣讲` 只更新讲法和证据索引，不允许单独维护第二份业务事实。

默认流程是：`领域模型 / 应用服务 -> contract tests -> 业务模块深讲 -> 专题或宣讲回链 -> docs hygiene`。

### 深讲页内容契约

业务模块子目录里的核心深讲页不能只写一张结论表。除目录级 `README.md` 和纯 SOP 页外，每篇深讲页默认必须回答下面七个问题：

| 必答问题 | 要写清楚什么 | 常用表达 |
| -------- | ------------ | -------- |
| 模块要解决什么问题 | 业务痛点、变化原因、为什么值得单独成界 | 问题表、边界表 |
| 架构设计是什么 | domain / application / infra / runtime 的职责和调用方向 | 分层图、组件图、调用图 |
| 领域模型怎么设计 | 聚合根、实体、值对象、领域服务、应用服务、仓储接口 | 类图、模型表 |
| 用了什么设计模式 | 状态机、职责链、策略、工厂、Builder、防腐层等真实源码中的协作方式 | 模式表、流程图 |
| 为什么这样设计 | 替代方案、选择理由、设计意图 | 取舍矩阵 |
| 付出了什么取舍 | 收益、代价、不支持能力、维护风险 | trade-off 表 |
| 如何验证 | 源码锚点、测试锚点、配置锚点、Verify 命令 | 锚点表、命令块 |

如果某篇文档没有真实使用某个模式，不要为了完整性虚构；应明确写成“未使用该模式，原因是当前变化点不需要”。如果某项能力只存在于规划或历史中，必须使用状态标签，不能写成当前事实。

---

## 维护清单

提交文档前，至少自检下面几项：

- 这篇文档的边界是否清楚，没有和相邻文档重复维护同一事实
- 关键结论是否能在一屏内读到
- 是否给了足够的代码、配置或契约锚点
- 是否把现状、规划、历史区分清楚
- 是否误把 archive 内容当成现行事实

---

修订本文时，如调整目录层级或阅读入口，请同步更新 [README.md](./README.md)。
