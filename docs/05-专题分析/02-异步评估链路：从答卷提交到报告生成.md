...
本文说明 `qs-server` **最关键的运行时设计**：同步入口只负责「接住请求 + 落主状态 + 发事件」，计分、建测评、评估引擎与报告在 **MQ + qs-worker + apiserver internal gRPC** 上推进；并分别讲清 **提交排队、异步边界、MQ、worker、evaluation engine（职责链）** 各自做什么。
...
9. [evaluation engine：职责链（pipeline）如何应用](#evaluation-engine职责链pipeline-如何应用)
...
下列时序与 [configs/events.yaml](../../configs/events.yaml) 中 Topic **`assessment.lifecycle`**（配置键 `assessment-lifecycle`）下的测评域事件一致。为兼顾**总览**与**BFF 内 SubmitQueue**，本节用**两张图**：先全链路，再展开小程序 → collection（含队列）→ apiserver → MQ → worker 的边界。
...
- **Topic 通道**：如运行时名 `assessment.lifecycle`（见 [events.yaml](../../configs/events.yaml) `topics.*.name`），把同一类业务事件交给订阅方。
...
当前 worker 默认走 **NSQ** 时（`messaging.provider: nsq`，见 [worker.dev.yaml](../../configs/worker.dev.yaml)），订阅端在 [server.go](../../internal/worker/server.go) `createSubscriber` 里组装 **NSQ Consumer 配置**：
...
Topic 运行时名以 [events.yaml](../../configs/events.yaml) 为准（如 `assessment.lifecycle`）。
