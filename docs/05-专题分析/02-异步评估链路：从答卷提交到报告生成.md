...
- [../03-基础设施/event/02-Publish与Outbox.md](../03-基础设施/event/02-Publish与Outbox.md)
- [../03-基础设施/event/03-Worker消费与AckNack.md](../03-基础设施/event/03-Worker消费与AckNack.md)
...
| 事件细节入口 | 本文讲评估主链叙事；事件契约和状态机细节回到 [事件系统深讲](../03-基础设施/event/README.md) |

---

## 本专题要解决什么问题

这篇专题回答“为什么从答卷提交到报告生成必须拆成同步受理 + 异步评估 + durable outbox”。这不是单纯为了性能，而是为了同时满足三个约束：

| 约束 | 设计选择 |
| ---- | -------- |
| 用户请求不能长时间等待评估 | collection-server 只负责受理，长任务交给 MQ/worker |
| 答卷保存和事件出站必须一致 | Mongo/MySQL outbox 按持久化边界落库 |
| worker 不能成为第二套业务中心 | worker 只调用 apiserver internal gRPC，业务写模型仍在 apiserver |

## 设计判断矩阵

| 能力 | 同步实现 | 当前异步实现 | 为什么选择当前方案 |
| ---- | -------- | ------------ | ------------------ |
| 答卷保存 | HTTP 请求内保存 | 仍同步保存 | 保存是用户可见提交结果，必须立即返回明确状态 |
| 创建 Assessment | 可同步创建 | 由 `answersheet.submitted` 触发 | 降低提交延迟，并让重复处理有 worker gate + 业务唯一约束 |
| 执行评估 pipeline | 可同步执行 | 由 `assessment.submitted` 触发 | pipeline 可能耗时且有失败补偿 |
| 报告出站 | 可 direct publish | 保存报告后进入 Mongo outbox | 报告和事件出站要在同一持久化边界内保持一致 |
| 下游副作用 | 可在 apiserver 内完成 | worker 消费 `report.generated` | 标签、统计、通知等副作用解耦 |

## 设计模式总览

| 模式 | 链路位置 | 价值 |
| ---- | -------- | ---- |
| Outbox | answer sheet / assessment / report 出站 | 解决业务写入和事件发布的一致性 |
| 职责链 | Evaluation Engine Pipeline | 把评估拆成校验、计分、风险、报告、通知等步骤 |
| 幂等 / duplicate gate | SubmitGuard、worker Redis gate、业务唯一约束 | 降低重复提交和重复消费影响 |
| Adapter | worker handler -> internal gRPC | worker 不持有业务 repository |
| 状态机 | Assessment 状态 | 评估失败、成功、提交状态显式化 |
...
事件系统的通用事实不在本文重复维护：delivery class、outbox 状态机、worker Ack/Nack 和 MQ 选型分别见 [事件契约与 Catalog](../03-基础设施/event/01-事件契约与Catalog.md)、[Publish 与 Outbox](../03-基础设施/event/02-Publish与Outbox.md)、[Worker 消费与 Ack/Nack](../03-基础设施/event/03-Worker消费与AckNack.md)。
...
- 不能把旧式 consumer 字段继续讲成当前运行时并发 / 重试配置
