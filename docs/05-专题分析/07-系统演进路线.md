...
POST /answersheets
  ...
  -> CalculateAnswerSheetScore + CreateAssessmentFromAnswerSheet
  -> assessment.submitted (MySQL outbox)
  -> worker consume assessment.submitted
  -> EvaluateAssessment (计分 + 解读 + 报告，当前同步完成)
  -> assessment.interpreted + report.generated
```

> **说明**：`assessment.created`、`assessment.completed`、`interpretation.completed` 为历史演进目标名，当前代码未使用；正式事件见 [`03-领域事件总览.md`](../02-业务模块/03-领域事件总览.md) §2。
...
| 横切能力 | 需要补充 |
| -------- | -------- |
| Event | `report.changed`、`interpretation.completed`、`interpretation.failed` |
