...
这篇文档不是重新讲 Survey、Scale、Evaluation 的所有细节；它回答一个架构判断：为什么测评业务不能做成一个“Assessment 大模块”。这个判断背后有三个变化原因：

| 变化原因 | 如果不拆界会怎样 | 拆界后的归属 |
| -------- | ---------------- | ------------ |
| 问卷题型和提交校验变化 | 评估 pipeline 被题型细节污染 | Survey |
| 量表计分和解读规则变化 | 历史 Assessment 与规则定义互相污染 | Scale |
| 测评状态、报告和失败补偿变化 | 采集模型被异步执行状态污染 | Evaluation |

三界分离不是为了目录好看，而是为了让变化沿着业务生命周期发生：采集模板和答卷事实有自己的生命周期，规则有自己的生命周期，测评实例也有自己的生命周期。

## 设计判断矩阵

| 判断问题 | Survey | Scale | Evaluation |
| -------- | ------ | ----- | ---------- |
| 这是用户提交的原始事实吗 | 是 | 否 | 否 |
| 这是可复用的医学/心理规则吗 | 否 | 是 | 否 |
| 这是某一次测评的状态或产出吗 | 否 | 否 | 是 |
| 需要状态机吗 | Questionnaire lifecycle / AnswerSheet submit | Scale lifecycle | Assessment state machine |
| 主要模式 | 校验策略、题型扩展、durable submit | 策略模式、领域服务、值对象 | 职责链、状态机、Builder、Outbox |
| 最大取舍 | 版本和答卷兼容复杂 | 规则变更不自动重算历史 | 异步补偿和失败排障复杂 |

读者可以用这张表判断新需求应该落在哪个模块：如果答案落在多个列，通常说明需求需要拆成多个模型，而不是硬塞进一个聚合。
