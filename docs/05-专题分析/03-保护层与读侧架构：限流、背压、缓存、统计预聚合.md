...
**与前后专题互参**：异步链路上的 **SubmitQueue、MQ、worker 消费**见 [02-异步评估链路：从答卷提交到报告生成.md](./02-异步评估链路：从答卷提交到报告生成.md)（本篇只写 SubmitQueue 在「保护层」中的定位，机制细节链到 `02`）；三界模型见 [01-测评业务模型：survey、scale、evaluation 为什么分离.md](./01-测评业务模型：survey、scale、evaluation%20为什么分离.md)。扩写过程用的工作底稿见 [03-保护层与读侧架构-扩写提纲.md](./03-保护层与读侧架构-扩写提纲.md)。
...
1. [开篇摘要（先读这几条）](#开篇摘要先读这几条)
2. [指标与日志索引（观测）](#指标与日志索引观测)
3. [30 秒了解系统](#30-秒了解系统)
4. [三类压力与主要手段对照](#三类压力与主要手段对照)
5. [核心架构](#核心架构)
6. [请求路径：读链路与写链路](#请求路径读链路与写链路)
7. [核心设计原则](#核心设计原则)
8. [入口保护层：HTTP 限流与 SubmitQueue](#入口保护层http-限流与-submitqueue)
9. [依赖保护层：下游背压](#依赖保护层下游背压)
10. [读侧加速：对象查询缓存](#读侧加速对象查询缓存)
11. [示例读链路（问卷详情与量表列表对照）](#示例读链路问卷详情与量表列表对照)
12. [读侧加速：统计预聚合与 worker 写入](#读侧加速统计预聚合与-worker-写入)
13. [worker 中的 Redis（与 apiserver 对照）](#worker-中的redis与-apiserver-对照)
14. [失败表现与可观测性](#失败表现与可观测性)
15. [与专题 02 的衔接（吞吐与压测）](#与专题-02-的衔接吞吐与压测)
16. [为什么不把它简单概括成「三级缓存」](#为什么不把它简单概括成三级缓存)
17. [关键设计点（概要）](#关键设计点概要)
18. [边界与注意事项](#边界与注意事项)
...
*写作约定见 [CONTRIBUTING-DOCS.md](../CONTRIBUTING-DOCS.md)。扩写提纲见 [03-保护层与读侧架构-扩写提纲.md](./03-保护层与读侧架构-扩写提纲.md)。*
