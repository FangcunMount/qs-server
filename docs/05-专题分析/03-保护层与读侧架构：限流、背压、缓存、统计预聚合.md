...
| 事件重复或乱序 | worker 重复处理、投影延迟 | Lock lease / pending / reconcile |
...
| Lease / Gate | Lock lease、SubmitGuard、duplicate gate | 明确 leader/idempotency/duplicate 三类语义 |
...
- Lock lease 没有自动续租或 fencing token；长任务需要单独评估 TTL 和失败语义。
