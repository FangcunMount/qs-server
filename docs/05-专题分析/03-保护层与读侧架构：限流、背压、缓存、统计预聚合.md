...
1. **三层分工**：入口（HTTP 限流、SubmitQueue）→ 依赖（MySQL/Mongo/IAM **in-flight 背压**）→ 读侧（apiserver **对象缓存** + **统计查询缓存 / MySQL 统计读模型**；worker 侧 Redis 现在主要用于锁，**不是**第二套对象缓存）。
...
5. **观测入口**：HTTP 见 **Gin Prometheus `/metrics`**（需开启 `metrics`）；缓存 family 状态、readiness 与 payload 指标统一走 [internal/pkg/cacheobservability/metrics.go](../../internal/pkg/cacheobservability/metrics.go)；细节见下节「指标与日志索引」。
...
| 指标与 family 状态 | [internal/pkg/cacheobservability/metrics.go](../../internal/pkg/cacheobservability/metrics.go) + [internal/pkg/cacheobservability/runtime_snapshot.go](../../internal/pkg/cacheobservability/runtime_snapshot.go)：统一维护 family 级命中、写入、payload、hotset 和 readiness 快照 |

### 统计与 worker

| 项 | 说明 |
| -- | ---- |
| worker | 当前 worker 侧 Redis 主要承担分布式锁，不再承担统计 Redis 增量写入；典型入口见 [answersheet_handler.go](../../internal/worker/handlers/answersheet_handler.go) 与 [internal/pkg/redislock/lock.go](../../internal/pkg/redislock/lock.go) |
| Redis 键前缀 | 统计查询缓存见 [statistics/cache.go](../../internal/apiserver/infra/statistics/cache.go) 的 `BuildStatsQueryKey`；量表列表键见 [global_list_cache.go](../../internal/apiserver/application/scale/global_list_cache.go) 中对 `BuildScaleListKey` 的使用 |
...
- **读侧层**：apiserver 内 **对象查询缓存**（Redis）+ **统计读模型**（MySQL 统计表 + 查询结果缓存）；worker 侧再用 Redis 做 **分布式锁** 等控制能力，与 apiserver 的对象读缓存**不是同一职责**。
...
- [internal/apiserver/infra/statistics/cache.go](../../internal/apiserver/infra/statistics/cache.go)、[internal/apiserver/application/statistics/sync_service.go](../../internal/apiserver/application/statistics/sync_service.go)、[internal/apiserver/runtime/scheduler/statistics_sync.go](../../internal/apiserver/runtime/scheduler/statistics_sync.go)
...
- **目录**：[internal/apiserver/infra/cache/](../../internal/apiserver/infra/cache/)：各 `*_cache.go` 为领域对象装饰器；[singleflight.go](../../internal/apiserver/infra/cache/singleflight.go) 防缓存击穿；[store.go](../../internal/apiserver/infra/cache/store.go) 定义最小存储接口；[hotset.go](../../internal/apiserver/infra/cache/hotset.go) 负责热榜记录。
- **TTL 与抖动**：统一收口到 [internal/apiserver/infra/cachepolicy/policy.go](../../internal/apiserver/infra/cachepolicy/policy.go) 与 [internal/apiserver/infra/cachepolicy/catalog.go](../../internal/apiserver/infra/cachepolicy/catalog.go)，由对象级策略和 family 默认策略共同决定。
...
[internal/apiserver/infra/statistics/cache.go](../../internal/apiserver/infra/statistics/cache.go) 当前已经收口成 3 类统计键：

- 查询结果缓存
- 事件幂等键
- 问卷 daily 中转键

它们不仅是「查不到再回源」的 cache，更是当前读侧统计模型的一部分。

### 当前统计读侧已从“Redis 增量写入”收口为“查询缓存 + MySQL 统计读模型”

[internal/apiserver/infra/statistics/cache.go](../../internal/apiserver/infra/statistics/cache.go) 当前只负责统计查询结果缓存：

- `stats:query:{cacheKey}`

MySQL 统计表的重建则由 [sync_service.go](../../internal/apiserver/application/statistics/sync_service.go) 与 [statistics_sync.go](../../internal/apiserver/runtime/scheduler/statistics_sync.go) 负责，从原始业务表重建 `statistics_daily / statistics_accumulated / statistics_plan`。夜间任务通过共享锁层串行化执行，并在完成后调用治理层触发统计 warmup。旧的 `event:processed:*`、`stats:daily:*`、window / accumulated / distribution 键族已经不再是当前运行时代码的一部分。
...
| 用途 | 说明 | 代码入口（示例） |
| ---- | ---- | ---------------- |
| 分布式锁 | 处理消息时防并发重入（如答卷相关） | [answersheet_handler.go](../../internal/worker/handlers/answersheet_handler.go) + [internal/pkg/redislock/lock.go](../../internal/pkg/redislock/lock.go) |
| 计划相关串行控制 | 计划调度和同类后台任务通过共享锁层串行化 | [internal/apiserver/runtime/scheduler/plan_scheduler.go](../../internal/apiserver/runtime/scheduler/plan_scheduler.go) + [internal/pkg/redislock/spec.go](../../internal/pkg/redislock/spec.go) |
