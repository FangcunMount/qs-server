...
| 认证与范围投影 | JWT、UserIdentity、TenantDomain、OrgScope、AuthzSnapshot |
...
## 5. 第二层保护：JWT / OrgScope / AuthzSnapshot 投影
...
它和 apiserver 对齐 Principal / OrgScope / AuthzSnapshot 语义，但不做 ActiveOperator 校验，因为 collection 面向前台用户，不是后台 operator。
...
所以 collection 必须理解当前用户是谁、在哪个 IAM 授权域 / QS 业务组织范围内。
