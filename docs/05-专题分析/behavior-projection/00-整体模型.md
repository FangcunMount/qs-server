...
**本文回答**：行为投影中的 footprint、pending event、assessment episode 和 journey daily aggregate 分别是什么，以及它们为什么不能合并成一个表。
...
| `statistics_journey_daily` | 面向统计查询的行为旅程日聚合读模型 |
...
flowchart LR
    ...
    episode --> projection["statistics_journey_daily"]
    pending --> episode
    pending --> projection
```
...
flowchart TB
    ...
    subgraph Projection["apiserver statistics projector"]
        ...
        projection["statistics_journey_daily"]
    end
...
| `StatisticsJourneyDaily` | 读模型 | 面向统计 API 的行为旅程日聚合结果 |
