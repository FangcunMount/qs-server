...
| 8 | IAM 安全链路图 | Principal / TenantScope / AuthzSnapshot / Capability 如何分层授权 |
...
```text
JWT、Principal、TenantScope、AuthzSnapshot、CapabilityDecision 分别负责什么？
...
- 讲 MBTI / BigFive 扩展时
- 讲 `interpretation-model.changed` 和 `interpretation.completed` 区别时
...
```mermaid
flowchart TB
    Changed["interpretation-model.changed<br/>规则 / 目录变化"]
...
```text
这里最重要的是区分两类事件。
interpretation-model.changed 是规则变化，主要触发缓存失效、列表重建、Context warmup 和读模型刷新。
...
| 8 | 高并发治理：分层保护链 |
| 9 | 安全链路：Principal / TenantScope / AuthzSnapshot |
| 10 | 工程质量：代码、契约、文档、证据 |
| 11 | 演进路线：从稳定性到多模型平台化 |
...
| 高并发治理 | `06-高并发治理讲法.md`、`../03-基础设施/resilience/README.md` |
