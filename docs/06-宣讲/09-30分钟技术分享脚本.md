...
| 26:00 - 28:00 | IAM 与安全 | 讲清 Principal / OrgScope / AuthzSnapshot / Capability |
...
| 横切能力 | 影响 |
| -------- | ---- |
| Event | `report.changed`、`interpretation.completed` |
...
> **qs-server 不自己实现完整 IAM，而是通过 IAMModule 嵌入 IAM SDK。HTTP 和 gRPC 都先用 IAM TokenVerifier 验证 token，投影成 Principal；再用 OrgScope 确定当前 tenant_domain / org_id；需要业务授权时，通过 IAM GetAuthorizationSnapshot 拉取当前用户在当前 domain 下的 resource/action 权限，再用 CapabilityDecision 判断能不能读取问卷、管理解释模型、读取答卷或触发评估。**
>
