...
5. `qs-apiserver` 会在一个 Mongo 事务里同时写入答卷、业务幂等记录和 `answersheet.submitted` outbox；真正出站由 apiserver relay 补发，把链路交给 worker。证据见 [durable_submit.go](../../internal/apiserver/infra/mongo/answersheet/durable_submit.go) 和 [server.go](../../internal/apiserver/server.go)。
...
| `已实现` | 答卷写库成功后，`answersheet.submitted` 通过 outbox/relay 最终补发，不再暴露“保存成功但事件直接 publish 失败就丢失”的窗口 | [durable_submit.go](../../internal/apiserver/infra/mongo/answersheet/durable_submit.go)、[server.go](../../internal/apiserver/server.go) |
...
| 写库后发事件 | 当前可以明确讲成“答卷 + outbox 同事务，relay 最终补发” | 不能把这个结论外推到所有事件；目前只覆盖 `answersheet.submitted` |
...
- “主状态成功、异步没起”：当前对 `answersheet.submitted` 已经用 outbox/relay 收口；若继续追问，要说明其它事件目前大多仍是保存后 best-effort publish。
...
- 不要说“所有事件都已经有 outbox”；当前出站一致性增强只覆盖 `answersheet.submitted`。
