...
| 关键 RPC | `CalculateAnswerSheetScore`、`CreateAssessmentFromAnswerSheet`、`EvaluateAssessment`、`TagTestee`、`ProjectBehaviorEvent` |
...
sequenceDiagram
    ...
    W->>A: TagTestee / ProjectBehaviorEvent / SendTaskOpenedMiniProgramNotification
    A->>R: 标签、统计投影、通知等副作用
```
...
| -------- | ------------------------ |
| 给受试者打标签 | `TagTestee` |
| 标记重点关注 | `TagTestee(mark_key_focus)` |
