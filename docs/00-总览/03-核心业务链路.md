...
| 关键事件 | `answersheet.submitted`、`assessment.submitted`、`assessment.evaluated`、`assessment.interpreted`、`assessment.failed`、`report.generated` |
...
| Interpretation | 基于 execution outcome 与 Definition report map 选择 builder，组装 InterpretReport |
...
目标边界中，Evaluation 成功后 Assessment 停在 `evaluated` 并发布 `assessment.evaluated`；Interpretation 生成 Report 后发布 `report.generated`。`assessment.interpreted` 仅作为迁移期兼容事件或查询投影，不再驱动 Assessment 聚合状态。报告失败也不能把已成功的 Assessment 标记为 failed。
