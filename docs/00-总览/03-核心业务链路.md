...
flowchart LR
    ...
    e[EvaluationCommitter<br/>Outcome + evaluated]
    i[Interpretation Outcome Use Case<br/>报告生成与状态机]

	    v --> f --> r --> e -->|assessment.evaluated| i
```
...
| EvaluationCommitter | 可靠提交 EvaluationOutcome、Run、score projection、Assessment evaluated 和 outbox |
| Interpretation Outcome Use Case | 只读 EvaluationOutcome，维护 Report 独立状态机并持久化报告 |

### 4.3 成功与失败事件

| 场景 | 状态变化 | 事件 |
| ---- | -------- | ---- |
| 评估成功 | submitted -> evaluated | `assessment.evaluated` |
| 报告成功 | Assessment 不变；Report -> generated | `report.generated` |
| 评估失败 | submitted -> failed | `assessment.failed` |
| 报告失败 | Assessment 保持 evaluated；Report -> failed | Report 状态持久化 |
| 不需要评估 | 跳过 | 以 service 分支为准 |

`assessment.interpreted` 仅作为历史数据/消费者兼容契约；新写路径不再发布它，也不再驱动 Assessment 聚合状态。旧 API 需要 `interpreted` 时，由查询层根据 Assessment 和已生成 Report 派生。
