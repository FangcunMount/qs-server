...
| 进程 | 开发配置 | 生产基线 | 独立 cache policy | 环境变量前缀 |
| --- | --- | --- | --- | --- |
| `qs-apiserver` | [`configs/apiserver.dev.yaml`](../../configs/apiserver.dev.yaml) | [`configs/apiserver.prod.yaml`](../../configs/apiserver.prod.yaml) | `configs/cache/apiserver.{dev,prod}.yaml` | `QS_APISERVER_` |
| `collection-server` | [`configs/collection-server.dev.yaml`](../../configs/collection-server.dev.yaml) | [`configs/collection-server.prod.yaml`](../../configs/collection-server.prod.yaml) | `configs/cache/collection-server.{dev,prod}.yaml` | `COLLECTION_SERVER_` |
| `qs-worker` | [`configs/worker.dev.yaml`](../../configs/worker.dev.yaml) | [`configs/worker.prod.yaml`](../../configs/worker.prod.yaml) | 无 | `QS_WORKER_` |
...
apiserver 与 collection-server 主配置只通过 `cache.policy_file` 引用 cache policy。相对路径以主配置文件所在目录解析，因此从其他工作目录启动不会改变解析结果。policy、匹配的主配置与镜像是一个原子发布单元；旧 inline `cache.capabilities/defaults/governance` 不再兼容。`report_status` 不属于业务 Cache-Aside，本次切换随同 cache policy 独立化直接迁移到三进程统一的 `runtime_state.report_status`，不设置额外观察窗口。
...
| `cache.policy_file` | `qs-apiserver` | `QS_APISERVER_CACHE_POLICY_FILE` |
| `runtime_state.report_status.ttl_seconds` | `qs-worker` | `QS_WORKER_RUNTIME_STATE_REPORT_STATUS_TTL_SECONDS` |

`report_status` 的另外两个对应键是 `QS_APISERVER_RUNTIME_STATE_REPORT_STATUS_TTL_SECONDS` 与 `COLLECTION_SERVER_RUNTIME_STATE_REPORT_STATUS_TTL_SECONDS`；三进程 TTL 必须保持一致。
...
AI 解读另有 build-tag 隔离的事务与冷启动 schema 门禁。它们使用统一的 fail-closed Replica Set harness，动态创建并在结束后删除唯一测试库；显式加上 `integration` tag 后缺少 URI 会直接失败，不会静默跳过：

```bash
QS_SERVER_TEST_MONGO_URI='mongodb://...' \
QS_SERVER_TEST_MONGO_DB_PREFIX='qs_ai_explanation_contract' \
go test -count=1 -tags integration \
  ./internal/apiserver/infra/mongo/interpretation/aiexplanation \
  ./internal/pkg/migration \
  -run '^(TestAIExplanation.*IsAtomicOnReplicaSet|TestAIExplanationMongo(ColdStartEnforcesRuntimeSchema|TTLMonitorDeletesExpiredRecords))$'
```

该门禁包含四条测试链：Participant 注入请求 Outbox、Run 创建和终态 Outbox 三类失败，验证日预算、规范 Input Generation、活跃执行槽、Run/Generation 终态和 Outbox；Prompt 评测注入 start/continuation Outbox 失败，并验证完整预算、EvaluationRun、attempt checkpoint、后续 step 和同机构并发唯一性；空库按标准 migration 升级到最新版本后，验证规范 `input_json`/结构化 `content` 持久化、关键唯一约束、六个 TTL index 参数和主体导出索引顺序；另向六个 TTL 集合写入已过期记录并等待 Mongo TTL Monitor 实际删除。Mongo 默认扫描周期下测试最多等待 90 秒；仓库 CI 的一次性 Mongo 将 `ttlMonitorSleepSecs` 设为 1 秒，并把该测试等待上限收紧为 15 秒。它们仍不替代 MQ 崩溃恢复和目标环境部署验收。
...
新增或修改 Event/Signal 时，应按 [Event / Signal 扩展与验收](../03-基础设施/event/70-扩展与验收.md) 执行，不在本文复制完整操作步骤。
