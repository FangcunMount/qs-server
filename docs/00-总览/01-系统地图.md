...
| 领域模块 | `survey / assessment-model / evaluation / interpretation-model / actor / plan / statistics` 七个限界上下文主要装配在 `qs-apiserver` 内；当前代码包名见 `container/modules/registry.go` |
...
| Assessment Model | 模型资产 | `AssessmentKind`、发布快照、模型绑定、Scale / Personality payload | `../02-业务模块/20-assessment-model/README.md` |
| Evaluation | 测评执行 | `Assessment`、`EvaluationRun`、执行状态机、执行结果、失败 / 重试 | `../02-业务模块/30-evaluation/README.md` |
| Interpretation Model / Report | 解释报告 | `InterpretReport`、report builder、score/personality adapter、报告持久化 | `../02-业务模块/40-interpretation-model/README.md` |
...
| 4. 评估 | pipeline 执行校验、计分、风险、解读和结果保存 | Assessment Model、Evaluation | worker -> apiserver |
| 5. 报告 | builder / adapter 把执行结果聚合成 InterpretReport | Interpretation Model / Report | apiserver |
...
- `InterpretReport` / Statistics 解决**结果解释与读侧查询**问题。
...
| 领域模块是七个微服务 | 错。`survey / assessment-model / evaluation / interpretation-model / actor / plan / statistics` 是 apiserver 内的限界上下文 |
...
| 领域模型 | `../../internal/apiserver/domain/survey/`、`../../internal/apiserver/domain/assessmentmodel/`、`../../internal/apiserver/domain/evaluation/`、`../../internal/apiserver/domain/report/`、`../../internal/apiserver/domain/actor/`、`../../internal/apiserver/domain/plan/`、`../../internal/apiserver/domain/statistics/` |
