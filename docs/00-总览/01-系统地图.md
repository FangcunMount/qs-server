...
| 领域模块 | `survey / model-catalog / evaluation / report / actor / plan / statistics` 七个限界上下文主要装配在 `qs-apiserver` 内；当前代码包名见 `container/modules/registry.go` |
...
| 领域模块是七个微服务 | 错。`survey / model-catalog / evaluation / report / actor / plan / statistics` 是 apiserver 内的限界上下文 |
