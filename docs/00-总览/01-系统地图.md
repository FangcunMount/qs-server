...
| 维度 | 结论 |
| ---- | ---- |
| 系统定位 | `qs-server` 是问卷与测评后端：负责前台答卷收集、模型资产治理、异步测评、解释报告、统计聚合和计划任务推进 |
...
| 领域模块 | `survey / model-catalog / evaluation / interpretation / actor / plan / statistics` 七个限界上下文主要装配在 `qs-apiserver` 内；当前代码包名见 `container/modules/registry.go` |
...
一句话概括：**qs-server 把一次问卷作答，稳定地转化为可解释、可追踪、可查询的测评结果与解释报告。**
...
qs-server 的领域模块主要装配在 `qs-apiserver` 内，不是七个独立微服务。
...
| ---- | ---------- | --------------- | -------- |
| Survey | 采集事实 | `Questionnaire`、`Question`、`AnswerSheet`、`Answer`、题型校验、答卷提交 | `../02-业务模块/10-survey/README.md` |
| Assessment Model | 模型资产 | `AssessmentKind`、发布快照、模型绑定、Scale / Personality payload | `../02-业务模块/20-model-catalog/README.md` |
| Evaluation | 测评执行 | `Assessment`、执行状态机、`AssessmentOutcome`、失败 / 重试 | `../02-业务模块/30-evaluation/README.md` |
| Interpretation Model / Report | 解释报告 | `ReportGeneration`、`InterpretationRun`、report builder、不可变 `InterpretReport`、查询 catalog | `../02-业务模块/40-interpretation/README.md` |
| Actor | 参与者视图 | `Testee`、Clinician、Operator、关系与标签 | `../02-业务模块/50-actor/README.md` |
| Plan | 计划与任务 | `AssessmentPlan`、`AssessmentTask`、开放 / 完成 / 过期 / 取消 | `../02-业务模块/60-plan/README.md` |
| Statistics | 读侧聚合 | 机构概览、漏斗、测评服务、计划统计、缓存与同步 | `../02-业务模块/70-statistics/README.md` |

这七个模块共同支撑主叙事：

```text
Survey 采集作答事实
  -> Assessment Model 提供测评模型资产
  -> Evaluation 执行一次测评
  -> Interpretation Model / Report 生成最终解释报告
...
| 阶段 | 业务含义 | 主要模块 | 主要运行时 |
| ---- | -------- | -------- | ---------- |
| 1. 建模 | 后台维护问卷、模型资产、发布快照、解读文案 | Survey、Assessment Model、Interpretation Model / Report | apiserver |
...
| 4. 评估 | 运行时描述符执行输入组装、计算、Outcome 组装与可靠提交 | Assessment Model、Evaluation | worker -> apiserver |
| 5. 报告 | `evaluation.outcome.committed` 驱动 Generation / Run，builder 生成并可靠提交 InterpretReport | Interpretation Model / Report | worker -> apiserver |
| 6. 追踪 | 报告、标签、统计、计划任务继续推进 | Actor、Plan、Statistics | apiserver / worker / scheduler |

因此，读 qs-server 时不要把 `Questionnaire`、`PublishedModelSnapshot`、`Assessment`、`InterpretReport` 混成一个对象：

- `Questionnaire` 解决**采集结构**问题；
- `PublishedModelSnapshot` / `Model Payload` 解决**模型资产与发布快照**问题；
- `Assessment` 解决**一次测评执行事实与结果**问题；
- `ReportGeneration` / `InterpretationRun` 解决**报告幂等生成与重试**问题，`InterpretReport` / Statistics 解决**结果解释与读侧查询**问题。
...
| gRPC 契约 | `../../api/grpc/gen/` | collection / worker 调 apiserver 的服务定义 |
| internal gRPC | `../../api/grpc/gen/internalapi/internal.proto` | worker 回调 apiserver 的内部动作，例如创建测评、执行评估、打标签 |
...
| 领域模块是七个微服务 | 错。`survey / model-catalog / evaluation / interpretation / actor / plan / statistics` 是 apiserver 内的限界上下文 |
...
| 想改问卷、模型资产、测评执行、解释报告、计划等业务 | `../02-业务模块/README.md` |
...
| 领域模型 | `../../internal/apiserver/domain/survey/`、`../../internal/apiserver/domain/modelcatalog/`、`../../internal/apiserver/domain/evaluation/`、`../../internal/apiserver/domain/interpretation/`、`../../internal/apiserver/domain/actor/`、`../../internal/apiserver/domain/plan/`、`../../internal/apiserver/domain/statistics/` |
| REST 契约 | `../../api/rest/apiserver.yaml`、`../../api/rest/collection.yaml` |
| gRPC 契约 | `../../api/grpc/gen/` |
