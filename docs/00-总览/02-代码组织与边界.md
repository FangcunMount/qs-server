...
| `domain/ruleset/scale/definition` | 医学量表编排、因子、计分与解读规则定义 |
| `domain/evaluation/assessment` | 一次测评行为、状态机、风险等级、失败与重试 |
| `domain/interpretation/scale` | Scale 运行时计分与解读 |
...
| Scale | `internal/apiserver/domain/ruleset/scale/definition` | `internal/apiserver/application/scale` | Mongo/MySQL scale mapper、规则适配等 | [`docs/02-业务模块/scale`](../02-业务模块/scale/) |
...
| 量表因子、计分规则、解读规则 | `domain/ruleset/scale/definition`、`application/scale` |
