...
| 维度 | 典型路径 | 说明 |
| ---- | -------- | ---- |
| **运行时** | `cmd/*`、`internal/{apiserver,collection-server,worker}` | 三个可部署进程，各自 `app.go` / `run.go` / `process/*` 启动与装配 |
...
│   ├── worker/
│   │   ├── integration/eventing/       # 事件 dispatcher 与显式 handler registry 适配
│   │   ├── integration/messaging/      # MQ subscribe、message extract、Ack/Nack runtime
│   │   ├── handlers/                   # 事件处理器
│   │   ├── infra/                      # gRPC 客户端、通知、Lock lease adapter 等
│   │   ├── observability/              # metrics / governance HTTP
│   │   └── container/
