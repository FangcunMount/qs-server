...
`container/root.go` 以 `AssessmentModelModule` 作为模型目录组合根；人格模型仅保留兼容投影字段，不再存在 `ScaleModule`。文档层对应为 `survey / model-catalog / evaluation / report / actor / plan / statistics`。
