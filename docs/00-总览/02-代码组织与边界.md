...
| 业务模块注册 | 以 `internal/apiserver/container/modules/registry.go` 为准；当前业务包是 `survey / modelcatalog / evaluation / report / actor / plan / statistics` |
...
`container/root.go` 仍保留 `ScaleModule`、`PersonalityModelModule` 等字段作为 `AssessmentModelModule` 的兼容别名；这不是新的独立核心模块。文档层对应为 `survey / model-catalog / evaluation / report / actor / plan / statistics`。
...
| `domain/modelcatalog` | 测评模型资产、模型身份、发布快照、Scale / Personality 等具体模型 payload |
| `domain/modelcatalog/scale/definition` | 医学量表模型资产的因子、计分与解读规则定义 |
| `domain/evaluation/assessment` | 一次测评行为、状态机、风险等级、失败与重试 |
| `domain/evaluation/scale` | Scale 运行时计分与解读 |
| `domain/interpretation` | 文档层 `interpretation` 的当前代码落点，负责 `InterpretReport`、报告 builder、adapter 与解读产物 |
...
| Assessment Model | `modelcatalog`，兼容注册 `scale/personalitymodel` | `internal/apiserver/domain/modelcatalog`、`internal/apiserver/domain/modelcatalog/scale` | `internal/apiserver/application/modelcatalog`，旧能力路径仍包括 `application/scale`、`application/personalitymodel` | modelcatalog/scale Mongo mapper、catalog、规则适配等 | [`docs/02-业务模块/20-model-catalog`](../02-业务模块/20-model-catalog/) |
...
| Interpretation Model / Report | `interpretation` | `internal/apiserver/domain/interpretation` | 当前主要由 `container/modules/interpretation` 装配 builder registry 与 durable saver | report builder registry、report repo、adapter | [`docs/02-业务模块/40-interpretation`](../02-业务模块/40-interpretation/) |
...
| 测评模型资产、发布快照、模型 payload | `domain/modelcatalog`、`application/modelcatalog` |
| 医学量表因子、计分规则、解读规则 | `domain/modelcatalog/scale/definition`、`application/scale` |
| 测评状态、评估、报告、失败重试 | `domain/evaluation`、`application/evaluation` |
| 最终解释报告、报告 builder、adapter | `domain/interpretation`、`container/modules/interpretation` |
