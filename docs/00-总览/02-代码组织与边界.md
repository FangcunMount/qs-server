...
| 机器契约 | REST 看 `api/rest/*.yaml`，gRPC 看 `api/grpc/gen/`，事件看 `configs/events.yaml`，配置看 `configs/*.yaml` |
| 业务模块注册 | 以 `internal/apiserver/container/modules/registry.go` 为准；当前业务包是 `survey / assessmentmodel / evaluation / report / actor / plan / statistics` |
...
`qs-apiserver` 是系统的业务中心。它负责装配七个业务包，持有 MySQL / Mongo / Redis / MQ / IAM / WeChat / OSS 等基础设施适配，暴露 REST 和 gRPC，并启动 outbox relay、scheduler、cache warmup 等后台 runtime。
...
业务模块注册事实以 [`container/modules/registry.go`](../../internal/apiserver/container/modules/registry.go) 为准。当前 `BusinessPackages` 包含七个业务包：
...
`container/root.go` 仍保留 `ScaleModule`、`PersonalityModelModule` 等字段作为 `AssessmentModelModule` 的兼容别名；这不是新的独立核心模块。文档层对应为 `survey / assessment-model / evaluation / interpretation-model / actor / plan / statistics`。

这说明业务模块不是微服务，而是 apiserver 内部的限界上下文。容器初始化顺序大体是：事件发布器、Survey、AssessmentModel（兼容 scale/personalitymodel）、Actor、Report、Evaluation、Plan、Statistics、cache governance、protected scope、CodesService、QRCode 等。
...
| `domain/assessmentmodel` | 测评模型资产、模型身份、发布快照、Scale / Personality 等具体模型 payload |
| `domain/assessmentmodel/scale/definition` | 医学量表模型资产的因子、计分与解读规则定义 |
| `domain/evaluation/assessment` | 一次测评行为、状态机、风险等级、失败与重试 |
| `domain/evaluation/scale` | Scale 运行时计分与解读 |
| `domain/report` | 文档层 `interpretation-model` 的当前代码落点，负责 `InterpretReport`、报告 builder、adapter 与解读产物 |
...
- Survey、Assessment Model、Evaluation、Interpretation Model / Report 等业务聚合。
- 只服务某个模块的应用服务。
...
| gRPC 契约 | [`api/grpc/gen/`](../../api/grpc/gen/) | collection / worker 调 apiserver 的 proto 契约 |
| internal gRPC | [`internalapi/internal.proto`](../../api/grpc/proto/internalapi/internal.proto) | worker 回调 apiserver 的内部服务契约 |
...
七个业务模块不是微服务，而是 apiserver 内的限界上下文。文档名称优先表达业务边界；代码包名保留当前实现事实。

| 文档模块 | 代码包 / 注册名 | 领域落点 | 应用层落点 | 典型基础设施落点 | 文档落点 |
| -------- | --------------- | -------- | ---------- | ---------------- | -------- |
| Survey | `survey` | `internal/apiserver/domain/survey` | `internal/apiserver/application/survey` | Mongo answersheet、questionnaire repo/cache 等 | [`docs/02-业务模块/10-survey`](../02-业务模块/10-survey/) |
| Assessment Model | `assessmentmodel`，兼容注册 `scale/personalitymodel` | `internal/apiserver/domain/assessmentmodel`、`internal/apiserver/domain/assessmentmodel/scale` | `internal/apiserver/application/assessmentmodel`，旧能力路径仍包括 `application/scale`、`application/personalitymodel` | assessmentmodel/scale Mongo mapper、catalog、规则适配等 | [`docs/02-业务模块/20-assessment-model`](../02-业务模块/20-assessment-model/) |
| Evaluation | `evaluation` | `internal/apiserver/domain/evaluation` | `internal/apiserver/application/evaluation` | assessment repo、evaluation input resolver、report saver 调用边界 | [`docs/02-业务模块/30-evaluation`](../02-业务模块/30-evaluation/) |
| Interpretation Model / Report | `report` | `internal/apiserver/domain/report` | 当前主要由 `container/modules/report` 装配 builder registry 与 durable saver | report builder registry、report repo、adapter | [`docs/02-业务模块/40-interpretation-model`](../02-业务模块/40-interpretation-model/) |
| Actor | `actor` | `internal/apiserver/domain/actor` | `internal/apiserver/application/actor` | actor read model、testee/clinician repo/cache | [`docs/02-业务模块/50-actor`](../02-业务模块/50-actor/) |
| Plan | `plan` | `internal/apiserver/domain/plan` | `internal/apiserver/application/plan` | plan/task repo、scheduler、通知上下文 | [`docs/02-业务模块/60-plan`](../02-业务模块/60-plan/) |
| Statistics | `statistics` | `internal/apiserver/domain/statistics` | `internal/apiserver/application/statistics` | statistics read model/cache/projection | [`docs/02-业务模块/70-statistics`](../02-业务模块/70-statistics/) |
...
| 测评模型资产、发布快照、模型 payload | `domain/assessmentmodel`、`application/assessmentmodel` |
| 医学量表因子、计分规则、解读规则 | `domain/assessmentmodel/scale/definition`、`application/scale` |
| 测评状态、评估、报告、失败重试 | `domain/evaluation`、`application/evaluation` |
| 最终解释报告、报告 builder、adapter | `domain/report`、`container/modules/report` |
...
| “`internal/pkg` 是业务领域层” | 错。它是三进程共享横切实现，不承载 Survey/Assessment Model/Evaluation 等业务语义 |
...
3. [`internal/apiserver/container/modules/registry.go`](../../internal/apiserver/container/modules/registry.go)：看当前业务模块注册事实和 legacy 注册名。
...
| 七个业务模块如何划界 | [`../02-业务模块/README.md`](../02-业务模块/README.md) |
