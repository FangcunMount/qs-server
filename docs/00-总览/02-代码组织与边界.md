...
这说明业务模块不是微服务，而是 apiserver 内部的限界上下文。容器初始化顺序大体是：事件发布器、Survey、AssessmentModel（兼容 scale/typologymodel）、Actor、Report、Evaluation、Plan、Statistics、cache governance、protected scope、CodesService、QRCode 等。
...
| Assessment Model | `modelcatalog`，兼容注册 `scale/typologymodel` | `internal/apiserver/domain/modelcatalog`、`internal/apiserver/domain/modelcatalog/scale` | `internal/apiserver/application/modelcatalog`，旧能力路径仍包括 `application/scale`、`application/typologymodel` | modelcatalog/scale Mongo mapper、catalog、规则适配等 | [`docs/02-业务模块/20-model-catalog`](../02-业务模块/20-model-catalog/) |
