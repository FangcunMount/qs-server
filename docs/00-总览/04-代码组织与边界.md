...
>
> 本文解释 qs-server 的代码为什么这样组织、依赖允许朝哪里走，以及开发者应当怎样定位和修改代码。系统上下文和三个进程的运行关系见[系统地图](./03-系统地图.md) 与[三进程协作总览](../01-运行时/00-三进程协作总览.md)；
> 单个业务模块的领域模型与关键链路见 `docs/02-业务模块/`。
...
例如，Worker 接收 `answersheet.submitted` 并驱动 Evaluation，不代表 Evaluation 属于 Worker；collection-server 接收 AnswerSheet 提交，
也不代表 AnswerSheet 聚合属于 collection-server。
...
当前源码通过 `TestApplicationsUsePortsForInfraBoundaries` 明确禁止 Application 导入 Mongo Driver、IAM 生成包和 `internal/apiserver/infra`。
这说明“应用层依赖抽象”已经是可执行约束，而不只是文档愿景。
...
Container 是允许同时看见 Application、Infra、Transport 和模块装配包的最外层。它的职责正是把依赖连接起来，因此“组合根依赖很多包”本身不是分层违规；真正需要控制的是业务规则不能向组合根泄漏，
以及组合根不能退化成随处可写的 Service Locator。
...
- platform 与 IAM 集成包；
- 当前模块安装顺序；
- 业务模块需要具备的装配与 bootstrap 文件；
- Transport export 文件约定；
- platform 集成 bootstrap 文件。

当前 `ModuleAssemblyFiles` 与 `ModuleBootstrapFiles` 已覆盖七个业务模块；`ModuleTransportExportFiles` 记录对外 Transport 约定，`PlatformBootstrapFiles` 单独记录平台集成入口。
这些注册表与架构测试共同保护 `container/modules/*` 的组合根边界。

### 6.4 当前安装顺序：`BusinessInitializationSequence`

模块内部装配已经迁入各自 package，最外层 `Container.Initialize` 仍通过 `module_init.go` 中的
`initSurveyModule`、`initReportModule`、`initModelCatalogModule` 等显式方法控制安装顺序。注册表使用 `BusinessInitializationSequence` 把这条顺序固化为机器可验证的当前契约。
...
显式顺序本身不是遗留兼容代码。后续只有在真实装配复杂度需要模块描述符或依赖图时，才应替换这条契约；替换时必须同步更新注册表与架构测试。
...
| 目录 | 责任 |
| --- | --- |
| `handlers/` | 事件解码、业务调用和结构化结果分类；不直接拥有 transport ACK/NACK |
| `integration/eventing/` | 事件目录与 Handler 注册 |
| `integration/messaging/` | MQ 消费、持久化 retry hold 和 ACK/NACK 传输结算 |
...
事件输入
  ...
  -> 返回成功、持久化业务处置或未分类错误
  -> messaging runtime 决定 ACK / NACK / hold
```
...
| 责任 | 所有者 |
| --- | --- |
| 判断 Evaluation/Interpretation 失败是否可自动恢复 | 对应业务模块与 Run |
| 记录下一次业务 attempt 的授权 | Run Repository + 可靠 retry event |
| 控制 Outbox 发布预算 | Mongo/MySQL Outbox Store |
| 处理消息 ACK、NACK、暂停 hold 与运输耗尽 | Worker messaging runtime / 通用 event transport |
| 查询候选并执行人工 Retry、Force Retry 或 Replay | apiserver System Governance |

`internal/pkg/retrygovernance` 只提供跨层共享的处置术语、策略快照和硬上限，不拥有任何业务聚合，也不直接执行重试。
...
| `BusinessInitializationSequence` 显式维护安装顺序 | 当前有效的组合根契约 | 只有出现真实依赖图需求时再演进，并同步架构测试 |
