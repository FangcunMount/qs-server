...
当前 `ModuleAssemblyFiles` 与 `ModuleBootstrapFiles` 已覆盖七个业务模块；`ModuleTransportExportFiles` 记录对外 Transport 约定，`PlatformBootstrapFiles` 单独记录平台集成入口。这些注册表与架构测试共同保护 `container/modules/*` 的组合根边界。

### 6.4 当前安装顺序：`BusinessInitializationSequence`

模块内部装配已经迁入各自 package，最外层 `Container.Initialize` 仍通过 `module_init.go` 中的 `initSurveyModule`、`initReportModule`、`initModelCatalogModule` 等显式方法控制安装顺序。注册表使用 `BusinessInitializationSequence` 把这条顺序固化为机器可验证的当前契约。
