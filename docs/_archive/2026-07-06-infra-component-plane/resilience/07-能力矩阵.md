...
| Runtime Status | 三进程 | 当前保护能力只读状态 | `resilience.RuntimeSnapshot` | 只展示，不调参、不 drain、不 release | ready/degraded snapshot | [05-观测降级与排障.md](./05-观测降级与排障.md) |
...
| 能力 | 必测路径 | 代码锚点 |
| ---- | -------- | -------- |
| RateLimit | allowed / limited / Retry-After / degraded-open | `internal/pkg/resilience/ratelimit`、`internal/pkg/middleware` |
| SubmitQueue | accepted / full / duplicate / failed / TTL cleanup / snapshot | `internal/collection-server/application/answersheet` |
| Backpressure | nil limiter / acquire / timeout / release / snapshot | `internal/pkg/resilience/backpressure` |
...
| RuntimeSnapshot | degraded count / ready / bounded status | `internal/pkg/resilience` |
| Metrics | low-cardinality labels / outcome mapping | `internal/pkg/resilience` |
