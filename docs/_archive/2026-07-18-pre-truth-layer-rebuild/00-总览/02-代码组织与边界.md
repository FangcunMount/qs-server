...
| 业务模块注册 | 以 `internal/apiserver/container/modules/registry.go` 为准；当前业务包是 `survey / modelcatalog / evaluation / interpretation / actor / plan / statistics` |
...
`container/root.go` 以 `AssessmentModelModule` 作为模型目录组合根；人格模型仅保留兼容投影字段，不再存在 `ScaleModule`。文档层对应为 `survey / model-catalog / evaluation / interpretation / actor / plan / statistics`。

这说明业务模块不是微服务，而是 apiserver 内部的限界上下文。容器初始化顺序大体是：事件发布器、Survey、AssessmentModel、Actor、Report、Evaluation、Plan、Statistics、cache governance、protected scope、CodesService、QRCode 等。
...
| `domain/modelcatalog` | 测评模型资产、模型身份、`DefinitionV2`、测量/校准/结论等领域语义 |
| `domain/evaluation/assessment` | 一次测评行为、状态机、风险等级、失败与重试 |
| `domain/calculation/scoring` | 共享因子计分规则 |
| `domain/interpretation` | 报告生成边界，负责 `ReportGeneration`、`InterpretationRun`、渲染 registry、Draft 与不可变 `InterpretReport` |
...
| Assessment Model | `modelcatalog` | `internal/apiserver/domain/modelcatalog` | `internal/apiserver/application/modelcatalog` 五类用例服务和 definition strategy | modelcatalog Mongo mapper、published resolver、payload adapter | [`docs/02-业务模块/20-model-catalog`](../02-业务模块/20-model-catalog/) |
...
| Interpretation Model / Report | `interpretation` | `internal/apiserver/domain/interpretation` | `internal/apiserver/application/interpretation` 生成编排与多角色查询 | Mongo Generation / Run / Artifact / Catalog repository、builder registry、Outcome fact port | [`docs/02-业务模块/40-interpretation`](../02-业务模块/40-interpretation/) |
...
| 医学量表因子、计分规则、解读规则 | `domain/modelcatalog/definition`、`application/modelcatalog/definition`、`domain/interpretation/scoring` |
...
| Redis / Cache / Lock | `internal/pkg/cacheplane`、`internal/pkg/resilience/locklease`、相关 infra/cache、resilience docs |
