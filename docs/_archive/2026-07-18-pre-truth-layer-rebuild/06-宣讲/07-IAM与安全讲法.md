...
**本文回答**：对外介绍 qs-server 时，如何把 IAM 接入、认证、授权、TenantDomain、OrgScope、业务主体、服务间调用、CapabilityDecision、AuthzSnapshot、模型管理权限和报告访问权限讲清楚；如何说明“JWT 证明你是谁，但不等于你能做什么”；为什么不能直接用 JWT roles 做业务授权；为什么能管理 MBTI 模型规则，不等于能查看用户 MBTI 报告；以及如何在面试中把安全边界讲得准确、不夸大。
...
> **JWT 证明“你是谁”，OrgScope 证明“在哪个 IAM 授权域和 QS 业务组织范围”，AuthzSnapshot 判断“你能做什么”，CapabilityDecision 把 IAM 权限翻译成 qs-server 的业务能力。**
...
> **qs-server 不直接用 JWT roles 做业务权限，而是用 IAM 验证身份，再通过 OrgScope、AuthzSnapshot 和 CapabilityDecision 判断当前用户能不能读答卷、看报告、管理模型或触发测评执行。**

---

## 3. 30 秒讲法

> **qs-server 通过 IAMModule 接入 IAM。请求进来后，HTTP / gRPC 中间件先验证 JWT 或 Service Token，投影成 Principal；再解析 OrgScope，确认当前调用发生在哪个 IAM 授权域和 QS 业务组织范围；如果业务接口需要授权，就通过 AuthzSnapshotLoader 向 IAM 拉取当前 subject 在 qs-server domain 下的 resource/action 权限快照；最后由 CapabilityDecision 判断是否具备具体业务能力，比如读取问卷、提交答卷、读取 Assessment、管理解释模型、读取用户报告或查看统计。这里不会直接信任 JWT roles，因为 roles 可能滞后、粒度粗，也无法表达 resource/action 和业务关系。**
...
>
> **请求进来以后，HTTP 和 gRPC 都会先通过 IAM 的 TokenVerifier 验证 token。token 验证后不会直接进入业务，而是先投影成 Principal，表达当前调用者是谁；再通过 OrgScope 确定当前请求发生在哪个 IAM 授权域和 QS 业务组织范围内。对于需要业务授权的接口，qs-server 会通过 AuthzSnapshotLoader 调 IAM 的 GetAuthorizationSnapshot，拿到当前用户在当前 domain 下的 roles、permissions、authz_version，再通过 CapabilityDecision 判断某个业务能力是否允许。**
>
...
>
> **所以这套设计的核心不是“我用了 JWT”，而是把身份、授权域 / 组织范围、业务主体、授权快照、业务 capability、服务身份和本地投影拆开，不把它们混成一个 roles 判断。**
...
OrgScope 说明调用发生在哪个 IAM 授权域 / QS 业务组织范围。
...
| tenant / org 声明 | OrgScope 使用和业务隔离 |
...
> **OrgScope 解决 IAM 授权域和 QS 业务组织范围问题。**

它通常同时保存：

```text
TenantDomain：IAM 授权域，来自 JWT tenant_id
OrgID：QS 业务组织范围，来自 JWT org_id
HasOrgID：JWT org_id 是否是有效业务组织 ID
```

### 11.2 为什么 tenant_id 和 org_id 不能混为一谈

IAM 的 tenant_id 是授权域声明，可能是 `fangcun` / `platform` 这样的字符串。

QS 的 org_id 是业务数据隔离的组织 ID。

如果混用，会出现：

- 把授权域误当业务组织范围。
- org_id=0 误当合法组织。
- 每个 handler 自己解析 claim。
- Casbin domain 和 QS org 边界混乱。

### 11.3 面试讲法

> **我没有在业务 handler 里把 tenant_id 当 org_id，而是把 TenantDomain 和 OrgID 统一投影成 OrgScope。这样能明确区分 IAM/Casbin domain 与 QS 业务数据范围。**
...
| 无上下文 | 难表达 tenant_domain/org_id/resource/scope |
...
> **服务间调用和用户请求是两个安全问题。用户请求要校验 Principal、OrgScope 和 Capability；内部调用要校验 ServiceIdentity，必要时再做 mTLS identity match。**
...
> **HTTP 入口先验证 token，再投影身份、TenantDomain 和 OrgScope，再加载授权快照，最后按业务 capability 判断。这个顺序很重要：没有身份就没有 scope，没有 scope 就不能加载正确 domain 下的 snapshot，没有 snapshot 就不能做 capability。**

### 19.2 collection 和 apiserver 差异

| 进程 | HTTP 安全重点 |
| ---- | ------------- |
| collection-server | 前台用户身份、TenantDomain、OrgScope、监护关系、AuthzSnapshot |
...
并且按 tenant_domain / org_id 范围过滤。

讲法：

> **统计不是天然安全的。聚合数据在小样本范围下仍可能暴露隐私，因此也要按 tenant_domain / org_id 和 capability 控制。**
...
```text
JWT -> Principal -> OrgScope -> Guardian/Testee relationship -> submit_answersheets -> collection submit -> apiserver durable save
```
...
```text
JWT -> Principal -> OrgScope -> Clinician relationship -> read_interpretation_reports -> report query
```

讲法：

> **医生能查看报告，不是因为他登录了，而是因为他在当前 tenant_domain / org_id 范围内与受试者或任务有关联，并且具备 read_interpretation_reports capability。**
...
```text
JWT -> Principal -> OrgScope -> Operator -> manage_interpretation_models -> MBTIModel publish / archive
```
...
```text
Operator JWT -> Principal -> OrgScope -> manage_governance / manage_assessment_repair -> repair action -> audit
```
...
> **认证上，通过 IAM TokenVerifier 验证 JWT 或 service token，形成 Principal；授权域和组织范围通过 OrgScope 表达；授权上，不直接信 JWT roles，而是通过 AuthzSnapshotLoader 从 IAM 获取 resource/action 权限快照，再由 CapabilityDecision 判断具体业务能力。**
...
> **不能。collection 是前台保护层，可以做入口校验和幂等保护，但主业务事实和最终授权仍应在 apiserver 收口。collection 调 apiserver 需要 ServiceAuth，同时业务请求仍携带用户身份和 OrgScope。**
...
> **需要。统计虽然是聚合数据，但仍可能泄露某个机构或小范围人群的敏感测评分布，所以至少要按 tenant_domain / org_id 过滤，并需要 `read_statistics` capability。**
...
```text
统计数据也要按 tenant_domain / org_id 和 capability 控制，小样本统计仍可能泄露隐私。
```
...
第二部分是授权域、组织范围和授权。
Principal 里有 TenantDomain，OrgScope 里有 TenantDomain + OrgID；TenantDomain 来自 JWT tenant_id，OrgID 来自 JWT org_id。
...
> **qs-server 的安全设计不是简单“接口加 JWT”。我把它分成认证、授权域/组织范围、业务主体、授权快照和业务能力判断五层：JWT 或 Service Token 先通过 IAM TokenVerifier 验证，形成 Principal；OrgScope 确定当前 tenant_domain / org_id；Actor 投影说明这个人在测评业务里是 testee、clinician 还是 operator；AuthzSnapshot 从 IAM 拉取 resource/action 权限快照；CapabilityDecision 再判断能不能读取问卷、提交答卷、读取报告、管理解释模型、查看统计或执行治理动作。**
>
> **这里最重要的是不要把 JWT roles 当成业务权限真值。JWT 证明“你是谁”，OrgScope 证明“在哪个授权域和组织范围”，AuthzSnapshot 才判断“你能做什么”。Operator 本地 roles 只是业务投影，不是授权事实源。多解释模型之后，还要把模型规则管理和用户报告访问拆开：能管理 MBTI 模型规则，不等于能查看用户 MBTI 报告。服务间调用则通过 ServiceAuth 和可选 mTLS identity match 保护，worker 和 collection 的服务身份不能替代用户权限。**
...
| 判断 | 证据 |
| ---- | ---- |
| Security Control Plane 统一 Principal、OrgScope、AuthzSnapshot、CapabilityDecision、ServiceIdentity | `docs/03-基础设施/security/01-安全模块整体架构.md` |
| IAM 是认证和授权真值，JWT roles 不是 capability 真值 | `docs/03-基础设施/security/01-安全模块整体架构.md` |
| HTTP/gRPC 都有 snapshot 注入链路 | `docs/03-基础设施/security/01-安全模块整体架构.md` |
| ServiceIdentity 来自 service auth bearer metadata 与 mTLS identity | `docs/03-基础设施/security/01-安全模块整体架构.md` |
| OperatorRoleProjection 是本地投影，不是权限真值 | `docs/03-基础设施/security/01-安全模块整体架构.md` |
