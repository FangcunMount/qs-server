...
- hotset。
- sdk_token。
- `redis_runtime.families.lock_lease` 选择 Redis profile/namespace；`lock_lease.renewal_enabled` 控制自动续租，代码默认 `false`。三个 dev 配置显式为 `true`，三个 prod 配置在分组上线前显式为 `false`。
...
- gRPC max in-flight。
- worker concurrency。
- `system_governance.resilience.*` 动作默认全部关闭；生产按动态限流、队列 drain/resume、Leader 让权的顺序逐项启用。
- collection 的 `resilience.control.enabled` 默认 `true`；为 true 时首次 desired state 同步完成前 `/readyz` 保持 503。仅在全部 resilience action 关闭且不存在 paused 维护状态时，才允许显式设为 `false`。
- 实例心跳 key 包含每次启动生成的 generation；治理命令和结果仍按稳定的 instance_id 定位。
- generation 使用系统随机源生成；随机源失败会阻止控制代理启动，不会降级为固定 generation。
- MySQL 审计终态写失败时，ops Redis 使用无 TTL 的 `governance:audit-replay:<org_id>:<request_id>` 保存待恢复终态；回填完成前不得人工删除。
