...
- 抢到锁：执行本轮。
- 抢不到：skip。
- 执行期按 `TTL / 3` 续租，任务使用 Runner 提供的 child context。
- 丢锁或续租 Redis 故障：取消本轮任务并返回 tick failure，下轮重试。
- 正常父 context 取消不记为 renew error；body 不响应取消时按续租周期持续告警。
...
3. Redis `lock_lease` family、`qs_locklease_operation_total` 的 renew/lost/canceled 结果以及 scheduler tick failure；canceled 不应伴随 family degraded。
