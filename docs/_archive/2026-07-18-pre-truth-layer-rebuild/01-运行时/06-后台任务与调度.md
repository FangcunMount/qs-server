...
apiserver 后台 runtime 统一启动 EventSubsystem。Subsystem 内有两个 Outbox profile：

| Profile | 条件 | 作用 |
| ----- | ---- | ---- |
| `mongo_domain_events` | Mongo store 存在，且 MQ publisher 可用 | 排空答卷与 Interpretation Mongo durable events |
| `assessment_mysql_events` | MySQL store 存在，且 MQ publisher 可用 | 排空 Evaluation durable events |

代码锚点：

- [`internal/apiserver/process/runtime_bootstrap.go`](../../internal/apiserver/process/runtime_bootstrap.go)
- [`internal/apiserver/eventing/subsystem/subsystem.go`](../../internal/apiserver/eventing/subsystem/subsystem.go)

relay loop 的核心语义：

```text
按 outbox_relay.<store>.interval：
  -> DispatchDue(ctx)
  -> 单轮最多 claim outbox_relay.<store>.batch_size 条
  -> 最多 outbox_relay.<store>.publish_workers 个并发 publish
  -> 成功 publish 的事件按 worker 尺寸分批 MarkEventsPublished
  -> 成功则继续
  -> 失败记录 warning
...
`mongo-domain-events` 在 claim pending 事件时按核心业务事件优先级排空；行为旅程不经过 Mongo Outbox，而由 `behavior_journey_scan` 独立轮询投影。
...
| `lock_lease.renewal_enabled` | apiserver / worker / collection | 启用 TTL/3 自动续租；代码默认关闭 |
...
2. `org_ids` 是否包含目标机构；
3. Redis lock 是否可用，并检查 `lock_lost` / `lock_renew_error`；
...
4. `outbox_relay.<store>.interval` / `batch_size` 是否足以覆盖写入速率；
5. outbox due events 是否 claim 成功；
6. 事件对应 topic 是否在 `events.yaml` 中配置；
7. worker 是否订阅了该 topic。
