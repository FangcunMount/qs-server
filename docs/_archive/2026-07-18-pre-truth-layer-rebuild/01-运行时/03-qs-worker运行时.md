...
| 容器阶段 | 创建 worker container，持有 logger、LockLease Runner/key builder、event catalog 和后续注入的 gRPC clients |
...
| 阶段 | 代码锚点 | 主要产物 | 失败影响 |
| ---- | -------- | -------- | -------- |
| resources | `process/resource_bootstrap.go` | Redis runtime、LockLease subsystem、EventCatalog | Redis / event config 初始化失败会阻断启动 |
...
container 的初始化不是在 container stage 完成的，而是在 integration stage 注入 gRPC ClientBundle 后才执行 `container.Initialize()`。`answersheet_processing` 初次 Redis 获取失败仍 degraded-open；但获取后丢锁或续租失败会取消 handler context 并返回错误，由 MQ 进入 NACK/retry。父 context 取消不走 degraded-open。

原因是：event dispatcher 创建 handler factory 时，需要 gRPC clients、LockLease Runner、notifier 等依赖；如果没有 gRPC client，handler 虽然能被注册，但真正处理事件时就缺少执行能力。
...
| ModelCatalog | 响应 `assessment_model.changed` 触发发布后动作 | AssessmentModel、DefinitionV2 和已发布模型的权威来源 |
...
| Redis lock | worker duplicate suppression 依赖 LockLease Runner |
...
| 新增锁或重复抑制 | LockLease 能力目录、keyspace、Runner 调用方、resilience docs |
