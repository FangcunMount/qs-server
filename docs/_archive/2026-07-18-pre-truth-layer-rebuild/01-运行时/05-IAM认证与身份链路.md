...
**本文回答**：`qs-server` 三进程运行时中 IAM 如何嵌入，JWT、TenantDomain、OrgScope、AuthzSnapshot、Guardianship、ServiceAuth、mTLS / ACL 分别发生在哪一层，以及 collection、apiserver、worker 在身份链路中的职责边界。
...
| 前台入口身份链路 | Client 带 JWT 进入 collection REST；collection 使用 IAM TokenVerifier、本地 JWKS 或远程验证，并投影 Principal、TenantDomain、OrgScope |
| 后台入口身份链路 | 管理端或内部调用进入 apiserver REST；apiserver protected routes 使用 JWT、TenantDomain、OrgScope、ActiveOperator、AuthzSnapshot 中间件链 |
...
| 监护关系 | collection 提交答卷时通过 IAM ProfileLinkService 校验填写者与受试者关系 |
...
1. **用户身份边界**：JWT、user_id、tenant_domain、org_id 进入 collection 或 apiserver REST；JWT `tenant_id` 表达 IAM 授权域，JWT `org_id` 表达 QS 业务组织范围。
...
| ---- | ------------ | ------------ | ---------- |
| collection-server | 前台 JWT 验签、用户身份投影、TenantDomain/OrgScope 检查、authz snapshot、监护关系校验、可选 service auth 出站凭证 | `internal/collection-server/container/iam_module.go`、`transport/rest/router.go`、`application/answersheet/submission_service.go` | 不做后台 operator active 校验；不拥有业务权限最终真值 |
| qs-apiserver | 后台 REST JWT 验签、TenantDomain、OrgScope、ActiveOperator、AuthzSnapshot、gRPC auth/mTLS/ACL/Audit、IAM client 与授权快照 loader | `internal/apiserver/container/iam.go`、`transport/rest/registrars.go`、`process/transport_bootstrap.go` | 不把 IAM 当成本地业务模块；不替代 IAM 权限源 |
...
sequenceDiagram
    ...
    R->>R: project user_id / tenant_domain / org_id
...
3. 如果 IAM ProfileLinkService 启用，并且 testee 绑定了 IAM profile，则调用 `HasActiveProfileLink` 校验填写者是否有权为该受试者提交。
...
| TenantDomain | JWT `tenant_id` / Casbin domain | IAM 授权域 |
| OrgScope | JWT `org_id` | QS 业务组织范围 |

collection 和 apiserver 都会要求 `tenant_domain` 和 `org_id`。apiserver 还可能要求 ActiveOperator，因为它是后台和主业务权威入口。
...
4. `tenant_domain` / `org_id` 是否存在且格式正确。
...
3. testee 是否绑定 IAM profile。
4. ProfileLinkService 是否启用。
5. `HasActiveProfileLink` 返回是否为 true。
...
| 修改 collection 监护校验 | collection SubmissionService、IAM ProfileLinkService adapter | 提交链路文档、survey AnswerSheet 文档 |
