...
> **Principal 说明“你是谁”，OrgScope 说明“在哪个 IAM 授权域和 QS 业务组织范围”，AuthzSnapshot + CapabilityDecision 才说明“你能不能做这件事”。**
...
sequenceDiagram
    ...
    participant Scope as Identity + OrgScope
    ...
    Scope->>MW: user_id + tenant_domain + org_id
    MW->>IAM: Load(tenantDomain,userID)
...
1. loader nil -> 503。
2. tenantDomain/userID 缺失 -> 400。
3. loader.Load(tenantDomain,userID) 失败 -> 503。
4. 成功后：
...
2. 从 context 读取 tenantDomain/userID。
...
```text
Principal.TenantDomain
  -> OrgScope
  -> tenantDomain + userID
  -> SnapshotLoader.Load(tenantDomain,userID)
  -> Snapshot.CasbinDomain / permissions
  -> CapabilityDecision
```

如果 `tenant_domain` 缺失或缺少有效 `org_id`：

- HTTP 业务路由会被 scope middleware 拒绝。
- gRPC authz snapshot interceptor 会对缺失 `org_id` 返回 InvalidArgument。
