...
| 解释模型扩展 | 新增 MBTI / BigFive 等解释模型时，需要明确规则模型、执行结果、报告快照、统计读模型分别落在 Mongo、MySQL 或 ReadModel 的哪一层 |
...
Interpretation Model persistence
...
04-ReadModel与Statistics
  -> ../../02-业务模块/70-statistics/README.md
  -> 05-新增持久化能力SOP
```
...
- 解释模型规则是否是文档型聚合。
- 规则模型是否需要版本快照。
- `MBTIModel / TypeProfile / DimensionRule` 更适合 Mongo Document 还是 MySQL 表。
- `EvaluationResult / InterpretReport / EvaluationRun` 是否仍归 Evaluation 持久化。
- MBTI 维度分数、TypeCode、ProfileResult 如何进入结果快照。
- 统计报表是否需要新增 read model 表或扩展现有统计投影。
- 是否需要 migration、新索引、唯一约束和回填脚本。
...
| 解释模型规则，例如 MBTIModel / TypeProfile | Mongo 优先 | 规则结构嵌套、版本化、整体加载更自然 |
| EvaluationRun / 执行尝试记录 | MySQL 优先 | 需要按 Assessment、attempt、状态查询和审计 |
| EvaluationResult / ProfileResult 快照 | Mongo 或 MySQL，按当前 Evaluation 事实源统一 | 需要稳定追溯，不能散落在具体模型模块 |
| InterpretReport 快照 | Mongo 优先，或沿用当前 Report 持久化方案 | 报告结构通常是文档型、可版本化、可渲染 |
...
天然是大文档和快照的聚合 -> Mongo
解释模型规则结构复杂、需要版本快照 -> Mongo 优先
一次测评执行状态和尝试记录 -> MySQL 优先
一次测评结果和报告快照 -> 归 Evaluation 事实源统一保存
...
| 数据 | 建议存储 | 说明 |
| ---- | -------- | ---- |
| `MBTIModel` | Mongo Document | 模型规则、维度、题目映射、版本信息适合整体加载 |
| `DimensionRule` | Mongo 内嵌或子文档 | 属于 MBTIModel 的规则组成部分 |
| `TypeProfile` | Mongo 内嵌或独立 collection | 16 类型画像结构较文档化，可随模型版本快照 |
| `MBTIContext` | Redis cache，可回源 Mongo | 只读规则上下文，不是事实源 |
| `EvaluationResult` | Evaluation 结果仓储 | 保存 DimensionScore、TypeCode、ProfileResult 等执行结果 |
| `InterpretReport` | Evaluation 报告仓储 | 保存报告快照和渲染数据 |
| MBTI 统计 | Statistics ReadModel | 用于按模型、组织、时间、TypeCode、维度分布查询 |

核心原则：

```text
具体解释模型保存“规则事实”；
Evaluation 保存“本次执行事实”；
Statistics ReadModel 保存“查询优化事实”；
Redis 只缓存“可回源的派生数据”。
```
...
| Interpretation Model Document | MBTI / BigFive 等解释模型规则文档、版本快照和查询索引 |
...
如果新增 MBTI 等解释模型，统计读模型可以按需要新增或扩展：
...
这些表只服务查询和统计，不反向成为 MBTIModel 或 EvaluationResult 的主事实源。
...
assessment.created / assessment.completed
    应与 Assessment 状态变化处于同一可靠边界。

interpretation.completed / interpretation.failed
    应与解释结果、失败状态或执行记录处于同一可靠边界。

report.generated
    应与 InterpretReport 保存处于同一可靠边界。
```
...
### 12.6 新解释模型不反向污染 Evaluation 表

新增 MBTI / BigFive 等解释模型时，不应为每种模型在 `Assessment` 主表上新增大量专用字段。

推荐方式：

```text
Assessment 保存 ModelRef / 状态 / 结果引用；
具体模型模块保存规则事实；
EvaluationResult 保存结构化执行结果快照；
InterpretReport 保存报告快照；
Statistics ReadModel 保存统计查询投影。
```

如果某个字段只属于 MBTI 的规则或画像，不应该塞进 Assessment 主模型。
...
### 13.7 “新增 MBTI 就要改 Assessment 表结构”

不一定。

Assessment 是一次测评执行事实，不是具体解释模型规则表。

MBTI 的规则、维度、类型画像应优先由 MBTI 模块持久化；MBTI 的执行结果应进入 EvaluationResult / InterpretReport 快照；统计查询再投影到 Statistics ReadModel。
...
| MBTI 规则加载失败 | Mongo 模型文档 / version index / published snapshot |
| MBTI 结果查不到 | EvaluationResult / InterpretReport 事实源 |
| MBTI 统计不准 | Statistics ReadModel / projector / QueryCache |
...
### Interpretation Model / Evaluation Result

- Interpretation Model domain：[../../../internal/apiserver/domain/](../../../internal/apiserver/domain/)
- Evaluation domain：[../../../internal/apiserver/domain/evaluation/](../../../internal/apiserver/domain/evaluation/)
- Evaluation infra：[../../../internal/apiserver/infra/](../../../internal/apiserver/infra/)
- Statistics read model：[../../../internal/apiserver/infra/mysql/statistics/readmodel/](../../../internal/apiserver/infra/mysql/statistics/readmodel/)
