...
| `report.changed` | `interpretation_model_changed_handler` | 解释模型或报告模型变化后动作，例如 Context cache 失效、读模型刷新 |

注意：`report.changed` 表示规则变化，不表示某次 Assessment 的解释执行完成；某次测评中的解释结果应由 `interpretation.completed` / `interpretation.failed` 表达。
