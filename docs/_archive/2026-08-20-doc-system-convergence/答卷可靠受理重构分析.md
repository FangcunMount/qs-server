...
### 13.3 已退役的 SubmitQueue（历史决策）

方案 B 的进程内队列与 worker-pool 文件已删除，不再作为当前源码入口。防回流边界由 [`reliable_submission_architecture_test.go`](../../internal/collection-server/application/answersheet/reliable_submission_architecture_test.go) 保护。
...
| SubmitQueue 退役防回流 | [`reliable_submission_architecture_test.go`](../../internal/collection-server/application/answersheet/reliable_submission_architecture_test.go) |
