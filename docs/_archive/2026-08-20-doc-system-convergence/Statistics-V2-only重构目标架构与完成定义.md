...
Run 的 `as_of_date` 等于请求的包含式 `to_date`。其中 `publish` 必须结束于最近完整上海自然日；历史局部窗口只允许 `validate/repair`，非法 publish 在创建 SyncRun 前返回参数错误。
...
MySQL 来源通过显式二元 keyset 推进，不使用只按主键续页的 `FindInBatches`。MongoDB 来源必须同时声明稳定排序和匹配的机构级联合索引；跨机构扫描后再过滤不是允许的实现。
...
- [x] 运行时只有 Collector → Projection → Read Service；
- [x] Scheduler 只执行唯一 publish Coordinator；
- [x] validate、repair、publish、resume-cache 均有测试；
- [x] `/api/v2/statistics`、Plan Enrollment 与 System Governance 可用；
- [x] Statistics V1 REST/gRPC/配置/缓存键全部缺席；
- [x] qs-operating-system 只调用 canonical API；
- [ ] migration 前向、空库和 down 演练通过；
- [ ] 冷启动 E2E 与重复运行幂等验证通过；
- [ ] 所有机构首个 publish 成功并具有明确 freshness；
- [x] 全量 Go 测试、前端测试与构建、OpenAPI 和静态 ratchet 通过。

> 2026-07-22 本地验收记录：MySQL 9.3 一次性空库已通过全量 migration、57/56 down/up 和 000005→000007 字段归属演练；冷启动 `integration` 合同已编写并通过编译。当前本地 Mongo Replica Set 端口需要认证，但未提供独立测试凭据，因此 Mongo 索引 up/down 和完整冷启动 E2E 仍未勾选，不以静态合同替代真实环境证据。
