# repair_stranded_plan_tasks 历史生产参数快照（归档）

> 原页混入机构、cutoff、候选数量、日期和内部 URL，仅供追溯。现行工具契约见 `scripts/oneoff/repair_stranded_plan_tasks/README.md`；任何真实执行参数与结果进入基础设施生产证据台账。
