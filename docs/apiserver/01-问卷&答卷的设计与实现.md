...
func (VersionService) Publish(q *Questionnaire) error {
    ...
    if q.GetStatus() != STATUS_DRAFT {
        return errors.WithCode(code.ErrStatusInvalid, "只有草稿状态才能发布")
    }
...
// 注册单选问题
func init() {
    RegisterQuestionFactory(question.RadioQuestion, func(builder *QuestionBuilder) Question {
        // 创建单选问题
...
// 注册多选问题
func init() {
    RegisterQuestionFactory(question.CheckboxQuestion, func(builder *QuestionBuilder) Question {
        // 创建多选问题
...
// 注册文本问题
func init() {
    RegisterQuestionFactory(question.TextQuestion, func(builder *QuestionBuilder) Question {
        // 创建文本问题
...
// 注册数字问题
func init() {
    RegisterQuestionFactory(question.NumberQuestion, func(builder *QuestionBuilder) Question {
        // 创建数字问题
...
// NewAnswerSheet 创建答卷
func NewAnswerSheet(questionnaireCode string, Version string, opts ...AnswerSheetOption) *AnswerSheet {
    a := &AnswerSheet{
...
func WithVersion(Version string) AnswerSheetOption {
    return func(a *AnswerSheet) {
        a.Version = Version
    }
}
...
radioQuestion := question_types.BuildQuestionConfig(
    ...
    question_types.WithQuestionType(question.RadioQuestion),
...
func TestRadioQuestion_Creation(t *testing.T) {
    ...
    assert.Equal(t, question.RadioQuestion, question.GetType())
...
func TestAnswerValue_Creation(t *testing.T) {
    // 测试单选答案值创建
    value := answer_values.NewAnswerValue(question.RadioQuestion, "A")
    
    assert.NotNil(t, value)
    assert.Equal(t, "A", value.Raw())
    
    // 测试数字答案值创建
    numberValue := answer_values.NewAnswerValue(question.NumberQuestion, 42)
