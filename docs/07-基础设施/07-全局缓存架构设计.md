...
- **redis-cache** (6379)：用于缓存，临时数据，可丢失
- **redis-store** (6380)：用于持久化存储，队列，发布订阅

### 2.2 缓存数据应该使用 redis-cache

**所有缓存数据（量表、问卷、统计查询等）都应该使用 redis-cache**，原因：

1. **语义清晰**：redis-cache 是"缓存"，redis-store 是"存储"
2. **数据可恢复**：缓存数据可以从 MongoDB 恢复，不需要持久化
3. **TTL 策略**：缓存需要设置合理的 TTL，自动过期
4. **架构职责**：符合"缓存"和"存储"的职责分离

**详细说明**：参见 `docs/07-基础设施/08-Redis双实例架构说明.md`
...
**注意**：所有缓存装饰器都应该使用 `redis-cache`，而不是 `redis-store`。
