...
本规划只聚焦问卷&量表 BC 在 `qs-apiserver` 内部的 **领域层**，把我们前面已经讨论清楚的内容（领域模块拆分、聚合设计、校验/计分策略、Evaluator 职责链、qs-worker 评估流程）整理成一组可维护的设计文档。

目标：

* 与现有的《01-架构设计总览》《01-01-问卷领域设计》《10-Redis 消息队列实现》等文档风格保持一致；
* 让“领域建模 → 代码结构 → 校验&计分 → 评估工作流”形成一条闭环；
* 后续可以按本文档规划逐篇落地为独立的 Markdown 文件。
...
当前“问卷&量表 BC”在 qs-apiserver 内部，适合新增/细化以上 4 篇设计文档：
...
下一步可以按优先级依次落地正文，一般建议先写 **01-02 总览** 和 **01-03 代码结构**，再把第三、第四篇作为“细化章节”补上。
