...
部署验收：`make perf-mixed280-models`（确认 setup 日志 `report_mode=websocket`、`report_ws_query`）；4C/8G 目标 **checks 100%**。
...
前置：`mixed_280_models` 全绿 + apiserver 线 B 已部署 + `report_events.enabled=true`。
