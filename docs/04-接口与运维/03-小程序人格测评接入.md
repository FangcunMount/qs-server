...
> 本文已由 [15-小程序接入文档.md](./15-小程序接入文档.md) 取代。当前 `submit-status=done` 会同时返回 `answersheet_id` 与 `assessment_id`；下文保留为历史说明，不再作为现行契约。

**本文回答**：小程序如何通过 collection-server REST 完成「选模型 → 拿题 → 提交 → 等报告 → 看结果」全链路；哪些接口必须带登录态、`testee_id`、精确题版 `version`。

> **R121 破坏性变更（待前端联调更新 assessment_id 获取方式）**：`GET /api/v1/answersheets/{id}/assessment` 与过渡期 `/api/v2/assessments*` 均已下线。当前详情/报告只保留 `/api/v1/typology-assessments*`；`assessment_id` 需要由提交后的异步事件、业务上下文或后续新契约提供，不能再通过 public REST 按答卷反查。
...
| 维度 | 结论 |
| ---- | ---- |
| 推荐入口 | `POST /api/v1/typology-assessment-sessions`（一次拿齐模型、题版、提交字段、后续 URL 模板） |
...
| 不推荐 | 用 `algorithm=mbti/sbti` 做产品分支；继续调用已下线的 `/api/v2/assessments*` 或按答卷反查测评端点 |

> **R108**：旧路径 `/api/v1/personality-*` 已下线，请全部改用 `/api/v1/typology-*`。

一句话：

> **先 session 锁定题版，再 submit，再取得 assessment_id，最后带 testee_id 通过 typology-assessments 拉报告。**
...
sequenceDiagram
    ...
    MP->>CS: POST /typology-assessment-sessions
    ...
    Note over MP,AS: assessment_id 由异步事件、业务上下文或后续新契约提供；旧按答卷反查 REST 已下线

    loop 直到 interpreted
        MP->>CS: GET /typology-assessments/{id}/wait-report?testee_id=
        CS-->>MP: status=interpreted
    end

    MP->>CS: GET /typology-assessments/{id}/report?testee_id=
...
成功后可得 `answersheet_id`。R121 后 public REST 不再提供按答卷反查测评的端点，`assessment_id` 需通过异步事件、业务上下文或后续新契约取得。

---

### 3.3 获取 assessment_id

R121 后 public REST 不再支持 `GET /api/v1/answersheets/{answersheet_id}/assessment`，也不再提供 `/api/v2/assessments*` 过渡读面。

| 情况 | 行为 |
| ---- | ---- |
| 已有 `assessment_id` | 进入报告等待与报告读取 |
| 只有 `answersheet_id` | 等待异步事件、业务上下文或后续新契约提供 `assessment_id` |
...
**兼容（长轮询）**：`GET /api/v1/typology-assessments/{id}/wait-report?testee_id={testee_id}&timeout=20`
...
| 步骤 | 接口 |
| ---- | ---- |
| 模型详情 | `GET /api/v1/typology-models/{code}` → 读 `questionnaire_code` / `questionnaire_version` |
| 精确题版 | `GET /api/v1/questionnaires/{code}?version={version}` |
| 提交 | 同上 `POST /answersheets` |
| 列表 | `GET /api/v1/typology-assessments?testee_id=&page=` |
| 详情 | `GET /api/v1/typology-assessments/{id}?testee_id=` |

**过滤策略**：列表不再支持 `algorithm` 旧过滤参数；产品侧应使用 `model.code` 与 `categories`，不要硬编码 `mbti`/`sbti` 分支。
...
| assessment pending | public REST 不再支持按答卷反查；等待异步事件、业务上下文或后续新契约提供 `assessment_id` |
...
idle
  ...
  → assessment_id_pending（等待异步事件/业务上下文/新契约）
...
- [ ] IAM 登录态可访问 `/api/v1/typology-assessment-sessions`
- [ ] session 返回的 `questionnaire.questions` 非空
- [ ] submit 使用 session 的 `questionnaire_version`（非“当前最新”）
- [ ] `assessment_id` 获取方式已按 R121 后新契约确认，不再依赖 `answersheets/{id}/assessment`
