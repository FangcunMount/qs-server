...
| 报告安全 | `report` / `wait-report` / `详情` 均要求 `testee_id`，且必须与测评归属一致 |
...
| wait-report | 使用服务端返回的 `next_poll_after_ms` |
| report 404 | 先确认 `wait-report` 已到 `interpreted` |
...
- [ ] `wait-report` / `report` 均带同一 `testee_id`
