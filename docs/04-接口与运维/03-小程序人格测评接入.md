...
> **R110 破坏性变更（待前端联调更新本文流程）**：`GET /api/v1/answersheets/{id}/assessment`（按答卷解析测评 + pending）已下线，v2 无同名替代。本文中依赖该端点获取 `assessment_id` 的步骤已失效，`assessment_id` 的获取方式需按新契约重新确定后回填本文。测评详情/报告请改用 `/api/v2/assessments/{id}`、`/api/v2/assessments/{id}/report` 或 `/api/v1/typology-assessments/{id}`。
