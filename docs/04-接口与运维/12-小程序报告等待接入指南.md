...
字段、路径与安全声明以当前 OpenAPI 为准；变更前先执行[接口契约核验 Runbook](./00-接口契约总览.md)。目标环境是否启用 WebSocket 只能由本次 effective config 与握手验证证明。

> **可靠受理破坏性变更**：`GET /api/v1/answersheets/{id}/assessment`、`GET /answersheets/submit-status` 与过渡期 `/api/v2/assessments*` 均已下线。
> `POST /answersheets` 直接返回已可靠持久化的 `answersheet_id`；`assessment_id` 只通过 `GET /answersheets/{id}/assessment-readiness?testee_id=...` 获取。
...
| 接入门禁 | 未取得目标环境 `report_events.enabled=true` 证据时按 WS 不可用处理；**短轮询必须可独立跑通** |
...
人格测评全链路（session → submit → assessment）见 [15-小程序接入文档.md](./15-小程序接入文档.md)。
...
- 仓库配置只是版本化意图；发布前按[配置检查 Runbook](./05-配置与环境变量.md)核对目标环境 effective value。
...
建议持久化：`kind`、`testee_id`、`fingerprint`、`idempotency_key`、`request_id`、`answersheet_id`、`assessment_id`。
readiness pending 超过 60 秒时展示“答卷已接收，测评生成延迟”，不得改写为答卷提交失败。
...
| 目标环境证据 | 前端建议 |
| --- | --- |
| effective `report_events.enabled=true` 且握手/首帧验收通过 | WS + `report-status` 降级 |
| effective value 为 `false`、未知，或握手/首帧失败 | 仅 `report-status`；不得假定 WS 可用 |
...
- 人格测评全链路：[15-小程序接入文档.md](./15-小程序接入文档.md)
- collection REST 总览：[02-collection-REST.md](./02-collection-REST.md)
- OpenAPI：`api/rest/collection.yaml`（`/report-events`、`/report-status`、`/wait-report`、`/typology-assessments/.../report`）；
  医学报告见 `api/rest/apiserver.yaml`（`/evaluations/assessments/{id}/report`）
