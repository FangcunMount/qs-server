...
> **拿到 `assessment_id` 后，优先 WS 订阅；不可用则按 `next_poll_after_ms` 轮询 `report-status`；终态 `interpreted` 再拉 `report`。**
...
`personality-assessment-sessions` 返回的 `endpoints` 目前含 `wait_report`、`report`，**不含** `report_status` / `report_events`；前端按上表自行拼接即可。
...
- [ ] JWT 可访问 `report-status` 与 `report`
