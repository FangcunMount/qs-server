...
> **拿到 `assessment_id` 后，优先 WS 订阅；不可用则按 `next_poll_after_ms` 轮询 `report-status`；终态 `interpreted` 再拉 `interpretation`。**

人格测评全链路（session → submit → assessment）见 [03-小程序人格测评接入.md](./03-小程序人格测评接入.md)。
...
| ---- | ------------------------- | --------------------- |
| 短轮询状态 | `{base}/personality-assessments/{id}/report-status?testee_id={testee}` | `{base}/assessments/{id}/report-status?testee_id={testee}` |
| 长轮询（legacy） | `{base}/personality-assessments/{id}/wait-report?testee_id={testee}&timeout=20` | `{base}/assessments/{id}/wait-report?testee_id={testee}&timeout=20` |
| 报告正文 | `{base}/personality-assessments/{id}/report?testee_id={testee}` | `{base}/assessments/{id}/report?testee_id={testee}` |
| WebSocket | `wss://collect.fangcunmount.cn/api/v1/report-events` | 同左 |

`personality-assessment-sessions` 返回的 `endpoints` 目前含 `wait_report`、`interpretation`，**不含** `report_status` / `report_events`；前端按上表自行拼接即可。
...
```http
GET /api/v1/personality-assessments/{id}/report-status?testee_id={testee_id}
...
```http
GET /api/v1/personality-assessments/{id}/wait-report?testee_id={testee_id}&timeout=20
```
...
sequenceDiagram
    ...
    Note over MP: 已完成 submit，持有 answersheet_id

    MP->>CS: GET /answersheets/{id}/assessment
    CS-->>MP: pending 或 assessment_id
...
```text
assessment_pending     → 轮询 answersheets/{id}/assessment（间隔 ≥ 1s）
assessment_ready       → 持有 assessment_id
...
- [ ] JWT 可访问 `report-status` 与 `interpretation`
- [ ] 人格、`medical` 两条线 `kind` 与 URL 前缀正确
- [ ] `testee_id` 与提交时一致（字符串传参）
- [ ] 非终态不请求 `interpretation`
...
- 人格测评全链路：[03-小程序人格测评接入.md](./03-小程序人格测评接入.md)
- collection REST 总览：[02-collection-REST.md](./02-collection-REST.md)
- OpenAPI 源文件：`api/rest/collection.yaml`（`/report-events`、`/report-status`、`/wait-report`）
