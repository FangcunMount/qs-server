...
> **R121 破坏性变更**：`GET /api/v1/answersheets/{id}/assessment` 与过渡期 `/api/v2/assessments*` 均已下线。本文只描述“已有 `assessment_id` 后如何等待报告”；`assessment_id` 获取路径需由异步事件、业务上下文或后续新契约提供。
...
sequenceDiagram
    ...
    Note over MP: 已完成 submit，并通过异步事件/业务上下文/新契约取得 assessment_id
