...
当前 collection Compose 已移除固定 `container_name` 与宿主机端口映射，由 CD 使用固定 project `qs-collection` 执行 `docker compose up --scale`。Nginx 通过 Docker DNS 服务别名负载均衡。该模式只提供同机进程级冗余；跨宿主机高可用仍应使用 LB/K8s/Swarm。
