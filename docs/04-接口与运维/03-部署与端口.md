...
- **`qs-apiserver`**：同时监听 HTTP(S) 与 gRPC。
- **`collection-server`**：监听 HTTP(S)，但只作为 gRPC 客户端连 apiserver。
- **`qs-worker`**：不对外暴露业务 HTTP/gRPC，只做 MQ 与 gRPC 出站。
...
## 配置监听、容器内端口和宿主机映射为什么要分三层看

只要排障涉及“端口不通”，就必须先分清这三层：

1. **配置监听地址**：yaml 里服务声称自己监听在哪里。  
2. **容器内端口**：进程在容器内部真正监听的端口。  
3. **宿主机映射**：Compose 是否把容器端口暴露到宿主机。  

### Docker Compose 宿主机映射（当前仓库）
