...
本文档按 [CONTRIBUTING-DOCS.md](../CONTRIBUTING-DOCS.md) 的讲解维度组织。**领域事件与 Topic / handler**见 [03-基础设施/01-事件系统](../03-基础设施/01-事件系统.md)；**统计模块业务语义**见 [02-业务模块/06-statistics](../02-业务模块/06-statistics.md)。本文区分 **Crontab 触发的 internal REST**、**apiserver 进程内统计同步 ticker**、**worker 事件驱动 gRPC** 三条路径。
...
| 类型 | 触发方式 | 主入口 |
| ---- | -------- | ------ |
| **周期任务** | 宿主机 **Crontab + Shell** 调 **apiserver internal REST** | [configs/crontab/qs-scheduler](../../configs/crontab/qs-scheduler)、[api-call.sh](../../configs/crontab/api-call.sh)、[refresh-token.sh](../../configs/crontab/refresh-token.sh) |
...
| 共用服务 | 同一套同步服务可能同时被 internal REST 和 ticker 复用 |
...
| | 内容 |
| -- | ---- |
| **负责（摘要）** | Crontab 与 internal REST 路径对齐；ticker 默认与配置键；事件型后台与 worker gRPC 的边界 |
| **不负责（摘要）** | Handler 内业务规则全文；**IAM 登录脚本密钥**（脚本内占位符，勿提交真实值） |
| **关联** | [02-gRPC契约](./02-gRPC契约.md)（InternalService worker 回调契约）、[05-专题分析](../05-专题分析/) 异步叙事 |
...
**同一套同步/调度服务**可被 **internal REST** 与 **ticker** 调用；**worker 的 internal gRPC** 则用于事件回调型业务动作，和按时间触发解耦。
...
这三类后台不是“同一机制的不同写法”，而是面向不同问题：

- **Crontab + internal REST**：解决“按墙钟触发”的扫描、同步和校验。  
- **进程内 ticker**：解决“进程自己兜底同步”的保底机制。  
- **MQ + worker**：解决“由业务事件驱动”的异步后处理。  

## Crontab 触发的 REST 路径今天怎么工作

### Crontab 与 internal REST 路径（Verify）

[`qs-scheduler`](../../configs/crontab/qs-scheduler) 当前示例（每小时错开）：

| 时间偏移 | 路径 | 作用（摘要） |
| -------- | ---- | ------------ |
| `:00` | `POST /internal/v1/statistics/sync/daily` | 每日统计 Redis→MySQL |
| `:05` | `POST /internal/v1/statistics/sync/accumulated` | 累计统计同步 |
| `:10` | `POST /internal/v1/statistics/sync/plan` | 计划统计同步 |
| `:15` | `POST /internal/v1/statistics/validate` | 一致性校验 |
| `:20` | `POST /internal/v1/plans/tasks/schedule` | 扫描并开放待调度任务 |
...
事件型后台只回答一件事：**它由业务事件触发，而不是由墙钟触发**。也就是说，worker 何时启动处理，取决于 **`qs-apiserver` 是否发布了事件**，而不是取决于 Crontab 或 ticker。

因此本篇只保留运维视角下的边界：

- **Crontab / ticker** 解决“按时间扫描、同步、校验”。
- **MQ + worker** 解决“业务写完成之后的异步后处理”。
- `plans/tasks/schedule` 处理 **任务开放与扫描**；答卷提交与评估仍在 **survey → evaluation → worker** 主链路。

具体到 **Topic、Ack/Nack、`events.yaml`、`RoutingPublisher`、worker handler 分发**，统一回看 [03-基础设施/01-事件系统](../03-基础设施/01-事件系统.md)；异步评估主链的完整时序回看 [05-专题分析/02-异步评估链路：从答卷提交到报告生成](../05-专题分析/02-异步评估链路：从答卷提交到报告生成.md)。

## Internal gRPC 在这里承担什么角色

`InternalService` 现在只承载 **worker 事件回调**，不再承载统计同步、校验或计划调度这类系统动作。运维默认入口统一是 **internal REST**。详见 [02-gRPC契约](./02-gRPC契约.md)。
