...
当前生产写路径只写三对象；生产查询读取三对象和只读 `archived_reports` 历史档案投影。旧 `interpret_reports` 已完成归档、对账和离线备份后退役，不再作为运行时或 one-off 数据源。ArchivedReport 无 Outcome、不可重试，也不进入 Generation/Run 状态机。
