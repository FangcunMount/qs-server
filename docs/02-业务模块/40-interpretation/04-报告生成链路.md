...
列表查询通过轻量 `report_query_catalog` 统一分页。目录以 AssessmentID 唯一，artifact 永远优先于 archive，多个 artifact 以 `generated_at + report_id` 选择最新项。目录只返回当前页来源引用，正文仍从原成品集合批量读取；ReportID 查询只接受真实 InterpretReport ID，不再把 ArchivedReport 的 AssessmentID 当作 ReportID。
