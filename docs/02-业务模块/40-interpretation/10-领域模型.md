...
| 当前报告索引 | `report_query_catalog` 中按 Assessment 指向当前 Artifact 正文的读模型 |
...
当前它们分别存在 `report_generations` 和 `interpretation_runs` collection，通过 GenerationID、LatestRunID 以及唯一索引保持关系。
这种拆分避免每新增一次 attempt 都扩大并重写 Generation 文档。
...
当前只注册 `standard` ReportType；TemplateVersion 已有历史 `legacy-v1` 与当前 `2026-08-v1` 两个不可变发布版本。
ModelCatalog 的 active snapshot 显式冻结 TemplateID/TemplateVersion，运行时不再为缺失版本补默认值，也不跨 TemplateVersion 回落。当前限制是只运营一种 ReportType，而不是模板版本仍停留在身份骨架。
...
Builder 暴露的 `BuilderIdentity` 和 `ContentSchemaVersion` 会同时固化到 InterpretReport 与 `interpretation.report.generated` 事件。
成功提交会校验请求中的来源身份与 Artifact 完全一致，Mongo mapper 也会持久化并恢复这两个字段。

历史上缺少来源字段的 Artifact 读取时不会冒充当前 Builder，而会映射成显式的 legacy/unknown 标记。由此可以区分：

- 当前写入：成品可独立说明 Builder 与 Content schema；
- 历史存量：缺失来源时保持显式未知，不根据当前代码静默猜测；
- 事件消费：generated payload 复制 Artifact 的同一份来源证据。

## 10. report_query_catalog：不是领域聚合的查询投影

### 10.1 为什么不直接查报告正文

报告正文体积较大，列表查询如果直接在 `interpret_report_artifacts` 上执行组织、受试者、模型和风险条件筛选，会让分页和正文大小直接绑定。
...
当 Catalog 指向不存在的 Artifact 时，读模型会返回 `CatalogDanglingSourceError`，而不是悄然忽略数据损坏。
...
模板发布与成品来源两项原缺口已经关闭；下表只保留当前仍需准确表达的边界。

| 问题 | 当前实现 | 目标边界 |
| --- | --- | --- |
| 报告模板版本 | 5 个 TemplateID × `legacy-v1` / `2026-08-v1` 共 10 个 release；active ModelCatalog 路由显式冻结 | 新版本继续按不可变 manifest 发布，保留仍需重放的旧版本 |
| 成品来源证明 | BuilderIdentity 和 ContentSchemaVersion 已进入 Artifact、Mongo 与 generated 事件 | 历史缺失字段只能标记 legacy/unknown，不得冒充当前来源 |
...
| gRPC 命名 | `GenerateReportFromOutcome` 是 canonical RPC；`GenerateReportFromAssessment` 仅保留 deprecated 兼容入口并有命中指标 | 观察窗满足且取得单项删除授权后再移除旧入口 |
