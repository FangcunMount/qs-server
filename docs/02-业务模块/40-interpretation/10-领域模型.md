...
当前只注册 `standard` ReportType；TemplateVersion 已有历史 `legacy-v1` 与当前 `2026-08-v1` 两个不可变发布版本。ModelCatalog 的 active snapshot 显式冻结 TemplateID/TemplateVersion，运行时不再为缺失版本补默认值，也不跨 TemplateVersion 回落。当前限制是只运营一种 ReportType，而不是模板版本仍停留在身份骨架。
...
Builder 暴露的 `BuilderIdentity` 和 `ContentSchemaVersion` 会同时固化到 InterpretReport 与 `interpretation.report.generated` 事件。成功提交会校验请求中的来源身份与 Artifact 完全一致，Mongo mapper 也会持久化并恢复这两个字段。
...
报告正文体积较大，列表查询如果直接在 `interpret_report_artifacts`
上执行组织、受试者、模型和风险条件筛选，会让分页和正文大小直接绑定。
