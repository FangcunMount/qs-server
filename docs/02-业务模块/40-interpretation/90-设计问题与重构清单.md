...
> 状态：**实施中**。本文是 Interpretation 的活动重构台账，汇总冻结输入、Builder 路由、Generation / Run、报告成品、可靠事件、查询授权、Audience 和组合状态中已经由当前源码确认的问题。本文中的目标设计不等于已实现能力；状态必须同时记录实现阶段与验收结论。
...
| --- | --- | --- | --- | --- | --- |
| IR-R001 | P0 | medical / typology 状态缓存命中可绕过 Assessment ownership | 已发布 | 待数据/运行验收 | collection-server、报告状态、患者隐私 |
| IR-R002 | P0 | Catalog 加载正文时不复验 Assessment / Org / Testee 关联 | 已接线 | 待数据/运行验收 | 报告正文越界、一致性损坏 |
| IR-R003 | P0 | Administration Audience 不随真实行为人收窄 | 已接线 | 待集成验收 | 医生与管理员内容可见性 |
| IR-R004 | P0 | 非法 typology runtime spec 与未知 TemplateID 静默回落 | 已接线 | 待数据/运行验收 | 模板路由、冻结输入正确性 |
| IR-R005 | P0 | 解释规则缺口使用最后一条规则兜底 | 已接线 | 待数据/运行验收 | 因子解释正确性、发布门禁 |
| IR-R006 | P0 | Interpretation 可能补写缺失 Level，越过 Outcome 边界 | 已接线 | 待集成验收 | Evaluation → Interpretation 结果语义 |
| IR-R007 | P1 | InterpretReport 构造器缺少最小内容完整性契约 | 已接线 | 待集成验收 | 空成品、机制不匹配内容 |
| IR-R008 | P1 | Outcome/Input 解码失败发生在 Generation / Run 之前 | 已接线 | 待集成验收 | 准入失败证据、运维恢复 |
| IR-R009 | P1 | `report.failed` 不携带完整 RetryDecision | 已接线 | 待集成验收 | Worker、等待端、治理工作台 |
| IR-R010 | P1 | `manual_required` 没有稳定的客户端状态投影 | 已接线 | 待业务验收 | 患者等待体验、人工治理 |
| IR-R011 | P1 | lease 时长、扫描周期和恢复历史没有形成统一治理契约 | 部分实现 | 待数据/运行验收 | 崩溃恢复、Run 审计 |
| IR-R012 | P1 | Run duplicate 未完全归一为 claim conflict | 已接线 | 待集成验收 | 并发重放、无意义 NACK |
| IR-R013 | P1 | TemplateVersion 仍是固定 `legacy-v1`，没有发布资产闭环 | 已接线 | 待集成验收 | 模板版本、历史重放、回滚 |
| IR-R014 | P1 | Artifact 未固化 BuilderIdentity 与 ContentSchemaVersion | 已接线 | 待数据/运行验收 | 成品来源证明、schema 迁移 |
| IR-R015 | P1 | Catalog 与运行时集合缺少持续对账和单一 migration 契约 | 已接线 | 待数据/运行验收 | 灾备、新环境、数据修复 |
| IR-R016 | P1 | Participant gRPC 未端到端绑定 IAM User 与 Testee | 已接线未启用 | 阻塞 | 内部调用信任边界 |
| IR-R017 | P1 | 历史报告维度展示读取当前发布配置 | 已接线 | 待数据/运行验收 | 历史报告展示语义 |
| IR-R018 | P1 | attention sync 是 best-effort，失败没有持久化补偿 | 已接线未启用 | 待数据/运行验收 | 高风险关注投影 |
| IR-R019 | P2 | ReportRow 与外部协议缺少统一来源包络和 V2 契约 | 未实施 | 未进入验收 | 多模型报告、追溯与兼容 |
| IR-R020 | P2 | Operations 按 ReportID 查询在授权前加载完整 Artifact | 已接线 | 待集成验收 | 最小读取原则、运维查询 |
| IR-R021 | P2 | Assessment 组合状态列表形成 N+1 Catalog 查询 | 已接线 | 待数据/运行验收 | 运营列表性能 |
| IR-R022 | P2 | Catalog 深分页、大 `$in`、count/page 不是稳定快照 | 未实施 | 待数据/运行验收 | 大机构工作台 |
| IR-R023 | P2 | Preview 与生产使用不同 Builder 解析路径 | 已接线 | 待集成验收 | 发布前预览可信度 |
| IR-R024 | P2 | `GenerateReportFromAssessment` 名称与 Outcome 驱动语义漂移 | 已接线 | 待集成验收 | gRPC 契约、开发者理解 |
...
- 尚缺预发联调、改造前后容量快照、指定 k6 场景和发布后 24 小时观察，因此状态保持“已实现待验收”；
