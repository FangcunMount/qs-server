...
> 状态（2026-08-31）：产品边界、三份核心机器契约、Participant 主链、OpenAI/DeepSeek Provider Adapter、确定性校验、耐久 Prompt 评测、双角色 Review、Profile/容量/生命周期治理和管理面已有首版代码。生产已启用治理/评测、注入 DeepSeek 密钥、确认 `ai-explanation-retention-v1` 180/365/35 天期限，并保持 `participant_enabled=false`。生成/裁判活动 Route 已回退并发布为 v6/v3 Responses，部署排空修复后最新 Run `635231960356106798` 未再出现 `provider_result_unknown`，但仍有 1 条输出内容契约失败和 1 条被通用化的语义裁判失败。当前固定 35 次、任一技术失败即禁止全部人工审核的状态机已被生产证据证明不适合作为长期发布模型。当前工作树已完成第一批 Semantic Outcome、7 个稳定失败码、v1 `AttemptRecord` 兼容证据、Mongo BSON 映射和治理详情投影；这些仍是未发布的仓库证据，尚未用生产 Recheck 验证。生产只读核查确认当前没有未完成 Mongo Outbox，但仍有 3 个 `awaiting_review` v1 Run；BSON/输出大小分布仍是证据缺口。下一批才在现有 Run 集合内实现轻量 Slot 和最小发布闭环；多集合、增量审核和完整双运行时不进入近期实现。首个 Profile、70 条真实人工复核、金额成本、SLO/告警、备份恢复/导出验收、客户端和用户流量发布仍未完成。本节不得据此宣称用户生产能力已经开放。
...
| AIX-D007 | 运行时/评测/预算/活跃槽集合已有 PO/Mapper/Repository、CAS、事务提交器与 migration 25～33。Participant Generation 保存规范 Input，Artifact 保存通过校验的 Content，不保存渲染 Prompt 或 Provider 原始正文；Prompt 评测为审计目的保存生成 raw/normalized output，按 365 天保留。当前工作树已在原 Run BSON 中内嵌保存语义裁判的 raw/normalized output、receipt 和 typed failure，旧记录仍可读。系统不引入应用层数据密钥。生命周期、TTL、Artifact 导出和 Replica Set 集成测试已有仓库证据 | 轻量 Slot/v2 发布证据、容量并发竞争、lifecycle backfill、备份恢复、真实授权/大数据量导出和生产 TTL 行为 |
...
| AIX-D009 | strict JSON、typed schema、ref、Profile、层级/数量、确定性 safety gate 与独立 semantic evaluator adapter 已实现；裁判 Prompt/Schema/Route 均冻结并校验。当前工作树已把裁判 Provider、result unknown、missing/size、Schema、Decode、Receipt 和 Decision 失败拆为 7 个稳定码，并保留完整执行证据 | 生成内容契约低基数子码、生产 Recheck、真实裁判方向性/新增事实结果和实际人工复核 |
...
| AIX-D011 | 当前 Prompt 评测每机构最多 1 个 collecting Run，启动时固定预留 70 次调用；生产机构 UTC 日预算为 1024。Participant global/user token bucket、机构/用户/Assessment 日预算和活跃槽已实现 | EvaluationExecutionPolicy 最坏生成/裁判执行预算、余额处置、token/金额/历史预算、供应商数据处理清单和真实竞争验收 |
| AIX-D012 | 请求、容量、lease recovery、Provider 调用、token、校验和 Prompt 评测 `stage/code` 低基数指标已实现，Mongo ledger 作为预算事实；当前工作树已增加低基数 Semantic typed code | Slot/Generation/Semantic/Gate 分层进度、分层 Dashboard、route revision 对比、金额 cost、SLO、部署告警、脱敏 trace/log、历史/跨机构治理和生产阈值验证 |
| AIX-D013 | 当前已实现 7×5 固定 35 次生成、最多 35 次裁判、稳定 InvocationID、Outbox/CAS、result unknown、双角色 70-review Gate 和只读单条 Recheck。Run `635231960356106798` 证明现行 v1 任一技术失败会冻结全部审核，且 `AttemptRecord` 混合样本、生成和裁判语义。当前工作树已完成 Semantic 失败完整 Outcome 和 v1 兼容写入 | 生产 Recheck、单文档轻量 Slot、有限补生成/补裁判、v1 只读、Policy 总预算、可靠性 Gate、70 条实际复核和首份 v2 approved Run |
...
1. Semantic Execution 失败证据和 typed code 已在当前工作树实现；先发布并用单条生产 Recheck 验证；
2. 在现有 Run 集合内分离 Slot、Generation Execution、Candidate 和 Semantic Execution，只实现有界补生成、补裁判与 blocked/recovery；
3. 使用冻结 Policy 计算总预算和可靠性 Gate，收集完整后再统一审核；
4. 完成 v1 只读、v2 新写、Replica Set 并发/事务/lease/result unknown 测试和治理台最小投影；
5. 保持 Participant 关闭，执行受控探针和新的完整生产 Run，再完成 70 条双角色复核与首个 Profile；
