...
| AIX-D002 | `Capability`、`Request`、`Get` 已实现，先授权、重复语义复用同一 Generation；REST/gRPC 状态已接线。failed Participant Generation 只能由机构管理员提交带 expected attempt、稳定 request ID、成本确认和未知结果风险确认的 governed retry；Prompt 评测只允许在 Provider dispatch 前取消。Participant Generation 不提供用户取消：dispatch 后无法证明外部调用已停止，也不返还日预算 | 客户端端到端状态/错误样例与真实兼容验收                                                                                           |
...
1. retention/TTL、数据主体 Artifact 导出及专用 Replica Set CI 已完成仓库级验证，但生产具体期限仍必须审批；下一步完成导出真实授权/大数据量和备份恢复验收，并演练历史 lifecycle backfill；
