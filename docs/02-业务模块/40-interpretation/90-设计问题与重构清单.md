...
> 状态（2026-08-27）：产品边界、机器契约、领域模型、Participant REST/gRPC、内部 Automation、Outbox/Worker、OpenAI Responses Adapter、确定性校验、独立生成/裁判 Route 配置、耐久合成评测执行、两类 lease 周期扫描、Prompt 评测机构级并发/UTC 日调用预算、Participant 独立到达层速率预算、机构/用户/Assessment 三层 UTC 日调用准入以及三层 Provider 活跃执行槽/治理查询、Participant 规范 Input/结构化 Artifact 持久化、显式 retention/TTL、数据主体 Artifact 导出、独立 semantic evaluator adapter、不可变 PromptEvaluationRun、双角色 ReviewService、Profile draft/publish/disable 治理及其 operator-only REST 管理面、Mongo migration 和 composition root 已有首版代码，功能默认关闭。v1 已明确废弃应用层数据密钥；OpenAI API 密钥、已发布 Profile、真实模型评测、实际人工复核、金额成本、SLO/告警部署、生产保留期限审批、备份恢复/导出真实验收、客户端和生产发布链仍未完成。本节同时记录实现进度与剩余门禁，不得据此宣称生产能力已存在。
...
| AIX-D002 | `Capability`、`Request`、`Get` 已实现，先授权、重复语义复用同一 Generation；REST/gRPC 状态已接线                                                                                                                                                                                                                                                                                                                                                                               | 人工 retry/cancel 契约和客户端样例                                                                                           |
...
| AIX-D005 | Provider/Route port、冻结 revision/fingerprint、one-shot executor 和 OpenAI Responses Structured Output Adapter 已实现                                                                                                                                                                                                                                                                                                                                         | OpenAI API 密钥注入后的真实 schema/model 验证、供应商幂等/回查/保留证据                                                                             |
...
| AIX-D007 | 运行时/评测/预算/活跃槽集合已有 PO/Mapper/Repository、CAS、事务提交器与 migration 25～30。v1 与标准解读保持同一持久化基线：Generation 保存机器契约约束后的规范 Input，Artifact 保存通过确定性校验的结构化 Content；不保存渲染 Prompt 或 Provider 原始响应，也不引入应用层数据密钥。显式 lifecycle policy 不提供法务默认值，启用时强制配置版本和三类正保留期；终态 Participant/Prompt 评测及 UTC 日预算写 `expires_at + retention_policy_version`，retry 清除 Generation 旧 expiry，migration 31 为六个集合创建 TTL index，启动扫描拒绝缺 lifecycle 元数据的历史终态文档。数据主体导出已通过 Participant REST、purpose-bound gRPC、稳定快照/keyset Application 与 Artifact Repository 接线，migration 32 增加 Org/Testee/audience 导出索引；输出只包含标准来源、版本收据和最终正文，排除内部执行/成本数据。三份专用 integration test 文件、四条测试链已覆盖 Participant request/start/failure、Prompt 评测 start/attempt/org concurrency，以及空库标准 migration、规范 Input/Content、关键唯一/TTL/导出索引和 TTL Monitor 实际删除的真实 Mongo 行为，并通过 integration 编译门禁；qs-server 当前没有 Testee 删除业务，v1 不实现删除传播或主体擦除状态机 | 本轮未提供专用 Replica Set URI且分支 CI 尚未运行，测试尚无实际通过证据；仍需容量并发竞争、历史 lifecycle backfill、生产期限审批与配置、备份恢复验证；导出尚无真实授权/大数据量验收 |
| AIX-D008 | Participant requested 重投可恢复过期 prepared lease；默认关闭的 HA scanner 会把精确 Run/lease/phase wake-up 与 Outbox 原子提交，Worker 校验证明后恢复同一 attempt；非幂等 route 的过期 dispatch 落 `provider_result_unknown` 且不重放。failed Run 已实现仅限机构管理员的人工 governed retry，完整授权、每 attempt 日预算与 durable retry event 原子提交，未知结果需显式接受重复调用/计费风险，Worker 校验 exact authorization 后再竞争活跃槽，硬上限 3 attempt；Prompt 评测已实现 start/step checkpoint、过期 lease 人工恢复和 dispatch 前取消，并增加默认关闭的 prepared-only HA scanner | 两类 scanner 的告警部署、runbook 与真实 Mongo/MQ 故障演练；自动 retry 仍不启用                                                           |
...
AIX-D007 Replica Set、保留期限与备份/导出治理
...
已经落地的 Schema、单测、ACL 和 Adapter 只能证明各自层级；它们不能替代真实模型质量、真实事务、隐私合规或生产验收。
...
1. retention/TTL 与数据主体 Artifact 导出已完成仓库级实现，TTL Monitor 删除测试也已接入分支 CI，但生产具体期限仍必须审批；下一步完成 CI 实跑、导出真实授权/大数据量和备份恢复验收，并演练历史 lifecycle backfill；
2. 在显式授权的专用 Replica Set 或分支 CI 上先运行已落地的三份 integration test 文件、四条测试链；再为两类日预算、Participant 活跃槽和两类 scanner 补告警部署，并完成容量竞争与 MQ 重投验收；
3. 使用 evaluation/review/governance 管理面完成真实身份与跨进程联调，再实际完成双角色复核；
4. 最后手工注入 OpenAI API 密钥，完成真实 Structured Output、拒绝、限流和超时测试；
5. 完成客户端、灰度和生产验收。
