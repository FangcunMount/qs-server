...
> 状态（2026-08-28）：产品边界、机器契约、领域模型、Participant REST/gRPC、内部 Automation、Outbox/Worker、OpenAI/DeepSeek Responses API Adapter、确定性校验、独立生成/裁判 Route 配置、耐久合成评测执行、两类 lease 周期扫描、Prompt 评测机构级并发/UTC 日调用预算、Participant 独立到达层速率预算、机构/用户/Assessment 三层 UTC 日调用准入以及三层 Provider 活跃执行槽/治理查询、Participant 规范 Input/结构化 Artifact 持久化、显式 retention/TTL、数据主体 Artifact 导出、独立 semantic evaluator adapter、不可变 PromptEvaluationRun、双角色 ReviewService、Profile draft/publish/disable 治理及其 operator-only REST 管理面、Mongo migration 和 composition root 已有首版代码。生产已启用治理/评测、注入 DeepSeek 密钥、确认 `ai-explanation-retention-v1` 180/365/35 天三类期限，并保持 `participant_enabled=false`。三次真实生产 Run 都形成技术失败证据：前两轮暴露 Worker 外层 deadline、DeepSeek output token/多分片兼容和 gRPC connection-age 中断；第三轮进一步暴露评测 Attempt lease、Worker RPC 与 NSQ message timeout 的层级倒挂、确定性硬门导致裁判漏跑以及失败码观测不足。第三轮后的修复候选已通过全仓测试但尚未部署，仍无可人工复核的通过候选。v1 已明确废弃应用层数据密钥；已发布 Profile、实际人工复核、金额成本、SLO/告警部署、备份恢复/导出真实验收、客户端和生产流量发布仍未完成。本节同时记录实现进度与剩余门禁，不得据此宣称用户生产能力已经开放。
...
| AIX-D013 | P1  | Prompt 评测与发布门禁            | 评测 runner、semantic evaluator、人工复核和不可变报告如何实现；Provider/模型/Prompt/Profile 任一变化怎样触发全量回归                             | 评测运行器设计、发布 gate、review 记录和 artifact schema                         |
...
| AIX-D005 | Provider/Route port、冻结 revision/fingerprint、one-shot executor 和 OpenAI/DeepSeek Responses API Structured Output Adapter 已实现；两个 Provider profile 均有请求/响应/错误分类契约测试 | 选定 Provider API 密钥注入后的真实 schema/model 验证、供应商幂等/回查/保留证据 |
...
| AIX-D007 | 运行时/评测/预算/活跃槽集合已有 PO/Mapper/Repository、CAS、事务提交器与 migration 25～33。v1 与标准解读保持同一持久化基线：Generation 保存机器契约约束后的规范 Input，Artifact 保存通过确定性校验的结构化 Content；不保存渲染 Prompt 或 Provider 原始响应，也不引入应用层数据密钥。显式 lifecycle policy 不提供法务默认值，启用时强制配置版本和三类正保留期；终态 Participant/Prompt 评测及 UTC 日预算写 `expires_at + retention_policy_version`，retry 清除 Generation 旧 expiry，migration 31 为六个集合创建 TTL index，启动扫描拒绝缺 lifecycle 元数据的历史终态文档。`ai-explanation-retention-v1` 已于 2026-08-27 确认并在生产配置候选中设置 Participant 180 天、Prompt 评测 365 天、容量账本 35 天。数据主体导出已通过 Participant REST、purpose-bound gRPC、稳定快照/keyset Application 与 Artifact Repository 接线，migration 32 增加 Org/Testee/audience 导出索引；输出只包含标准来源、版本收据和最终正文，排除内部执行/成本数据。四份专用 integration test 文件、五条测试链已覆盖 Participant request/start/failure、Prompt 评测 start/attempt/org concurrency、治理目录稳定分页，以及空库标准 migration、规范 Input/Content、关键唯一/TTL/导出索引和 TTL Monitor 实际删除的 Mongo 行为；这些测试与后续 current-runtime E2E 已在 PR #34 CI 33057698726 的专用 Replica Set 实际通过。qs-server 当前没有 Testee 删除业务，v1 不实现删除传播或主体擦除状态机 | 仍需容量并发竞争、历史 lifecycle backfill、备份恢复验证；导出尚无真实授权/大数据量验收，生产 TTL 行为尚未验收 |
...
| AIX-D009 | strict JSON、typed schema、ref、Profile、层级/数量、确定性 safety gate 与独立 semantic evaluator adapter 已实现；裁判 Prompt/Schema/Route/receipt 均冻结并校验                                                                                                                                                                                                                                                                                                                    | 真实裁判方向性/新增事实结果和实际人工复核                                                                                              |
...
| AIX-D011 | operator Prompt 评测已实现每机构最多 1 个 `collecting` run，并在 start 事务内按 UTC 日保守预留完整 70 次 Provider 调用；Participant 首次语义 Generation 已实现独立 global/user token-bucket，并在 request 事务内同时预留机构/用户/Assessment 三层各 1 次调用，默认日上限 `500/5/3`；人工 retry 按 attempt 再预留 1 次三层日预算，同一完整治理动作幂等不重复扣减；Worker 开始 Run 时在同一事务取得默认 `10/2/1` 三层活跃 Provider 槽，终态事务精确释放，容量不足不推进 Generation；超额返回稳定 429；日预算和实时活跃槽均以 Mongo ledger 为最终裁决，并提供可信机构治理查询                                                 | token/金额/历史预算、供应商数据处理清单和真实竞争验收                                                                                     |
| AIX-D012 | Prompt 评测 prepared scanner、Participant lease scanner、评测启动准入、Participant request/execution admission 与人工 retry authorization 已有低基数计数器，区分固定恢复、预算、活跃容量、授权创建/复用/冲突和内部错误；精确日预留与活跃槽审计分别落 Mongo ledger，治理查询同时投影当前机构的 UTC 日预算和实时 Provider 执行槽。Participant 已增加事务成功后的 queue、terminal outcome 与 end-to-end duration；Responses API Adapter 已增加固定 `generation|semantic_evaluator|unknown` purpose 下的调用结果、延迟和 Provider-reported input/output token，并为失败增加固定白名单 `code` 维度，未知动态值统一归入 `other`；Provider 返回后的 strict Output/引用/Profile、安全/Artifact 构造已有固定结果与 validation duration。所有 label 不含供应商、模型、Route 或业务/调用身份                                                                                     | 金额 cost、SLO、部署告警、脱敏 trace/log、历史/跨机构治理和生产阈值验证                              |
| AIX-D013 | 离线 preflight 固定规划 7×5=35 次生成且实际 Provider 调用为 0；operator-only start 将 Run 与首个 step outbox 原子提交，Worker 经内部 gRPC 单步执行，attempt checkpoint 与下一事件原子推进；稳定 InvocationID、重复事件、active release/organization execution 唯一性、过期 dispatch `result_unknown`、人工恢复、prepared-only 周期恢复、取消和进度/剩余成本投影均有仓库测试；Prompt 评测 start/attempt/org concurrency 的专用 Replica Set integration test 已在 PR #34 CI 33057698726 实跑通过；独立 semantic adapter、Prompt/Output Schema/hash 防漂移、35+1 inventory gate、rubric 阈值、不同主体双角色 review、70-review finalize gate 和不可变终态已实现。第三轮后补充显式 `Attempt lease > Worker RPC`、`NSQ message timeout > Worker RPC` 配置契约；结构合法但确定性硬门失败的候选仍执行独立裁判，无规范化 Output 的候选不调用裁判 | 修复部署后的无技术失败 35+35、70 条实际复核、首份 approved report、跨进程/MQ 崩溃恢复证据 |
...
### 17.3 建议的下一轮交付顺序与文档预算

当前仓库已经有 165 份 active Markdown，达到 `docs/document-closure.json` 的硬上限。后续不能机械新增多份 AI 设计文档。[AI 解读核心设计](./25-核心设计-AI解读.md) 已完成领域知识、架构、运行治理、发布阶梯和未来演进的整合；下一轮按以下顺序补齐环境与生产证据，并同步更新本台账：

1. retention/TTL、数据主体 Artifact 导出及专用 Replica Set CI 已完成仓库级验证，生产期限已确认为 180/365/35 天并进入配置；下一步完成导出真实授权/大数据量和备份恢复验收，并演练历史 lifecycle backfill；
2. 为两类日预算、Participant 活跃槽和两类 scanner 补告警部署，并完成容量竞争、跨进程与 MQ 重投验收；
3. 使用 evaluation/review/governance 管理面完成真实身份与跨进程联调，再实际完成双角色复核；
4. 最后手工注入选定 Provider 的 API 密钥，完成真实 Structured Output、拒绝、限流和超时测试；
5. 完成客户端、灰度和生产验收。
