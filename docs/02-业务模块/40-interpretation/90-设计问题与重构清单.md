...
| ID | 优先级 | 问题 | 状态 | 主要影响面 |
| --- | --- | --- | --- | --- |
| IR-R001 | P0 | medical / typology 状态缓存命中可绕过 Assessment ownership | 规划改造 | collection-server、报告状态、患者隐私 |
| IR-R002 | P0 | Catalog 加载正文时不复验 Assessment / Org / Testee 关联 | 规划改造 | 报告正文越界、一致性损坏 |
| IR-R003 | P0 | Administration Audience 不随真实行为人收窄 | 规划改造 | 医生与管理员内容可见性 |
| IR-R004 | P0 | 非法 typology runtime spec 与未知 TemplateID 静默回落 | 规划改造 | 模板路由、冻结输入正确性 |
| IR-R005 | P0 | 解释规则缺口使用最后一条规则兜底 | 规划改造 | 因子解释正确性、发布门禁 |
| IR-R006 | P0 | Interpretation 可能补写缺失 Level，越过 Outcome 边界 | 规划改造 | Evaluation → Interpretation 结果语义 |
...
`findInterpretRuleWithRangeFallback` 在没有任何区间命中时返回最后一条规则。这会掩盖：
...
某些常模报告路径会根据冻结 Norm、T 分和解释规则补写缺失 Level。这样做改善了历史或不完整 Outcome 的展示，但与已确认边界存在张力：
