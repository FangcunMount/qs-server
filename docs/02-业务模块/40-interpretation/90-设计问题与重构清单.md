...
- 当前闭环证据：`reportwait.Service.GetStatus` / `Wait` 在 cache、DB fallback 和 notifier 前调用独立 `AuthorizeAssessment`；
  ...
  详细当前授权边界统一由[查询模型、授权与 Audience 投影](./24-核心设计-查询模型、授权与Audience投影.md)维护。
...
#### IR-R003 实施结果（2026-07-21）

- Administration Access 改为返回 `ReportAccessDecision` / 带 Audience 的 `ListScope`；
  `GetReport` / `ListReports` 使用 `decision.Audience`，不再硬编码 `AudienceAdmin`；
- Audience 由 `actor.access.ResolveAccessScope` 决定：`IsAdmin → AudienceAdmin`，受限 clinician → `AudienceClinician`；
  即使按指定 TesteeID 列表过滤时 `ListScope.Restricted=false`，Audience 仍按真实角色收窄；
...
旧 AI 引擎、治理接口和恢复任务已在 M5 分支删除；新工作流通过 qs-ai 执行。详见 [当前责任与验收边界](./25-核心设计-AI解读.md)。
