...
| AIX-D007 | 运行时/评测/预算/活跃槽集合已有 PO/Mapper/Repository、CAS、事务提交器与 migration 25～33。Participant Generation 保存规范 Input，Artifact 保存通过校验的 Content，不保存渲染 Prompt 或 Provider 原始正文；Prompt 评测为审计目的保存生成 raw/normalized output，按 365 天保留。当前缺少语义裁判失败时同等级的 raw/normalized output、receipt 和 validator evidence。系统不引入应用层数据密钥。生命周期、TTL、Artifact 导出和 Replica Set 集成测试已有仓库证据 | 新 Evaluation Evidence v2、历史只读兼容、容量并发竞争、lifecycle backfill、备份恢复、真实授权/大数据量导出和生产 TTL 行为 |
...
| AIX-D009 | strict JSON、typed schema、ref、Profile、层级/数量、确定性 safety gate 与独立 semantic evaluator adapter 已实现；裁判 Prompt/Schema/Route 均冻结并校验。当前内容契约失败只暴露一个粗 code，裁判 Schema/Decode/Receipt/Decision 错误统一压缩为 `semantic_evaluation_failed` | 内容契约低基数子码、独立裁判失败证据、真实裁判方向性/新增事实结果和实际人工复核 |
...
| AIX-D012 | 请求、容量、lease recovery、Provider 调用、token、校验和 Prompt 评测 `stage/code` 低基数指标已实现，Mongo ledger 作为预算事实。当前没有 Slot/Generation/Semantic/Gate 分层进度，语义失败 code 过粗 | 分层 Dashboard、route revision 对比、金额 cost、SLO、部署告警、脱敏 trace/log、历史/跨机构治理和生产阈值验证 |
| AIX-D013 | 当前已实现 7×5 固定 35 次生成、最多 35 次裁判、稳定 InvocationID、Outbox/CAS、result unknown、双角色 70-review Gate 和只读单条 Recheck。Run `635231960356106798` 证明任一技术失败会冻结全部审核，且 `AttemptRecord` 混合样本、生成和裁判语义 | `EvaluationExecutionPolicy v1`、`GatePolicy v1`、35 个 Candidate Slot、Generation/Semantic Execution、typed disposition、有界补样、增量审核、G1～G5、70 条实际复核和首份 approved Run |
...
1. 先补齐 Semantic Execution 失败证据、typed code 和内容契约子码，使下一次失败可证明；
2. 发布 EvaluationExecutionPolicy/GatePolicy 与 Evidence v2，分离 Slot、Generation Execution、Candidate 和 Semantic Execution；
3. 实现有界补生成、补裁判、blocked/recovery、增量审核和 G1～G5，容量按最坏调用数预留；
4. 完成历史只读兼容、Replica Set 并发/事务/lease/result unknown 测试和治理台分层投影；
