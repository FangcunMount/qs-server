...
| IR-R002 | P0 | Catalog 加载正文时不复验 Assessment / Org / Testee 关联 | 部分实现 | 阻塞 | 报告正文越界、一致性损坏 |
...
| IR-R008 | P1 | Outcome/Input 解码失败发生在 Generation / Run 之前 | 部分实现 | 阻塞 | 准入失败证据、运维恢复 |
...
| IR-R013 | P1 | TemplateVersion 仍是固定 `legacy-v1`，没有发布资产闭环 | 部分实现 | 阻塞 | 模板版本、历史重放、回滚 |
| IR-R014 | P1 | Artifact 未固化 BuilderIdentity 与 ContentSchemaVersion | 已接线 | 待数据/运行验收 | 成品来源证明、schema 迁移 |
| IR-R015 | P1 | Catalog 与运行时集合缺少持续对账和单一 migration 契约 | 部分实现 | 阻塞 | 灾备、新环境、数据修复 |
| IR-R016 | P1 | Participant gRPC 未端到端绑定 IAM User 与 Testee | 已接线未启用 | 阻塞 | 内部调用信任边界 |
| IR-R017 | P1 | 历史报告维度展示读取当前发布配置 | 部分实现 | 阻塞 | 历史报告展示语义 |
| IR-R018 | P1 | attention sync 是 best-effort，失败没有持久化补偿 | 部分实现 | 阻塞 | 高风险关注投影 |
...
- IR-R002 / IR-R015：在线读取与离线 reconcile 共用关联字段校验；reconcile 已进入 HA 一致性调度器，并提供组织范围只读运维查询。Archive 缺失 OrgID 的权威回填与受控 Repair 尚未完成，因此仍为阻塞。
...
- IR-R008：AdmissionFailure 已增加带审计权限的按 Outcome 查询入口；稳定 typed reason 与受控 readmit 动作仍未完成。
