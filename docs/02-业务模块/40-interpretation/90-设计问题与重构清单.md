...
> 状态（2026-09-01）：产品边界、三份核心机器契约、Participant 主链、OpenAI/DeepSeek Provider Adapter、确定性校验、耐久 Prompt 评测、双角色 Review、Profile/容量/生命周期治理和管理面已有首版代码。生产已启用治理/评测、注入 DeepSeek 密钥、确认 `ai-explanation-retention-v1` 180/365/35 天期限，并保持 `participant_enabled=false`。生成/裁判活动 Route 已回退并发布为 v6/v3 Responses，部署排空修复后最新 Run `635231960356106798` 未再出现 `provider_result_unknown`，但仍有 1 条输出内容契约失败和 1 条被通用化的语义裁判失败。当前固定 35 次、任一技术失败即禁止全部人工审核的 v1 状态机已被生产证据证明不适合作为长期发布模型。当前工作树已完成 Semantic Outcome、7 个稳定失败码、v1 兼容证据，以及单文档 v2 Slot/Generation/Candidate/Semantic、Mongo CAS、Policy 总预算、Outbox/Worker/gRPC、分步 Runner、统一审核、G1～G5 Gate、管理 API 和 Profile 发布约束；这些仍是未发布的仓库证据，尚未用生产 Recheck 或完整 v2 Run 验证。生产只读核查确认当前没有未完成 Mongo Outbox，但仍有 3 个 `awaiting_review` v1 Run；BSON/输出大小分布仍是证据缺口。多集合、增量审核和完整双运行时不进入近期实现。首个 Profile、70 条真实人工复核、金额成本、SLO/告警、备份恢复/导出验收、客户端和用户流量发布仍未完成。本节不得据此宣称用户生产能力已经开放。
...
| AIX-D007 | 运行时/评测/预算/活跃槽集合已有 PO/Mapper/Repository、CAS、事务提交器与 migration 25～33。Participant Generation 保存规范 Input，Artifact 保存通过校验的 Content，不保存渲染 Prompt 或 Provider 原始正文；Prompt 评测为审计目的保存生成 raw/normalized output，按 365 天保留。当前工作树已在原 Run 集合中按明确版本保存 v2 Slot、两类 Execution、receipt、raw/normalized output 和 typed failure，旧 v1 记录只读。系统不引入应用层数据密钥。生命周期、TTL、Artifact 导出和 Replica Set 集成测试已有仓库证据 | 生产 BSON/输出分布、容量并发竞争、lifecycle backfill、备份恢复、真实授权/大数据量导出和生产 TTL 行为 |
...
| AIX-D013 | Run `635231960356106798` 证明现行 v1 任一技术失败会冻结全部审核，且 `AttemptRecord` 混合样本、生成和裁判语义。当前工作树已完成 Semantic 完整 Outcome、固定 35 Slot、两类有界 Execution、Candidate 不可替换、`result_unknown` 人工确认、v1 只读、Policy 总预算、Outbox/CAS、统一双角色审核、可靠性 Gate、管理 API 和 Profile 仅接受 approved v2 Evidence | 生产 BSON/输出测量、生产 Recheck、70 条实际复核、首份 v2 approved Run 和生产 Profile |
...
1. Semantic Execution 完整证据、轻量 Slot、Policy 总预算、可靠性 Gate、v1 只读/v2 新写、Replica Set 测试和治理最小投影均已在当前工作树完成；
2. 取得生产 Run BSON 与输出大小分布，确认单文档方案具备余量；
3. 推送并通过远端 CI，在维护窗口部署 v2 新写路径，Participant 继续关闭；
4. 用单条生产 Recheck 验证 typed failure，再执行新的完整 v2 Run；
