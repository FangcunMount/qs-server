...
> 状态（2026-08-27）：产品边界、机器契约、领域模型、Participant REST/gRPC、内部 Automation、Outbox/Worker、OpenAI/DeepSeek Responses API Adapter、确定性校验、独立生成/裁判 Route 配置、耐久合成评测执行、两类 lease 周期扫描、Prompt 评测机构级并发/UTC 日调用预算、Participant 独立到达层速率预算、机构/用户/Assessment 三层 UTC 日调用准入以及三层 Provider 活跃执行槽/治理查询、Participant 规范 Input/结构化 Artifact 持久化、显式 retention/TTL、数据主体 Artifact 导出、独立 semantic evaluator adapter、不可变 PromptEvaluationRun、双角色 ReviewService、Profile draft/publish/disable 治理及其 operator-only REST 管理面、Mongo migration 和 composition root 已有首版代码。生产配置候选已启用治理/评测运行时并保持 `participant_enabled=false`；`ai-explanation-retention-v1` 已确认 180/365/35 天三类期限。v1 已明确废弃应用层数据密钥；已发布 Profile、真实模型评测、实际人工复核、金额成本、SLO/告警部署、备份恢复/导出真实验收、客户端和生产流量发布仍未完成。本节同时记录实现进度与剩余门禁，不得据此宣称用户生产能力已经开放。
