...
| IR-R007 | P1 | InterpretReport 构造器缺少最小内容完整性契约 | 规划改造 | 空成品、机制不匹配内容 |
| IR-R008 | P1 | Outcome/Input 解码失败发生在 Generation / Run 之前 | 规划改造 | 准入失败证据、运维恢复 |
| IR-R009 | P1 | `report.failed` 不携带完整 RetryDecision | 规划改造 | Worker、等待端、治理工作台 |
| IR-R010 | P1 | `manual_required` 没有稳定的客户端状态投影 | 规划改造 + 待业务决策 | 患者等待体验、人工治理 |
| IR-R011 | P1 | lease 时长、扫描周期和恢复历史没有形成统一治理契约 | 规划改造 + 待时延证据 | 崩溃恢复、Run 审计 |
| IR-R012 | P1 | Run duplicate 未完全归一为 claim conflict | 规划改造 | 并发重放、无意义 NACK |
| IR-R013 | P1 | TemplateVersion 仍是固定 `legacy-v1`，没有发布资产闭环 | 规划改造 | 模板版本、历史重放、回滚 |
| IR-R014 | P1 | Artifact 未固化 BuilderIdentity 与 ContentSchemaVersion | 规划改造 | 成品来源证明、schema 迁移 |
| IR-R015 | P1 | Catalog 与运行时集合缺少持续对账和单一 migration 契约 | 规划改造 + 待部署核验 | 灾备、新环境、数据修复 |
| IR-R016 | P1 | Participant gRPC 未端到端绑定 IAM User 与 Testee | 规划改造 | 内部调用信任边界 |
| IR-R017 | P1 | 历史报告维度展示读取当前发布配置 | 规划改造 | 历史报告展示语义 |
| IR-R018 | P1 | attention sync 是 best-effort，失败没有持久化补偿 | 规划改造 | 高风险关注投影 |
