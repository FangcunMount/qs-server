...
报告正文较大，如果直接在 Artifact 集合上做组织、受试者、模型、风险和时间筛选，
列表分页会和正文大小直接绑定，每个上层消费者也会重复实现同样的筛选与排序逻辑。

`report_query_catalog` 把这些问题收敛为“一次 Assessment 对应一个当前查询条目”。
Artifact 成功提交时同步更新 catalog。
