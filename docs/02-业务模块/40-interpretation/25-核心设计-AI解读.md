...
> 当前状态：**代码发布候选，生产治理与评测运行时已启用，用户能力仍关闭，生产质量尚未通过**。v1 的机器契约、领域模型、Participant API、异步执行、OpenAI/DeepSeek Responses API Adapter、Mongo 持久化、输出校验、在线评测、双角色人工审核、Profile 发布、容量治理和数据生命周期均已有仓库实现；生产已注入 DeepSeek 密钥、固化 180/365/35 天保留策略并发起四次冻结评测 Run。四次 Run 都形成不可改写的技术失败证据，尚无可进入人工审核的通过候选；第四轮 Run `634833605914276398` 的 35 次生成中仅 20 次成功、12 次独立模型裁判成功，15 条技术失败由 9 条输出 token 截断、4 条输出基数异常和 2 条裁判超时组成。当前 v2 修复候选已将生成与裁判的思考强度冻结为 `low`，将生成输出上限提高至 12000，将裁判超时提高至 180 秒，并同步拉开 Worker RPC、Attempt lease、NSQ message timeout 和 gRPC connection-age grace 的外层边界；修复目前只有仓库测试证据，尚未部署。因此 `participant_enabled=false`，Profile 未发布，也没有用户灰度或生产可用性结论。
...
2026-08-28 的首轮生产 35 次评测验证了冻结参数的必要性：默认 high 思考、8000 total output tokens 和 120 秒裁判超时组合产生 9 次 token 截断、4 次无最终 message、2 次裁判超时。修复后的 `v2` Route 将生成/裁判思考强度显式设为 `low`，生成上限提高到 12000，裁判超时提高到 180 秒；旧 Run 继续保留原 `v1` 身份和失败证据，不做原地改写。
...
| OpenAI/DeepSeek Responses API Adapter | 已完成四轮生产实调，v2 修复候选待发布 | 前三轮定位并修复 Worker 外层 deadline、output token incomplete、多 `output_text` 分片、gRPC connection-age 中断、Attempt lease/NSQ 重投、裁判漏跑和粗粒度失败码观测。第四轮在这些修复上完成 35 个 generation attempt，只有 20 个生成成功和 12 份裁判成功；新证据将 15 条技术失败收敛为 `provider_output_token_limit` 9 条、`provider_output_cardinality_invalid` 4 条和语义裁判 `provider_timeout` 2 条。v2 候选显式使用 `reasoning.effort=low`，将生成 token 上限提高至 12000、裁判 timeout 提高至 180s，并建立 `Attempt lease 8m > Worker AI RPC 7m`、`NSQ message timeout 9m > Worker AI RPC 7m`、connection-age grace 10m 的外层边界；广域 Go 测试与文档事实门禁通过 | 修复部署后的无技术失败 35+35、质量、供应商保留和真实成本 |
| 输出校验与 Safety | 已实现首版 | typed JSON、ref、Profile、确定性规则单测 | 真实内容忠实度与人工安全结论 |
| 在线评测与审核 | 已实现框架，四次生产 Run 均因技术失败不可审核 | 35+35 编排、证据 CAS、70 条双角色 gate、Org/status 审核目录；技术失败 Run 禁止 review/finalize 并支持审计取消；第四次 Run 完整形成 35 个 generation attempt，并用于定位 DeepSeek 默认 high 思考下的 token 挤占、无结构化消息和裁判超时 | v2 修复后无技术失败的生产 Run、真实 reviewer 和 approved run |
