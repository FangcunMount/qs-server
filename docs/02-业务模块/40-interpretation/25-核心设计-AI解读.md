...
> 当前状态：**代码发布候选，生产配置已进入治理与评测阶段，用户能力仍关闭，生产效果尚未验收**。v1 的机器契约、领域模型、Participant API、异步执行、OpenAI/DeepSeek Responses API Adapter、Mongo 持久化、输出校验、在线评测、双角色人工审核、Profile 发布、容量治理和数据生命周期均已有仓库实现，分支 CI 和专用 Replica Set 测试已经通过；生产配置已固化保留策略并准备启用治理/评测运行时，但 `participant_enabled=false`。尚缺真实模型评测、正式 Profile、真实身份与跨进程联调、灰度和生产运行证据。
...
| OpenAI/DeepSeek Responses API Adapter | 已完成首轮生产实调，修复候选待发布 | Provider profile、JSON Schema 请求、响应/用量投影、错误/大小/超时分类测试；首轮生产评测发现 Worker 30 秒外层 deadline 早于两段 Provider 执行上限，已增加 AI 专用 3 分钟 RPC deadline | 修复部署后的完整 35+35 重跑、质量、供应商保留和真实成本 |
| 输出校验与 Safety | 已实现首版 | typed JSON、ref、Profile、确定性规则单测 | 真实内容忠实度与人工安全结论 |
| 在线评测与审核 | 已实现框架，首轮生产 Run 因技术失败不可审核 | 35+35 编排、证据 CAS、70 条双角色 gate、Org/status 审核目录；技术失败 Run 禁止 review/finalize 并支持审计取消 | 修复后无技术失败的生产 Run、真实 reviewer 和 approved run |
