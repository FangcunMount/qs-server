...
> 当前状态：**代码发布候选，用户能力默认关闭，生产效果尚未验收**。v1 的机器契约、领域模型、Participant API、异步执行、OpenAI/DeepSeek Responses API Adapter、Mongo 持久化、输出校验、在线评测、双角色人工审核、Profile 发布、容量治理和数据生命周期均已有仓库实现，分支 CI 和专用 Replica Set 测试已经通过；尚缺真实模型评测、正式 Profile、真实身份与跨进程联调、灰度和生产运行证据。
...
| 生产开关 | 模块开关 `enabled=false`；用户流量开关 `participant_enabled=false` | 尚未开放 |
...
| 数据生命周期 | 已实现框架 | retention policy、TTL migration、导出索引与 Replica Set TTL Monitor 实跑 | 正式期限、backfill、备份恢复和生产数据行为 |
