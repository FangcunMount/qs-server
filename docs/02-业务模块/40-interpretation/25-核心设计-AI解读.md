...
> 当前状态：**代码发布候选，用户能力默认关闭，生产效果尚未验收**。v1 的机器契约、领域模型、Participant API、异步执行、OpenAI Adapter、Mongo 持久化、输出校验、在线评测、双角色人工审核、Profile 发布、容量治理和数据生命周期均已有仓库实现，分支 CI 和专用 Replica Set 测试已经通过；尚缺真实模型评测、正式 Profile、真实身份与跨进程联调、灰度和生产运行证据。
