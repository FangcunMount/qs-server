...
第一版复核随尚未提交的 Candidate/角色审核一起记录，保留既有审核只追加、已终结 Run 不可改写的约束；已记录的角色审核不能事后补改复核。历史无复核记录的证据保持原门禁结果。此机制不需要重新生成 Candidate 或重新调用裁判，但实际 Profile 发布仍需完整双角色审核和 Finalize 通过。
