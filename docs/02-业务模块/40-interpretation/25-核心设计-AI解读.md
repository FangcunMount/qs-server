...
> 截至 2026-09-01，当前状态：**生产治理与 v1 评测运行时已启用，用户能力仍关闭，v2 改造位于已推送但未合并、未发布的分支**。生产 Run `635231960356106798` 的内容契约失败与通用语义裁判失败，证明当前瓶颈是评测编排、失败分类、裁判证据和零失败发布门禁。分支现已完成 Semantic Outcome、稳定失败码、v1 兼容证据，以及单文档 v2 Slot/Generation/Candidate/Semantic 聚合、Mongo CAS、容量预留、Outbox/Worker/gRPC、分步 Runner、`result_unknown` 人工决策、管理 REST、G1～G5 终审和 Profile 仅接受 approved v2 Evidence；全仓 Go 回归、OpenAPI、文档事实、diff lint 和分层边界检查已通过。生产只读核查确认没有未完成 Mongo Outbox，但仍有 3 个 `awaiting_review` v1 Run；历史 BSON 与 Generation 输出分布已取得，Semantic 输出样本和生产 Recheck 尚未取得。以上仓库证据不能替代发布、生产探针或完整 v2 Run。多集合和增量审核只保留为未来架构；`participant_enabled=false`，首个生产 Profile 尚未发布。
...
| 评测收集模型 | 7 个 case × 5 个有效 Candidate；执行失败可有界补样，所有执行仍保留 | v2 领域、Mongo/Application、Outbox/Worker 和分步 Runner 已在未合并分支接线；待发布与完整 Run |
| 质量发布 | 35 个有效 Candidate、35 份完整裁判证据、70 条双角色人工复核，并检查全部执行的可靠性/契约符合率 | v2 G1～G5 和管理写闭环已实现；待真实审核和生产证据 |
...
| 裁判证据 | 独立保存裁判请求身份、Provider receipt、原始/规范化输出、校验与失败分类 | v1 兼容证据与 v2 独立 Semantic Execution 均已实现；待生产 Recheck |
...
这组映射用于统一领域沟通和对外宣讲，不是当前的代码重命名计划。裁判失败证据、失败分类和有界补样语义已经在未合并分支完成，生产历史 BSON 与 Generation 输出测量也已取得；下一步是受控发布、完整生产 Run 和 Semantic 容量复测。只有发布验证后 `OnlineRunner` 仍表现出职责过多、变更放大或测试隔离困难，才启动独立结构性重构；届时先以表征测试保护行为，再分批提取 Candidate Pipeline，而不是只做模式化改名。
...
四份 Schema、服务端嵌入访问器和契约编译测试均已落库。轻量 v2 已接入同一 Mongo 集合的独立 PO/Mapper/Repository、CAS Application Service、容量事务、Outbox 事件、Worker/gRPC 路由和分步 Runner；新管理写入口使用 `/internal/v2`，v1 评测 REST 只保留查询。该结论只证明未合并分支中的代码和测试，不证明生产切换或完整 Run 成功。
...
| `CandidateSlot` | 7 个 case × 5 个分层样本位中，这一位由哪个 Candidate 占用 | 身份为 `case_id + slot_ordinal`；只接受第一个符合执行契约的 Candidate | v2 Run 已接线；待完整生产 Run |
...
生产 v1 实现把这些目标直接解释为“恰好 35 次生成 + 最多 35 次裁判”，所以任何一次无法形成 Candidate 或裁判收据，整个 Run 都不能进入人工审核。当前分支的 v2 把**样本目标**和**外部执行预算**分开：样本目标仍然是固定的 35 个 Slot；生成和裁判允许在冻结上限内补充执行，但绝不无限“跑到通过”。
...
| Evaluation Slot/Execution/Candidate 模型 | 未合并分支已完成端到端仓库接线 | 契约、领域、Mongo CAS、容量、Outbox/Worker/gRPC、分步 Runner 与管理 API 聚焦测试 | 发布后行为、完整生产 Run、完整 v2 BSON 余量 |
...
| 在线评测与审核 | v2 仓库闭环已实现，尚未发布 | 固定 35 Slot、两类有界 Execution、统一审核、管理 `/internal/v2` 写接口和防重放测试 | 真实 reviewer、approved Run 与生产行为 |
| 失败分类与裁判证据 | 未合并分支已实现，尚未发布 | `SemanticEvaluationOutcome`、稳定失败码、v1 兼容证据、v2 Semantic Execution、Mongo 往返和详情 API | 生产 Recheck、治理台真实展示和告警 |
...
| Run BSON 大小 | [Database Operations 33443439888](https://github.com/FangcunMount/qs-server/actions/runs/33443439888) 全量读取 15 个历史 Run：`$bsonSize` P50=`329,413` 字节、P95/最大=`437,143` 字节；排除已存输出后的 P50=`113,166`、最大=`141,781` 字节 | 当前历史体量远低于 Mongo 16 MiB 上限，近期继续使用单 Run 文档；首个完整 v2 Run 后复测，不立即拆集合 |
| raw/normalized 输出分布 | 同一只读审计覆盖 525 次 Generation：raw P50/P95/最大=`4,955/6,514/8,558` 字节，normalized=`4,308/5,518/6,657` 字节；历史数据没有 Semantic Execution 样本 | Generation 已有生产量级；Semantic 与完整 v2 上界仍只能在首个 v2 Run 后确认，不能宣称容量永久安全 |

因此当前决策是：**生产实测支持先用单 Run 文档完成快速验证；首个包含 Semantic Execution 的完整 v2 Run 仍要复测 BSON，但不再用理论上限驱动多集合重构。**

### 3. 当前代码链路与真正的修改点

本轮改造开始时的生产 v1 运行链路已经可以准确定位；它用于解释问题来源，不代表当前分支仍以该链路承接新 Run：
...
> 实现状态：当前未合并分支已完成本批仓库改造与聚焦测试；尚缺发布后的单条生产 Recheck，因此不宣称生产问题已关闭。
...
> 实现状态：本批已在未合并分支完成领域、Mongo、Application、事务提交、事件、Worker/gRPC、Runner 和管理入口接线；尚未发布。生产历史 BSON 与 Generation 输出分布核查已完成；Semantic 输出和完整 v2 余量要在首个完整 Run 后复测。
...
生产 v1 的 `$bsonSize` 与 Generation 输出分布已由只读工具 `scripts/oneoff/audit_ai_explanation_prompt_evaluation_size` 完成全量扫描。该工具直接使用 Mongo 返回的原始 BSON 字节，并在生产 `infra-network` 内以无 capability、只读文件系统的临时容器执行；凭据只通过受保护环境变量传递。当前 15 个 Run、525 次 Generation 的结果支持首版继续使用单文档，但因为没有 Semantic Execution 样本，工具没有给出完整 v2 投影上界。
...
若首个完整 v2 Run 的复测不能证明有足够余量，则停止继续放量，转入“未来架构：独立 Execution 集合”。
...
> 实现状态：统一 Candidate 审核、G1～G5 终审、Profile 仅接受 approved v2 Evidence、Policy 计算 140 次总预留和 `/internal/v2` 管理写接口已在未合并分支实现并通过聚焦测试；仍缺完整 Run、真实人工审核和生产发布证据。
...
分支验收审计结论：第 1～2、4～11 项已有直接领域、应用、路由或契约测试；第 3 项已由单条受控 Recheck 测试证明“独立终态且不修改源 Run”，但生产 Recheck 仍未执行；第 12 项已证明 140 次最坏预留、原始 BSON 统计口径、缺样本时失败关闭，以及 `15,569,768` 字节完整 v2 文档在 Mongo 7 Replica Set 的 round-trip。同一组集成测试还验证了 v2 同集合版本隔离、CAS，以及容量预留、Evidence 和 Outbox 在启动及 Generation 完成失败时整体回滚；对应 integration tests 已纳入现有 CI Mongo job。生产历史 BSON 与 Generation 输出分布已经取得，Semantic 输出分布和完整 v2 Run 仍是发布后的复核门禁。因此当前是“代码实现完成、真实 Mongo 行为已验证、生产验收未完成”，不能标记为已发布或 production ready。
