...
> 当前状态：**生产治理与评测运行时已启用，用户能力仍关闭，生产质量尚未通过**。v1 的机器契约、领域模型、Participant API、异步执行、OpenAI/DeepSeek Responses API Adapter、Mongo 持久化、输出校验、在线评测、双角色人工审核、Profile 发布、容量治理和数据生命周期均已有仓库实现；生产已注入 DeepSeek 密钥、固化 180/365/35 天保留策略并完成七次冻结评测 Run。第七轮 Run `634926884852871726` 的 35 次生成调用均获得 completed response 和可解析 JSON，但 `json_object` 没有约束业务字段：35/35 均缺少 `schema_version` 和 `integrated_insights`，33/35 使用未被契约接受的 `insights`，33/35 把 `limitations` 输出成字符串。因此规范化输出、独立模型裁判和 case assertion 通过数均为 0。当前 `v5` 修复把生成 Route 恢复为 DeepSeek Responses API 已声明的 `json_schema`，并把 Schema 不合格响应明确记录为 `output_validation` 技术失败；独立裁判仍使用 `v2 / json_schema / low / 8000 / 180s`。因此 `participant_enabled=false`，Profile 未发布，也没有用户灰度或生产可用性结论。
...
- 当前主 Provider 为 DeepSeek，默认模型固定为其 Responses API 模型 `deepseek-v4-flash`；OpenAI 保留为显式配置的备选 Provider；
...
- Provider Route 对思考强度和 Structured Output 模式使用显式枚举；两者都属于冻结执行语义。`v5` 生产候选的生成 Route 使用 `none + json_schema`，独立模型裁判仍使用 `low + json_schema`；
...
2026-08-28 的第四轮生产 35 次评测验证了冻结参数的必要性：默认 high 思考、8000 total output tokens 和 120 秒裁判超时组合产生 9 次 token 截断、4 次无最终 message、2 次裁判超时。`v2` 将生成/裁判思考强度显式设为 `low`，生成上限提高到 12000，裁判超时提高到 180 秒；第五轮消除了 token 截断与超时，但仍有 1 次 `completed/no_message`。`v3` 关闭生成端 thinking 后，第六轮的 35 次 Provider 调用全部返回非空正文，但 27 份正文带 Markdown JSON 围栏。`v4` 切换到 `json_object` 后，第七轮 35 份正文均为合法 JSON，证明包络问题已收敛；但 35/35 均不符合 v1 Schema，证明 `json_object` 并不是业务结构化输出。`v5` 因此恢复官方 Responses API 契约中的 `json_schema`，并将不合格结构记录为技术失败，而不是可审核候选。旧 Run 继续保留原冻结身份和失败证据，不做原地改写。
...
| OpenAI/DeepSeek Responses API Adapter | 已完成七轮生产实调，v5 候选开发中 | 前六轮定位并修复 Worker 外层 deadline、output token incomplete、多 `output_text` 分片、gRPC connection-age 中断、Attempt lease/NSQ 重投、裁判漏跑、粗粒度失败码和 Markdown 围栏。第七轮 Run `634926884852871726` 的 35 份正文都是合法 JSON，但 35/35 均未通过 v1 Schema；v5 将生成端冻结为 `json_schema / none / 12000 / 120s`，独立裁判保持 `v2 / json_schema / low / 8000 / 180s` | v5 部署后的无技术失败 35+35、质量、供应商保留和真实成本 |
| 输出校验与 Safety | 已实现首版 | typed JSON、ref、Profile、确定性规则单测 | 真实内容忠实度与人工安全结论 |
| 在线评测与审核 | 已实现框架，七次生产 Run 均未达到发布门禁 | 35+35 编排、证据 CAS、70 条双角色 gate、Org/status 审核目录；技术失败 Run 禁止 review/finalize 并支持审计取消；v5 把 Schema 不合格从普通 assertion 失败提升为 `output_validation` 技术失败 | v5 修复后无技术失败的生产 Run、真实 reviewer 和 approved run |
