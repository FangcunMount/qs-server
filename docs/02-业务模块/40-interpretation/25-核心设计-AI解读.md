...
> 当前状态：**生产治理与评测运行时已启用，用户能力仍关闭，生产质量尚未通过**。v1 的机器契约、领域模型、Participant API、异步执行、Mongo 持久化、输出校验、在线评测、双角色人工审核、Profile 发布、容量治理和数据生命周期均已有仓库实现；生产已注入 DeepSeek 密钥并固化 180/365/35 天保留策略。生产 Run `634965161702076974` 证明 `v7/v4 strict-tool` 发布候选出现明显回归：35 条中 22 条技术失败，其中包含裁判请求拒绝和 Provider 输出包装层偏差。当前修复候选将活动路由回退到已有生产证据的 `v6/v3 Responses`，拆分生成与独立裁判的协议配置，并只对已观测的唯一 JSON 包装做确定性解包。新增的单条诊断复测用于上线前 1+1 调用探针，但不修改旧 Run，不计入 35+35，不能作为 Profile 发布证据。该修复尚未取得新的生产证据，因此 `participant_enabled=false`，Profile 未发布，也没有用户灰度或生产可用性结论。
...
| 架构视角 | 当前含义 | 主要代码载体 | 边界 |
| --- | --- | --- | --- |
| Release Workflow | 多服务协作的整体发布治理流程 | 评测、证据、人工审核、Profile 发布与容量治理服务 | 不是单一 `AIExplanationReleaseWorkflow` 大类，不自动发布 Profile，也不自动打开用户流量 |
| Candidate Pipeline | `OnlineRunner` 中的单次 attempt 处理路径 | 冻结资产解析 → Provider 生成 → 输出规范化 → 确定性校验 → 独立模型裁判 → 证据提交 | `OnlineRunner` 还承担冻结评测套件的多 attempt 编排，因此现在不宜只做类型改名 |
| Validation Rule Set | 对候选输出运行全部确定性规则并保留每条证据 | `EvaluateCandidate` + `Assertions` | 会收集 `passed` / `failed` / `blocked` 结果，不是“第一个 Handler 处理后立即结束”的经典职责链 |
| Run State Machine | 维护评测 Run 的状态转换和领域不变量 | `PromptEvaluationRun` 聚合 | 代码保留领域名，不改名为 `PromptEvaluationRunStateMachine` |
| Provider Strategy | 为应用层提供统一的模型生成契约 | `port.Provider` | 应用层依赖稳定 port，不依赖 OpenAI 或 DeepSeek SDK |
| Provider Adapter | 将冻结 Route 转换为具体 Provider 线协议，并规范化回执与错误 | `responsesapi.Provider` | 只处理可证明的协议差异，不修补业务内容，最终仍由服务端完整契约校验裁决 |

这组映射用于统一领域沟通和对外宣讲，不是当前的代码重命名计划。现阶段保持现有类型和包边界，优先完成新的生产评测、双角色人工审核和首个生产 Profile 发布。只有在这些门禁完成后，代码证据仍表明 `OnlineRunner` 存在职责过多、变更放大或测试隔离困难，才启动独立的结构性重构任务；届时应先补齐行为保护，再分批提取单次 Candidate Pipeline，而不是现在只做模式化改名。
...
索引与迁移由 Mongo migration 25～33 管理，覆盖运行集合、评测证据、日预算、活跃槽、人工重试、TTL、数据主体导出和治理目录稳定分页。专用 Replica Set 集成测试验证事务回滚、唯一约束、治理目录分页、冷启动 Schema 和 TTL Monitor；这些测试已在 [PR #34 CI 33057698726](https://github.com/FangcunMount/qs-server/actions/runs/33057698726) 实际通过。
...
- 当前主 Provider 为 DeepSeek；生产修复候选的生成 `v6` 与裁判 `v3` Route 均回到 `deepseek-v4-pro + responses`；OpenAI 保留为显式配置的备选 Provider；
...
当前不实现请求级多 Provider 路由、自动降级或一次请求并发多模型。Provider、模型或 Route revision 变更都会改变冻结执行身份，必须重跑真实评测和发布门禁。

2026-08-28 至 2026-08-29 的生产评测验证了冻结参数和本地校验都不可省略：早期组合先后出现 token 截断、无最终 message 和裁判超时；调整冻结思考强度、token 上限和 timeout 后，这些传输层问题收敛。`json_object` 一轮虽得到 35/35 合法 JSON，却得到 35/35 v1 Schema 不合格，证明 JSON 语法不是业务契约。Responses `v6/v3` 曾将技术失败压缩到 1 条；之后的 `v7/v4 strict-tool` 实验却在 Run `634965161702076974` 中上升到 22/35，其中 11 条是裁判 `provider_request_rejected`，另有 Provider 包装层字段与 JSON 语法失败，且没有形成可用的 semantic receipt。因此修复候选回退活动路由到 v6/v3，将 strict-tool 保留为未通过生产证据的实验协议，并用独立协议配置、低基数 HTTP 失败码和确定性已知包装解包收敛边界。旧 Run 继续保留原冻结身份和失败证据，不做原地改写。
...
collecting → awaiting_review → approved | rejected
    │             └→ canceled（仅存在不可变技术失败证据）
    └→ canceled（Provider 尚未 dispatch）
```

`generation_attempts=35` 只表示 35 个冻结执行记录均已落库，不表示 35 份候选输出和 35 次独立模型裁判全部成功。只要任一 generation attempt 带有技术 `failure`，该 Run 就必须满足以下约束：

- `can_review=false`、`can_finalize=false`，不能用 70 条人工意见掩盖基础设施故障；
- `can_cancel=true`，机构管理员可填写原因后审计取消；
- 取消保留 `closed_at`、attempt、失败、已有 review 和 recovery 证据，不修改历史、不退款；
- 修复故障后启动新的冻结 Run，成功的 `awaiting_review` Run 仍不能通过取消绕过人工审核。
...
审核工作台通过 `GET /prompt-evaluations` 按可信机构和状态读取稳定 keyset 分页摘要，再按 Run/attempt 显式读取单份证据；列表不批量返回 35 份 Input 和 Provider 原始输出。Profile 治理通过 `GET /profiles` 与 `GET /profiles/{profile_id}/versions/{version}` 读取版本目录和定义。游标绑定资源类型、机构、状态、`created_at` 与 `domain_id`，不能跨机构、跨筛选条件复用。
...
这个能力的首要用途是修复发布前的最小 1+1 生产探针。单条复测通过只能证明该条诊断样本在当前 Release 上可执行，仍然必须新建完整 35+35 Run 并完成 70 条双角色审核，才可以进入 Profile 发布决策。
...
v1 每个评测 Run 保守预留 70 次调用；同一机构最多一个 collecting Run。单条诊断复测每次独立预留 2 次调用，与完整 Run 共用机构 UTC 日预算账本，但不占用 collecting Run 发布槽。默认日预算 140；配置预算只需不少于 70，不能组成完整 70 次预留的余额不会放行新的 Run。取消、失败或 `result_unknown` 不退款，因为系统无法可靠证明外部调用未发生。

这些默认值只是功能关闭时的安全起点，不是生产容量结论。正式上线前需要结合目标机构规模、模型价格、实际 token 和灰度结果重新定标。

当前生产于 2026-08-28 经审计批准将每机构 UTC 日上限显式设为 1024，即最多 14 个完整 Run，并留下 44 次不能单独启用的余额。这只改变成本准入上限，不改变单 Run 70 次预留、同机构单活跃 Run、失败不退款、人工审核或用户总开关门禁。
...
| OpenAI/DeepSeek Provider Adapter | v6/v3 Responses 回退修复候选已实现，待部署探针 | 生成/裁判协议与 Endpoint 独立冻结；已知唯一 JSON 包装确定性解包；低基数 HTTP 失败码；v7/v4 回归证据保留 | 部署后单条 1+1 探针、新的无技术失败 35+35、内容质量和真实成本 |
| 输出校验与 Safety | 已实现首版 | typed JSON、ref、Profile、确定性规则单测 | 真实内容忠实度与人工安全结论 |
| 在线评测与审核 | 已实现框架，现有生产 Run 尚未达到发布门禁 | 35+35 编排、证据 CAS、70 条双角色 gate、Org/status 审核目录；失败 Run 只读证据；独立单条复测聚合、2 次成本确认、活动源防重与 `result_unknown` 边界 | 修复候选的生产 1+1 探针、无技术失败的新 Run、真实 reviewer 和 approved run |
...
| 生产启用 | 回退修复候选 | 治理/评测启用、Participant 流量独立关闭、缺少配置时 fail closed | 合并部署、单条探针、新 35+35、Profile、灰度、SLO、告警和生产可用性 |
...
截至 2026-08-27，上述自动化条目已由 [PR #34 CI 33057698726](https://github.com/FangcunMount/qs-server/actions/runs/33057698726) 证明；PR 仍需代码审查和合并决定，不能把自动化通过写成已经发布。
