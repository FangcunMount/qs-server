...
| Run State Machine | 维护评测 Run 的状态转换和领域不变量 | `PromptEvaluationRun` 聚合 | 支持 collecting 内补样、显式 blocked/recovery，收集完整后统一审核 | 代码保留领域名，不为模式而改名 |
...
| Run BSON 大小 | 现有生产 `status` 工作流只输出数据库级统计，没有执行 `$bsonSize` 分布查询；普通 SSH 账户也不能读取生产 Mongo 配置 | 不能据此宣称单文档永远安全，也不能据此立即拆集合 |
| raw/normalized 输出分布 | 当前管理 API 和生产审计没有输出 P50、P95、最大字节数 | 在启用完整 v2 Run 前补一次只读测量；未得到结果前，多集合保持未来架构 |

因此当前决策是：**先用单 Run 文档完成快速验证；把 BSON 实测设为完整生产 Run 的前置证据，而不是先用理论上限驱动多集合重构。**
...
- 现有生产 Run 的 `$bsonSize` 分布；
- generation/semantic raw 与 normalized 的 P50、P95、最大值；
- 按首版 Policy 推算的文档上界；
- 单文档 round-trip 和接近上界的 BSON 测试。

当前工作树已增加只读工具 `scripts/oneoff/audit_ai_explanation_prompt_evaluation_size`，直接使用 Mongo 返回的原始 BSON 字节统计上述分布，并按 70 次 Generation + 70 次 Semantic 上限推算观测载荷。扫描被截断或缺少任一类输出样本时，工具不给出可用的容量结论。

若实测不能证明有足够余量，则停止第二批启用，转入“未来架构：独立 Execution 集合”。
...
只有在这次局部改造后 `OnlineRunner` 仍无法隔离测试，才提取 `CandidateProcessingPipeline` 类型。
...
3. 单条 Recheck 能证明原 `semantic_evaluation_failed` 已可定位，且不修改源 Run；
...
工作树验收审计结论：第 1～2、4～11 项已有直接领域、应用、路由或契约测试；第 3 项已由单条受控 Recheck 测试证明“独立终态且不修改源 Run”，但生产 Recheck 仍未执行；第 12 项已证明 140 次最坏预留、原始 BSON 统计口径和缺样本时失败关闭。2026-09-01 本地隔离 Mongo 7 Replica Set 已进一步验证 v2 同集合版本隔离、CAS，以及容量预留、Evidence 和 Outbox 在启动及 Generation 完成失败时整体回滚；对应 integration tests 已纳入现有 CI Mongo job。生产 BSON/输出分布与完整 v2 Run 仍是启用前门禁。因此当前是“代码实现完成、真实 Mongo 行为已验证、生产验收未完成”，不能标记为已发布或 production ready。
