...
| Run BSON 大小 | 普通 SSH 账户不能读取生产 Mongo 配置；工作树已为 Database Operations 增加 `audit-ai-evaluation-size` 只读 operation，但尚未推送或执行 | 执行结果出来前不能宣称单文档永远安全，也不能据此立即拆集合 |
| raw/normalized 输出分布 | 只读 operation 会全量扫描并输出 P50、P95、最大字节数，但当前仍没有生产结果 | 在启用完整 v2 Run 前执行；未得到结果前，多集合保持未来架构 |
...
当前工作树已增加只读工具 `scripts/oneoff/audit_ai_explanation_prompt_evaluation_size`，直接使用 Mongo 返回的原始 BSON 字节统计上述分布，并按 70 次 Generation + 70 次 Semantic 上限推算观测载荷。Database Operations 的 `audit-ai-evaluation-size` operation 会在生产 `infra-network` 内以无 capability、只读文件系统的临时容器执行全量扫描，凭据只通过受保护环境变量传递。扫描被截断或缺少任一类输出样本时，工具不给出可用的容量结论。
