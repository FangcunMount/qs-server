...
> 截至 2026-09-01，当前状态：**轻量 v2 已合并、通过 CI 并部署生产，用户能力仍关闭**。历史 v1 Run `635231960356106798` 的内容契约失败与通用语义裁判失败，证明了评测编排、失败分类、裁判证据和零失败发布门禁的问题；v2 因此采用单 Run 文档中的 Slot/Generation/Candidate/Semantic 分离、有界补执行、Mongo CAS、容量总预留、Outbox/Worker/gRPC、`result_unknown` 人工决策、G1～G5 终审以及 Profile 仅接受 approved v2 Evidence。后端提交 `4f9dab9ea594cea192d83a9ef6bdfb12e62f85be` 的 [CI 33470001144](https://github.com/FangcunMount/qs-server/actions/runs/33470001144) 与 [Production Deploy 33470547988](https://github.com/FangcunMount/qs-server/actions/runs/33470547988) 均成功；生产 Run `635356837083886126` 已收集 35 个 review-ready Candidate 和 35 份裁判证据并进入 `awaiting_review`，治理台显示 `0 / 70` 条人工审核。历史 BSON 与 Generation 输出分布已取得；包含 Semantic Execution 的完整 v2 BSON 复测和生产 Recheck 仍未完成。多集合、增量审核和自动 Prompt 反馈只保留为未来触发项；`participant_enabled=false`，首个 approved Evidence 和生产 Profile 尚未形成。
...
四份 Schema、服务端嵌入访问器和契约编译测试均已落库。轻量 v2 已接入同一 Mongo 集合的独立 PO/Mapper/Repository、CAS Application Service、容量事务、Outbox 事件、Worker/gRPC 路由和分步 Runner；新管理写入口使用 `/internal/v2`，v1 评测 REST 只保留查询。后端提交 `4f9dab9ea594cea192d83a9ef6bdfb12e62f85be` 已通过 CI 并部署，生产 Run `635356837083886126` 已走完自动收集阶段；这仍不等于人工 Gate 通过或用户能力开放。
...
当前生产 Run `635356837083886126` 仍冻结 Prompt/Suite/Profile v1 与 v6/v3，已经得到 35 个 review-ready Candidate 和 35 份 semantic receipt，并进入 `awaiting_review`。服务端尚无该 Run 的双角色人工审核；对候选的本地预审发现的主要问题是把共现写成因果或机制、在缺少等级/常模时创造分类、把 focus area 当作事实，以及在不允许时组合父子维度。Prompt/Suite v2 只针对这四类已观测问题增加约束，仍需部署后创建全新的 v2 Run 验证，不能把本地预审或旧 Run 伪装成 v2 发布证据。
...
历史 v1 实现中，`generation_attempts=35` 只表示 35 个冻结执行记录均已落库，不表示 35 份候选输出和 35 次裁判全部成功；任一技术 `failure` 都使 `can_review=false`、`can_finalize=false`。生产 v1 Run `635231960356106798` 因此在 33 条可用证据之外仍被 2 条不同失败整体冻结。该缺陷已由 v2 替代：生产 v2 Run `635356837083886126` 已完成 35 个 Slot 和 35 份裁判证据，并进入 `awaiting_review`；当前阻塞是尚未提交 70 条人工审核，不是自动收集失败。
...
| 在线评测与审核 | v2 已部署并进入生产待审阶段 | 固定 35 Slot、两类有界 Execution、管理 `/internal/v2` 写接口、防重放测试；治理台显示 35 个待审 Candidate | 70 条真实审核、approved Run、Finalize/Gate |
...
| 生产启用 | 治理/评测已启用，Participant 关闭 | v6/v3 与轻量 v2 已部署；生产 Run 已形成 35 Candidate/35 Semantic 并进入待审；治理台空数组兼容修复已部署 | 70 review、approved Evidence、Profile、灰度、SLO、告警和用户生产可用性 |
