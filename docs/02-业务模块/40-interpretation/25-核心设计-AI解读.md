...
> 截至 2026-08-31，当前状态：**生产治理与评测运行时已启用，用户能力仍关闭，生产质量尚未通过**。v1 的机器契约、Participant 主链、异步执行、Mongo 持久化、Provider Adapter、输出校验、在线评测、双角色人工审核、Profile 治理、容量治理和数据生命周期均已有仓库实现；生产已注入 DeepSeek 密钥并固化 180/365/35 天保留策略。活动生成/裁判路由已回退并发布为 `v6/v3 Responses`，部署排空修复也已消除本轮 `provider_result_unknown`。但生产 Run `635231960356106798` 仍出现 2 条不同性质的失败：一条生成输出违反冻结内容契约，一条语义裁判失败被压缩成无法继续定位的通用错误。它证明当前瓶颈已经从“Provider 接口能否调用”转移到**评测编排、失败分类、裁判证据和零失败发布门禁**。四份发布治理机器契约及 Policy/Failure/Slot/Execution/Candidate/Gate v2 领域不变式已落库；现行 `PromptEvaluationRun`、Mongo、Application、Runner 和治理台尚未切换，因此评测补样和裁判补执行仍是规划改造。`participant_enabled=false`，首个生产 Profile 尚未发布，也没有用户灰度或生产可用性结论。
...
发布治理不能再把“35 个样本位”误建模为“恰好执行 35 次且全部零故障”。目标设计要求每个样本位自动接受**第一个满足生成结构与内容契约的 Candidate**，并为该 Candidate 独立补齐裁判证据；两类执行都只允许在冻结成本上限内补充。所有失败执行仍不可变保留并进入可靠性/契约符合率门禁，不能通过补样隐藏模型不稳定，也不能因一条已知可隔离的失败冻结其余有效样本。
...
| 评测收集模型 | 7 个 case × 5 个有效 Candidate；执行失败可有界补样，所有执行仍保留 | v2 机器契约与领域不变式已实现；Run/Mongo/Application 尚未接线 |
| 质量发布 | 35 个有效 Candidate、35 份完整裁判证据、70 条双角色人工复核，并检查全部执行的可靠性/契约符合率 | 部分实现；当前门禁需重构 |
...
| 失败模型 | 区分基础设施、结果未知、协议、内容契约、裁判执行和质量失败 | Taxonomy v1 与领域分类已实现；现行 Runner 尚未切换 |
| 裁判证据 | 独立保存裁判请求身份、Provider receipt、原始/规范化输出、校验与失败分类 | Evidence v2 契约与领域不变式已实现；持久化/应用接线未实现 |
...
| Candidate Pipeline | `OnlineRunner` 中的单次 attempt 处理路径 | 冻结资产解析 → Provider 生成 → 输出规范化 → 确定性校验 → 独立模型裁判 → 证据提交 | 将 Candidate Slot、Generation Execution 和 Semantic Execution 分离 | `OnlineRunner` 当前还承担多 attempt 编排，先修语义与证据，再决定是否提取类型 |
| Validation Rule Set | 对候选输出运行全部确定性规则并保留每条证据 | `EvaluateCandidate` + `Assertions` | 增加稳定的内容契约子码和规则版本收据 | 会收集 `passed` / `failed` / `blocked`，不是“首个 Handler 处理即结束”的经典职责链 |
| Run State Machine | 维护评测 Run 的状态转换和领域不变量 | `PromptEvaluationRun` 聚合 | 支持 collecting 内补样、显式 blocked/recovery、Candidate 就绪后增量审核 | 代码保留领域名，不为模式而改名 |
...
这组映射用于统一领域沟通和对外宣讲，不是当前的代码重命名计划。当前优先级已经因生产证据发生变化：在下一轮完整生产评测前，必须先补齐裁判失败证据、失败分类和有界补样语义；否则继续重跑只会反复抽样。完成行为改造和新一轮生产证据后，如果 `OnlineRunner` 仍表现出职责过多、变更放大或测试隔离困难，再启动独立结构性重构；届时先以表征测试保护行为，再分批提取 Candidate Pipeline，而不是只做模式化改名。
...
flowchart LR
    ...
    Pipe[CandidateProcessingPipeline]
...
业务契约要求交给服务端 validator 的内容是一个严格 JSON 对象，不接受额外说明或自由文本 fallback。Provider Adapter 可以针对已审阅、无歧义的线协议包装生成独立 `validation_output`，例如唯一完整 Markdown JSON 围栏；它必须同时保留原始正文，且不能补字段、改业务内容或修复非法 JSON。任何未知、含糊或多重包装仍然失败关闭。
...
四份 Schema、服务端嵌入访问器、契约编译测试以及领域层 Policy/Failure/Slot/Execution/Candidate 不变式已落库。这只证明契约与领域语义可编译、可校验；现行 `PromptEvaluationRun` 仍使用 `AttemptRecord v1`，新 Run 的 Mongo/Application/Runner 迁移是下一个独立批次。

这些契约属于治理层，不应塞入 `AIExplanationProfile v1`。Profile 决定用户解读内容与策略；Evaluation Policy 决定如何证明一个 Profile/Prompt/Route 组合可以发布。二者版本独立，但必须共同进入评测 Release Identity。
...
当前 `PromptEvaluationRun + AttemptRecord` 已能冻结 Release、运行 35 个生成记录、追加校验和人工审核证据，但 `AttemptRecord` 同时表示“固定样本”“生成执行”和“裁判结果”，导致一条裁判失败会让整个样本既无法完成也无法补足。目标模型拆为：

| 模型 | 回答的问题 | 关键不变量 | 状态 |
| --- | --- | --- | --- |
| `PromptEvaluationRun` | 这组 Release Identity 是否收集到足够发布证据 | 冻结 Release、Execution Policy 和 Gate Policy；证据只追加 | 旧聚合已实现；v2 领域证据/状态机已实现，现行聚合与持久化待迁移 |
| `CandidateSlot` | 7 个 case × 5 个分层样本位中，这一位由哪个 Candidate 占用 | 身份为 `case_id + slot_ordinal`；只接受第一个符合执行契约的 Candidate | v2 领域不变式已实现；Run 未接线 |
| `CandidateGenerationExecution` | 为某个 Slot 发起的一次生成调用发生了什么 | 一次执行最多调用生成 Provider 一次；成功、失败、未知都不可变 | v2 领域不变式已实现；现行仍由 `AttemptRecord` 承载 |
| `Candidate` | 哪份规范化输出进入质量评测和人工审核 | 必须来自 Slot 的首个结构可接受执行；不可人工挑选“更好答案” | v2 领域不变式已实现；Run 未接线 |
| `SemanticEvaluationExecution` | 对一个已接受 Candidate 的一次模型裁判调用发生了什么 | 与生成调用分开保存 Provider receipt、原始/规范化输出和失败 | v2 领域不变式已实现；Runner/Mapper 未接线 |
| `AssertionReceipt` | 某条确定性或语义规则的结论与依据是什么 | 规则类型、scope、ordinal、版本和状态稳定 | 已实现，需补失败子码 |
| `HumanReview` | 领域/安全责任人如何判断 Candidate | 两个角色、两个真实审核人、理由必填、不可覆盖 | 已实现 |
| `GateResult` | 为什么本 Run 被批准或拒绝 | 同时评估样本完整性、质量、可靠性、契约符合率和人工审核 | 已实现质量部分，规划扩展执行指标 |
...
因此 Candidate 形成门只负责回答“这是不是一个可以被稳定解析、展示和评测的 AIExplanationOutput”；引用、Profile、安全和语义质量回答“这个 Candidate 是否合格”。应用层应先独立完成 typed parse/normalization，再执行会返回 receipts 的 ValidationRuleSet，不能因为一条质量规则失败而丢失规范化 Candidate 或跳过模型裁判。
...
索引与迁移由 Mongo migration 25～33 管理，覆盖当前运行集合、评测证据、日预算、活跃槽、人工重试、TTL、数据主体导出和治理目录稳定分页。专用 Replica Set 集成测试验证事务回滚、唯一约束、治理目录分页、冷启动 Schema 和 TTL Monitor；这些测试已在 [PR #34 CI 33057698726](https://github.com/FangcunMount/qs-server/actions/runs/33057698726) 实际通过。
...
基础设施层把业务 Provider port、冻结 Route 和具体 HTTP 线协议拆开，当前支持 OpenAI Responses、DeepSeek Responses，以及 DeepSeek [Beta Strict Tool Calls](https://api-docs.deepseek.com/guides/tool_calls/)：

- 当前主 Provider 为 DeepSeek；生产生成 `v6` 与裁判 `v3` Route 均为 `deepseek-v4-pro + responses`；OpenAI 保留为显式配置的备选 Provider；
- OpenAI 默认端点为 `https://api.openai.com/v1/responses`，发送 `strict: true` JSON Schema 并显式设置 `store: false`；
- DeepSeek Responses 兼容协议使用 `https://api.deepseek.com/responses`；`json_schema` 请求不发送该契约未声明的 `strict`，也不发送 `store`；该协议继续用于历史 Route 语义和显式兼容配置；
- DeepSeek strict-tool 协议使用 `https://api.deepseek.com/beta/chat/completions`，只声明一个函数、设置函数 `strict: true` 并用 `tool_choice` 强制调用它一次；函数参数 Schema 会内联本地 `$defs`、把 `const` 转成单值 `enum`、把对象字段全部设为 required，并在每层对象设置 `additionalProperties: false`；
...
- `json_schema` 把完整 Schema 交给 Provider；`json_object` 只让 Provider 保证有效 JSON 包络，因此服务端仍对原始正文执行完整 v1 Schema 校验，绝不把“有效 JSON”误判成“业务契约合格”；
- 非流式 Response 允许一个 message 内出现多个有序 `output_text` 内容分片，Adapter 必须先按顺序合并，再执行 JSON Schema 校验；多个 message 仍视为协议异常；
- DeepSeek Responses API 默认启用思考模式，`max_output_tokens` 同时覆盖推理 token 与最终结构化输出；`reasoning.effort`、token 上限和 timeout 都是执行语义，必须进入冻结 Route 身份，不能在既有 Run 上静默修改；
- Provider Route 对模型、线协议、思考强度、Structured Output 模式和确定性包络兼容规则使用显式 revision；`provider_protocol` 进入 Route 指纹。历史空值等价于 Responses，切换 strict-tool 必须发布新 revision，不能静默改变旧 Run；
- DeepSeek 返回唯一且完整的 Markdown `json` 代码围栏时，Adapter 保留原始正文用于审计，只把围栏内部的单个合法 JSON object 交给严格校验。对生产已观测的 `parameters`、`json`、`json_string` 唯一单键包装也只做确定性解包；额外字段、数组、非法 JSON、尾随解释或任何歧义正文都不修补并继续失败关闭；
...
发布治理中的“独立模型裁判”目前只实现了独立 Prompt、独立 Route 和独立调用，生产仍由同一个 DeepSeek Provider/model 承担生成和裁判，因此并不具备 Provider 故障域独立性。目标设计允许为裁判冻结不同 Provider/model，或者冻结一个明确且有上限的 fallback chain；fallback 的顺序、适用错误和每一跳执行身份都必须进入 Release Identity，不能在运行时静默换模型。

2026-08-28 至 2026-08-31 的生产评测验证了冻结参数和本地校验都不可省略：早期组合先后出现 token 截断、无最终 message 和裁判超时；调整冻结思考强度、token 上限和 timeout 后，这些传输层问题收敛。`json_object` 一轮虽得到 35/35 合法 JSON，却得到 35/35 v1 Schema 不合格，证明 JSON 语法不是业务契约。之后的 `v7/v4 strict-tool` 实验在 Run `634965161702076974` 中出现 22/35 技术失败，其中包含裁判请求拒绝、包装层字段和 JSON 语法问题。因此活动路由回退并发布为 v6/v3，strict-tool 保留为未通过生产证据的实验协议。

最新 Run `635231960356106798` 在 v6/v3 上得到 35/35 生成执行，但仍有 2 条失败：`PROMPT-EVAL-006 #2` 的当前 code 是 `provider_output_content_contract_invalid`，按目标分类属于 `output_contract_conformance`，说明 Provider 返回了可读取 JSON，但没有满足冻结内容约束；`PROMPT-EVAL-004 #5` 被压缩为通用 `semantic_evaluation_failed`，当前证据无法区分裁判 Schema、Decode、Receipt 或 Decision 契约问题。本轮没有 `provider_result_unknown`，说明部署排空修复与此次失败无关。它进一步证明下一步不能继续只调 token、兼容包装或重跑完整 Run，而应先升级失败证据和评测状态机。
...
当前实现把这些目标直接解释为“恰好 35 次生成 + 最多 35 次裁判”，所以任何一次无法形成 Candidate 或裁判收据，整个 Run 都不能进入人工审核。目标设计把**样本目标**和**外部执行预算**分开：样本目标仍然是固定的 35 个 Slot；生成和裁判允许在冻结上限内补充执行，但绝不无限“跑到通过”。
...
独立模型裁判的执行条件是“Provider 输出已通过结构解析并形成规范化 Output”，而不是“所有确定性质量门已经通过”。因此，结构合法但命中确定性硬门的候选仍要保留独立裁判证据；无法解析、无规范化 Output 或 Provider 调用失败的候选不送裁判。前者仍由硬门阻止发布，后者作为技术失败或缺失裁判阻止评测批准。
...
infrastructure_success_rate
  = 得到确定 Provider 终态的调用数 / 已 dispatch 调用数
...
`result_unknown` 不得从基础设施分母排除；否则系统会通过“不知道结果”美化成功率。业务质量分数只基于预先冻结的 35 个 accepted Candidate，执行可靠性则基于该 Run 的全部调用。第一版阈值必须通过历史 Run 分布、目标 SLO 和成本共同校准后写入 `AIExplanationReleaseGatePolicy v1`，不能在本文中把未经验证的数字伪装成生产标准。

当前实现中，`generation_attempts=35` 只表示 35 个冻结执行记录均已落库，不表示 35 份候选输出和 35 次裁判全部成功；任一技术 `failure` 都使 `can_review=false`、`can_finalize=false`。生产 Run `635231960356106798` 因此在 33 条可用证据之外仍被 2 条不同失败整体冻结。这是**当前实现事实和待改缺陷**，不是目标设计。

目标设计允许已经接受且裁判证据完整的 Candidate 在 Run 仍处于 collecting 时进入增量人工审核队列。失败执行只读展示、禁止审核；其他 review-ready Candidate 不再被连带冻结。Run 只有在 G2 完整后才能进入 `awaiting_review`，只有 G1～G5 全部计算完成后才能 `approved|rejected`。这不会让人工意见掩盖技术故障，因为 G3 仍独立评估全部失败执行。

只有 approved 证据与待发布 Profile 的 Profile/Prompt/Schema/Route release identity 完全一致，并且 published selector slot 不冲突时，才允许 `draft → published`。评测完成不会自动发布 Profile，Profile 发布也不会自动打开用户流量开关 `participant_enabled`。

模块运行与用户流量采用两个独立开关：`enabled` 装配 Profile 治理、审计和可选的在线评测；`participant_enabled` 才注册 Participant API、用户侧 Worker 执行器、数据主体导出和 Participant lease recovery。后者必须依赖前者，默认关闭。这使生产环境能够在没有任何用户请求进入 AI 链路时，先完成真实评测、双角色审核和 Profile 发布。
...
审核工作台通过 `GET /prompt-evaluations` 按可信机构和状态读取稳定 keyset 分页摘要，再按 Run/Slot/Candidate 显式读取单份证据；列表不批量返回完整 Input 和 Provider 原始输出。目标查询投影必须分别显示 `slots_required/accepted`、`generation_executions`、`semantic_receipts`、`human_reviews`、各类失败和剩余预算，避免继续把“35/35 执行”误写成“35/35 有效 Candidate”。Profile 治理通过 `GET /profiles` 与 `GET /profiles/{profile_id}/versions/{version}` 读取版本目录和定义。游标绑定资源类型、机构、状态、`created_at` 与 `domain_id`，不能跨机构、跨筛选条件复用。
...
当前 v1 每个评测 Run 保守预留 70 次调用；同一机构最多一个 collecting Run。单条诊断复测每次独立预留 2 次调用，与完整 Run 共用机构 UTC 日预算账本，但不占用 collecting Run 发布槽。取消、失败或 `result_unknown` 不退款，因为系统无法可靠证明外部调用未发生。
...
Run 启动前一次性预留最坏成本，运行中不得越过上限。若第一版策略采用“每 case 最多 2 个额外生成执行、每个 Candidate 最多 1 个额外裁判执行”，最坏调用数会高于 70，必须重新计算日预算能支持的 Run 数量；本文不把建议参数直接写成生产配置。上线前还要决定并测试“可证明未调用的余额是否返还”，默认仍按不返还设计。
...
发布评测的目的不同：它必须保存生成和裁判的受限原始/规范化输出，才能审计 Provider 协议、内容契约和人工判断。此类正文只属于内部评测证据，按 365 天保留，不进入 Participant 导出、普通列表、日志或 Prometheus label。当前生成证据已经保存 raw/normalized output；目标改造需为失败的语义裁判补齐同等级证据。
...
首个生产策略已于 2026-08-27 确认为 `ai-explanation-retention-v1`：Participant 终态记录保留 180 天，Prompt 评测终态证据保留 365 天，UTC 日容量账本保留 35 天。生产 YAML 使用 Go `time.Duration` 可解析的 `4320h`、`8760h`、`840h` 表达；这项确认关闭了“期限未决”，但不替代历史数据 backfill、备份恢复、真实授权导出和 TTL 生产行为验收。
...
当前缺口是：生成链路已有细分 Provider code 和原始输出，但语义裁判中 Schema、Decode、Receipt 和 Decision 错误会被统一压缩为 `semantic_evaluation_failed`；失败时也没有保存裁判 raw/normalized output 和 Provider receipt。因此治理台无法回答“是裁判模型输出问题，还是 qs-server 本地校验问题”。
...
内容契约也应从单一 `provider_output_content_contract_invalid` 拆成低基数子码，例如 output item count、insight evidence count、suggestion origin/source、plain-text constraint 和 duplicate reference。原始解码器错误、Provider message 和用户内容仍不得直接成为 label 或普通日志。
...
| Evaluation Slot/Execution/Candidate 模型 | 机器契约与领域不变式已实现，运行时未接线 | `PromptEvaluationEvidence v2`、Policy/Failure 契约编译测试与领域测试 | Mongo/Application/Runner 迁移、容量和生产行为 |
| Participant 主链 | 已实现 | REST/gRPC、授权边界、Outbox/Worker、查询与导出单测 | 客户端 UX 和跨进程预发 |
| OpenAI/DeepSeek Provider Adapter | v6/v3 Responses 已发布 | 生成/裁判协议与 Endpoint 独立冻结；已知唯一 JSON 包装确定性解包；低基数 HTTP 失败码；v7/v4 回归证据保留；最新 Run 无 `provider_result_unknown` | DeepSeek 对完整业务契约的稳定符合率、跨 Provider 裁判独立性 |
| 输出校验与 Safety | 已实现首版 | typed JSON、ref、Profile、确定性规则单测 | 真实内容忠实度与人工安全结论 |
| 在线评测与审核 | 部分实现，需先改造再重跑 | 当前 35+35 编排、证据 CAS、70 条双角色 gate、失败 Run 只读证据、单条复测；Run `635231960356106798` 暴露零失败门禁 | 有效 Candidate 补样、增量审核、完整裁判失败证据、真实 reviewer 和 approved run |
| 失败分类与裁判证据 | Taxonomy v1/Evidence v2 契约与领域模型已实现，现行链路未切换 | `ClassifiedFailure`、独立 Generation/Semantic Execution 证据校验 | 现行 `semantic_evaluation_failed` 尚未映射子码，Runner/Mapper 尚未写入新证据 |
| Profile 治理 | 已实现 | draft/publish/disable、发布门禁、唯一 selector slot 和版本目录查询 | 已发布生产 Profile |
| 容量治理 | 当前 70 次模型已实现；补样预算规划改造 | 日预算、活跃槽、限流和治理查询单测 | 新 Policy 最坏预算、实际价格和竞争定标 |
...
| 生产启用 | 治理/评测已启用，Participant 关闭 | v6/v3 已部署、治理台可运行和诊断、缺少配置时 fail closed | 新评测模型发布、新 35 Candidate、70 review、Profile、灰度、SLO、告警和生产可用性 |
...
目标是下一次生产失败能够回答“Provider 返回了什么、在哪一条规则失败、是否可以安全继续”，不改变 Run Gate：

1. 为语义裁判引入独立执行证据，保存 receipt、raw/normalized output、route identity 和 validator receipts；
2. 将 `semantic_evaluation_failed` 拆成稳定 typed code；
3. 将内容契约错误拆成低基数子码，同时保留安全详情和原始证据；
4. 治理台按 case/slot/execution 展示失败，失败执行禁止审核，accepted Candidate 证据可正常查看；
5. 增加按 stage/kind/code/route revision 的低基数指标和跨进程关联日志。

这一批是后续状态机改造的前置条件。没有证据，不应继续通过修改 Prompt、token 或包装兼容来猜测根因。

### 2. 第二批：分离 Slot、生成执行和裁判执行

目标是建立正确领域语义：

1. 发布 `EvaluationExecutionPolicy v1` 和 `GatePolicy v1` 机器契约；
2. `PromptEvaluationRun` 冻结 35 个 Slot、执行策略、Gate 策略和最坏成本；
3. 新增 Generation Execution 与 Semantic Execution 持久化模型；
4. 以 CAS 接受每个 Slot 的第一个结构/内容契约合格 Candidate；
5. 质量失败 Candidate 不允许替换；裁判执行失败只补裁判；
6. 增加 `blocked`、受控恢复和预算耗尽语义；
7. 历史 `AttemptRecord` 只读兼容，不做伪造迁移。

### 3. 第三批：重写发布 Gate 和审核投影

目标是停止“一条失败冻结全部有效样本”：

1. G1～G5 分别计算并持久化原因；
2. collecting 阶段的 accepted Candidate 可增量进入双角色审核；
3. Gate 同时检查 35 个有效 Candidate 和全部失败执行的可靠性/契约符合率；
4. 治理 API 分别返回 Slot、Generation、Semantic、Review、Cost 和 Gate 进度；
5. Profile 发布只接受新 GatePolicy 下的 approved Run；旧 Run 保留诊断价值，但不能被解释为新门禁已通过。

### 4. 第四批：容量、迁移与生产验证

1. 容量账本按 Policy 最坏调用数预留，不再固定 70；
2. 对重复事件、并发补样、双 Worker claim、事务回滚、lease 过期和 result unknown 做 Replica Set 集成测试；
3. 用 stub 故障矩阵验证每个 stage/code 的 disposition；
4. 部署时保持 Participant 关闭，验证历史 Run 与新 Run 查询兼容；
5. 先执行少量受控生产探针，再启动完整 Release Run；
6. 完成 35 个 accepted Candidate、35 份裁判、70 条人工审核和 Profile 发布后，才进入用户灰度决策。

### 5. 代码落点

| 设计职责 | 当前主要载体 | 预期改造 |
| --- | --- | --- |
| Run 聚合与 Gate | `domain/.../evaluation/PromptEvaluationRun` | Slot、Execution Policy、blocked/recovery、G1～G5 |
| Candidate Pipeline | `application/.../evaluation/OnlineRunner` | 按 Slot 推进、生成/裁判执行分离、typed disposition |
| 确定性规则 | `EvaluateCandidate + Assertions` | 内容契约子码、规则版本和全量 receipts |
| 裁判执行 | `infra/.../semantic/Evaluator` | 返回成功或完整失败证据，不再只返回普通 error |
| Provider Strategy | `application port.Provider` | 能力等级和冻结 Route 语义 |
| Provider Adapter | `infra/.../responsesapi.Provider` | 保持协议适配，不进入业务补偿 |
| 持久化 | `infra/mongo/.../aiexplanation` | 新证据版本、唯一索引、历史兼容和 TTL |
| 管理 API | `transport/rest/.../ai_explanation_administration` | Slot/Execution/Gate 查询与受控恢复命令 |
| 治理前端 | `qs-operating-system` AI 治理台 | 分层进度、失败证据、增量审核、成本与 Gate 展示 |

完成这些语义改造后再评估是否提取同名 `CandidateProcessingPipeline` 类型。若 `OnlineRunner` 仍可通过小范围重构保持清晰，就不为了模式名称制造额外抽象。
...
当前首要问题是领域语义和证据缺口，而不是类型名称。先定义 Slot、Execution、Candidate、失败 disposition 和 Gate，再用测试落实；如果随后 `OnlineRunner` 仍然同时承担过多职责，再提取单次 Pipeline。直接把现有代码改名或拆成多个 Handler，不会自动修复错误的零失败门禁。
