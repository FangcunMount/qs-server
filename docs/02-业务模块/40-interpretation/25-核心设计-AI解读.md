...
> 当前代码将新 Run 默认资产切换为不可变 Prompt/Suite/Profile v4：显式要求生成建议来源空数组、纠正稳定结果的模式选择、允许基于已有描述比较情境，并要求关注点在正文和建议组织中体现。七组输入、用例断言、Profile 策略、v2 发布门槛和执行预算均保持；历史 v1–v3 按冻结身份继续读取。
> v4 尚未形成生产通过证据。已有流程测试覆盖管理员启动、35 候选、双角色批量审核、终审和独立发布，但测试替身不证明模型质量或生产存储。`participant_enabled=false`；仍需新的 approved Run、独立 Profile 发布及参与者链路验收后才能开启用户流量。
