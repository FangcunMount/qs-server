...
> 当前状态：**生产治理与评测运行时已启用，用户能力仍关闭，生产质量尚未通过**。v1 的机器契约、领域模型、Participant API、异步执行、Mongo 持久化、输出校验、在线评测、双角色人工审核、Profile 发布、容量治理和数据生命周期均已有仓库实现；生产已注入 DeepSeek 密钥并固化 180/365/35 天保留策略。多轮冻结评测已依次暴露并收敛 timeout、token 截断、空消息、Markdown 围栏和粗粒度失败证据，但最近一次 `v6/v3 + deepseek-v4-pro` 复测仍有 1 条 Provider 输出未通过 `AIExplanationOutput v1`。当前 `v7/v4` 发布候选改用 DeepSeek Beta 的强制单函数 `strict=true` 调用，同时继续由服务端执行完整 Schema、引用、Profile 和 Safety 校验，并修复输出克隆把空数组改变为 `null` 的语义漂移。该候选尚未取得新的 35+35 生产通过证据，因此 `participant_enabled=false`，Profile 未发布，也没有用户灰度或生产可用性结论。
...
- 当前主 Provider 为 DeepSeek；生产候选的生成 `v7` 与裁判 `v4` Route 均冻结为 `deepseek-v4-pro + deepseek_strict_tool_call`，开发默认值仍可独立配置；OpenAI 保留为显式配置的备选 Provider；
...
- strict-tool 的 `thinking.type` 与顶层 `reasoning_effort` 分别承载开关和强度，非流式请求显式发送 `stream: false`；生成 `none` 映射为 disabled，裁判 `low` 映射为 enabled/low，这些值随 Route 指纹冻结；
...
2026-08-28 至 2026-08-29 的生产评测验证了冻结参数和本地校验都不可省略：早期组合先后出现 token 截断、无最终 message 和裁判超时；调整冻结思考强度、token 上限和 timeout 后，这些传输层问题收敛。`json_object` 一轮虽得到 35/35 合法 JSON，却得到 35/35 v1 Schema 不合格，证明 JSON 语法不是业务契约。恢复 Responses `json_schema` 后又暴露 Markdown 围栏偏差，`v6` 只做证据保留型的确定性单围栏解包。最近一次 v6 复测将技术失败压缩到 1 条，但该条仍是 first-attempt hard gate 失败，不能忽略或进入人工审核。`v7/v4` 因此把结构约束移到 DeepSeek strict-tool 通道，并保留服务端完整校验与细分的低基数 Schema 失败码；旧 Run 继续保留原冻结身份和失败证据，不做原地改写。
...
| OpenAI/DeepSeek Provider Adapter | v7/v4 strict-tool 发布候选已实现，待部署复测 | 多轮生产实调已定位 Worker deadline、token incomplete、空消息、围栏、Schema 边界和失败指标；最近一次 v6 复测仍有 1 条 Schema 技术失败。v7/v4 强制单次 strict function call，协议进入冻结 Route 指纹，服务端继续完整校验 | v7/v4 部署后的无技术失败 35+35、内容质量、供应商保留和真实成本 |
| 输出校验与 Safety | 已实现首版 | typed JSON、ref、Profile、确定性规则单测 | 真实内容忠实度与人工安全结论 |
| 在线评测与审核 | 已实现框架，现有生产 Run 尚未达到发布门禁 | 35+35 编排、证据 CAS、70 条双角色 gate、Org/status 审核目录；技术失败 Run 禁止 review/finalize、允许查看安全失败证据并支持审计取消；Schema 失败已细分为低基数 stage/code 指标 | v7/v4 部署后无技术失败的生产 Run、真实 reviewer 和 approved run |
