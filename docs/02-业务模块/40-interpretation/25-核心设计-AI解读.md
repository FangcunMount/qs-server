...
> 当前状态：**生产治理与评测运行时已启用，用户能力仍关闭，生产质量尚未通过**。v1 的机器契约、领域模型、Participant API、异步执行、OpenAI/DeepSeek Responses API Adapter、Mongo 持久化、输出校验、在线评测、双角色人工审核、Profile 发布、容量治理和数据生命周期均已有仓库实现；生产已注入 DeepSeek 密钥、固化 180/365/35 天保留策略并完成五次冻结评测 Run。第四轮 Run `634833605914276398` 的 15 条技术失败由 9 条输出 token 截断、4 条输出基数异常和 2 条裁判超时组成。v2 修复发布后，第五轮 Run `634846378593366574` 将生成成功收敛到 34/35，形成 26 份独立模型裁判，无 token 截断或超时；唯一技术失败是 DeepSeek 在 7.54 秒、956 output tokens 后以 `completed/no_message` 结束的 `provider_output_cardinality_invalid`。因此当前 v3 候选仅将生成端思考强度从 `low` 调整为 `none`，保留 12000 token 和 120 秒上限；独立裁判继续使用 `v2 / low / 8000 / 180s`。因此 `participant_enabled=false`，Profile 未发布，也没有用户灰度或生产可用性结论。
...
基础设施层将 Responses API 线协议与具体供应商拆开，当前显式支持 `openai` 和 `deepseek`。DeepSeek 能力基线以其[Responses API 官方契约](https://api-docs.deepseek.com/guides/responses_api/)为准：
...
- DeepSeek 默认端点为 `https://api.deepseek.com/responses`，使用其 Responses API 声明的 `json_schema` 格式；请求不发送其契约未声明的 `strict` 和 `store`，服务端本身是无状态 API；
...
- Provider Route 对思考强度使用显式枚举；未配置表示沿用供应商默认，生产 DeepSeek Route 则显式使用 `low`，避免默认 high 思考挤占最终 JSON 的 token 和调用时限；
...
2026-08-28 的第四轮生产 35 次评测验证了冻结参数的必要性：默认 high 思考、8000 total output tokens 和 120 秒裁判超时组合产生 9 次 token 截断、4 次无最终 message、2 次裁判超时。修复后的 `v2` Route 将生成/裁判思考强度显式设为 `low`，生成上限提高到 12000，裁判超时提高到 180 秒。第五轮生产 Run 验证 v2 消除了 token 截断与超时，但仍有 1 次生成调用在远低于 token 上限时返回 `completed/no_message`。因此 `v3` 只关闭生成端 thinking，不放宽 Output Schema 或技术失败门禁；旧 Run 继续保留原冻结身份和失败证据，不做原地改写。
...
| OpenAI/DeepSeek Responses API Adapter | 已完成五轮生产实调，v3 候选待发布 | 前四轮定位并修复 Worker 外层 deadline、output token incomplete、多 `output_text` 分片、gRPC connection-age 中断、Attempt lease/NSQ 重投、裁判漏跑和粗粒度失败码观测。第五轮 Run `634846378593366574` 在 v2 上完成 35 个 generation attempt：34 个生成成功、26 份裁判成功，没有 token 截断或超时；唯一失败是 `completed/no_message`，在 7.54s 和 956 output tokens 后被正确分类为 `provider_output_cardinality_invalid`。v3 候选将生成端设为 `reasoning.effort=none`，独立裁判继续使用 `v2 / low / 8000 / 180s`，同时保留既有超时外层边界 | v3 部署后的无技术失败 35+35、质量、供应商保留和真实成本 |
| 输出校验与 Safety | 已实现首版 | typed JSON、ref、Profile、确定性规则单测 | 真实内容忠实度与人工安全结论 |
| 在线评测与审核 | 已实现框架，五次生产 Run 均因技术失败不可审核 | 35+35 编排、证据 CAS、70 条双角色 gate、Org/status 审核目录；技术失败 Run 禁止 review/finalize 并支持审计取消；第五次 Run 将技术失败降至 1/35，并用于定位 low thinking 下的偶发 `completed/no_message` | v3 修复后无技术失败的生产 Run、真实 reviewer 和 approved run |
