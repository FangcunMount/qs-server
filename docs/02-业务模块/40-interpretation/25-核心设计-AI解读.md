...
> v6 与 Semantic Prompt v2 尚未形成生产通过证据。v5 已完成 35 次生成与 35 次语义调用，但存在 6 个候选的硬断言失败，不能据此发布。新版本测试覆盖历史父子混合输出拒绝与最小同级组合通过、裁判身份和消息绑定；测试替身不证明模型质量。`participant_enabled=false`；仍需新 approved Run、独立 Profile 发布和参与者验收。
