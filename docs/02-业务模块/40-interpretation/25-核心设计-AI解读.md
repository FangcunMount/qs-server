...
> 截至 2026-09-01，当前状态：**轻量 v2 已合并、通过 CI 并部署生产，用户能力仍关闭**。生产 Run `635426176763965998` 已验证 v8/v5 Route：35 次生成和 35 次裁判均一次成功，G3 可靠性与契约符合率均为 100%。
...
第一版阈值必须通过历史 Run 分布、目标 SLO 和成本共同校准后写入 `AIExplanationReleaseGatePolicy v1`，不能在本文中把未经验证的数字伪装成生产标准。
