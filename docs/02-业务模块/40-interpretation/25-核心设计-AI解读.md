...
> 当前状态：**代码发布候选，生产治理与评测运行时已启用，用户能力仍关闭，生产质量尚未通过**。v1 的机器契约、领域模型、Participant API、异步执行、OpenAI/DeepSeek Responses API Adapter、Mongo 持久化、输出校验、在线评测、双角色人工审核、Profile 发布、容量治理和数据生命周期均已有仓库实现；生产已注入 DeepSeek 密钥、固化 180/365/35 天保留策略并发起三次冻结评测 Run。三次 Run 都形成不可改写的技术失败证据，尚无可进入人工审核的通过候选；第三轮后的超时层级、NSQ 重投、裁判漏跑和失败码观测修复仅有仓库测试证据，尚未部署。因此 `participant_enabled=false`，Profile 未发布，也没有用户灰度或生产可用性结论。
...
- DeepSeek 的 `max_output_tokens` 同时覆盖推理 token 与最终结构化输出；该上限和 timeout 都属于冻结 Route 身份，生产值必须由真实评测校准，不能在既有 Run 上静默修改；
...
| OpenAI/DeepSeek Responses API Adapter | 已完成三轮生产实调，第三轮后修复候选待发布 | 前两轮定位 Worker 外层 deadline、output token incomplete、多 `output_text` 分片和 gRPC connection-age 中断；第三轮完成 35 个 generation attempt，但只有 25 个生成成功、11 份裁判成功，进一步暴露评测 Attempt lease 短于 Worker RPC、NSQ 默认消息超时造成在途重投、确定性硬门提前返回导致裁判漏跑，以及指标只能看到聚合 `invalid_response`。当前补丁显式建立 `Attempt lease 6m > Worker AI RPC 5m`、`NSQ message timeout 6m > Worker AI RPC 5m`，把 connection-age grace 提高到 7m，对结构合法候选继续裁判，并增加固定白名单 failure-code 指标；全仓测试通过 | 修复部署后的无技术失败 35+35、质量、供应商保留和真实成本 |
| 输出校验与 Safety | 已实现首版 | typed JSON、ref、Profile、确定性规则单测 | 真实内容忠实度与人工安全结论 |
| 在线评测与审核 | 已实现框架，三次生产 Run 均因技术失败不可审核 | 35+35 编排、证据 CAS、70 条双角色 gate、Org/status 审核目录；技术失败 Run 禁止 review/finalize 并支持审计取消；第三次 Run 完整形成 35 个 generation attempt，并用于定位跨层超时与裁判漏跑 | 修复后无技术失败的生产 Run、真实 reviewer 和 approved run |
