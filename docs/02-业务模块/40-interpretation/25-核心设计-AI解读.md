...
> 当前状态：**生产治理与评测运行时已启用，用户能力仍关闭，生产质量尚未通过**。v1 的机器契约、领域模型、Participant API、异步执行、OpenAI/DeepSeek Responses API Adapter、Mongo 持久化、输出校验、在线评测、双角色人工审核、Profile 发布、容量治理和数据生命周期均已有仓库实现；生产已注入 DeepSeek 密钥、固化 180/365/35 天保留策略并完成六次冻结评测 Run。第六轮 Run `634853879149769262` 的 35 次生成调用均获得 completed response 和非空正文，未发生 transport、timeout 或 token incomplete；但 27/35 正文被 DeepSeek 包在 Markdown JSON 代码围栏中，因而被服务端严格 Schema gate 正确拒绝。剩余 8 份进入规范化校验，其中 6 份完成独立模型裁判，另有确定性或语义质量失败。当前 `v4` 候选把生成 Route 的原生结构化模式从 `json_schema` 切换为 `json_object`，同时继续由服务端执行完整 v1 Schema、引用、Profile 和 Safety 校验；独立裁判仍使用 `v2 / json_schema / low / 8000 / 180s`。因此 `participant_enabled=false`，Profile 未发布，也没有用户灰度或生产可用性结论。
...
- Provider Route 对思考强度和 Structured Output 模式使用显式枚举；两者都属于冻结执行语义。`v4` 生产候选的生成 Route 使用 `none + json_object`，独立模型裁判仍使用 `low + json_schema`；
...
| OpenAI/DeepSeek Responses API Adapter | 已完成六轮生产实调，v4 候选开发中 | 前五轮定位并修复 Worker 外层 deadline、output token incomplete、多 `output_text` 分片、gRPC connection-age 中断、Attempt lease/NSQ 重投、裁判漏跑和粗粒度失败码观测。第六轮 Run `634853879149769262` 的 35 次生成调用都有 completed response 和非空正文，但 27 份带 Markdown JSON 围栏；v4 候选将生成端冻结为 `json_object / none / 12000 / 120s`，增加无内容输出的包络分类指标，独立裁判保持 `v2 / json_schema / low / 8000 / 180s` | v4 部署后的无技术失败 35+35、质量、供应商保留和真实成本 |
| 输出校验与 Safety | 已实现首版 | typed JSON、ref、Profile、确定性规则单测 | 真实内容忠实度与人工安全结论 |
| 在线评测与审核 | 已实现框架，六次生产 Run 均未达到发布门禁 | 35+35 编排、证据 CAS、70 条双角色 gate、Org/status 审核目录；技术失败 Run 禁止 review/finalize 并支持审计取消；第六次 Run 将故障定位到 27 条 Markdown 围栏和少量确定性/语义质量失败 | v4 修复后无技术失败的生产 Run、真实 reviewer 和 approved run |
