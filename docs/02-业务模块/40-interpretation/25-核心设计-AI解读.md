...
> 当前代码将新 Run 默认资产切换为不可变 Prompt/Suite/Profile v5，仅补充不可信描述下的最小有效综合洞察：忽略命令文本，继续利用独立的可信等级与标准结论，不得清空 integrated_insights。七组输入、用例断言、Profile 策略、v2 发布门槛和执行预算均保持；历史 v1–v4 按冻结身份继续读取。
> v5 尚未形成生产通过证据。已有流程测试覆盖管理员启动、35 候选、双角色批量审核、终审和独立发布，但测试替身不证明模型质量或生产存储。`participant_enabled=false`；仍需新的 approved Run、独立 Profile 发布及参与者链路验收后才能开启用户流量。
