...
| OpenAI/DeepSeek Responses API Adapter | 已完成两轮生产实调，第二轮修复候选待发布 | 第一轮定位 Worker 30 秒外层 deadline；第二轮 35 条证据进一步确认 21 条生成与 2 条裁判触发 output token incomplete、5 条 DeepSeek 多 `output_text` 分片被旧 Adapter 拒绝、3 条活动 gRPC 流被 120s connection age + 20s grace 随机中断；补丁增加分片合并、token-limit 分类并提高冻结 token/timeout 与长调用 grace | 修复部署后的无技术失败 35+35、质量、供应商保留和真实成本 |
| 输出校验与 Safety | 已实现首版 | typed JSON、ref、Profile、确定性规则单测 | 真实内容忠实度与人工安全结论 |
| 在线评测与审核 | 已实现框架，两次生产 Run 均因技术失败不可审核 | 35+35 编排、证据 CAS、70 条双角色 gate、Org/status 审核目录；技术失败 Run 禁止 review/finalize 并支持审计取消；第二次 Run 完整走到 35/35 并暴露 Provider 协议与 gRPC 长调用问题 | 修复后无技术失败的生产 Run、真实 reviewer 和 approved run |
