...
> 截至 2026-09-01，当前状态：**生产治理与 v1 评测运行时已启用，用户能力仍关闭，v2 改造只存在于未发布工作树**。生产 Run `635231960356106798` 的内容契约失败与通用语义裁判失败，证明当前瓶颈是评测编排、失败分类、裁判证据和零失败发布门禁。工作树现已完成 Semantic Outcome、稳定失败码、v1 兼容证据，以及单文档 v2 Slot/Generation/Candidate/Semantic 聚合、Mongo CAS、容量预留、Outbox/Worker/gRPC、分步 Runner、`result_unknown` 人工决策、管理 REST、G1～G5 终审和 Profile 仅接受 approved v2 Evidence；全仓 Go 回归、OpenAPI、文档事实、diff lint 和分层边界检查已通过。生产只读核查确认没有未完成 Mongo Outbox，但仍有 3 个 `awaiting_review` v1 Run；BSON/输出大小分布和生产 Recheck 尚未取得。以上仓库证据不能替代发布、生产探针或完整 v2 Run。多集合和增量审核只保留为未来架构；`participant_enabled=false`，首个生产 Profile 尚未发布。
...
| 评测收集模型 | 7 个 case × 5 个有效 Candidate；执行失败可有界补样，所有执行仍保留 | v2 领域、Mongo/Application、Outbox/Worker 和分步 Runner 已在工作树接线；待发布与完整 Run |
...
| Run State Machine | 维护评测 Run 的状态转换和领域不变量 | v1 `PromptEvaluationRun` 只读；v2 `PromptEvaluationEvidenceV2` 新写 | 完成 BSON 测量、受控发布和完整生产 Run | 代码保留领域名，不为模式而改名 |
...
这组映射用于统一领域沟通和对外宣讲，不是当前的代码重命名计划。裁判失败证据、失败分类和有界补样语义已经在工作树完成，下一步是先取得 BSON 测量、受控发布和完整生产 Run 证据。只有发布验证后 `OnlineRunner` 仍表现出职责过多、变更放大或测试隔离困难，才启动独立结构性重构；届时先以表征测试保护行为，再分批提取 Candidate Pipeline，而不是只做模式化改名。
...
四份 Schema、服务端嵌入访问器和契约编译测试均已落库。轻量 v2 已接入同一 Mongo 集合的独立 PO/Mapper/Repository、CAS Application Service、容量事务、Outbox 事件、Worker/gRPC 路由和分步 Runner；新管理写入口使用 `/internal/v2`，v1 评测 REST 只保留查询。该结论只证明未发布工作树中的代码和聚焦测试，不证明生产切换或完整 Run 成功。
...
生产 v1 实现把这些目标直接解释为“恰好 35 次生成 + 最多 35 次裁判”，所以任何一次无法形成 Candidate 或裁判收据，整个 Run 都不能进入人工审核。当前工作树的 v2 把**样本目标**和**外部执行预算**分开：样本目标仍然是固定的 35 个 Slot；生成和裁判允许在冻结上限内补充执行，但绝不无限“跑到通过”。
...
| Evaluation Slot/Execution/Candidate 模型 | 工作树已完成端到端仓库接线 | 契约、领域、Mongo CAS、容量、Outbox/Worker/gRPC、分步 Runner 与管理 API 聚焦测试 | 发布后行为、完整生产 Run、BSON 余量 |
...
| 失败分类与裁判证据 | 工作树已实现，尚未发布 | `SemanticEvaluationOutcome`、稳定失败码、v1 兼容证据、v2 Semantic Execution、Mongo 往返和详情 API | 生产 Recheck、治理台真实展示和告警 |
...
本轮改造开始时的生产 v1 运行链路已经可以准确定位；它用于解释问题来源，不代表当前工作树仍以该链路承接新 Run：
...
> 实现状态：当前工作树已完成本批仓库改造与聚焦测试；尚缺发布后的单条生产 Recheck，因此不宣称生产问题已关闭。
...
> 实现状态：本批已在工作树完成领域、Mongo、Application、事务提交、事件、Worker/gRPC、Runner 和管理入口接线；尚未发布，BSON/输出分布前置核查仍未完成。
...
> 实现状态：统一 Candidate 审核、G1～G5 终审、Profile 仅接受 approved v2 Evidence、Policy 计算 140 次总预留和 `/internal/v2` 管理写接口已在工作树实现并通过聚焦测试；仍缺完整 Run、真实人工审核和生产发布证据。
...
工作树验收审计结论：第 1～2、4～11 项已有直接领域、应用、路由或契约测试；第 3 项已由单条受控 Recheck 测试证明“独立终态且不修改源 Run”，但生产 Recheck 仍未执行；第 12 项已证明 140 次最坏预留、原始 BSON 统计口径、缺样本时失败关闭，以及 `15,569,768` 字节完整 v2 文档在 Mongo 7 Replica Set 的 round-trip。同一组集成测试还验证了 v2 同集合版本隔离、CAS，以及容量预留、Evidence 和 Outbox 在启动及 Generation 完成失败时整体回滚；对应 integration tests 已纳入现有 CI Mongo job。生产 BSON/输出分布与完整 v2 Run 仍是启用前门禁。因此当前是“代码实现完成、真实 Mongo 行为已验证、生产验收未完成”，不能标记为已发布或 production ready。
