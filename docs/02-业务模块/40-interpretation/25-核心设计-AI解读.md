...
> 当前状态：**生产治理与评测运行时已启用，用户能力仍关闭，生产质量尚未通过**。v1 的机器契约、领域模型、Participant API、异步执行、OpenAI/DeepSeek Responses API Adapter、Mongo 持久化、输出校验、在线评测、双角色人工审核、Profile 发布、容量治理和数据生命周期均已有仓库实现；生产已注入 DeepSeek 密钥、固化 180/365/35 天保留策略并完成八次冻结评测 Run。第八轮 Run `634935458379346478` 的 35 次生成调用使用 `deepseek-v4-flash`，均获得 completed response，没有超时、限流、截断、空消息或网络错误；但生成端 `v5 / json_schema` 仍有 24/35 正文带 Markdown JSON 围栏，只有 11/35 为裸 JSON object，因此正好产生 24 条 `output_validation` 技术失败，只有 8 份候选进入使用 `deepseek-v4-pro` 的独立模型裁判。当前 `v6` 候选将生成与裁判统一为 `deepseek-v4-pro`，保留原始 Provider 正文作为审计证据，只对唯一且完整的 `json` 围栏做确定性解包，再执行原有严格 Schema、引用、Profile 和 Safety 校验；独立裁判 Route 同步升为 `v3`。因此 `participant_enabled=false`，Profile 未发布，也没有用户灰度或生产可用性结论。
...
requested Outbox
  ...
  → 持久化 dispatching
  → 已配置 Provider 的 Responses API 单次调用
...
### 4. Responses API 适配层与 Provider Profile

基础设施层将 Responses API 线协议与具体供应商拆开，当前显式支持 `openai` 和 `deepseek`。DeepSeek 能力基线以其[Responses API 官方契约](https://api-docs.deepseek.com/api/create-response/)为准：

- 当前主 Provider 为 DeepSeek；生产 `v6/v3` 生成与裁判 Route 均冻结为 `deepseek-v4-pro`，开发默认值仍可独立配置；OpenAI 保留为显式配置的备选 Provider；
- OpenAI 默认端点为 `https://api.openai.com/v1/responses`，发送 `strict: true` JSON Schema 并显式设置 `store: false`；
- DeepSeek 默认端点为 `https://api.deepseek.com/responses`；Route 可冻结为其 Responses API 声明的 `json_schema` 或 `json_object`。`json_schema` 请求不发送其契约未声明的 `strict`，两种模式都不发送 `store`；
...
- Provider Route 对模型、思考强度、Structured Output 模式和确定性包络兼容规则使用显式 revision；它们都属于冻结执行语义。`v6` 生产候选的生成 Route 使用 `deepseek-v4-pro + none + json_schema`，独立模型裁判 `v3` 使用 `deepseek-v4-pro + low + json_schema`；
- DeepSeek 返回唯一且完整的 Markdown `json` 代码围栏时，Adapter 保留原始正文用于审计，只把围栏内部的单个合法 JSON object 交给严格校验。无语言标记、多围栏、数组、非法 JSON、尾随解释或任何歧义正文都不做修补并继续失败关闭；
- 两者都只执行非流式、无工具、无业务 metadata 的单次请求，并继续通过本地 Schema/ref/Profile/Safety 确定性校验；
- `ai_explanation.provider` 选择当前进程的 Provider，两条生成/裁判 route 暂时共用同一 Provider 和 Endpoint，只允许模型不同；
...
2026-08-28 至 2026-08-29 的生产 35 次评测验证了冻结参数和本地校验都不可省略：第四轮默认 high 思考、8000 total output tokens 和 120 秒裁判超时组合产生 9 次 token 截断、4 次无最终 message、2 次裁判超时。`v2` 将生成/裁判思考强度显式设为 `low`，生成上限提高到 12000，裁判超时提高到 180 秒；第五轮消除了 token 截断与超时，但仍有 1 次 `completed/no_message`。`v3` 关闭生成端 thinking 后，第六轮的 35 次 Provider 调用全部返回非空正文，但 27 份正文带 Markdown JSON 围栏。`v4` 切换到 `json_object` 后，第七轮 35 份正文均为合法 JSON，证明包络问题已收敛；但 35/35 均不符合 v1 Schema，证明 `json_object` 并不是业务结构化输出。`v5` 恢复 Responses API `json_schema` 后，第八轮 35 次 Provider 调用全部成功，但 24 份正文仍带围栏并被正确标记为技术失败。`v6` 因此增加证据保留型的确定性单围栏解包，不改变或补齐任何业务字段；旧 Run 继续保留原冻结身份和失败证据，不做原地改写。
...
v1 每个评测 Run 保守预留 70 次调用；同一机构最多一个 collecting Run。默认日预算 140；配置预算只需不少于 70，不能组成完整 70 次预留的余额不会放行新的 Run。取消、失败或 `result_unknown` 不退款，因为系统无法可靠证明外部调用未发生。
...
| OpenAI/DeepSeek Responses API Adapter | 已完成八轮生产实调，v6 候选开发完成待部署 | 前七轮定位并修复 Worker deadline、token incomplete、多 `output_text` 分片、连接中断、lease/重投、裁判漏跑、粗粒度失败码及 JSON 语法/Schema 边界。第八轮 Run `634935458379346478` 的 35 次 flash 生成调用全部 completed，但 24/35 正文带 Markdown 围栏并成为技术失败；v6 保留原文，只确定性解包唯一完整 JSON 围栏，并将生成/裁判统一为 `deepseek-v4-pro`、冻结身份升为 `v6/v3` | v6 部署后的无技术失败 35+35、内容质量、供应商保留和真实成本 |
| 输出校验与 Safety | 已实现首版 | typed JSON、ref、Profile、确定性规则单测 | 真实内容忠实度与人工安全结论 |
| 在线评测与审核 | 已实现框架，八次生产 Run 均未达到发布门禁 | 35+35 编排、证据 CAS、70 条双角色 gate、Org/status 审核目录；技术失败 Run 禁止 review/finalize 并支持审计取消；v5 把 Schema 不合格提升为 `output_validation` 技术失败，v6 修复已识别的单围栏包络偏差 | v6 部署后无技术失败的生产 Run、真实 reviewer 和 approved run |
...
| 生产启用 | RC2 配置候选 | 治理/评测启用、Participant 流量独立关闭、缺少配置时 fail closed | 部署验收、真实评测、Profile、灰度、SLO、告警和生产可用性 |
