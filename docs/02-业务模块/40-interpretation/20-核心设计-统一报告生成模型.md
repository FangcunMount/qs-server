...
当前 input adapter 会根据冻结常模表和 Outcome 中的 T 分恢复维度解释。该实现的目标是恢复展示文案，但它也会在 Level 缺失时填充 level code。因此，
是否已越过“Interpretation 不重新判定 Outcome”的边界，必须作为已识别的审计点，不能只将它描述为普通报告渲染。
...
| 失败阶段 | 当前分类 | 持久证据 |
| --- | --- | --- |
| 读取 Outcome 失败 | repository / application error | Starter 前不形成 Run；可分类拒绝写 AdmissionFailure |
| 解码 Outcome 或 ReportInput 失败 | input adapter / stable admission kind | AdmissionFailure，不制造 Generation/Run |
| RenderingKey 无法形成 | input / `unsupported_mechanism` | 已准入时形成 Run Failure |
| Registry 找不到 Builder | template / `builder_not_found` | Run Failure |
| Builder 返回错误 | build / `build_failed` | Run Failure |
| Builder 返回空 Draft | build / `empty_draft` | Run Failure |
| Draft 无法构造合法 Report | build / `invalid_artifact` | Run Failure |
...
### 16.1 TemplateVersion 已成为显式发布资产

当前内置发布目录包含 `standard`、`mbti`、`sbti`、`bigfive`、`enneagram` 五个 TemplateID，每个均发布历史 `legacy-v1` 与当前 `2026-08-v1`，共 10 个不可变 release manifest。
manifest 将 ReportType、DecisionKind、BuilderIdentity、ContentSchemaVersion 与 AdapterKey 固化，并由测试校验其覆盖全部已注册机制。

ModelCatalog 发布时必须为报告 section 显式填写 TemplateID/TemplateVersion，并校验 release 已发布、模型类型匹配且同一 ReportMap 路由一致。Outcome 冻结这组身份后，Interpretation 按精确版本解析；
缺失版本或未知 release 会 fail-closed，不再使用默认版本或动态目录猜测。
...
### 16.6 Preview 与生产共享 Builder 解析

当前 Preview 与生产都通过默认 Builder Registry 和 `ResolveByMechanism` 解析 Builder，Preview 不再直接实例化 TypologyBuilder。架构测试禁止重新引入这条旁路。

Preview 仍保持进程内、无 Generation/Run、无生产 ReportID、无 Catalog 和无生产事件；共享的是纯解析与 Build 机制，不是生产生命周期。
