...
当前内置发布目录包含 `standard`、`mbti`、`sbti`、`bigfive`、`enneagram` 五个 TemplateID，每个均发布历史 `legacy-v1` 与当前 `2026-08-v1`，共 10 个不可变 release manifest。manifest 将 ReportType、DecisionKind、BuilderIdentity、ContentSchemaVersion 与 AdapterKey 固化，并由测试校验其覆盖全部已注册机制。

ModelCatalog 发布时必须为报告 section 显式填写 TemplateID/TemplateVersion，并校验 release 已发布、模型类型匹配且同一 ReportMap 路由一致。Outcome 冻结这组身份后，Interpretation 按精确版本解析；缺失版本或未知 release 会 fail-closed，不再使用默认版本或动态目录猜测。
