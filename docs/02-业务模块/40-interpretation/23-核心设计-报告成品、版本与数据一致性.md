...
Catalog 是后来建立的读模型。它在新成功事务中会随 Artifact 一起更新，但无法自动为之前已经存在的 Archive 和 Artifact 创建历史索引。

`scripts/oneoff/backfill_interpretation_report_catalog` 用于：

- 从 Archive 回填 Catalog；
- 从 Artifact 回填或更新 Catalog；
- 保证 Artifact 优先、最新 Artifact 获胜；
- 检查缺失 Catalog、悬空 Source 和关联异常。

工具默认 dry-run，只有显式 `--apply` 才会写入；`--verify-only` 可以单独做对账。它还支持按来源、起止 ID、batch size 和 worker 控制分阶段执行。

### 14.2 Archive 回填为什么还要查 MySQL

Archive 中的 `domain_id` 在历史契约中被当作 AssessmentID，但 Archive 本身不是可信的完整授权关联源。回填时，工具使用 MySQL Assessment 核对 OrgID 和 TesteeID，遇到 Assessment、Org 或 Testee 缺失会跳过并记录，而不是盲目信任历史报告中的冗余字段。

Artifact 已经固化 OrgID、AssessmentID 和 TesteeID，因此 Artifact 回填只需 MongoDB。

### 14.3 为什么 Artifact 回填强制单 worker

同一 Assessment 可能存在多份 Artifact。如果多个分页并发回填，较旧 Artifact 可能在不同页面与较新 Artifact 竞争。虽然 Catalog 更新条件本身具有防逆序能力，工具仍将 Artifact 阶段 workers 固定为 1，降低大批量历史重建的竞争与运维噪声。
...
### 16.6 Catalog 只有回填工具，没有持续对账闭环

新成功路径会同事务写 Catalog，历史数据可以使用 one-off 工具回填与 verify。但当前没有后台对账任务持续发现：
...
| Catalog 回填和 verify | `scripts/oneoff/backfill_interpretation_report_catalog/main.go` |
...
go test ./scripts/oneoff/backfill_interpretation_report_catalog
```
...
- Backfill dry-run、apply、verify 和多 Artifact 获胜规则。
