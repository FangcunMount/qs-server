...
> 状态：本文已按当前源码重写。InterpretReport 不可变成品、Generation / Run / Artifact 关系、成功事务和 `report_query_catalog` 当前报告选择已落地；空 `archived_reports` 已由 Mongo migration 22 退役。
...
| ContentSchemaVersion | Interpretation | 报告内容契约按哪种结构解码 | InterpretReport、Mongo 与 generated event |

BuilderIdentity 不是又一个业务版本，它是“哪个生成机制实现了这份成品”的可追溯身份。当前 BuilderIdentity 已进入 InterpretReport、Mongo、generated event 和日志，成功提交会校验这些来源证据一致。
...
- 内置目录发布 5 个 TemplateID × `legacy-v1` / `2026-08-v1`，共 10 个不可变 release；
- ModelCatalog active snapshot 显式冻结 TemplateID/TemplateVersion；
- Registry 按精确版本解析，不跨 TemplateVersion 回落；缺失或未知 release 会 fail-closed。

所以准确表述是：

> 当前已建立可发布、可冻结、可审计的双版本模板资产体系；新版本通过新 Generation/Artifact 与旧版并存。当前没有开放“按任意历史版本查询”的外部 API，Catalog 仍只选择每个 Assessment 的当前正文。

### 5.3 ContentSchemaVersion 决定“读取端如何理解内容”

字段增加、字段语义改变、某个可选字段变为必填，都不应只靠代码版本猜测。一份长期保存的报告成品应该明确声明其内容 schema。

当前 Committer 要求 Builder 提供 ContentSchemaVersion，并把它同时固化到 Artifact 与 `interpretation.report.generated` 事件；提交时校验 Artifact、Builder 声明和事件 payload 使用同一值。历史缺失字段会恢复为显式 legacy schema 标记，不根据文档形状静默猜测当前 schema。
...
`interpret_report_artifacts` 保存完整报告正文。如果列表查询直接在大文档上执行组织、受试者、
模型、风险和时间筛选，查询成本会与正文大小直接绑定。

`report_query_catalog` 把“当前正文选择”预先投影为每个 Assessment 一行，列表先在紧凑索引上筛选和分页，
只批量加载当前页面所需的 Artifact 正文。
...
| SourceKind | 正文来源；当前只允许 `artifact` |
| SourceID | 被选中正文的 ID |
| ModelCode | 按测评模型筛选 |
...
生产只读预检确认 `archived_reports` 文档数为 0，`report_query_catalog` 的 archive 来源引用也为 0。Mongo migration 22 已在再次验证这两个条件后删除集合；读模型、Catalog 修复与定时审计只接受 canonical Artifact 来源。
...
| Catalog 原子选择条件 | 防止旧 Artifact 覆盖新 Artifact |
...
### 11.1 七阶段 Mongo audit 中的 Interpretation 边界

跨集合 `mongoconsistency` auditor 另外以三个阶段检查已经提交或进入 retry 的 Interpretation 事实：

| 阶段 | 检查内容 | high finding |
| --- | --- | --- |
| `generation_run` | 非 pending Generation 必须引用存在且属于自己的 LatestRun，Generation/Run 状态映射一致 | `generation_missing_run`、`generation_run_state_mismatch` |
| `generated_terminal` | generated Generation 必须引用属于自己的 Artifact，并存在 `interpretation.report.generated` Outbox | `generated_missing_artifact`、`generated_missing_terminal_outbox` |
| `retry_outbox` | 带 retry_event_id 的 Run 必须存在匹配 Generation、event identity 和 next_attempt_at 的 retry Outbox | `retry_missing_scheduled_outbox` |

auditor 只读取 Mongo 业务集合；它不会把 Generation 改成 generated、补建 Artifact/Outbox 或重发事件。scheduler 只通过 MySQL checkpoint 保存 phase/cursor/statistics，并使用 Redis 自动续租 leader lock。正式生产 one-off 中这三个阶段 finding 为 0，但生产 scheduler 仍是默认关闭，不能把时点证据描述成持续运行保证。
...
| 历史报告正文可读，但来源字段为 legacy/unknown | 旧 Artifact 创建时尚未持久化来源字段 | 保持显式历史标记；不得根据当前 Builder 或数据形状伪造来源 |
...
Catalog 是由 Artifact 投影得到的读模型。新成功事务会在提交 Artifact 时同步更新 Catalog，current-only 路径不依赖历史回填脚本。

原有 Catalog 单次回填工具已经退役。当前维护窗口采用“删除不兼容的历史测评事实，再由新链路生成新报告”的策略，因此不能再把 Archive、Artifact 或 Catalog 单独回填到新的事实层：

- 删除历史报告时，Artifact、Generation、Run 与 Catalog 必须按同一 Assessment 范围成组清理；
...
当前 Runtime Repository 只读取 `interpret_report_artifacts`，查询先读 `report_query_catalog`。
Mongo migration 22 已删除空 `archived_reports`，运行时不会重新创建该集合。

旧集合没有转换为 canonical 数据的保证。发布恢复边界是完整 MongoDB 备份，详见
[seeddata 历史能力退役 ADR](../../05-决策记录/01-架构决策总表.md#7-seeddata-历史能力永久退役)。

## 16. 当前设计边界与剩余缺口

### 16.1 Artifact 构造器保护来源与内容契约

`NewInterpretReport` 已经检查 ReportID、GenerationID、OutcomeID、RunID、Association、ReportType、TemplateVersion、BuilderIdentity、ContentSchemaVersion 和 GeneratedAt，并执行跨机制及 Builder 特定成品契约。仍需关注的是：

- 新机制接入时是否同步定义 Builder 特定成品契约；
- 跨机制最小契约是否仍适用于认知任务等异类报告；
- Restore 路径是否只负责历史反序列化，而没有被新写入绕过。

当前写路径不能提交缺少来源身份或违反 Builder 内容契约的 Artifact。领域构造器、Committer 和 Mongo mapper 测试共同保护该边界。

后续新增 Builder 时，应扩展机制特定契约，而不是把某一种报告的必填字段升级成所有报告的通用假设。

### 16.2 Artifact 来源自证已关闭

BuilderIdentity 与 ContentSchemaVersion 已进入 Artifact、Mongo 与成功事件；Committer 校验请求、Artifact 和事件一致。历史缺失字段会恢复为显式 unknown/legacy 标记。
...
- 历史兼容只做显式标记，不冒充当前 schema。

### 16.3 TemplateVersion 发布资产已关闭

当前 10 个 release manifest 覆盖五个 TemplateID 和两个版本，ModelCatalog 发布、Outcome 冻结与 Interpretation 路由都携带显式身份。新写入使用 `2026-08-v1`，历史 `legacy-v1` 继续保留用于历史事实。

后续发布第三个版本时仍必须新建不可变 manifest、冻结模型路由并保留旧版本，不得原地替换 checksum 或删除仍需重放的 release。
...
### 16.6 Catalog 对账能力已建立，周期执行仍是运维策略

新成功路径会同事务写 Catalog；当前还提供带 checkpoint 的分批 audit、漂移列表、dry-run repair plan 和受控 apply，用于发现：
...
当前生产治理已用该能力验证 Catalog 与 Artifact/Archive 一致。系统没有在每次查询时重扫正文库，也没有把修复设为无条件后台自动写入；是否周期执行 audit、何时对 repair plan apply，仍由运维策略和显式授权决定。

这里需要区分两条 audit：Report Catalog audit 专门检查/规划 `report_query_catalog` winner，具备独立 repair plan；全局 Mongo consistency audit 检查 Generation/Run/Artifact/Outbox 等七阶段引用，只产生 finding，没有 repair port。前者的 repair 授权不能复用为后者的数据修复，后者 scheduler 的生产启用条件见[调度任务](../../04-接口与运维/07-调度任务.md)。
...
4. ReportType、TemplateVersion、BuilderIdentity、ContentSchemaVersion 和 GeneratedAt 必须存在。
...
9. TemplateVersion 必须解析到已发布的不可变 release，不能补默认版本或跨版本回落。
10. Catalog audit 与 repair plan 必须分离，apply 只能执行已冻结且仍匹配当前状态的计划。

### 17.2 需要在后续重构中补强

1. 新增 Builder 时必须同步定义并测试机制特定 Content 契约。
2. 当前版选择必须是明确业务规则，不得永远隐含等于“最后生成者获胜”。
3. 多 ReportType 或多 TemplateVersion 并存后，查询必须能指定业务身份，而不是只按 Assessment 取一行。
4. Archive 的推导字段必须与原始持久化事实可区分。
5. Migration 必须能完整重建 Runtime 实际依赖的 collection 和 index。
...
写侧版本化和发布治理已经完成：双版本 release、ModelCatalog/Outcome 显式路由、严格解析、Artifact 来源自证都已落地。尚未完成的是多 ReportType / 多版本同时对外查询：当前 Catalog 仍按 Assessment 选择一个当前正文，外部接口也没有按版本切换契约。
...
| 报告 Content 值对象 | `internal/apiserver/domain/interpretation/report` |
...
| 跨集合 audit 定义/checkpoint | `internal/apiserver/application/mongoconsistency/audit.go` |
| Interpretation 三阶段 scanner | `internal/apiserver/infra/mongo/mongoconsistency/scanner.go` |
...
go test ./internal/apiserver/application/mongoconsistency
go test ./internal/apiserver/infra/mongo/mongoconsistency
```
