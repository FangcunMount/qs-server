...
> 状态：本文已按当前源码重写。InterpretReport 不可变成品、Generation / Run / Artifact 关系、成功事务和 `report_query_catalog` 当前报告选择已落地；
> 空 `archived_reports` 已由 Mongo migration 22 退役。
...
当前 Committer 要求 Builder 提供 ContentSchemaVersion，并把它同时固化到 Artifact 与 `interpretation.report.generated` 事件；
提交时校验 Artifact、Builder 声明和事件 payload 使用同一值。历史缺失字段会恢复为显式 legacy schema 标记，不根据文档形状静默猜测当前 schema。
...
`interpret_report_artifacts` 保存完整报告正文。如果列表查询直接在大文档上执行组织、受试者、模型、风险和时间筛选，查询成本会与正文大小直接绑定。

`report_query_catalog` 把“当前正文选择”预先投影为每个 Assessment 一行，列表先在紧凑索引上筛选和分页，只批量加载当前页面所需的 Artifact 正文。
...
生产只读预检确认 `archived_reports` 文档数为 0，`report_query_catalog` 的 archive 来源引用也为 0。Mongo migration 22 已在再次验证这两个条件后删除集合；
读模型、Catalog 修复与定时审计只接受 canonical Artifact 来源。
...
auditor 只读取 Mongo 业务集合；它不会把 Generation 改成 generated、补建 Artifact/Outbox 或重发事件。
scheduler 只通过 MySQL checkpoint 保存 phase/cursor/statistics，并使用 Redis 自动续租 leader lock。正式生产 one-off 中这三个阶段 finding 为 0，
但生产 scheduler 仍是默认关闭，不能把时点证据描述成持续运行保证。
...
当前 Runtime Repository 只读取 `interpret_report_artifacts`，查询先读 `report_query_catalog`。Mongo migration 22 已删除空 `archived_reports`，运行时不会重新创建该集合。

旧集合没有转换为 canonical 数据的保证。发布恢复边界是完整 MongoDB 备份，详见 [seeddata 历史能力退役 ADR](../../05-决策记录/01-架构决策总表.md#7-seeddata-历史能力永久退役)。

## 16. 当前设计边界与剩余缺口

### 16.1 Artifact 构造器保护来源与内容契约

`NewInterpretReport` 已经检查
ReportID、GenerationID、OutcomeID、RunID、Association、ReportType、TemplateVersion、BuilderIdentity、ContentSchemaVersion 和
GeneratedAt，并执行跨机制及 Builder 特定成品契约。仍需关注的是：
...
当前 10 个 release manifest 覆盖五个 TemplateID 和两个版本，ModelCatalog 发布、Outcome 冻结与 Interpretation 路由都携带显式身份。新写入使用 `2026-08-v1`，
历史 `legacy-v1` 继续保留用于历史事实。
...
这里需要区分两条 audit：Report Catalog audit 专门检查/规划 `report_query_catalog` winner，具备独立 repair plan；
全局 Mongo consistency audit 检查 Generation/Run/Artifact/Outbox 等七阶段引用，只产生 finding，没有 repair port。
前者的 repair 授权不能复用为后者的数据修复，后者 scheduler 的生产启用条件见[调度任务](../../04-接口与运维/07-调度任务.md)。
...
写侧版本化和发布治理已经完成：双版本 release、ModelCatalog/Outcome 显式路由、严格解析、Artifact 来源自证都已落地。尚未完成的是多 ReportType / 多版本同时对外查询：
当前 Catalog 仍按 Assessment 选择一个当前正文，外部接口也没有按版本切换契约。
