...
当前 Runtime Repository 读取 `archived_reports`，新成品写入 `interpret_report_artifacts`，查询先读
`report_query_catalog`。标准 migration 创建 Generation、Run、Artifact 及 canonical indexes；
旧报告集合已由退休 migration 删除，运行时不会重新创建。
