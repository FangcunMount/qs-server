...
> 状态：本文已按当前源码重写。InterpretReport 不可变成品、Generation / Run / Artifact 关系、成功事务和 `report_query_catalog` 当前报告选择已落地；空 `archived_reports` 将由 Mongo migration 22 退役。
...
生产只读预检确认 `archived_reports` 文档数为 0，`report_query_catalog` 的 archive 来源引用也为 0。Mongo migration 22 将在再次验证这两个条件后删除集合；读模型、Catalog 修复与定时审计只接受 canonical Artifact 来源。
...
Mongo migration 22 将删除空 `archived_reports`，运行时不会重新创建该集合。
