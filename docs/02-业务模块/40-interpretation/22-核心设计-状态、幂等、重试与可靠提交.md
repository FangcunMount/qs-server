...
`isClaimConflict` 同时识别 Generation duplicate/version conflict 与 Run `(generation_id, attempt)` duplicate。极端并发首先撞到任一唯一约束时，失败方都会重读 winner 并返回 processing，而不是把已知 claim 竞争放大为传输 NACK。
...
Outcome 与 ReportInput 的解码发生在 Starter 之前，因此不会制造虚假的 Generation/Run；失败会幂等写入 MySQL `interpretation_admission_failure`，并可通过 Operations API 按组织或 Outcome 查询。
...
Run duration 由 `system_governance.retry.lease.run_duration` 管理，默认 5 分钟；恢复周期、批量与 leader lock 由独立的 `interpretation_lease_recovery` 管理，默认 10 秒、每批 100 条，且与 Evaluation 恢复和一致性审计隔离。
