...
- reconcile interval：统一配置，默认 10 秒；
- reconcile jitter fraction：与 interval 同一配置块；
- batch limit：100；
- 分布式锁 TTL：5 分钟；
- `lease_reconcile_enabled=true`。

重复业务事件也可能触发过期 lease reclaim；scheduler 是没有新事件到来时的兜底恢复。配置对象同时提供 lease 过期后和进程崩溃后的最坏恢复窗口计算。
...
### 23.2 lease 配置与恢复窗口已统一

Run duration、reconcile interval 与 jitter 已统一进入 `system_governance.retry.lease`。默认值为 5 分钟、10 秒、0 jitter，并能计算最坏恢复窗口。
