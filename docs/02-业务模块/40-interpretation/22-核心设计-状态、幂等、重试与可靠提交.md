...
两个 Worker 可能同时发现 Generation 不存在。双方都尝试创建相同业务 Key，唯一索引只允许一个成功。失败方把 Generation duplicate 或 version conflict 视为 claim
conflict，重新读取一次：
...
`isClaimConflict` 同时识别 Generation duplicate/version conflict 与 Run `(generation_id, attempt)` duplicate。极端并发首先撞到任一唯一约束时，
失败方都会重读 winner 并返回 processing，而不是把已知 claim 竞争放大为传输 NACK。
...
lease 给 running Run 一个有限执行权期限。当前时长来自 `system_governance.retry.lease.run_duration`，默认 5 分钟。
...
Interpretation lease recovery 已接入独立的 HA scheduler：
...
- run duration：统一配置，默认 5 分钟；
- recovery interval：`interpretation_lease_recovery.interval`，默认 10 秒，校验范围 10～30 秒；
- batch limit：100；
- 分布式锁：Interpretation 专用 leader lock，TTL 30 秒；
- Evaluation 租约恢复和一致性审计使用各自独立的 runner、配置与锁。

重复业务事件也可能触发过期 lease reclaim；scheduler 是没有新事件到来时的兜底恢复。恢复失败只进入 Interpretation 自身的日志和指标流，不阻塞 Evaluation 恢复或全量一致性审计。

### 10.6 lease 恢复审计

Reclaim 保留 Run 原始 `attempt_origin`，并追加 ClaimHistory、RecoveryCount 与 LastReclaimedAt。由此可以同时回答“这次业务 attempt 为什么创建”和“执行权何时、被哪个 trace 接管”。
...
Outcome 与 ReportInput 的解码发生在 Starter 之前，因此不会制造虚假的 Generation/Run；失败会幂等写入 MySQL `interpretation_admission_failure`，
并可通过 Operations API 按组织或 Outcome 查询。

AdmissionFailure 保存稳定 kind、safe code/message、retryable、decision、fingerprint 与时间证据；它与进入 Starter 后的 Run Failure 是两类明确事实。

### 23.2 lease 时长与恢复调度已显式治理

Run duration 由 `system_governance.retry.lease.run_duration` 管理，默认 5 分钟；
恢复周期、批量与 leader lock 由独立的 `interpretation_lease_recovery` 管理，默认 10 秒、每批 100 条，且与 Evaluation 恢复和一致性审计隔离。

仍需用生产指标持续验证：

- Builder 正常执行时长分布；
- lease 与 recovery interval 的比例；
- expired lease 数量和恢复耗时指标；
- 是否需要 heartbeat 续租。

### 23.3 lease recovery 审计已关闭

原始 attempt origin 保持不变；每次 reclaim 追加 ClaimHistory，并累计 RecoveryCount/LastReclaimedAt。

### 23.4 Run duplicate 已纳入 claim conflict

Starter 已识别 `interpretationrun.ErrAlreadyExists`，重读 winner 后返回 processing。唯一索引与 claim conflict 归一化共同保护并发路径。

### 23.5 report.failed 已携带 RetryDecision

failed event 复制持久化 RetryDecision，包含 disposition、attempt、policy version、下一执行时间和授权身份；下游可以明确区分 automatic、manual_required 与 terminal。

### 23.6 manual_required 已有稳定等待投影

Worker 对 automatic 保持 in-flight，对 manual_required 写入 `waiting_manual_action` 临时不可用，对 terminal 写入 failed。后续成功仍可覆盖投影。

Redis 仍是可重建状态，最终判断应以 Generation、Run 与 RetryDecision 为准。
...
| 共享重试策略 | [`internal/pkg/retrygovernance`](../../../internal/pkg/retrygovernance/) |
