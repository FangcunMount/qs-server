...
本文从 REST/gRPC 提交入口开始，说明服务端如何选择精确问卷版本、校验原始答案、可靠提交 AnswerSheet 与 Outbox，并由 worker 驱动基础计分和 Assessment intake。重点是每一阶段的成功含义、幂等依据和失败恢复边界。
...
Survey 的同步成功点是 `AnswerSheet + answersheet.submitted Outbox` 已可靠提交，不是 Assessment、Evaluation 或报告已经完成。Survey 只拥有 AnswerSheet 和基础题分；跨 ModelCatalog、Plan、Evaluation 的编排属于 `application/journey/assessmentintake`。
...
`answersheet.submitted` 使用 durable Outbox；MQ 发布失败由 relay 重试。调用方不需要为了等待下游测评而保持同步请求。
...
计分失败是硬失败：journey 不继续创建 Assessment，worker 等待重试。读取 active version 代替历史版本会破坏重放一致性，因此禁止。

### 7.3 Binding、Plan 与 Assessment
...
- Survey 计分失败：不创建新 Assessment，消息重投后按精确版本重试。
