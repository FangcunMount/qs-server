...
当前最终裁决来自 `answersheets` 上 `(submit_meta.writer_id, submit_meta.idempotency_key)` 的局部唯一索引。Redis guard、事务前查询和客户端禁止重复点击都只能降低竞争，不是最终一致性来源。旧 `answersheet_submit_idempotency` 只在内嵌元数据未命中时提供历史回查。
