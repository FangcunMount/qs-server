...
本文说明一次正式答卷提交如何从 collection-server 或管理端进入 Survey，如何按精确问卷版本校验题型、答案值、选项、显示条件和 validation rules，
以及如何将 AnswerSheet、幂等约束与 `answersheet.submitted` Outbox 原子提交。
...
如果把计分、Assessment 和报告纳入 HTTP 提交事务，会使用户请求承担跨 Survey、ModelCatalog、Evaluation 和 Interpretation 的全链路耗时与失败率。
当前设计只要求 HTTP 等待不可再放弃的作答事实提交，将后续过程交给可恢复异步链路。
...
`EnsureSubmittable` 要求 Questionnaire 为 published 且 code/version 完整。精确版本查询当前允许读取仍保留的历史 published snapshot，不要求它必须是当前 active release。
这保证了已领取精确版本的作答会话不会因新版发布而语义漂移。
...
6. 只要提交中包含本次计算为不可见的题目，就拒绝整份提交，不会静默丢弃隐藏题答案。
7. 当前可见且带 required rule 的可作答题必须存在且非空。
8. 执行 `min_length`、`max_length`、`min_value`、`max_value`、`min_selections`、`max_selections` 和 `pattern`。
9. 已发布问卷如果携带两端不支持的校验规则，将被视为发布配置不可执行，而不是用户答案错误。

可选题可以不出现在最终提交中。但只要客户端提交了某道题，它的题型、选项和规则就必须全部合法；服务端不做“忽略错误题、保存其它题”的部分接受。

### 7.3 仍未完成的严格契约

不可见题答案已由共享校验器在归一化后、required 检查前逐题拒绝；collection 预检和 apiserver 最终裁决共用该行为。下列 Section 约束仍是未完成项：
...
当前最终裁决来自 `answersheets` 上 `(submit_meta.writer_id, submit_meta.idempotency_key)` 的局部唯一索引。Redis guard、事务前查询和客户端禁止重复点击都只能降低竞争，不是最终一致性来源。
旧 `answersheet_submit_idempotency` 已由 Mongo migration 22 退役。
...
3. 插入带 `submit_meta` 的 AnswerSheet document，由局部唯一索引裁决并发。
4. 将聚合事件 stage 到 `domain_event_outbox`，并携带 request ID。
5. 提交 MongoDB transaction。
6. commit 成功后才清空聚合内存事件。
7. 调用 post-commit dispatcher 尝试立即唤醒 relay。

AnswerSheet（含 `submit_meta`）或 Outbox stage 任一步失败，事务都回滚。post-commit dispatcher 失败不能回滚已经 commit 的 AnswerSheet；常规 Outbox relay 扫描仍可恢复投递。
...
### 10.3 已实现：幂等元数据收入 AnswerSheet

新写已经将 writer ID、idempotency key、fingerprint、request ID 和 accepted time 收入 AnswerSheet MongoDB document，并建立局部唯一索引。

物理存储已经简化，但旧集合仍保留只读 fallback；这不改变本文中的可靠受理契约：
