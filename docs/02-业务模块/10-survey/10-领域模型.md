...
`Answer.Score` 是一次作答的基础题分事实。人格模型默认通过 `question_score` 消费该值，只另外声明题目贡献的目标因子、方向和权重，不应复制问卷选项分值。只有显式 `option_override` 才按 `Answer.Value` 使用模型覆盖分值；完整规则见 [ModelCatalog：人格题目贡献与因子计分](../20-model-catalog/23-核心设计-人格题目贡献与因子计分.md)。
