...
AnswerSheet 没有“草稿 → 已提交”的状态转换。客户端的提交请求就是完整的最终答卷；系统不定义草稿上传、继续编辑或部分保存协议。`Submit` 成功创建聚合时立即产生 `AnswerSheetSubmittedEvent`，
但只有后续 DurableStore 事务提交后才算可靠受理。

新提交统一使用 `Submit`，使提交上下文与 `AnswerSheetSubmittedEvent` 同时入模。`ReconstructSubmissionContext` 只用于从历史数据重建，新提交使用 `NewSubmissionContext`。
...
这里的不可变性专指**原始作答事实不可变**：QuestionnaireRef、SubmissionContext、AnswerValue 和 FilledAt 在提交后不能被计分流程修改。基础题分根据这些事实和精确问卷快照计算，
属于 AnswerSheet 的可重建延迟派生属性，可以由受控的 Survey scoring 补充或重建。

AnswerSheet 当前没有显式 scoring status 或 scored timestamp，而新建时单题分和总分都使用 `0`。因此持久化层无法只根据分值区分“尚未计分”和“已计分且真实结果为零”。这是当前设计不足；
后续可以通过显式状态、完成时间或可空派生值解决，但本文不预先指定字段方案。Evaluation 的 requested/running/completed/failed 属于 Evaluation 模块，不能直接借来表达 Survey scoring 状态。

`Answer.Score` 是由一次作答派生出的基础题分。计分成功并持久化后，下游可以把它作为稳定输入使用；在此之前不能把初始 `0` 当成已经成立的零分事实。人格模型默认通过 `question_score` 消费该值，只另外声明题目贡献的目标因子、方向和权重，不应复制问卷选项分值。
只有显式 `option_override` 才按 `Answer.Value` 使用模型覆盖分值；完整规则见 [ModelCatalog：因子与计分模型](../20-model-catalog/23-核心设计-因子与计分模型.md)。

### 4.4 领域事件

`Submit` 成功后产生 `AnswerSheetSubmittedEvent`，payload 包含 AnswerSheet ID、questionnaire code/version、testee/org/filler/task 与 submitted_at。
该事件表示“作答事实已建立”，不表示后续测评已执行。

该事件也不承诺一定存在后续测评。业务上，独立信息收集问卷在 AnswerSheet 与基础题分完成后结束；只有冻结 Admission 明确要求 Assessment 且携带完整 AssessmentModel release identity 时才进入 Assessment。
无 Admission 的历史事件仍走 live binding 兼容入口。
...
`domain/survey` 不依赖 application、infra、transport 或 worker；`application/survey` 不直接编排 Evaluation/Interpretation。
跨模块作答到测评链路放在 `application/journey/assessmentintake`。
...
- AnswerSheet 和 `answersheet.submitted` Outbox 在同一 Mongo transaction 中落库；
  独立 Questionnaire 的 Publish / Unpublish / Archive 各自在一个 Mongo transaction 中更新 head 与 published snapshot，
  已绑定问卷则通过 Assessment Release 与模型发布事实共用一个更大的 Mongo transaction。
