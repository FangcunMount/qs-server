...
提交幂等元数据已经收入 AnswerSheet MongoDB document；旧 `answersheet_submit_idempotency` 经生产空集合和零引用验证后已由 Mongo migration 22 退役。
...
因此 `questions` 以嵌套数组保存在 Questionnaire document 中。Option、ValidationRule、CalculationRule 和 ShowController 继续嵌套在对应 Question 内部。
一次读取即可得到完整提交契约，一次快照复制即可冻结完整版本。
...
先以 archived 形式插入新 snapshot，是为了在 active 切换过程中满足“一个 code 只有一个 active snapshot”的约束。当前上述步骤位于同一 Mongo transaction；
Unpublish 和 Archive 的 head 变更与 active clear 也使用同一事务。Repository 在 session context 中不改写 Redis，本地缓存失效、跨实例信令和领域事件均在 commit 后执行，避免读侧观察到最终回滚的版本。
...
Repository 现在只按 `answersheets.submit_meta` 查询幂等结果。Mongo migration 22 在删除旧集合前强制确认其文档数为 0；运行时不再持有旧集合 Repository、fallback 指标或双读路径。
down migration 只能恢复空结构，不能恢复历史数据。
...
AnswerSheet 列表通过 aggregation projection 返回 ID、questionnaire code/title、filler、total score、filled time 和 answer count。
`answer_count` 在查询管道中由 `answers` 数组长度计算，列表不返回全部答案。
...
正式 Mongo migration `000013_unified_modelcatalog_schema` 已按 head、snapshot、active 和 release history 建立局部唯一/查询索引，并有 contract 与 integration tests。
部署验收仍必须用生产 `getIndexes()` 核对名称、键、unique 和 partial filter；仓库 migration 存在不等于生产一定已执行。
