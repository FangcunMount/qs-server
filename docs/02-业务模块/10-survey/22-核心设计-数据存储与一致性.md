...
Repository 现在只按 `answersheets.submit_meta` 查询幂等结果。Mongo migration 22 在删除旧集合前强制确认其文档数为 0；运行时不再持有旧集合 Repository、fallback 指标或双读路径。down migration 只能恢复空结构，不能恢复历史数据。
