...
> 状态：主体已实现；独立问卷已在 Survey 结束且不创建 Assessment，基础计分显式状态仍是规划改造。

## 1. 本文回答

本文从一份已经可靠受理的 AnswerSheet 出发，说明它如何经由 `answersheet.submitted`、Outbox relay、qs-worker 和 Assessment Intake Journey，
完成基础题分派生、测评模型绑定解析、Plan 来源识别、Assessment 幂等创建，并在满足条件时发出 `evaluation.requested`。
...
上一篇 [答卷校验与可靠受理](./31-关键链路-答卷校验与可靠受理.md) 以 AnswerSheet 与 Outbox 共同提交为终点；本文从这个持久化事实继续展开。
Evaluation 内部怎样执行模型、保存 Outcome 和驱动报告，不在本文重复，见 Evaluation 的
[从 AnswerSheet 到 Assessment](../30-evaluation/30-关键链路-从AnswerSheet到Assessment.md) 与
[从执行请求到 Outcome 提交](../30-evaluation/31-关键链路-从执行请求到Outcome提交.md)。
...
flowchart LR
    ...
    J --> B{"冻结 Admission purpose"}
    B -->|"independent_questionnaire"| U["正常结束<br/>不创建 Assessment"]
    B -->|"assessment + exact release"| P["识别 adhoc / Plan 来源"]
...
6. **独立问卷的终点是 AnswerSheet。** Journey 返回 `no_assessment_required`，不创建 Assessment，也不进入报告等待。
...
事件携带 AnswerSheet ID、Questionnaire code/version、testee、filler、org、TaskID、request ID 和提交时间等路由上下文。它不是 AnswerSheet 的完整副本，也不是 Evaluation 的执行快照；
后续业务仍以持久化对象和精确身份为事实源。
...
`answersheet.submitted` 的恢复基础不是“在 Worker 内写一个无限循环”，而是：持久 Outbox 保留发布意图、MQ 提供重投、Journey 可重入。
具体 transport attempt、dead-letter 与治理策略属于 Event 基础设施文档。
...
> **当前不足。** 通用题目 `CalculationRule` 当前没有进入这条 scoring task 装配链路。文档不能把所有题级转换都描述成已经被异步基础计分实现；新增反向题或其它题级计算规则时，
> 需要先核对其究竟编码在 option score 中，还是需要扩展 scoring task 契约。
...
legacy kind 会先归一为当前 canonical kind/subKind/algorithm。Evaluation 创建有模型引用的 Assessment 时，
还会通过 `EvaluationModelValidator` 重新验证模型与 QuestionnaireRef 是否可执行。
...
项目当前通过“发布版本的 Questionnaire 与 Model 共同构成独立测评发布版本”来约束正常测评入口；Plan 和门诊入口只保存编码，用户实际开始任务或扫码时选择最新发布版本。
历史执行仍依赖 AnswerSheet 已冻结的精确 Questionnaire code/version。
...
`assessmentintake.Service.Ensure` 优先消费提交时冻结的 Admission：

| 场景 | 当前实现 |
| --- | --- |
| `assessment + frozen exact release` | 使用 `retained_exact` 校验保留的精确版本，创建/复用 Assessment 并自动提交 |
| legacy live binding | 使用 `active_release` 校验当前活动版本 |
| `independent_questionnaire` | 基础计分后结束，不创建 Assessment，返回 `no_assessment_required` |
| 应有 Assessment 但冻结 identity 不完整 | fail closed，不降级成独立问卷 |

历史无 Admission 事件仍走 legacy binding；这是一条兼容入口，不改变新提交必须冻结意图与 release 的要求。
...
4. 未找到时按 Admission 返回 `no_assessment_required` 或 `pending`；
5. Assessment 为 `pending` 时继续返回 `pending`；
6. Assessment 为 `submitted|evaluated` 时返回 `ready + assessment_id`，失败时返回 `failed + failure_reason`；
7. 记录 submit-to-assessment-ready 时延。

客户端只对 `pending` 继续轮询；`ready` 进入 report-status/事件订阅，`no_assessment_required` 回到 AnswerSheet 详情，`failed` 停止自动轮询并展示原因。
`ready` 不表示 Interpretation 已完成。
...
### 15.1 历史无 Admission 事件仍保留 live binding 兼容

新提交已冻结 `independent_questionnaire` 或 `assessment + exact release`：独立问卷在基础计分后正常结束，不创建 Assessment；完整测评使用冻结版本进入 Evaluation。
只有无 Admission 的历史事件仍查询当前 active binding，查不到时返回 `legacy_unclassified` 并重投，不再创建 unbound pending Assessment。该兼容入口退出前需要历史事件排空和生产命中证据。
...
`EnsureAssessment` 当前把多数 Journey 错误统一翻译为 gRPC Internal。Worker 可以据此失败重投，但运维侧不易区分配置错误、暂时依赖错误和不可重试数据错误。
具体错误分类和 retry governance 应在 Event 基础设施文档中设计，不能由 Survey 文档擅自承诺已经实现。
