...
上一篇 [答卷校验与可靠受理](./31-关键链路-答卷校验与可靠受理.md) 以 AnswerSheet 与 Outbox 共同提交为终点；本文从这个持久化事实继续展开。Evaluation 内部怎样执行模型、保存 Outcome 和驱动报告，不在本文重复，见 Evaluation 的 [从 AnswerSheet 到 Assessment](../30-evaluation/30-关键链路-从AnswerSheet到Assessment.md) 与 [从执行请求到 Outcome 提交](../30-evaluation/31-关键链路-从执行请求到Outcome提交.md)。
...
客户端只对 `pending` 继续轮询；`ready` 进入 report-status/事件订阅，`no_assessment_required` 回到 AnswerSheet 详情，`failed` 停止自动轮询并展示原因。`ready` 不表示 Interpretation 已完成。
...
新提交已冻结 `independent_questionnaire` 或 `assessment + exact release`：独立问卷在基础计分后正常结束，不创建 Assessment；完整测评使用冻结版本进入 Evaluation。只有无 Admission 的历史事件仍查询当前 active binding，查不到时返回 `legacy_unclassified` 并重投，不再创建 unbound pending Assessment。该兼容入口退出前需要历史事件排空和生产命中证据。
