...
两种 transport 当前各遗漏一部分 application result：REST `EnrollmentResponse` 已返回 planID、enrollmentID、round、完整 Task 列表和 `Idempotent`，但没有 `CreatedTaskCount`；内部 gRPC 返回 planID、Task 列表、`Idempotent` 和 `CreatedTaskCount`，但 `EnrollmentResultMessage` 没有 enrollmentID 和 round。
...
command service 仍会为响应中的预估 count 做一次 plan/testee 范围查询；真正执行终止时先读取 active Enrollment，再按 enrollment_id 读取本轮 Task，不再扫描 Plan 下所有患者任务。两次读取仍有响应计数竞态，但执行成本和业务归属已经限定到当前参与轮次。

### 7.6 持久化与事件顺序

application service 在事务内逐条保存 canceled Task，任一保存失败会使 Task 与 Enrollment 的全部变更回滚。事务成功后才 best-effort 发布 `task.canceled`；事件仍是通知，MySQL Task/Enrollment 才是权威事实。

### 7.7 事务失败不会返回成功

任一 Task 保存、Enrollment 保存或事务提交失败都会向上返回错误，不再出现“部分 Task 已取消但 API 成功”的旧语义。`AffectedTaskCount` 仍是事务前预估值，不能当作提交后逐行核对结果；成功的强语义是 active Enrollment 与本轮非终态 Task 已共同提交。

### 7.8 串行重放按 active Enrollment 幂等收敛

第一次事务失败时没有部分业务状态需要补偿，调用方可以安全重试。第一次已经成功时，active Enrollment 不再存在，后续终止为空操作成功；Task 状态不会被重复改写。`task.canceled` 仍是 best-effort 通知，事件丢失需要按 MySQL 事实对账，而不是重做终止事务。
