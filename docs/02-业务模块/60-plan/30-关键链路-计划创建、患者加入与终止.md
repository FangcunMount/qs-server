...
> 状态：**已按当前源码重写**。本文沿管理侧写命令还原 Plan 从模板创建、患者加入到终止参与的完整调用链。周期算法的数学语义见[《周期策略与任务生成》](./20-核心设计-周期策略与任务生成.md)，
> 状态、幂等和事务边界见[《状态、幂等与数据一致性》](./21-核心设计-状态、幂等与数据一致性.md)。

当前 Scope 增量把总部公共计划维护与受试者计划操作分开。后台由 `application/plan/operator_commands.go` 的显式 Operator 用例接入：校验可信公司、动作权限和 Actor 返回的范围；内部调度及扫码入组仍由各自独立入口执行，不能把后台范围检查直接套到内部服务。

| 操作 | 范围约束 | 一致性边界 |
|---|---|---|
| 创建、暂停、恢复、结束或取消公共计划 | 对应计划动作必须具有公司全部门店范围 | 公共计划不因某店获授权而成为该店私有内容 |
| 受试者加入、终止及任务变更 | 按实际动作校验受试者当前门店 | 外层事务锁定受试者归属，再调用原命令；并发转店不能使用过时归属通过写入 |
| Enrollment / Task 查询 | 应用入口将动作范围传入查询，分页和计数之前限制当前受试者集合 | 不返回范围外任务或以未过滤总数泄露数据 |
| 计划事务通知 | 原通知适配器延迟到最外层事务提交后发送 | 提交后发送不等于新增了持久化 Outbox；失败恢复能力按原机制验收 |

源码依据包括 `application/plan/enrollment_query.go`、`task_query_scope.go`、`infra/mysql/plan/ownership_lock.go` 和 `infra/plan/commit_publisher.go`。本地事务与范围测试已覆盖关键分支；真实总部身份、门店运营身份及转店后历史任务访问交接仍需上线验收。
...
它不会复用 REST Handler 中的 `TesteeAccessService.ValidateTesteeAccess`。这符合“内部系统动作”与“运营人员操作”分离的传输边界，但意味着内部调用方的服务认证和租户授权必须可靠；
application 层只会继续校验 Plan 的机构归属和 ID/状态，不会再次验证 clinician-patient relation。
...
Plan module 正常由 composition root 注入 `PublishedModelLister`。但 `NewPublishedScaleCatalog(nil)` 会得到 nil，
create workflow 遇到 nil catalog 时会直接跳过 scale 校验，而 Plan module 初始化不会因此失败。
...
两种 transport 当前各遗漏一部分 application result：REST `EnrollmentResponse` 已返回 planID、enrollmentID、round、完整 Task 列表和
`Idempotent`，但没有 `CreatedTaskCount`；内部 gRPC 返回 planID、Task 列表、`Idempotent` 和 `CreatedTaskCount`，
但 `EnrollmentResultMessage` 没有 enrollmentID 和 round。
...
sequenceDiagram
    ...
    E->>DB: 开启事务并校验 Plan 属于 org
    E->>DB: 查询 active Enrollment
    E->>DB: 按 enrollment_id 查询本轮 Task
    loop 每条非终态 Task
        E->>E: TaskLifecycle.Cancel
        E->>DB: Save(task)
    end
    E->>DB: Enrollment.Terminate + Save
    DB-->>E: commit
    loop 已提交的 canceled Task
...
这个 count 是命令开始前的**预估受影响数**，不是数据库提交后核对出来的实际成功数。预查和执行之间还存在并发窗口：Task 可能被打开、完成或取消；但终止事务本身不会再静默提交一部分 Task。

### 7.4 领域终止规则

`enrollmentService.TerminateEnrollment` 当前会：

1. 校验 Plan 属于当前 org；
2. 查询该 plan/testee 的 active Enrollment；
3. 按 enrollment_id 查询本轮 Task；
4. 对所有非终态 Task 调 `TaskLifecycle.Cancel` 并保存；
5. 将 Enrollment 标记为 terminated 并保存；
6. 在同一 MySQL 事务提交以上变更。
...
### 7.5 执行查询已收敛到 active Enrollment

command service 仍会为响应中的预估 count 做一次 plan/testee 范围查询；真正执行终止时先读取 active Enrollment，再按 enrollment_id 读取本轮 Task，不再扫描 Plan 下所有患者任务。
两次读取仍有响应计数竞态，但执行成本和业务归属已经限定到当前参与轮次。

### 7.6 持久化与事件顺序

application service 在事务内逐条保存 canceled Task，任一保存失败会使 Task 与 Enrollment 的全部变更回滚。事务成功后才 best-effort 发布 `task.canceled`；
事件仍是通知，MySQL Task/Enrollment 才是权威事实。

### 7.7 事务失败不会返回成功

任一 Task 保存、Enrollment 保存或事务提交失败都会向上返回错误，不再出现“部分 Task 已取消但 API 成功”的旧语义。`AffectedTaskCount` 仍是事务前预估值，不能当作提交后逐行核对结果；
成功的强语义是 active Enrollment 与本轮非终态 Task 已共同提交。

### 7.8 串行重放按 active Enrollment 幂等收敛

第一次事务失败时没有部分业务状态需要补偿，调用方可以安全重试。第一次已经成功时，active Enrollment 不再存在，后续终止为空操作成功；Task 状态不会被重复改写。
`task.canceled` 仍是 best-effort 通知，事件丢失需要按 MySQL 事实对账，而不是重做终止事务。
...
系统已经持久化 Enrollment 的 startDate、joinedAt、terminatedAt、termination reason、round 和 record origin。当前仍未独立保存 joinedBy/terminatedBy；
如业务要求追溯具体操作者，应扩展 Enrollment 审计字段，而不是从 Task 或日志反推。
