...
> 历史验收判定（2026-08-06）：当时为**有条件通过，无开放 `FINAL-P1`**。286,552 个 Task 均有 revisioned schedule fact，legacy fulfillment fallback 为 0，计划、入组、任务链可回放；缓存、通知、失败恢复与容量仍是黄色运行证据。该生产数据和验收没有针对当前 checkout/源码复核基线重新执行，不能提升为当前 CI、部署或生产结论；当前版本证据以[当前版本定档验收台账](../../00-总览/09-当前版本定档验收台账.md)为准。
...
上述条目是风险/演进台账，不表示当前生产已经发生对应事故。schedule revision 治理、生产历史修复、Statistics repair/validate/publish 与 cohort 对账是在 2026-08-06 历史截点完成；下文状态在实施前必须按当前代码和数据重新复核。
...
`TerminateEnrollment` 已改为按 active Enrollment 加载本轮 Task，并在同一 MySQL 事务中取消非终态 Task、终止 Enrollment；任一步失败整体回滚，旧的非事务领域终止入口已经删除。Resume 方向仍可能先把 Plan 保存为 active，再在后续 Task 保存失败时返回错误，形成“调用方收到失败、数据库却已经部分恢复”的状态。
...
部分查询会按 Plan 加载全部 Task 后在内存中过滤；REST 和 gRPC 对 Enrollment 结果的表达也不完全一致：两者均返回 `Idempotent`，REST 另有 enrollmentID/round 但缺 `CreatedTaskCount`，gRPC 有 `CreatedTaskCount` 但缺 enrollmentID/round。
