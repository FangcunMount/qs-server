...
> 历史验收判定（2026-08-06）：当时为**有条件通过，无开放 `FINAL-P1`**。286,552 个 Task 均有 revisioned schedule fact，legacy fulfillment fallback 为 0，计划、入组、任务链可回放；
> 缓存、通知、失败恢复与容量仍是黄色运行证据。该生产数据和验收没有针对当前 checkout/源码复核基线重新执行，不能提升为当前 CI、部署或生产结论；当前版本证据以[当前版本定档验收台账](../../00-总览/09-当前版本定档验收台账.md)为准。
...
当前最需要继续观察的不是“再增加一种周期策略”，而是四组运行时边界：
...
4. **调度与扩展模型仍面向当前规模**：容量、公平性、时区和多模型/滚动生成由真实增长或产品需求触发，不作为本版预建项。

上述条目是风险/演进台账，不表示当前生产已经发生对应事故。schedule revision 治理、生产历史修复、Statistics repair/validate/publish 与 cohort 对账是在 2026-08-06 历史截点完成；
下文状态在实施前必须按当前代码和数据重新复核。
...
P0 不表示要在一个 PR 中完成所有重构。它表示必须先补特征测试和数据审计，再以最小事务、条件写或持久补偿逐项关闭，不能直接用一次大规模结构调整掩盖现有行为。它属于 `PL-Rxxx` 模块风险命名空间，不与 `FINAL-P*` 互换。
...
`TerminateEnrollment` 已改为按 active Enrollment 加载本轮 Task，并在同一 MySQL 事务中取消非终态 Task、终止 Enrollment；任一步失败整体回滚，旧的非事务领域终止入口已经删除。
Resume 方向仍可能先把 Plan 保存为 active，再在后续 Task 保存失败时返回错误，形成“调用方收到失败、数据库却已经部分恢复”的状态。

#### PL-R001 风险

- paused、finished 或 canceled Plan 下仍存在 pending/opened Task；
- Plan 模板暂停/结束/取消返回成功后，患者仍可能继续使用未取消入口；
...
持久化对象包含公共 `version` 字段，但领域实体和 Mapper 没有携带、递增和比较该版本；更新也没有形成 `WHERE id = ? AND version = ?` 或 `WHERE status = opened` 的条件写。
因此当前并发语义本质上是 last-write-wins。
...
AnswerSheet 的 `202 Accepted` 已表示 AnswerSheet 与 `answersheet.submitted` Outbox 在 MongoDB 事务中可靠持久化。
Worker/Journey 随后创建并提交 Assessment，再调用：
...
提交上下文携带明确 `task_id` 时，Resolver 会检查 Task 的机构、受试者、opened 状态和模型 code。若不匹配，当前链路记录 warning 并返回 nil；
AnswerSheet 仍可继续创建 Assessment，但这次测评会被当成非 Plan 测评，原 Task 留在 opened。
...
Task 有 created/updated、openAt、expireAt、completedAt，但没有独立 expiredAt、canceledAt；Plan 也没有 pausedAt、finishedAt、canceledAt。
best-effort 事件不能作为完整审计来源，普通 updatedAt 又无法说明发生了哪种操作。
...
部分查询会按 Plan 加载全部 Task 后在内存中过滤；REST 和 gRPC 对 Enrollment 结果的表达也不完全一致：两者均返回 `Idempotent`，
REST 另有 enrollmentID/round 但缺 `CreatedTaskCount`，gRPC 有 `CreatedTaskCount` 但缺 enrollmentID/round。
...
Plan 继续晚绑定发布版本；Task 冗余同一通用 identity；ModelCatalog 提供按 kind/code 的 active admission。迁移可把现有记录映射为 `kind=scale`，
但必须同步修改入口、Resolver、Statistics、API 和索引，不能只重命名字段。
...
引入独立 Enrollment 后保存 `generated_until / next_seq / schedule_revision`，Scheduler 或生成器在有界窗口内补齐未来任务。已经生成的 Task 仍是不可随意改写的安排事实；
修改计划只影响尚未生成部分，或通过显式 reschedule workflow 处理。
