...
4. 独立 Enrollment、多模型 Plan、滚动生成等能力，哪些是当前缺陷，哪些应等待真实业务需求。
...
1. **状态提交不够真实**：Plan 生命周期和患者终止会先保存部分记录、吞掉后续 Task 保存错误，却仍返回成功；单 Task 更新也缺少真正的条件写和并发冲突语义；
...
4. **调度与参与模型仍面向当前规模**：扫描无分页和公平性治理，时区依赖进程环境，Enrollment 由 Task 集合隐式表达，多模型和滚动生成仍是未来演进项。

治理顺序应当是：

> 先让每个成功响应都对应完整、可证明的持久化结果，再让已受理测评与 Task 最终收敛；之后治理入口、提醒和调度；最后才讨论独立 Enrollment、多模型 Plan 和滚动生成。
...
| ID | 优先级 | 问题 | 状态 | 主要影响面 |
| --- | --- | --- | --- | --- |
| PL-R001 | P0 | Plan 生命周期与患者终止可能部分保存却返回成功 | 规划改造 | Plan / Task 一致性、管理端成功语义 |
...
| PL-R014 | P2 | 创建与并发加入的业务幂等仍不完整 | 规划改造 | API 重试、并发写入 |
| PL-R015 | P2 | 隐式 Enrollment 无法表达参与审计和重新加入轮次 | 待业务决策 | join / terminate / rejoin |
...
### 5.4 当前没有独立 Enrollment 聚合并不自动构成 P0

当前“患者加入 Plan”由 `(plan_id, testee_id)` 下的一组 Task 表达，满足一次加入、一次终止和简单重复请求。只有当业务需要保存加入来源、原始开始日期、终止原因、参与轮次或重新加入时，独立 Enrollment 才成为必要领域事实。
...
当前唯一键可以防止 `(plan_id,testee_id,seq)` 重复和一个 Assessment 关联多个 Task，但不能证明全部业务不变式。生命周期部分提交、并发覆盖和履约回写失败都可能留下结构合法、业务矛盾的数据。
...
### PL-R014：创建与并发加入需要稳定业务幂等

**当前事实**

重复加入已经有两层保护：领域对账会只生成缺失 Task，数据库有 `(plan_id,testee_id,seq)` 唯一键。但两个第一次加入请求并发时，二者可能都看到空集合，后提交者收到 duplicate key，而不是重新加载并归一成幂等成功。

CreatePlan 则没有调用方提供的业务幂等键；网络重试可能创建两个语义相同的 Plan 模板。

**目标设计**

- Enroll 捕获唯一冲突后重新加载、重新对账；若期望 Task 已完整存在则返回幂等成功；
- 若只存在部分 Task，则补齐缺失 Task并报告 reconciliation；
- CreatePlan 接受有作用域的 `idempotency_key`，或由外部治疗方案实例 ID 形成唯一业务来源键；
- 不用“名称 + 周期参数完全相同”推断重复，因为运营可能合法创建相同模板。

**验收条件**

- 两个并发首次加入最终都得到同一 Task 集合；
- 不吞掉真正不同 startDate 或 schedule 的冲突；
- CreatePlan 重试返回原 PlanID；
- 幂等记录有机构作用域、参数摘要、状态和过期/保留策略。

### PL-R015：何时需要独立 Enrollment 聚合

**当前事实**

`PlanEnrollment` 是领域服务，不是持久化聚合。加入事实由 Task 集合推导，因此系统无法直接回答：
...
终止后用同一开始日期重新加入时，既有 canceled Task 可能让系统返回幂等结果，却没有恢复出一轮真正可执行的新参与。

**引入条件**

只有出现以下需求之一，才建议正式引入 Enrollment：

- 必须审计医生要求和治疗方案来源；
- 支持终止后重新加入或多轮治疗；
- 支持滚动生成和独立暂停某个患者；
- 需要按参与实例而非 Plan 模板统计履约；
- Task 数量很大，不再适合通过全量集合推断加入状态。

**目标模型候选**

```text
Enrollment
  id, orgID, planID, testeeID, round
  startDate, joinedAt, joinedBy, sourceRef
  status(active/terminated/completed)
  terminatedAt, reason
```

Task 增加 `enrollment_id`，唯一性从 `(plan,testee,seq)` 演进为 `(enrollment,seq)`。迁移必须先为存量 Task 构造 round=1，不得直接破坏原唯一约束。
...
按业务优先级处理时区、内部 Testee scope、创建/加入幂等、审计和查询契约。独立 Enrollment 只有在业务确认后进入此阶段。
