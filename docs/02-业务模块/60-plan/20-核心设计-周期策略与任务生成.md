...
- `GenerateTasksUntil` 为什么不能写成当前运行能力，以及旧任务过滤/生成辅助入口为何已经删除。
...
它作为“患者参与基准日”保存在 Enrollment 中。即使 fixed_date 不使用 startDate 计算 Task 绝对日期，重复加入仍必须与 active Enrollment 的 startDate 一致。
...
旧 `FilterNewTasks` 只用 planID + seq 构造字符串键，没有 testeeID，还用 `string(rune(seq))` 表达整数序号，存在跨患者误判风险。当前主链已统一使用包含计划、患者、序号和时间轴语义的 task reconcile，该无调用辅助函数已删除。

### 11.4 重复任务生成入口已删除

旧 `GenerateTasksWithIDs` 绕过 AssessmentPlan，重复维护四套周期公式，且没有调用方。当前所有生成入口统一委托 `TaskGenerator` 与类型化 Plan 规则，避免 triggerTime、新 scheduleType 和校验语义双写漂移。
