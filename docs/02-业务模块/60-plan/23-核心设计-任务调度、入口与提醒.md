...
> 2026-09-27 增量：生产 `task_opened_reminder` 开关仍关闭，下文的 `task.opened` 最佳努力提醒仍是当前运行路径。默认关闭的新路径已有 Task 原事务提醒意图和逐收件账本：只有微信发送 API 明确返回成功才记 `confirmed`（平台接单，不代表最终送达或用户阅读）；[微信当前接口文档](https://developers.weixin.qq.com/miniprogram/dev/server/API/mp-message-management/subscribe-message/api_sendmessage)成功响应只保证 `errcode=0`／`errmsg=ok`，不保证 `msgid`，账本因而允许确认状态没有平台消息 ID 并标记 `platform_accepted_no_msgid`。网络或响应结果未知保留人工核对，不自动重发。新路径尚无生产真实发送验收，下文对旧路径“无送达账本”的描述不适用于未来启用的新路径。
