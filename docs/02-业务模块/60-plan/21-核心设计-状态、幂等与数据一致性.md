...
Task 已持久化 `planned_at`、`due_at`、`schedule_defined_at`、`open_at`、`expire_at`、`completed_at`、`expired_at` 和 `canceled_at`。Plan 自身的 paused/finished/canceled 时间仍没有独立业务列。`task.*` 事件是通知与集成信号，Task 表才是状态和时间真值。
