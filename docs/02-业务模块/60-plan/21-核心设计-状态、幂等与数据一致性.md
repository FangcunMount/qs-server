...
“先查 active Enrollment，没有再创建”能让串行重试幂等，但两个请求可能同时查到“没有”。因此数据库使用 active Enrollment 唯一约束拒绝第二轮 active 记录，并以 `(enrollment_id, seq)` 唯一键保护本轮 Task。
...
AnswerSheet 创建 Assessment 后再完成 Task，则跨越 Survey/Evaluation 与 Plan 应用边界；即使都在同一 MySQL，也需要考虑业务耦合、重放入口和补偿。
这一层通常应由“可靠事件 + 幂等消费 + 调和”完成，而不是把所有模块硬塞进一个长事务。
...
| `expired` | 错过开放窗口，或 opened 入口到期/人工终止 | `expired_at` 和 `expiration_reason` 已设置 |
| `canceled` | Task 被 Plan 命令取消 | `canceled_at` 已设置 |
...
stateDiagram-v2
    [*] --> Pending: 生成 Task
    Pending --> Opened: Open
    Pending --> Expired: missed_open_window
    Opened --> Completed: Complete(assessmentID)
    Opened --> Expired: entry_timeout / manual
...
- 替换 `planned_at`；
- 重新计算 `due_at`；
- `schedule_revision` 递增并记录 `schedule_defined_at`；
- 把 status 设为 pending；
- 清空 open/expire/completed/expired/canceled 时间和 expiration reason；
- 清空 assessmentID、entryToken 和 entryURL；
- 不生成独立领域事件。

Resume 在同一事务内先锁定 Plan 和待复用 Task，再原子保存 Plan active 与全部重排结果。重排 Task 使用专用全字段更新，把 nil/空值显式写入数据库，并以旧 `schedule_revision` 为 CAS 条件；任一更新失败会回滚整个 Resume。
...
Task 已持久化 `planned_at`、`due_at`、`schedule_defined_at`、`open_at`、`expire_at`、`completed_at`、`expired_at` 和 `canceled_at`。
Plan 自身的 paused/finished/canceled 时间仍没有独立业务列。`task.*` 事件是通知与集成信号，Task 表才是状态和时间真值。
...
| ResumePlan | Plan 已 active | 拒绝，因为不再是 paused | 首次事务失败会整体回滚，可安全重试；首次已成功则无需补齐 |
...
| 场景 | 当前结果 |
| --- | --- |
| 首次 Enroll | 同事务创建 active Enrollment 和全部期望 Task |
| 相同 Plan/Testee/startDate 串行重放 | 返回 active Enrollment 与本轮 Task，`Idempotent=true` |
| 相同 Plan/Testee 但 startDate 不同 | 与 active Enrollment 冲突并拒绝 |
| 两个首次 Enroll 并发 | active 唯一约束拒绝一方；相同 startDate 回读为幂等成功 |
| TerminateEnrollment 串行重放 | 第一次事务终止 active round；之后无 active，空操作成功 |
| 终止后再次 Enroll | 创建 round+1 和新的 Task 集合 |

Enrollment 的幂等身份来自持久化 `enrollment_id` 与 active 唯一约束；Task 的 seq 只在同一 Enrollment 内唯一。
...
TerminateEnrollment 只处理 active Enrollment：

```text
查询本轮 Task
  -> 内存中迁移非终态 Task
  -> 事务内逐条 Save
  -> Enrollment 标记 terminated
  -> 任一步失败整体回滚
```

它已经与 Pause/Finish/Cancel 的旧部分保存风险分开：终止成功明确表示 Enrollment 与本轮 Task 已共同提交。事件在提交后 best-effort 发布，不属于终止成功条件。

### 9.6 患者加入

Enroll 使用显式 transaction Runner 保存 Enrollment 与 Task；active 唯一约束解决并发首次创建窗口。

因此它的完整契约应是：

```text
串行重放：读取 active Enrollment 幂等成功
并发首次写：active 唯一约束决定最终唯一
唯一冲突后：回读 active；相同 startDate 转幂等，否则明确冲突
```

三步均已实现，仍需真实 MySQL 并发集成测试提供数据库级证据。
...
M5-05 的隔离候选为 `task.opened` 增加机构范围，并允许向 Plan 开放服务注入标准 MySQL Stager。注入时，手动与定时开放均在原 Task 事务内暂存 `task.opened.reminder.requested`，保留原开放事件 ID，成功 Stage 后不再直发旧事件；未注入时仍走上面的最佳努力路径。**当前生产装配没有注入该 Stager，事件目录也没有启用新的提醒事件**，因此不能把隔离候选解释为已上线的可靠提醒。收件人级持久处理、未知微信调用人工核验、最迟提醒时刻和旧新消息交接仍须完成。

隔离测试还模拟了事务成功提交但调用方丢失成功回执：重试开放会被已开放的 Task 拒绝，不会产生第二个入口或提醒意图。调用方此时必须查询 Task 和对应意图核对结果，不能把重试错误直接判断为原事务失败；这项模拟并不等同于真实数据库断线后的未知提交证明。
...
在 last-write-wins 下，有效 Assessment 已经创建，Task status 却可能最终被覆盖为 expired。又因 `Updates(struct)` 跳过 nil 零值，
B 不一定清除 A 写入的 assessmentID/completedAt，最终反而可能形成“status=expired，但 assessmentID/completedAt 非空”的不可解释组合，同时两个请求还可能分别发布 completed/expired 事件。
唯一 assessmentID 索引无法防止这类“不同状态的同行覆盖”。

#### 竞态 C：Plan Pause 与 Task Complete

Pause 在内存中把 opened Task 转 canceled，同时 Assessment Intake 可能基于旧 opened 快照完成同一 Task。没有事务和条件更新时，最后写入决定 status，零值跳过还可以使其他字段保留上一个写入的值；
最终状态来自时序偶然性，而不是显式业务优先级。
...
| Terminate Enrollment | active Enrollment 终止 + 本轮全部受影响 Task |
| Enroll | 新 Enrollment + 本轮完整 Task 集合 |
...
| 并发 Enrollment | 真实 MySQL 唯一冲突后是否回读为幂等成功 |
...
| 通用 MySQL 更新逻辑 | [`internal/pkg/database/mysql/base.go`](../../../internal/pkg/database/mysql/base.go) |
