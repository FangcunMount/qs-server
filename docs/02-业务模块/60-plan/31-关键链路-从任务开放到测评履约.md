...
新提交若冻结为 `independent_questionnaire`，Journey 不匹配 Plan Task、不创建 Assessment，Task 保持 `opened`。这符合业务边界：独立 Questionnaire 可以产生 AnswerSheet，但不能履约一个要求完成测评模型的 Plan Task。

无 Admission 的历史事件仍通过精确 Questionnaire code/version 查询 live binding。若 Binding 缺失，Journey 返回 `legacy_unclassified` 并让消息重投；它不会猜测 model code，也不会创建 unbound pending Assessment。只有解析出完整模型身份后，显式 Task 匹配和 opened Task 回退才会执行。
