...
### 6.2 Enrollment 是患者参与轮次

`Enrollment` 已经是独立持久化实体，保存机构、Plan、受试者、轮次、开始日期、状态、加入/关闭/终止时间、终止原因和记录来源。Task 通过 `enrollment_id` 归属于一次明确参与，序号只在该轮次内唯一。

同一患者同一 Plan 最多存在一个 active Enrollment；所有 Task 进入终态后自然 closed，显式终止则变为 terminated 并取消未完成 Task。终止后可以创建下一 round。详细分析见[领域模型](./10-领域模型.md)。
...
| 重复加入幂等与任务对账 | 已实现 | active Enrollment 唯一约束 + `(enrollment_id,seq)` 唯一键 |
