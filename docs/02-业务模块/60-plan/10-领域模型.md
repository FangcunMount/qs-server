...
> 状态：**已重写**。本文以 `internal/apiserver/domain/plan`、应用服务、MySQL 映射和迁移为事实基础，说明 Plan、Enrollment 与 Task 当前真实的领域边界。
> 目标模型和重构方向会明确标为规划，不会写成现状。
...
- enrollmentID 确定该患者的参与轮次，并间接确定 Plan 与受试者；
- seq 确定该周期中的第几次测评。
...
| `plan_id` | 来源 Plan |
| `enrollment_id` | 来源参与轮次；与 seq 组成业务唯一键 |
| `seq` | 该患者在该 Plan 下第几次测评 |
| `org_id` | 机构隔离和查询冗余 |
| `testee_id` | 真正接受测评的人 |
| `scale_code` | 本任务持续测评的模型族编码 |
| `planned_at` | 依据周期和 startDate 计算的计划开放时间 |
| `due_at` | 履约 cohort 截止：`planned_at + 7` 个上海自然日 |
| `schedule_revision` | 同一 Task ID 的调度轮次，创建为 1，每次 Reschedule 递增 |
| `schedule_defined_at` | 当前调度轮次被定义的业务时间 |
| `open_at` | 调度器实际开放时间 |
| `expire_at` | 入口硬失效时间：`open_at + 7` 个上海自然日 |
| `completed_at` | Task 被 Assessment 履约的时间 |
| `expired_at` | 进入 expired 的业务时间 |
| `canceled_at` | 进入 canceled 的业务时间 |
| `expiration_reason` | missed_open_window / entry_timeout / manual |
...
有一个需要特别说明的例外：`TaskStatus.IsTerminal()` 把 completed、expired、canceled 都视为终态；
但 Plan Resume 可以通过 `Reschedule` 复用 expired/canceled Task，把它重置为 pending。completed 永远不会被重新调度。
...
| EnrollTestee | Enrollment + 多个新 Task | 同一 MySQL 事务 |
...
| TerminateEnrollment | active Enrollment + 本轮非终态 Task | 同一 MySQL 事务，失败整体回滚 |

CreatePlan、Enrollment 加入/终止和单 Task 命令的边界已经明确。剩余问题集中在 Plan 模板生命周期与多 Task 联动：领域层完成了规则计算，但 Pause/Resume/Finish/Cancel 仍需继续核对事务和并发条件写，可能发生部分成功。
