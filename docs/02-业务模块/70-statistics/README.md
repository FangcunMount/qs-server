...
门店纯统计查询检查运营统计 read 权限、有效 Operator 与门店 Scope，服务概览、临床人员和计划执行按当前归属统计；详见 [门店专题分析](./41-门店专题分析.md)。后台专业统计查询在访问缓存或数据库前始终检查测评 statistics 权限。缓存命中不能绕过授权；原有组织范围与统计发布时间校验继续生效。
...
| [41-门店专题分析.md](./41-门店专题分析.md) | 门店专题、权限、分页与当前归属/历史开展边界 |
