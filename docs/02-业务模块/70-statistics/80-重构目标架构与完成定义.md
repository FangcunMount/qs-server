...
Coordinator 接受三种显式运行模式：

- `validate`：只读、映射、校验和计数；
- `repair`：写 Fact 并重建指定窗口 Daily，不修改全局发布水位和缓存；
- `publish`：在 Repair 能力之上重建 Fulfillment/Snapshot，并发布机构级 Cache Generation。

请求窗口只控制 Collector 和 Daily。对外 `as_of_date` 由服务端计算为前一上海完整自然日，并在结果事务中校验只能单调前进。
...
Redis 使用机构级 Generation。MySQL 结果事务提交成功后才允许切换 Generation，实际 Generation 必须写回 SyncRun。缓存失败不能回滚已提交的统计结果；Generation 尚未切换时，查询只能读取旧代缓存或快速拒绝，不能回源读取未发布结果；Generation 已切换但预热失败时才允许受 LoadGuard 保护地回源当前 MySQL 结果。
...
| `statistics_v2_org_snapshot` | 每机构一行 | OrganizationSnapshotProjection；与 V1 快照物理隔离 |
...
- Generation 实际切换时间，以及每次 `resume-cache` 的追加式审计记录。
...
sequenceDiagram
    ...
    Scheduler->>Coordinator: publish(org, repairWindow)
...
6. 历史 `repair` 不执行全局 Projection，也不发布 Cache Generation；多窗口回填完成后再执行一次当前水位 `publish`。
7. Repair 或未切换 Generation 的 Publish 不能让冷请求读取已改变的结果表；允许回源时必须在查询前后校验同一个可见 Publish Run；Content 累计查询必须受已发布 `as_of_date` 约束。
