...
> 状态：**V2 目标链路及核心实现**。Access、Assessment、Plan 的 Collector、Fact、Projection、缓存和 `/api/v2/statistics` 已串联；当前仍处于生产回填、影子对账和调用方切换前阶段。
...
Task 必须保存不可替代的业务时间，Statistics 不使用 `updated_at` 猜测动作。V2 第一阶段不统计重排次数；新一轮参与创建新 Enrollment 和 Task，同一 Enrollment 内如需重排，Plan 必须保存 Schedule Revision。
...
相比当前 V1，V2 不再在一个 Overview 请求中串行执行多组实时业务大查询。
...
它需要展示每一轮 Enrollment、每个 Task 的计划/到期/完成时间和状态，不适合从日聚合反推。因此 V2 继续直接读取 Plan 业务读模型，并应用患者资源授权。
