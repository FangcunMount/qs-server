...
Task 必须保存不可替代的业务时间，Statistics 不使用 `updated_at` 猜测动作。Task 复用同一 ID 时，每次重排递增 `schedule_revision` 并写入新的 `schedule_defined_at`。Collector 为每个轮次生成不可变 `task_schedule_defined`；当前轮次进入终态时生成同 revision 的 `task_schedule_terminal`。重排轮次只服务 Fulfillment，不计入 Activity。
...
PlanFulfillmentProjection 先按 `schedule_revision` 选择每个 Task 的最新 `task_schedule_defined`，再只读取同 revision 的 terminal Fact，以固定 `as_of` cutoff 计算：
...
没有 Schedule Fact 的旧数据依次回退到 `task_due_defined` 和旧生命周期 Fact。最新轮次存在时，旧轮次 `task_canceled` 不参与排除；只有最新 `task_schedule_terminal=canceled` 才排除该 Task。planned cohort 与 due cohort 都来自最新轮次，禁止用 `MAX(planned_at/due_at)` 猜测最新调度。
...
- 查询编排：[`application/statistics/read_service.go`](../../../internal/apiserver/application/statistics/read_service.go)
- 当前 ReadStore：[`infra/mysql/statistics/read_store.go`](../../../internal/apiserver/infra/mysql/statistics/read_store.go)
- 当前同步：[`application/statistics/coordinator.go`](../../../internal/apiserver/application/statistics/coordinator.go)
