...
> 状态：**核心代码已落地，生产迁移待验收**。本文是 Statistics V2 的活动实施台账。[重构目标架构与完成定义](./80-重构目标架构与完成定义.md) 已经确定最终状态。当前已经具备增量 migration、V2 运行链路、查询接口、回填工具和调用方版本开关，但尚未在真实环境执行历史回填、连续七天影子对账、生产切流和 V1 退役。旧 V1 问题台账已由本版路线图取代；V1 只做阻塞业务或迁移安全的必要修复，不再扩展其投影体系。
...
| ID | 优先级 | 事项 | 状态 | 依赖 |
| --- | --- | --- | --- | --- |
| ST-V2-001 | P0 | 持久化 PlanEnrollment | 代码完成，待迁移验收 | 无 |
| ST-V2-002 | P0 | 冻结 Assessment AttributionSnapshot | 代码完成，待生产验证 | 无 |
| ST-V2-003 | P0 | 建立上海时间统一组件与迁移审计 | v0.6.8 已升级并显式配置，待环境与历史审计 | 无 |
| ST-V2-004 | P0 | 新增 Statistics V2 九张表 | Migration 完成，待环境执行 | 001-003 契约 |
| ST-V2-005 | P0 | 实现三类 Data Collector | 代码完成，待真实数据验收 | 001-004 |
| ST-V2-006 | P0 | 实现 Typed Projection Engine 与五个 Projection | 代码完成，待真实数据验收 | 004-005 |
| ST-V2-007 | P0 | 固化 Plan Fulfillment cohort 算法 | 代码完成，待对账签收 | 001、005-006 |
| ST-V2-008 | P1 | 实现 SyncRun 与批次状态机 | Generation、并发 attempt 与恢复审计已收口，待故障演练 | 004-007 |
| ST-V2-009 | P1 | 建立 V2 查询契约与新鲜度字段 | 代码完成，待调用方验收 | 006-008 |
| ST-V2-010 | P1 | 建立机构级 Cache Generation 闭环 | 发布读屏障与 Content 水位已收口，待故障演练 | 008-009 |
| ST-V2-011 | P1 | 历史 Fact 回填与归属标记 | Validate/Repair/Publish 工具完成，待执行 | 001-008 |
| ST-V2-012 | P1 | V1/V2 影子对账与逐接口切换 | 服务端模式开关和调用方适配已提交，待影子期与逐接口验收 | 009-011 |
| ST-V2-013 | P1 | 建立测试、指标和运维闭环 | 部分完成 | 贯穿全部 |
| ST-V2-014 | P2 | 退役 V1 Projector/Scanner/旧表 | 未开始，受切流门槛阻塞 | 012-013 |
...
`PlanEnrollment` 已成为持久化实体，能够表达轮次、加入、自然关闭和显式终止；Enrollment 与初始 Task、终止与未完成 Task 取消均使用同一 MySQL 事务。当前剩余事实是 migration 尚未在真实环境验收，历史 Task 仍待按 `round=1 / derived_legacy` 回填核对。
...
新 AnswerSheet 已在可靠受理前校验 `OriginRef` 并冻结 `AttributionSnapshot`，随 AnswerSheet、Admission 和 Outbox 一起持久化；Statistics Collector 读取冻结归属。剩余问题集中在历史 AnswerSheet 的 `derived_legacy/unknown` 回填数量与偏差验收。
...
领域时间计算已显式使用 `Asia/Shanghai`；qs-server 已升级 `component-base v0.6.8`，并在 apiserver/worker 配置中显式设置 MySQL `location=Asia/Shanghai`、session `time_zone=+08:00`。剩余工作是环境连接验证和历史表时间语义审计。
...
```text
running -> failed
running -> succeeded                         # validate / repair
running -> data_committed -> succeeded       # publish
```

### 关键合同

- Run 在结果事务外创建；
- Repair 只重建 Daily，不修改 Snapshot、水位或 Cache Generation；
- Publish 的 Daily、Snapshot、`data_committed` 在同一事务；
- Publish 水位由服务端生成并只能单调前进；
- 缓存成功后才 `succeeded`；
- 缓存失败可以只重试闭环；
- 机构失败互不污染；
...
- 并发 attempt 分配遇到唯一键竞争时重新读取最大 attempt 并重试；
- Cache Generation 和切换时间写回 Run；每次 `resume-cache` 追加独立 JSON 审计项，不只覆盖最后一次摘要。
...
- 未形成可见 Generation 的新结果不得被冷请求回源；已经切换 Generation 的 `data_committed` 结果允许受控回源；
- Content 累计查询以已发布 `as_of_date` 为上界。
...
按组织和自然日分片执行 `repair`，回填三类 Fact 并重建 Daily。所有窗口完成并对账后，再执行一次当前完整日 `publish` 构建 Fulfillment/Snapshot 和 Cache Generation。
