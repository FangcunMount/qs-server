...
对正常七天窗口之外的历史来源执行采集和重建。回填必须提供机构、范围、原因和操作者。
...
禁止只依赖操作系统或容器的 `time.Local`。Go 应显式加载 Location，MySQL 连接和会话应明确 `+08:00`/上海语义。
...
`7d/30d` 的结束日是 `as_of_date`，均包含结束日。V1 `today` 不能静默映射为昨天，应通过 V2 契约和调用方迁移显式下线。
...
| data_committed | 已发生 | 已采集 | 新结果 | 尚未切换 | 数据正确，缓存待闭环 |
...
V2 实施时应尽量复用现有事务 Runner、LockLease 和缓存治理能力，但不能复用 V1 中已经被目标模型删除的实时投影语义。
