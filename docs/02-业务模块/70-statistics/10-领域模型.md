...
旧 `BehaviorFootprint`、`AssessmentEpisode`、`StatisticsJourneyMutation`、Scanner Checkpoint、Pending 和 Journey/Plan Daily 均不是当前领域模型，也不能作为新增指标的来源。
Resolve/Intake Log 继续存在，是因为它们属于不可从当前状态恢复的 Access 权威日志。
