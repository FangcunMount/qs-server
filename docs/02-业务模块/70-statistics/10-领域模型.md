...
> 状态：**目标模型已定稿，核心代码已实现，生产迁移中**。本文定义 Statistics V2 的概念、身份、生命周期和不变量；`BehaviorFootprint`、`AssessmentEpisode`、Checkpoint 与 Pending 仍作为 V1 回滚链路存在，但不属于 V2 目标模型。
...
OrganizationSnapshotProjection    -> statistics_v2_org_snapshot
```
...
| `statistics_v2_org_snapshot` | V2 独立快照表，明确 `as_of_date/snapshot_at`，不复用 V1 同名表 |
