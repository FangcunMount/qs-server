...
> 状态：**当前唯一领域模型**。本文定义 Statistics 的概念、身份、生命周期和不变量。旧 Journey/Episode、Checkpoint 与 Pending 模型已经退出运行时和最终 Schema。
...
每个阶段有独立业务意义、发生时间和失败可能，因此必须分别计数。Statistics 不使用“Assessment evaluated”同时近似 Outcome 和 Report。
...
当前不统计“重排次数”。新一轮参与应创建新 Enrollment 和新 Task；如果同一 Enrollment 内确实支持调整时间，Plan 必须持久化 Schedule Revision，Statistics 不使用 `updated_at` 猜测重排。
...
旧 `BehaviorFootprint`、`AssessmentEpisode`、`StatisticsJourneyMutation`、Scanner Checkpoint、Pending 和 Journey/Plan Daily 均不是当前领域模型，也不能作为新增指标的来源。Resolve/Intake Log 继续存在，是因为它们属于不可从当前状态恢复的 Access 权威日志。

当前对象关系固定为：

| 当前对象 | 责任 |
| --- | --- |
| Resolve/Intake Log | Access Fact 来源 |
| AttributionSnapshot | Assessment 归属来源 |
| PlanEnrollment/Task | Plan Fact 来源 |
| 三类 Fact | 唯一标准统计事实 |
| 四类 Daily + OrgSnapshot | 唯一统计结果 |
| SyncRun | 唯一运行账本 |
...
- [`domain/statistics/contracts.go`](../../../internal/apiserver/domain/statistics/contracts.go)
- [`application/statistics/coordinator.go`](../../../internal/apiserver/application/statistics/coordinator.go)
- [`application/statistics/read_service.go`](../../../internal/apiserver/application/statistics/read_service.go)
- [`infra/mysql/statistics/collectors.go`](../../../internal/apiserver/infra/mysql/statistics/collectors.go)
- [`infra/mysql/statistics/projections.go`](../../../internal/apiserver/infra/mysql/statistics/projections.go)
- [`infra/mysql/statistics/run_store.go`](../../../internal/apiserver/infra/mysql/statistics/run_store.go)
