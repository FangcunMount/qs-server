...
当前 canonical 链路已经按这条责任链执行：快照或 InputSnapshot 缺少 DecisionKind 时 fail closed，不从 Kind、Algorithm 或旧格式补齐。只有明确的历史读取适配器可在唯一映射成立时恢复缺失 DecisionKind，它不参与新发布或 Worker 路由。
...
这些规则已经从 DefinitionV2 的显式 Conclusion 语义出发，Kind 只选择应用哪类完整性规则，不能在缺少 Conclusion 时凭类型补齐 DecisionKind。当前 scale/cognitive/behavioral 仍各只支持一种 DecisionKind；未来扩展为多种 Decision 时，应扩展 Definition 语义和发布兼容校验，不能在运行时从 Kind 猜测。
...
Publisher 在写入前确实会解析 `AlgorithmFamily / Algorithm / DecisionKind` 并校验相容性，但 Mongo record 不单独持久化 ProductChannel、SubKind 或 AlgorithmFamily。Reader 返回的进程内 `port.AssessmentSnapshot` 会根据 Kind 派生 ProductChannel/SubKind，并根据 DecisionKind 派生 AlgorithmFamily；这些便利字段不是独立持久化事实。
...
Evaluation 不得根据 Kind/SubKind/Algorithm 或临时 DTO 重新决定 DecisionKind。但从 canonical DecisionKind 派生 AlgorithmFamily 正是当前实现，不是一次新的业务决策。DecisionKind 缺失、未知或与发布 identity 冲突时，必须在发布或执行边界直接失败。
...
`DefinitionV2`、AnswerSheet、Questionnaire、NormSubject 和其它执行事实位于 `InputSnapshot` 的其它字段，不属于 `ModelSnapshot` 身份。InputSnapshot 只穿透发布快照中的 canonical DecisionKind，不携带独立 AlgorithmFamily。
...
Registry 只以 `DecisionKind` 作为 DescriptorKey。注册每个 descriptor 时，它会验证 descriptor 声明的 AlgorithmFamily 与该 DecisionKind 的派生 family 一致；AlgorithmFamily 和 ExecutionPath 因此是已选 descriptor 的进程内属性，而不是输入提供的第二路由键。
...
Mongo published record、InputSnapshot、ModelRoute、DescriptorKey 和 Outcome RuntimeIdentity 都携带同一 DecisionKind。AlgorithmFamily 只在 publisher/reader/descriptor 边界根据该 DecisionKind 派生并校验。

### 11.2 Identity 不覆盖发布 Decision

运行时不从 identity 推导或替换 DecisionKind，也不把未知/不相容的 DecisionKind 替换为 family 默认值。冲突是终态 validation failure；从已验证 DecisionKind 派生 Family 则是 descriptor 注册契约的一部分。
...
历史 EvaluationOutcome/Interpretation 读取适配器会调用 `ResolveLegacyRuntime`：仅当 Kind + Algorithm 能唯一得到 DecisionKind 时才恢复缺失值，typology 这类非唯一映射或任何冲突都返回 `ErrLegacyRuntimeIdentity`。这是只读兼容收敛，不得用于新发布、重试或 descriptor dispatch。
