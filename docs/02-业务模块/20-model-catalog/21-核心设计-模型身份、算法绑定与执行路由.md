...
> 状态：当前实现。发布物化会解析并交叉校验 AlgorithmFamily / Algorithm / DecisionKind，但 canonical 持久化与 Evaluation 路由只冻结 `DecisionKind`；AlgorithmFamily 由它在进程内派生并校验。
...
4. AssessmentSnapshot、InputSnapshot、ModelRoute、DescriptorKey、RuntimeDescriptor 怎样把发布模型连接到 Evaluation 执行管线；
5. ExecutionPath 为什么只是当前装配机制，不应上升为业务模型身份；
6. 运行时为什么必须拒绝从 Kind/Algorithm 重新决定 DecisionKind，而 AlgorithmFamily 只能从 canonical DecisionKind 派生；
...
| `AlgorithmFamily` | 整条计算管线采用哪类执行机制 | 不是 canonical 持久化/路由键；发布时参与兼容校验，运行时由 DecisionKind 派生 |
| `Algorithm` | 调用哪项稳定代码能力 | 不直接选择 descriptor；用于发布兼容校验和 family-specific 输入物化 |
| `ExecutionSpec` | 该 Algorithm 需要哪些模型参数 | 是，作为执行输入契约 |
| `Strategy` | Factor、Norm 或 Decision 的局部算法是什么 | 由对应 Calculation 能力消费 |
| `DecisionKind` | 计算结果怎样形成稳定业务结果 | 是，发布时冻结，是当前唯一 canonical descriptor key |
...
ModelIdentity + AlgorithmBinding + DefinitionV2
  在发布阶段完成兼容校验
  解析 AlgorithmFamily / Algorithm / DecisionKind
  持久化 Algorithm / DecisionKind
                         ↓
Evaluation 从 InputSnapshot 构造 ModelRoute
  只使用发布快照中的 DecisionKind
                         ↓
RuntimeDescriptorRegistry
  按 DecisionKind 选择 descriptor
  派生/校验 AlgorithmFamily
                         ↓
EvaluationOutcome
  保存模型身份和 DecisionKind
```

当前 canonical 链路已经按这条责任链执行：快照或 InputSnapshot 缺少 DecisionKind 时 fail closed，不从 Kind、Algorithm 或旧格式补齐。
只有明确的历史读取适配器可在唯一映射成立时恢复缺失 DecisionKind，它不参与新发布或 Worker 路由。
...
| Kind | 当前推导 |
| --- | --- |
| `scale` | 必须有显式 RiskConclusion，然后得到 `score_range` |
| `typology` | 必须有 TypeConclusion，并从其 `Decision.Kind` 读取允许的分类策略 |
| `behavioral_rating` | 固定为 `norm_lookup`；缺 NormRef 或 Primary NormConclusion 时拒绝发布 |
| `cognitive` | 必须有显式 AbilityConclusion，然后得到 `ability_level` |

这些规则已经从 DefinitionV2 的显式 Conclusion 语义出发，Kind 只选择应用哪类完整性规则，不能在缺少 Conclusion 时凭类型补齐 DecisionKind。
当前 scale/cognitive/behavioral 仍各只支持一种 DecisionKind；未来扩展为多种 Decision 时，应扩展 Definition 语义和发布兼容校验，不能在运行时从 Kind 猜测。
...
### 9.1 当前 published record 持久化什么

当前 Mongo `PublishedAssessmentModelPO` 保存的与身份/路由相关的事实是：
...
Publisher 在写入前确实会解析 `AlgorithmFamily / Algorithm / DecisionKind` 并校验相容性，但 Mongo record 不单独持久化 ProductChannel、SubKind 或 AlgorithmFamily。
Reader 返回的进程内 `port.AssessmentSnapshot` 会根据 Kind 派生 ProductChannel/SubKind，并根据 DecisionKind 派生 AlgorithmFamily；这些便利字段不是独立持久化事实。

因此持久化证据能够证明某次发布使用了哪个 Kind/Algorithm/Model Version 和 DecisionKind；AlgorithmFamily 是当前代码按 DecisionKind 映射得到的执行机制，不能宣称为另一个已持久化的历史路由键。

### 9.2 运行时不得重新决定什么

Evaluation 不得根据 Kind/SubKind/Algorithm 或临时 DTO 重新决定 DecisionKind。但从 canonical DecisionKind 派生 AlgorithmFamily 正是当前实现，不是一次新的业务决策。
DecisionKind 缺失、未知或与发布 identity 冲突时，必须在发布或执行边界直接失败。

### 9.3 当前发布事实

当前发布契约分为“发布期校验值”和“持久化 canonical 值”：
...
AlgorithmFamily 在发布期由 identity 与 DecisionKind 双向校验，但不再作为 canonical 快照的独立存储字段。

ExecutionPath 不需要进入发布快照。它是服务装配细节，可以随着内部实现重构；只要 DecisionKind 的业务语义与对应执行结果不变，历史模型不应该感知 provider 或 descriptor 的代码路径名称。
...
`DefinitionV2`、AnswerSheet、Questionnaire、NormSubject 和其它执行事实位于 `InputSnapshot` 的其它字段，不属于 `ModelSnapshot` 身份。
InputSnapshot 只穿透发布快照中的 canonical DecisionKind，不携带独立 AlgorithmFamily。
...
`ModelRouteFromInput` 只从 `InputSnapshot.Model.DecisionKind` 构造路由。DecisionKind 缺失时拒绝执行，不回退 Assessment ModelRef。
...
DescriptorKey 直接复制冻结的 DecisionKind，并先确认它能映射到已知 AlgorithmFamily。DecisionKind 缺失、未知或没有对应 descriptor 时拒绝运行。
...
Registry 只以 `DecisionKind` 作为 DescriptorKey。注册每个 descriptor 时，它会验证 descriptor 声明的 AlgorithmFamily 与该 DecisionKind 的派生 family 一致；
AlgorithmFamily 和 ExecutionPath 因此是已选 descriptor 的进程内属性，而不是输入提供的第二路由键。
...
```text
ModelIdentity
  Kind / Algorithm / Code / Version / Title

RuntimeIdentity
  DecisionKind
```

这使结果能够说明“使用了哪版模型”和“使用了哪种结果判定契约”。查看 AlgorithmFamily 时必须根据 DecisionKind 使用当前映射派生，不能当成 Outcome 中独立持久化的字段。

## 11. 当前路由的 fail-closed 保证

### 11.1 Canonical DecisionKind 完整穿透

Mongo published record、InputSnapshot、ModelRoute、DescriptorKey 和 Outcome RuntimeIdentity 都携带同一 DecisionKind。
AlgorithmFamily 只在 publisher/reader/descriptor 边界根据该 DecisionKind 派生并校验。

### 11.2 Identity 不覆盖发布 Decision

运行时不从 identity 推导或替换 DecisionKind，也不把未知/不相容的 DecisionKind 替换为 family 默认值。冲突是终态 validation failure；
从已验证 DecisionKind 派生 Family 则是 descriptor 注册契约的一部分。
...
### 11.4 Canonical 路由没有 fallback，历史读取有限收敛

新 snapshot 的 DecisionKind 缺失、发布校验与运行时身份冲突、输入物化丢字段或 descriptor 未注册都会显式失败；canonical Worker 路由不使用旧 payload、format 或 family fallback。

历史 EvaluationOutcome/Interpretation 读取适配器会调用 `ResolveLegacyRuntime`：仅当 Kind + Algorithm 能唯一得到 DecisionKind 时才恢复缺失值，
typology 这类非唯一映射或任何冲突都返回 `ErrLegacyRuntimeIdentity`。这是只读兼容收敛，不得用于新发布、重试或 descriptor dispatch。
...
其中 Mongo published record 只持久化 Algorithm 和 DecisionKind；AlgorithmFamily 是发布期校验结果和进程内派生属性，不是第二个 canonical 存储键。
...
Evaluation 可以校验这些字段，但不重新决定 DecisionKind，也不要求输入另外携带 AlgorithmFamily。

### 12.3 Descriptor 路由

当前 ModelRoute 与 DescriptorKey 直接使用冻结值：
...
Kind / Algorithm 仍保留在 InputSnapshot 或执行上下文中，用于审计和 family-specific 输入物化，但不进入 ModelRoute，也不覆盖已冻结的 DecisionKind。

### 12.4 结果审计

EvaluationOutcome 继续记录 ModelIdentity 与仅包含 DecisionKind 的 RuntimeIdentity。这样即使未来 descriptor 的代码组织改变，历史结果仍然能够回答：

- 使用了哪份模型发布版本；
- 使用哪种结果判定；
- 按当前 DecisionKind 映射派生为哪类算法管线（不是独立持久化事实）；
...
当前 `AlgorithmFamilyFromIdentity`、`FamilyCapabilityByKind`、definition handler 和 runtime descriptor registry 分散承担了部分矩阵职责。
目标不是再增加一个重复 registry，而是逐步形成单一、可验证的兼容契约。
...
- 冻结的 DecisionKind 语义不变，且它与 AlgorithmFamily 的映射变更经过显式迁移/兼容评估；
...
- 从 Kind、SubKind 或临时 DTO 猜测新发布模型缺失的 DecisionKind；
- 让 DecisionKind 与发布快照不同，却静默使用 Family 默认值或运行时重新推导值；
...
- Mongo AssessmentSnapshot 保存 Algorithm、DecisionKind 和 DefinitionV2，进程内 reader snapshot 由 DecisionKind 派生 AlgorithmFamily；
- active admission reader 与 retained exact-version reader 已经分离；
- RuntimeDescriptor 以 DecisionKind 形成精确路由键，并校验 descriptor 的 AlgorithmFamily 与该键一致；
- RuntimeDescriptor 已组合 InputAssembler、Calculator 和 OutcomeAssembler；
- EvaluationOutcome 保存 ModelIdentity 与只包含 DecisionKind 的 RuntimeIdentity；
...
- 建立显式模型—算法兼容矩阵；
- 把当前分散的 identity 兼容校验、DecisionKind → AlgorithmFamily 映射和 descriptor manifest 收敛为单一可验证契约；
- 对 DecisionKind 语义或其 family 映射的变更引入显式兼容/迁移评估，不重新增加 snapshot AlgorithmFamily 双键；
- 继续用架构 ratchet 保护 DecisionKind-only ModelRoute、DescriptorKey 和 Outcome RuntimeIdentity；
