...
7. `DefinitionV2`、兼容 payload、QuestionnaireSnapshot、AnswerSheet、Norm 在执行输入中各自扮演什么角色？
...
| NormSubject | Actor/Testee 人口学资料 | 当前生产 resolver 尚未实现填充 |

---

## 20. DefinitionV2 是执行语义源，不从兼容 payload 补洞
...
`PublishedModel.Payload` 和 `PayloadFormat` 仍承担兼容传输、格式识别或迁移期契约，但不能在 DefinitionV2 缺失时反向补齐模型语义。
...
2. 校验 payload format 与 DefinitionV2；
3. 重建统一 typology payload；
4. 读取 AnswerSheet；
