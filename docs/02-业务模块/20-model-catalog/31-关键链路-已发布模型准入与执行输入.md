...
这是**当前实现与已确认业务边界之间的差距**。它不会误执行模型，但会制造一个没有后续业务意义的 pending Assessment，并可能污染列表或统计语义。
目标改造应让 unbound AnswerSheet 在 Journey 层明确结束，而不是借助 `NeedsEvaluation=false` 兜底。
...
如果 T3 不复核，就会在下架之后继续产生新测评。因此 `CreateForAnswerSheet` 在真正保存 Assessment 前调用 `PublishedEvaluationModelValidator`，
用完整精确 ref 再执行一次：
...
当前 `InputRef.QuestionnaireCode/Version` 会被构造，但各 provider 实际先读 AnswerSheet，再以 AnswerSheet 中的 questionnaire ref 加载 QuestionnaireSnapshot。
scale 的执行校验会再把它们与 Assessment 逐项比较；其它模型族的校验完整度并不完全一致，后文会单独说明。
...
从 `ModelInputProviderRegistry` 选择 provider。`SubKind` 不是 `InputRef.ModelRef` 的独立字段；
`ModelRef.ExecutionIdentity()` 会根据 `Kind` 派生 canonical SubKind，再与 Algorithm 组成 provider 键。
provider 集合由 Evaluation runtime descriptor registry 中已注册且可执行的 `ExecutionPath` 物化，而不是手工维护第二份随意列表。这一 provider 选择键不是后续 RuntimeDescriptor 的路由键。
...
InputSnapshot
├── Model
│   ├── kind / algorithm / decision kind / code / version / title
│   └── typed payload
├── ModelPayload
├── AnswerSheet
│   ├── questionnaire code/version
...
│   ├── code/version
│   └── questions/options
└── NormSubject（behavioral/cognitive 由 Actor Testee 资料按 Assessment 时点物化）
```
...
| Execution path | canonical DecisionKind 已注册，且 descriptor 声明的 family 与 DecisionKind 派生值一致 |
...
这意味着主链路设计正确，但防御式校验分散在 provider 和 mechanism 中。建议后续在 `RepositoryResolver` 物化完成后、descriptor routing 之前增加一个模型族无关的
`InputSnapshotInvariantValidator`，再保留 family-specific validator 检查各自细节。

---

## 24. NormSubject：当前源码已接入，部署证据单独记账
...
`factor_norm` 和 SPM 计算会读取它，用于年龄、性别等分层常模匹配。当前源码链路为：

```text
Assessment.TesteeID + SubmittedAt
  -> InputRef.TesteeID + AsOf
  -> Actor TesteeQuery-backed NormSubjectReader
  -> Gender + Birthday
  -> BuildNormSubjectSnapshot(AgeMonthsAt(asOf), Gender)
  -> behavioral/cognitive InputSnapshot.NormSubject
```

`internal/apiserver/container/modules/evaluation/install.go` 已把 Actor TesteeQuery 适配器装入 provider registry，
behavioral/cognitive provider 都会调用 `resolveNormSubject`。人口学依赖失败被分类为可重试 Actor dependency；年龄以 Assessment `SubmittedAt` 为 `AsOf`，不是执行时当前年龄。
缺失生日会保留“年龄不存在”，不能误写成零月龄。

因此当前证据必须区分：

```text
当前源码与测试已实现
  InputRef 冻结 TesteeID/AsOf
  composition root 接入 Actor NormSubjectReader
  behavioral/cognitive provider 物化 NormSubject
  v2 subject identity 区分缺失年龄、零月龄与性别

仍需单独证明
  当前 checkout 对应的 CI、精确 SHA 部署与真实目标模型 smoke
  Actor 人口学修订后的受控 force-retry/审计语义
```

对于依赖年龄/性别分层的常模，这不是展示问题，而是执行正确性问题。持续验收必须守卫：
...
```text
InputSnapshot.Model.DecisionKind
  -> ModelRoute{DecisionKind}
  -> DescriptorKey{DecisionKind}
  -> RuntimeDescriptor（内含已校验的 AlgorithmFamily）
  -> InputAssembler
  -> Calculator
  -> OutcomeAssembler
```

RuntimeDescriptor 的 canonical 路由只从已物化的 `InputSnapshot.Model.DecisionKind` 构造；DecisionKind 缺失、未知或没有精确 descriptor 时直接失败。
AlgorithmFamily 由 DecisionKind 在进程内派生，并在 descriptor 注册时校验，不是 `ModelRoute` 或 `DescriptorKey` 的第二个输入字段。

Engine 另外保留 `ExecutionIdentity = Kind + SubKind + Algorithm` 用于 evaluator 能力身份和观测；当 Assessment 的 Algorithm 为空时，
`resolveExecutionIdentity` 可使用 `InputSnapshot.Model` 中的 Algorithm。这是 Assessment/Input ModelRef 之间的兼容协调，
不会从 payload 猜测 RuntimeDescriptor，也不改变 DecisionKind-only 路由。
...
- AlgorithmBinding；
- DefinitionV2；
- canonical DecisionKind 与 Decision/ExecutionSpec；
- 精确 NormRef。
...
### 已关闭 P0：分层常模 NormSubject 物化

当前源码已从 Testee/Actor 获取人口学资料，以测评发生时间构造年龄和性别，并区分缺失资料语义；Evaluation `EV-R005` 已记录技术验收。剩余项是当前 checkout 的 CI/部署 smoke 和受控重试审计，不得把历史部署证据提升为本轮生产证据。
...
> active 是准入规则，解决“这个版本现在还允许不允许产生新测评”；exact-version 是历史执行规则，解决“已经受理的测评必须按当时哪一套规则执行”。新测评先通过 active release 绑定，Assessment 再冻结模型和问卷精确版本。
> Worker 重试只按 Assessment 的 exact ref 读取保留快照，不会取最新版本，因此发布、下架和异步延迟都不会改变历史结果语义。

### 33.2 为什么 Worker 不直接处理模型配置？

可以回答：

> Worker 只负责事件消费和执行控制。模型选择属于 ModelCatalog 与 Assessment Intake，执行引用属于 Assessment，算法属于 Evaluation/Calculation。
> Worker 只传 Assessment ID，既缩小事件契约，也避免消息和数据库形成两个业务真相。
...
> immutable exact payload 可以缓存，因为内容不变；但 active 状态会变化，所以新 Assessment 的 active exact validator 绕过精确 payload cache，直接回 Mongo 复核 release_status。
> 缓存负责性能，Mongo 中的发布状态负责准入正确性。

### 33.5 当前设计还缺什么？

可以回答：

> 主链路已经形成 active 准入、Assessment 冻结、exact history 执行、NormSubject 按 Assessment 时点物化和 Outbox 可靠唤醒，但仍有几个明确缺口：独立问卷会留下 unbound pending Assessment；
> 跨模型族不变式校验不统一；EvaluationRun 目前只记录 model:code@version，而不是完整复合输入证据；当前 checkout 的 CI/部署 smoke 尚未取得。这些问题都有清晰的改造边界。
