...
从 `ModelInputProviderRegistry` 选择 provider。`SubKind` 不是 `InputRef.ModelRef` 的独立字段；`ModelRef.ExecutionIdentity()` 会根据 `Kind` 派生 canonical SubKind，再与 Algorithm 组成 provider 键。provider 集合由 Evaluation runtime descriptor registry 中已注册且可执行的 `ExecutionPath` 物化，而不是手工维护第二份随意列表。这一 provider 选择键不是后续 RuntimeDescriptor 的路由键。
...
`internal/apiserver/container/modules/evaluation/install.go` 已把 Actor TesteeQuery 适配器装入 provider registry，behavioral/cognitive provider 都会调用 `resolveNormSubject`。人口学依赖失败被分类为可重试 Actor dependency；年龄以 Assessment `SubmittedAt` 为 `AsOf`，不是执行时当前年龄。缺失生日会保留“年龄不存在”，不能误写成零月龄。
...
RuntimeDescriptor 的 canonical 路由只从已物化的 `InputSnapshot.Model.DecisionKind` 构造；DecisionKind 缺失、未知或没有精确 descriptor 时直接失败。AlgorithmFamily 由 DecisionKind 在进程内派生，并在 descriptor 注册时校验，不是 `ModelRoute` 或 `DescriptorKey` 的第二个输入字段。

Engine 另外保留 `ExecutionIdentity = Kind + SubKind + Algorithm` 用于 evaluator 能力身份和观测；当 Assessment 的 Algorithm 为空时，`resolveExecutionIdentity` 可使用 `InputSnapshot.Model` 中的 Algorithm。这是 Assessment/Input ModelRef 之间的兼容协调，不会从 payload 猜测 RuntimeDescriptor，也不改变 DecisionKind-only 路由。
...
> 主链路已经形成 active 准入、Assessment 冻结、exact history 执行、NormSubject 按 Assessment 时点物化和 Outbox 可靠唤醒，但仍有几个明确缺口：独立问卷会留下 unbound pending Assessment；跨模型族不变式校验不统一；EvaluationRun 目前只记录 model:code@version，而不是完整复合输入证据；当前 checkout 的 CI/部署 smoke 尚未取得。这些问题都有清晰的改造边界。
