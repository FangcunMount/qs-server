...
> 状态：`behavioral_rating + factor_norm` 已支持 BRIEF-2 和感觉统合 SPM 的因子、复合指数、精确常模、T 分/百分位和等级投影。
> 正式发布已强制 NormRef、Primary NormConclusion 与 `norm_lookup`（见 `MC-R003`）。历史 default/`score_range` snapshot 的兼容读取与下线仍属后续治理。
...
`behavioralDecisionKind` 固定返回 `norm_lookup`；无 NormRef、无且仅一个 Primary NormConclusion 或身份不匹配时均在发布阶段拒绝。

### 已关闭防线：NormSubject 生产装配

生产 input provider 从 Actor/Testee 权威事实冻结性别与出生日期，并按 Assessment 提交时间计算 AgeMonths。缺少专用 cohort 所需人口学资料时只允许命中显式 generic 行，否则分类失败。
...
### 已关闭防线：canonical Algorithm

运行时仅接受 `brief2` 与 `spm_sensory`；Assessment、冻结 snapshot 和 descriptor 输入必须保存并精确匹配真实 Algorithm，不存在 default 或 payload 身份回退。
