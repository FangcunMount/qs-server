...
| Algorithms | `personality_typology`、`mbti`、`sbti`、`bigfive` 等历史/具体身份 |
...
12. 生成 `assessmentmodel.personality.typology.v1` payload。
...
### 16.1 Algorithm identity 尚未完全收敛

API options 对新建模型主要暴露 `personality_typology`，但领域枚举、初始化脚本、历史快照和 Assessment 可能保存 `mbti/sbti/bigfive`。provider registry 再用 typology alias 将不同 identity 路由到统一实现。

需要明确最终策略：

- Algorithm 保留理论/算法身份，统一运行时只是实现；或
- 所有配置模型统一 Algorithm，理论差异完全放进 Definition。

在迁移完成前不能随意改写历史 Assessment 的 Algorithm。

### 16.2 RuntimeSpec 与兼容 payload 字段重复

typology payload 同时保留 DimensionOrder、Dimensions、QuestionMappings、MatchingSpec 等兼容字段和显式 Runtime。长期应保持 DefinitionV2 单一创作源，逐步减少多份投影造成的差异。
