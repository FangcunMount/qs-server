...
>
> 数据库是发布事实源；Redis 缓存失效和生命周期事件都发生在 commit 之后，只提供最终一致传播。标准 migration、关键唯一索引、revision CAS、启动校验和 Replica Set 集成测试已经与 unified schema 对齐；后续重点是周期审计、删除/恢复原子性以及明确的 Mongo durability 部署证据。
