...
### 23.7 DefinitionV2 内容摘要可能漂移

风险：手工改库导致 snapshot 中的 Definition 内容摘要与 DefinitionV2 不一致。

目标：发布时保存 DefinitionV2 canonical content hash，并由只读 inventory audit 检查哈希和运行时可物化性；不提供回填。
