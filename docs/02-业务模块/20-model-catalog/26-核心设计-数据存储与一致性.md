...
> 状态：已按 2026-08-06 当前源码与生产台账校准。ModelCatalog 已采用 MongoDB 文档存储、单集合双角色 head/release 结构、不可变发布快照和 Questionnaire/Model 联合发布事务；
> 统一索引已进入标准 migration 与启动时 fail-closed 校验，Norm 唯一索引、Model/Questionnaire revision CAS 和真实 Replica Set 集成测试均已落地。
> 本文继续区分已实现事实与长期治理项。
...
11. 标准 migration、启动索引协调和 fail-closed 校验怎样共同保证 unified schema？
12. 删除、下架、归档和历史保留在物理存储上分别意味着什么？

本文不重复展开：

- Questionnaire 与模型为什么共同组成可执行测评，见 [问卷绑定与发布版本](./22-核心设计-问卷绑定与发布版本.md)；
- Factor、Norm 和 Decision 的领域语义，见 [因子与计分模型](./23-核心设计-因子与计分模型.md)、[常模资产与校准](./24-核心设计-常模资产与校准.md)
  和[结果判定、Outcome 与解释边界](./25-核心设计-结果判定、Outcome与解释边界.md)；
...
- DefinitionV2 runtime materialization；
...
- 简化备份、审计和模型族迁移；
- 避免 authoring 与 runtime snapshot 长期形成双存储真相；
...
旧 runtime snapshot 集合已随 seeddata 历史能力退役删除，恢复只能依赖完整数据库备份。
...
2. 解析 Norm 等外部资产；
3. 推导 DecisionKind；
4. 物化并校验 family-specific runtime DTO；
5. 只冻结 DefinitionV2、运行身份与 Definition 内容摘要。

`definition_v2` 是 canonical 领域定义和唯一持久化执行语义。family-specific DTO 只在保存、发布或运行时由它确定性物化，不保存 projection hash，也不保留 payload replay/backfill。
...
### 10.4 Questionnaire 并发编辑使用 revision CAS

Questionnaire head 具有独立于业务 `version` 的 `revision`。Repository 更新时：

1. 读取当前 revision 作为 expected revision；
2. 在内存对象中递增 revision；
3. 以 `code + head role + expected revision` 作为 `UpdateOne` filter，且不使用 upsert；
4. `MatchedCount=0` 时区分记录不存在与 `ErrRevisionConflict`；
5. application 将 revision conflict 映射为稳定的冲突错误，要求调用方刷新后重试。

这防止两个编辑者静默覆盖。它不等于自动合并：冲突方仍需重新读取最新 head，再由产品界面或调用方决定如何重放编辑。
...
旧的 model-only publication service 仍存在于应用层和组合结构中，但 REST 的 `assessmentModelPublicationRoutes` 当前为空。对绑定问卷的可执行测评，
`release.Service` 才是 canonical 外部入口。
...
### 13.4 专用联合发布 Replica Set 集成测试已落地，执行证据需逐次记录

仓库已有带 `integration` build tag 的 `internal/apiserver/container/modules/modelcatalog/transaction_integration_test.go`，
通过 `mongodbtest.ReplicaSetDatabase` 在独立数据库内同时写 `questionnaires` 与 `assessment_models`，覆盖 repository failure、commit failure、跨集合 rollback 和发布边界。
它补充了 release service 的应用层 rollback 单元测试。

这只能证明测试能力存在。每次验收仍必须在明确授权、可重置的非生产 Replica Set 上以 `-tags integration` 执行；普通 `go test` 未编译该文件，缺少环境或测试被跳过都不能记为本轮通过，更不能直接对生产库执行。
...
标准 Mongo migration 已包含统一 ModelCatalog、Questionnaire 与 Norm 的 canonical indexes；启动时 `IndexManager` 会幂等清理冲突索引并校验当前索引集合。
全新环境可以只通过标准 migration 重建当前 schema，不再依赖一次性 cutover 脚本或已退役集合。

索引变更必须继续使用不可变的后续 migration，并同步更新 contract test。缺少关键唯一索引时，服务不得把环境视为可写就绪。
...
```text
authoring draft rows
+ runtime snapshot rows
```
...
revision 乐观匹配使一个成功、一个失败。Repository 区分记录不存在与 revision conflict，application 将后者映射为稳定冲突响应，调用方刷新后再决定是否重放编辑。
...
当前 Model snapshot repository 不会像 Norm repository 那样在 duplicate-key 后重新读取并比较内容；上述“同时预读不存在”的输家会直接收到 Mongo 写错误。
后续可把它翻译为稳定的并发/版本冲突错误，同时保留调用方重试后的幂等语义。
...
Authoring consistency
  ...
  Questionnaire head: revision CAS, conflict requires refresh
...
### 23.1 标准 migration 与启动索引校验已闭环

原风险：新环境只有旧 `unique(code)`，head 与 snapshot 无法共存；旧 published collection 可能被误当真相。

当前实现：

- `000013_unified_modelcatalog_schema` 创建 role-based canonical indexes；
- `ReconcileUnifiedModelCatalogIndexes` 幂等删除冲突旧索引并补齐 canonical indexes；
- `VerifyUnifiedModelCatalogIndexes` 在服务启动阶段验证 required/forbidden 清单，失败即中止启动；
- fresh/legacy/dirty、索引幂等和 Replica Set rollback 契约已有集成测试；
- 当前生产 migration `21/false`，旧双集合和冲突索引已完成退役与独立复核。

### 23.2 Norm 唯一索引已闭环

`idx_assessment_norms_table_version` 已进入 migration 000013、IndexManager required 清单与启动校验，同一 TableVersion 的并发首次导入由唯一约束收敛。

### 23.3 Questionnaire head CAS 已闭环

Questionnaire domain、Mongo repository、application 冲突映射、revision 单测和真实 Mongo 双写者集成测试已经形成闭环；冲突不会再表现为 last-write-wins。

### 23.4 Model revision 冲突已使用专用错误

Model repository 在 CAS 未命中时区分 not found 与 `ErrRevisionConflict`，application 将后者映射为稳定冲突响应。

### 23.5 发布幂等与 active pair

风险：历史漂移下返回虚假成功，掩盖缺失 active pair。

当前发布服务在幂等路径验证 Questionnaire/Model active pair 与 release identity；人工改库或旧数据造成的漂移仍需通过只读 audit 发现，不能靠重复发布覆盖。

### 23.6 跨集合只读审计已实现，生产 scheduler 仍关闭

手工改库、旧脚本或迁移错误仍可能造成 active model 引用不存在/非 active Questionnaire、active snapshot 重复、binding 漂移或 NormRef 缺失。
当前 `mongoconsistency` 的最后两个阶段已经覆盖这些风险：

- `model_release` 以 published head 为 anchor，校验恰好一个 active published snapshot、head/snapshot kind/algorithm/questionnaire
  binding 一致，以及精确 Questionnaire active snapshot 存在；
- `published_model_runtime` 重新物化 active snapshot，校验 schema/decision/runtime identity、DefinitionV2 canonical content hash 和所有 NormRef；
- 除 Definition hash mismatch 为 medium 外，上述 ModelCatalog finding 均为 high；scanner 只读 Mongo，不提供 repair port。

正式生产 one-off 中这两个阶段 finding 为 0，但这只是 2026-08-16 01:13 CST 的时点证据。全局 scheduler 仍因 Survey 方向的 269 条 high drift 保持关闭；
不能把“ModelCatalog 两阶段为 0”写成“生产周期审计已经运行”。启用和处置门禁见[调度任务](../../04-接口与运维/07-调度任务.md)。

### 23.7 DefinitionV2 内容摘要已可审计

Publisher 保存 DefinitionV2 canonical content hash 与 hash schema；`AuditPublishedSnapshotInventory` 对比持久摘要、重新物化四类 runtime
identity，并对缺失、摘要不一致或不可物化结果 fail closed。

这能发现手工改库或历史数据漂移，但不会自动回填或覆盖异常 snapshot；处置仍需独立清单与授权。
...
### 23.9 真实 Mongo 联合发布集成测试

独立 Replica Set 测试覆盖 session context、partial unique index、跨集合 rollback、CAS、compatible down 与 incompatible down rejection。
它依赖明确授权的非生产 MongoDB，测试被跳过时不能算作取得该项证据。
...
1. unified `assessment_models` / `questionnaires` / `assessment_norms` 索引进入标准 migration；
2. 启动时 reconcile 并 fail-closed 验证关键索引；
3. Model 与 Questionnaire head 使用 revision CAS；
4. fresh database、历史升级和真实 Replica Set commit/rollback 具备集成测试；
5. 生产物理旧集合和冲突结构已经过授权退役并独立复核。

### 当前持续治理

1. 持续运行 model/questionnaire/norm 跨集合只读审计；
2. 持续运行 DefinitionV2 canonical content hash 与 runtime materialization 审计；
3. 为缓存 version token 和生命周期事件保留可观测、repair 与 outbox 升级路径；
4. 固化 Mongo read/write concern 的部署证据；
5. 对 Questionnaire draft 删除/恢复继续补强事务边界。

### P2：长期资产治理

1. 为 release migration/backfill 建立统一审计记录；
2. 明确已退役 collection 备份的保留周期与恢复验证责任；
...
| 明确 optimistic conflict error | 已实现 | CAS 未命中区分 revision conflict 与 not found，application 映射稳定冲突响应 |
| Questionnaire head CAS | 已实现 | 独立 revision、expected revision filter、无 upsert、冲突刷新重试 |
...
| Norm table_version unique index | 已实现 | migration 000013 + IndexManager required 清单 |
| Unified model/questionnaire indexes | 已实现 | 标准 migration + 启动 reconcile/verify；one-off 不再是正常部署入口 |
| 标准 migration 对齐 unified schema | 已实现 | fresh/legacy/dirty 与升级契约有测试，生产 migration 21/false |
| 服务启动关键索引验证 | 已实现 | required 缺失或 forbidden legacy index 存在时 fail closed |
| DefinitionV2 content audit | 已实现 | 发布保存 canonical hash，inventory audit 校验摘要与四 family runtime 可物化性 |
| active pair 与 runtime 周期一致性审计 | 已实现/生产关闭 | 七阶段 auditor 的 `model_release`、`published_model_runtime`；one-off 可运行，scheduler 尚未过启用门禁 |
| 真实 Replica Set release integration test | 已实现 | 授权非生产环境执行；跳过不能计为通过 |
...
| Unified schema 标准 migration | `internal/pkg/migration/migrations/mongodb/000013_unified_modelcatalog_schema.up.json`、`internal/pkg/migration/unified_modelcatalog_schema_contract_test.go` |
| 当前 Mongo migrations | `internal/pkg/migration/migrations/mongodb` |
| 跨集合只读审计定义 | `internal/apiserver/application/mongoconsistency/audit.go` |
| ModelCatalog 审计扫描 | `internal/apiserver/infra/mongo/mongoconsistency/scanner.go` |
...
>
> 数据库是发布事实源；Redis 缓存失效和生命周期事件都发生在 commit 之后，只提供最终一致传播。标准 migration、关键唯一索引、revision CAS、启动校验和 Replica Set 集成测试已经与 unified schema 对齐；
> 跨集合审计代码与 one-off 已实现，后续重点是通过生产 scheduler 启用门禁、补强删除/恢复原子性并固化 Mongo durability 部署证据。
