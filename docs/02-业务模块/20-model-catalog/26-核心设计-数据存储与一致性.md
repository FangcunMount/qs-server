...
> 状态：已按 2026-08-06 当前源码与生产台账校准。ModelCatalog 已采用 MongoDB 文档存储、单集合双角色 head/release 结构、不可变发布快照和 Questionnaire/Model 联合发布事务；统一索引已进入标准 migration 与启动时 fail-closed 校验，Norm 唯一索引、Model/Questionnaire revision CAS 和真实 Replica Set 集成测试均已落地。本文继续区分已实现事实与长期治理项。
...
仓库已有带 `integration` build tag 的 `internal/apiserver/container/modules/modelcatalog/transaction_integration_test.go`，通过 `mongodbtest.ReplicaSetDatabase` 在独立数据库内同时写 `questionnaires` 与 `assessment_models`，覆盖 repository failure、commit failure、跨集合 rollback 和发布边界。它补充了 release service 的应用层 rollback 单元测试。
...
标准 Mongo migration 已包含统一 ModelCatalog、Questionnaire 与 Norm 的 canonical indexes；
启动时 `IndexManager` 会幂等清理冲突索引并校验当前索引集合。全新环境可以只通过标准 migration
重建当前 schema，不再依赖一次性 cutover 脚本或已退役集合。

索引变更必须继续使用不可变的后续 migration，并同步更新 contract test。缺少关键唯一索引时，
服务不得把环境视为可写就绪。
...
手工改库、旧脚本或迁移错误仍可能造成 active model 引用不存在/非 active Questionnaire、active snapshot 重复、binding 漂移或 NormRef 缺失。当前 `mongoconsistency` 的最后两个阶段已经覆盖这些风险：

- `model_release` 以 published head 为 anchor，校验恰好一个 active published snapshot、head/snapshot kind/algorithm/questionnaire binding 一致，以及精确 Questionnaire active snapshot 存在；
...
正式生产 one-off 中这两个阶段 finding 为 0，但这只是 2026-08-16 01:13 CST 的时点证据。全局 scheduler 仍因 Survey 方向的 269 条 high drift 保持关闭；不能把“ModelCatalog 两阶段为 0”写成“生产周期审计已经运行”。启用和处置门禁见[调度任务](../../04-接口与运维/07-调度任务.md)。

### 23.7 DefinitionV2 内容摘要已可审计

Publisher 保存 DefinitionV2 canonical content hash 与 hash schema；`AuditPublishedSnapshotInventory` 对比持久摘要、重新物化四类 runtime identity，并对缺失、摘要不一致或不可物化结果 fail closed。
...
独立 Replica Set 测试覆盖 session context、partial unique index、跨集合 rollback、CAS、compatible down 与 incompatible down rejection。它依赖明确授权的非生产 MongoDB，测试被跳过时不能算作取得该项证据。
...
>
> 数据库是发布事实源；Redis 缓存失效和生命周期事件都发生在 commit 之后，只提供最终一致传播。标准 migration、关键唯一索引、revision CAS、启动校验和 Replica Set 集成测试已经与 unified schema 对齐；跨集合审计代码与 one-off 已实现，后续重点是通过生产 scheduler 启用门禁、补强删除/恢复原子性并固化 Mongo durability 部署证据。
