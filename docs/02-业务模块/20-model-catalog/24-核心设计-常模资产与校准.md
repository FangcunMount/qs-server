...
当前每个 FactorTable 可以包含 `Lookup`、`Bands`，也可以两者同时存在。运行时优先直接查表，未命中后再尝试参数换算。
...
### 7.3 参数化 Bands 当前存在不同的缺失值语义

当前 `bandMatchesSubject` 的实现比直接查表宽松：

- Band 限定性别，但受试者性别为空时，不会拒绝；
- Band 限定年龄，但受试者年龄为 0 时，不会拒绝；
- 最终可能选择数组中第一个 Band。

这意味着人口学信息缺失时，参数化路径可能误用某个特定人群参数。它与直接查表“缺失即不匹配”的语义不一致，属于当前实现缺口，不应被解释成领域设计。
...
| --- | --- |
| 模型不需要人口学分层 | 使用显式通用行 |
| 模型需要年龄，输入缺失 | 可重试的数据缺失错误或明确降级，不得随意匹配 |
...
当前代码尚未形成这些稳定错误分类；未命中时多数链路只是没有追加派生分。
...
但当前生产 `BehavioralRatingModelInputProvider` 和 `CognitiveModelInputProvider` 只装配模型、答卷和问卷，没有从 Actor/受试者事实构造 `NormSubject`。仓库中可见的 NormSubject 设置主要出现在测试 fixture。

因此目前需要诚实区分：

| 能力 | 状态 |
| --- | --- |
| Calculation 接受年龄和性别 | 已实现 |
| 常模表保存人口学分层 | 已实现 |
| Evaluation port 携带 NormSubject | 已实现 |
| 生产输入解析器从受试者事实装配 NormSubject | **尚未闭环** |

在这项缺口修复前，依赖人口学分层的端到端常模结果不能仅凭单元测试宣称已可靠支持。
...
`Projection.Apply` 在某个 Factor 找不到常模结果时，只保留原始 Dimension，不返回错误。

这意味着一次 `factor_norm` Evaluation 可能成功，但某些因子没有派生分。当前实现无法区分：

- 模型没有为该 Factor 配 Norm；
- Norm 缺少该 Factor；
- 人口学信息缺失；
- 人群分层未命中；
- raw score 超出范围；
...
对于业务明确要求常模结果的模型，这种静默缺失过于宽松。目标设计应由发布契约声明“必需校准因子”，运行时未命中时产生稳定、可重试性明确的失败原因。
...
### 14.4 当前认知 SPM 留痕弱于行为评定

认知 SPM 当前若存在 NormTables，只向 total dimension 追加 percentile 和可选 standard score，没有写 TScore，也没有写 NormReference。加上 canonical Norm 导入目前拒绝 `cognitive + spm`，这一链路尚未达到行为评定相同的资产和审计标准。

因此不能把 `cognitive.SPM.NormTables` 结构存在，等同于“认知常模已经完整产品化”。
...
| `behavioral_rating + spm_sensory` | 支持 | 支持，单一版本 | 复用行为评定 norm pipeline | 同上，但人口学装配缺口仍存在 | 已接入但需补端到端门禁 |
| `behavioral_rating + default` | 支持 | 支持 | 复用 factor_norm | 同上 | 通用行为评定入口 |
...
| `cognitive + spm` | 导入校验明确拒绝 | handler 可查 NormRef，但没有可通过 Import 的对应资产 | 代码存在 Norm 装配和部分换算 | 仅百分位/标准分，缺 NormReference | 尚未产品化闭环 |
...
当前 NormRepository 能按版本查 Norm，却没有“哪些 AssessmentSnapshot 正在引用这个版本”的专用查询。
...
后续可以增加 Norm usage/read model，但不应把引用列表反向写入 Norm 聚合并要求强一致更新；发布快照才是引用事实源。
...
| Norm 使用关系查询 | 未实现 | 无专用反向 usage read model |
