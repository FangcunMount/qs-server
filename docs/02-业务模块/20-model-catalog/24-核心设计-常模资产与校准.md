...
`ValidateImport` 要求 Kind 与 Algorithm 都非空且属于显式允许组合；缺少 identity 的旧 Norm 直接视为 `norm_invalid`。当前已支持 Behavioral（`brief2`、`spm_sensory`）与 Cognitive SPM（`cognitive + spm`），不通过 default 或 alias 推断算法。
...
发布快照不保存兼容 payload 或 payload format。运行时 DTO 只在内存中由冻结 DefinitionV2 与精确 Norm 组装。
