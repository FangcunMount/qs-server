...
> 状态：核心资产、完整导入校验、精确版本引用、人口学输入、Behavioral/Cognitive 运行时投影、发布兼容门禁与 Outcome 留痕已经实现；Mongo 唯一索引及更广泛的常模治理仍是独立事项。
...
- BRIEF-2、SPM 感觉统合和 Raven SPM 的完整算法语义，分别见 [behavioral_rating：行为评定](./40-模型类型/30-behavioral-rating-行为评定.md) 与 [cognitive：
  认知测验](./40-模型类型/40-cognitive-认知测验.md)。
...
`ValidateImport` 要求 Kind 与 Algorithm 都非空且属于显式允许组合；缺少 identity 的旧 Norm 直接视为 `norm_invalid`。
当前已支持 Behavioral（`brief2`、`spm_sensory`）与 Cognitive SPM（`cognitive + spm`），不通过 default 或 alias 推断算法。
...
3. 两者都未命中，按 subject/cohort/raw 范围返回精确 typed error。
...
- 每个引用版本必须能从仓储查到；
- 行为评定快照构建时只能使用一个常模版本；
- `ValidateImport` 完整验证 Norm identity、Factor、范围、人口学 scope、Mean/StdDev 和有限数值；
- Model Kind、Algorithm、FormVariant 与 Norm 必须相容；
- 每个 NormRef Factor 必须同时存在于 Definition 与 Norm；
- Decision 的派生分必须可产生；`standard_score` 要求只有 Lookup、没有 Band，且每一行都有 `StandardScore`。

### 11.2 仍由后续治理决定的内容

- 是否允许一份 Norm 包含当前模型未引用的额外 Factor；
- 是否引入多 Norm calibration profile；
- 是否为 Norm 建立独立 draft/publish/archive 生命周期和数据库复合唯一索引。
...
AssessmentSnapshot 会冻结完整 `DefinitionV2`，因此精确 `NormRefs` 随模型 release 保留。发布快照不保存兼容 payload 或 payload format。
运行时 DTO 只在内存中由冻结 DefinitionV2 与精确 Norm 组装。
...
- 维护窗口删除重建旧数据时，必须先重新导入可通过 `ValidateImport` 的 Norm，再发布引用它的 DefinitionV2。
...
生产 `BehavioralRatingModelInputProvider` 和 `CognitiveModelInputProvider` 通过 `NormSubjectReader` 从 Actor/Testee 权威事实构造快照。
`AsOf` 固定为 `Assessment.SubmittedAt`；缺失时不回退 `time.Now()`，年龄保持未知。

快照身份规则是：

- 所有可执行 Assessment 只接受 `isn:v2`，摘要明确编码年龄是否存在、年龄值和性别；
- automatic/manual/lease recovery 必须保持上一 attempt 的 v2 引用；
- 资料纠正只能通过经审计的 `evaluation.force_retry` 做 v2→v2 修订，记录 previous/current ref、action request ID 和 attempt origin；
- v1、可读 label 和畸形 v2 一律拒绝；不持久化出生日期或完整人口学明文。
...
认知 SPM 的 total factor 通过同一个 `ResolveNormScore` 解析，写入 percentile、可选 StandardScore 和实际 `NormReference`。
存在 total-factor NormRef 时，未命中是终态失败而不是静默缺少派生分。
...
| BRIEF-2 全新环境初始化（仅 `gXkk9W@4.0.1`） | `scripts/oneoff/seed_brief2` |
| SPM 感觉统合全新环境初始化（仅 `bJFKi3@4.0.1`） | `scripts/oneoff/seed_spm_sensory` |
