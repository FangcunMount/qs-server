...
collection 之间没有 Mongo foreign key。已绑定问卷的 Assessment Release 将 Questionnaire head/snapshot/active 切换、精确 binding、AssessmentModel head 更新与模型快照新增/旧 active 失效置于同一 Mongo transaction；未绑定模型的独立问卷发布仍由 Survey 自己完成。
