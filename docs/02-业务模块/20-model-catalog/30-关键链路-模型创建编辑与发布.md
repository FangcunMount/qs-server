...
Publish (one Mongo transaction)
  -> binding policy
  -> family publish validation
  -> build payload + AssessmentSnapshot
  -> append snapshot + deactivate former active
  -> persist AssessmentModel status/revision
  -> best-effort event/cache effects
```

模型 snapshot 与 head 在一个 Mongo transaction 中发布；问卷发布仍可能先于它们发生。失败排查应分别核对 Questionnaire 与 `assessment_models` 的 head/snapshot 记录。
...
sequenceDiagram
    ...
    S->>P: BuildSnapshot
    P->>PR: Append snapshot + deactivate former active
...
7. 追加新的 AssessmentSnapshot，并将同 code 的旧 active snapshot 标记为 inactive。
8. 乐观更新 `assessment_models` head 中的 status/revision。
...
| 动作 | 当前顺序与约束 |
| --- | --- |
| Unpublish | 聚合标记 draft → deactivate active snapshot → 更新 head → effects |
| Archive | 若已发布先 deactivate snapshot → 聚合标记 archived → 更新 head → effects |
...
| snapshot/head transaction 失败 | 无新 active snapshot，旧 active 保持 | transaction 回滚后修复配置并重试 |
| Unpublish/Archive 更新失败 | 需复核 inactive snapshot 与 head 状态 | 以 Mongo 记录和缓存失效日志为准 |
| event/cache effect 失败 | model 与 snapshot 已成功提交 | effect 不反转发布结果；以 Mongo 为事实源 |

发布不再依赖删除补偿；Mongo transaction 保证 snapshot append、active 切换和 head 更新不会出现半成品状态。

## 9. 排障顺序

1. 检查 `assessment_models` 的 code、status、revision、binding 和 DefinitionV2。
2. 检查 `assessment_models` 中是否有 active snapshot kind/code/release_version，DefinitionV2 是否存在。
