...
- 作为 compatibility payload 的唯一创作输入。
...
- 历史报告重建读取哪一版解释；
- 一份解释资产能否跨模型版本复用；
- 旧 DefinitionV2 和 compatibility payload 怎样迁移。
...
## 8. DefinitionV2、compatibility payload 与发布快照

### 8.1 两份数据的权威性不同

当前 AssessmentModel 同时保存：
...
发布后的 AssessmentSnapshot 也同时保存 DefinitionV2 和 handler 构建的 payload bytes。

它们不是两个平级事实源：

- 新的领域校验和运行时物化应读取 DefinitionV2；
- payload 继续服务现有 wire/runtime DTO 和历史兼容消费者；
- payload 必须能够从 DefinitionV2 单向生成；
- DefinitionV2 缺失时不能反向解析 payload 猜测领域语义；
- compatibility payload 的下线必须先迁移全部消费者。

这是一种迁移期的防腐层设计：保留旧契约，但不允许旧契约重新夺回领域语义所有权。
...
flowchart LR
    ...
    B["Build compatibility payload"]
    U["Update DefinitionV2 + payload"]
    M["Save AssessmentModel head"]

    J --> V --> F --> H --> B --> U --> M
```

`SaveDefinition` 先执行纯 Definition 校验，再在候选模型上构建 payload。只有 payload 构建成功且非空，canonical DefinitionV2 和 compatibility payload 才一起保存。

因此，保存成功表示：

- Definition 内部结构可以解析；
- 当前 handler 能生成兼容 artifact；
...
当前快照冻结 Algorithm、DecisionKind、DefinitionV2、ExecutionSpec 和 compatibility payload，但尚未持久化 AlgorithmFamily。目标设计要求在发布时通过兼容矩阵解析并冻结 AlgorithmFamily，Evaluation 运行时不再重复推导。
...
- DecisionKind 是否可推导；
- typology runtime payload 与绑定问卷题目、选项是否匹配；
- compatibility payload 是否可以构建。
...
8. compatibility payload 是投影，不是第二事实源；
...
| compatibility payload | [`port/modelcatalog/payload`](../../../internal/apiserver/port/modelcatalog/payload/) |
