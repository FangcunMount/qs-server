...
```text
assessment_models (published_snapshot, active)
  -> optional cache
...
```mermaid
flowchart LR
    M["assessment_models<br/>published_snapshot + active"]
    S["统一 Snapshot Repository"]
...
1. Mongo `Repository` 读写 `assessment_models` 中的 `published_snapshot` 记录。
2. 统一 Repository 同时实现 active admission reader 与 exact-version history reader。
...
published BSON 结构、活动快照键和 retained 历史行为见 [22-核心设计-数据存储模型.md](./22-核心设计-数据存储模型.md)。
...
| 方式 | 输入 | 适用场景 | 约束 |
| --- | --- | --- | --- |
| active by ref | kind/sub-kind/algorithm/code/version | 新建 Assessment intake | version 必填，且 snapshot 必须 `release_status=active` |
| exact by ref | kind/sub-kind/algorithm/code/version | Assessment 已冻结模型引用后的执行、重试与报告补偿 | version 必填，可读取 active 或 archived snapshot |
...
sequenceDiagram
    ...
    G->>M: GetPublishedModelByRef (exact version)
...
| active snapshot 不存在 | not found | 查询 head 继续执行 |
...
| 层 | canonical 契约 |
| --- | --- |
| Mongo runtime | `assessment_models` 中 active `published_snapshot` 的 AssessmentSnapshot |
...
| 环节 | 路径 |
| --- | --- |
| Published repository / runtime catalog | [`infra/mongo/modelcatalog/repo.go`](../../../internal/apiserver/infra/mongo/modelcatalog/repo.go)、[`infra/ruleset/runtime_catalog.go`](../../../internal/apiserver/infra/ruleset/runtime_catalog.go) |
