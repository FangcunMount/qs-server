...
- Algorithm、发布时冻结的 AlgorithmFamily、DecisionKind 与 DefinitionV2 中的 ExecutionSpec；
- DefinitionV2；
- Questionnaire 引用及 Definition 内容摘要。

快照不保存 compatibility payload。运行时必须使用冻结的 AlgorithmFamily 与 DecisionKind，不能从 Kind、格式或执行路径再次推导。
