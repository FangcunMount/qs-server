...
| Collection | 保存内容 |
| --- | --- |
| `assessment_models` | `head` 工作记录，以及 active/archived `published_snapshot`、DefinitionV2、wire payload 与精确问卷版本绑定 |
| `questionnaires` | `head` 工作记录，以及 active/archived `published_snapshot` 完整历史 |
| `assessment_norms` | 按 table version 保存的 Norm |

Redis 仅是 active/latest/list 与 exact-version 的普通读缓存，MongoDB 是唯一事实源。旧 `scales` 与 `published_assessment_models` collection 不是当前 ModelCatalog 生产读写路径。
