...
- Mongo `AssessmentSnapshot` 已持久化 Algorithm、DecisionKind 和 DefinitionV2，按当前契约不再单独持久化 AlgorithmFamily；Evaluation 直读 DecisionKind，只在进程内由它派生 family。

因此，上述规则中的历史身份保护和统一完整性契约仍未全部固化，不能在文档中假装已经完全实现；但 DecisionKind-only 的 canonical 持久化和 Evaluation 路由已经收口，不再把“冻结 AlgorithmFamily 字段”列为待完成目标。历史身份守卫适合放在需要读取 retained release 的应用层模型演进策略，模型完整性与兼容性适合放在发布校验。具体分别在 [问卷绑定与发布版本](./22-核心设计-问卷绑定与发布版本.md)、[DefinitionV2 与模型扩展](./20-核心设计-DefinitionV2与模型扩展.md) 与 [模型身份、算法绑定与执行路由](./21-核心设计-模型身份、算法绑定与执行路由.md) 中展开。
...
当前发布物化会同时根据 `Kind + SubKind + Algorithm` 和 Definition 的 DecisionKind 解析 AlgorithmFamily 并交叉校验；这只是现有兼容矩阵的实现，不是永久一对一规则。进程内 `AssessmentSnapshot` 有派生 AlgorithmFamily 便利字段，但 Mongo record 以 DecisionKind 为 canonical 路由事实，不单独持久化 family。
...
`AssessmentModel` 和发布后的 `AssessmentSnapshot` 都只保存 canonical `DefinitionV2`。保存时，application `definition.Registry` 根据模型身份执行 family-specific 物化校验；校验只证明 DefinitionV2 能构造运行时 DTO，不保存、序列化或哈希临时 DTO。
...
- 发布物化已校验 AlgorithmFamily 与 DecisionKind 一致，`AssessmentSnapshot` 持久化 canonical DecisionKind（MC-R006）；历史读取仅在 Kind + Algorithm 能唯一恢复缺失 DecisionKind 时进入显式 compatibility 适配；
