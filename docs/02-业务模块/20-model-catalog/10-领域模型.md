...
- 首次发布后 Algorithm 不可更换，当前缺少统一的历史 release 守卫；
- 首次发布后 questionnaire code 固定，当前缺少统一的历史 release 守卫；
- “Factor 必需、Norm 可选、Decision 必需”尚未形成所有模型族统一执行的发布完整性契约；
- `AssessmentSnapshot` 尚未冻结 AlgorithmFamily，当前运行时仍会从 identity 或 execution path 推导；
