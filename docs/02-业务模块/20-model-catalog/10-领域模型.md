...
因此，可发布测评模型必须同时具备 Factor 和 Decision，Norm 则按模型需要选配。只有 Factor 而没有 Decision 的配置不是完整测评模型；如果只需要收集答案或展示基础分数，
应让业务链路停在 Questionnaire / AnswerSheet，而不是创建一个残缺模型。
...
从目标领域语言看，Decision 只输出稳定结果事实，例如 `OutcomeCode`、`LevelCode`、分类代码或画像事实。
title、summary、description、traits、strengths、suggestions 等内容属于 Interpretation-oriented Assets，不应被误称为 Decision 结果。
当前结构尚未完成物理拆分，本文会一直同时标明“概念边界”和“代码现状”。
...
- DefinitionV2 已保存，但无法物化为声明的运行算法；
...
修改已经发布的 head 时，应用服务会先调用 `ForkDraftFromPublished`，把工作 head 变回 draft，再执行真正的编辑命令。`ForkDraftFromPublished` 本身不增加 revision，
后续实际修改只推进一次 revision，从而保持一次命令对应一次乐观锁修订。
...
- DefinitionV2 必须存在并能按声明的 AlgorithmFamily 物化；
- typology 必须使用 `sub_kind=typology` 并配置 algorithm；
- AlgorithmFamily 与 DecisionKind 必须完整且通过精确兼容校验；
...
4. 发布时应解析并交叉校验 AlgorithmFamily 与 DecisionKind，持久化 canonical DecisionKind；Evaluation 只从该 DecisionKind 派生 family，不能从 Kind/Algorithm 重新决定路由。
...
- Mongo `AssessmentSnapshot` 已持久化 Algorithm、DecisionKind 和 DefinitionV2，按当前契约不再单独持久化 AlgorithmFamily；
  Evaluation 直读 DecisionKind，只在进程内由它派生 family。

因此，上述规则中的历史身份保护和统一完整性契约仍未全部固化，不能在文档中假装已经完全实现；但 DecisionKind-only 的 canonical 持久化和 Evaluation 路由已经收口，不再把“冻结 AlgorithmFamily 字段”列为待完成目标。
历史身份守卫适合放在需要读取 retained release 的应用层模型演进策略，模型完整性与兼容性适合放在发布校验。具体分别在
[问卷绑定与发布版本](./22-核心设计-问卷绑定与发布版本.md)、[DefinitionV2 与模型扩展](./20-核心设计-DefinitionV2与模型扩展.md) 与 [模型身份、算法绑定与执行路由](./21-核心设计-模型身份、算法绑定与执行路由.md) 中展开。

## 5. 发布模型语义：CoreModel 与 AlgorithmBinding

### 5.1 它是什么

`DefinitionV2` 是 `AssessmentModel` 内部的组合值，不具有独立 code、独立 Repository 或独立发布状态。它随 working head 的 revision 演进，并在发布时整体复制进 `AssessmentSnapshot`。
它不是“任意 JSON 配置”；服务端以显式类型表达每层结构，并执行跨层引用校验。
...
`Algorithm` 是一项具体、稳定的代码能力；`ExecutionSpec` 是这项能力的模型参数契约。二者都不是代码实现本身。实际实现属于 Calculation，
Evaluation 负责把 ModelCatalog 的 Factor、Norm、Decision 与 ExecutionSpec 转成 Calculation 能理解的中性输入。
...
当前发布物化会同时根据 `Kind + SubKind + Algorithm` 和 Definition 的 DecisionKind 解析 AlgorithmFamily 并交叉校验；这只是现有兼容矩阵的实现，不是永久一对一规则。
进程内 `AssessmentSnapshot` 有派生 AlgorithmFamily 便利字段，但 Mongo record 以 DecisionKind 为 canonical 路由事实，不单独持久化 family。
...
### 5.4 DefinitionV2-only

`AssessmentModel` 和发布后的 `AssessmentSnapshot` 都只保存 canonical `DefinitionV2`。保存时，application `definition.Registry` 根据模型身份执行 family-specific 物化校验；
校验只证明 DefinitionV2 能构造运行时 DTO，不保存、序列化或哈希临时 DTO。

DefinitionV2 缺失、运行身份不完整或无法物化时必须 fail closed。运行时消费者只能从 published DefinitionV2 构造内存 DTO，不存在 payload 反向解码或兼容字段回退。
...
Decision 不负责生成 title、summary、description、suggestions，也不应该知道报告 section、template 或展示组件。
这些内容即便当前仍物理保存在 `Conclusions`、`Outcomes` 或 `ReportMap` 中，概念上也属于 Interpretation-oriented Assets。
...
- Algorithm、发布时冻结的 DecisionKind 与 DefinitionV2 中的 ExecutionSpec；发布期还会解析并校验 AlgorithmFamily，但 Mongo record 不单独存储它；
- DefinitionV2；
- Questionnaire 引用及 Definition 内容摘要。

快照不保存 compatibility payload。Evaluation 必须使用冻结的 DecisionKind，不能从 Kind、Algorithm、格式或执行路径重新决定它；AlgorithmFamily 只能由该 DecisionKind 在进程内派生。
...
其中 Algorithm、ExecutionSpec 与 DecisionKind 已经进入当前发布快照；AlgorithmFamily 尚未进入。目标改造完成后，Evaluation 应直接使用快照中的 AlgorithmFamily，
不再根据 `Kind / SubKind / Algorithm` 或产品分类重新推导。
...
Calculation 也不应反向依赖 ModelCatalog、Factor、Question 或 Evaluation 对象。它只提供无状态计算内核；跨领域对象转换由 Evaluation 应用层适配器承担。
这一约束防止算法实现重新长成“既查配置、又管状态、又出报告”的硬编码解析模块。
...
- DefinitionV2 是 head 和 snapshot 唯一持久化模型定义，family-specific DTO 只在内存物化；
...
- 发布物化已校验 AlgorithmFamily 与 DecisionKind 一致，`AssessmentSnapshot` 持久化 canonical DecisionKind（MC-R006）；
  历史读取仅在 Kind + Algorithm 能唯一恢复缺失 DecisionKind 时进入显式 compatibility 适配；
