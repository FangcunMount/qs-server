...
| 扩展原则 | 新增风险等级、区间规则或文案结构时，要同步 domain、DTO、report、统计、后处理和文档 |
...
| “高风险是否触发重点关注” | Evaluation 后处理 / Actor | 这是后续业务动作 |
...
| Tests | 区间匹配、报告、统计、后处理 |
| Docs | Scale / Evaluation / 接口文档 |

### 10.1 风险等级的稳定性

风险等级一旦进入报告、统计和后处理，就成为跨模块语义。不要轻易重命名或删除。
...
| 风险等级枚举化 | 可统计、可通知、可触发后续动作 | 新等级变更成本高 |
...
不建议。中文文案不适合统计、排序和接口兼容。应保留稳定枚举，再做展示转换。

### 14.3 “修改文案不会影响业务”

不一定。如果文案只是展示语言，影响较小；如果同时调整 riskLevel 或 scoreRange，就会影响报告、统计和后处理。
