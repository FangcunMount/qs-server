...
| 发布冻结 | `published` 后因子、解读规则和问卷绑定被冻结，展示字段仍可维护 |
...
| `published` | 已发布，可被 Evaluation 作为规则消费；规则字段冻结，展示字段可维护 |
...
| 字段类型 | published 后是否可直接修改 |
| -------- | -------------------------- |
| 因子、计分策略、解读规则 | 否 |
| 绑定问卷 code/version | 否 |
| title、description、category、stages、applicableAges、reporters、tags | 是 |

问卷发布后的绑定版本同步只作用于 `draft` Scale；`published` 和 `archived` 不会被自动改写 questionnaireVersion，避免线上规则事实漂移。
