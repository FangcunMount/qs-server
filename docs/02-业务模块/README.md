...
Survey、ModelCatalog、Evaluation、Interpretation、Actor、Plan 与 Statistics 已保留独立的领域模型、核心设计、关键路径和设计问题文档。后续深拆必须以当前代码复核为前提，不能把归档模板直接搬回；代码治理应优先引用各模块设计问题文档中的稳定编号，逐项补保护、实施和验收。
