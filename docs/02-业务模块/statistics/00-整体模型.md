...
- 当前 MySQL 聚合层收敛为 `statistics_journey_daily`、`statistics_content_daily`、`statistics_plan_daily`、`statistics_org_snapshot` 四张表；旧 `statistics_daily / statistics_accumulated / statistics_plan` 和拆散的 `analytics_*_daily` 不再作为运行时读写入口。
