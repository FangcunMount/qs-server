...
角色管理已收敛为 IAM 原子替换直接角色，qs-server 分开保存直接/有效角色、policy version 与 pending 状态；IAM 已提交但投影延迟不会被伪装成失败。仍需在生产观察中验证 pending 能由请求期同步与 IAM 版本事件收敛。部署只接受 IAM AuthZ v3 真值，禁止请求级切换或恢复本地角色真值。
