...
Actor 主体装配、核心身份对象、照护关系、公开入口和访问范围已经实现，但不能据此签署安全闭环：公开 Intake 仍缺 Profile 代表权校验，
collection-server 的 Testee 详情、照护上下文和更新入口仍缺调用用户到目标 Testee 的 ownership gate。两项均是当前源码可复现的 P0；生产证据与仓库测试必须分开记账。

## 2. 设计问题与重构清单

> 本节是模块风险台账。目标设计不等于已实现；Issue、分析报告、提交和 PR 优先引用 `AR-Rxxx`。条目关闭必须同时具备代码、migration、数据、测试、观测与文档证据。`AR-Rxxx` 的 P0/P1 是固有风险等级，不等于开放 `FINAL-P1`；
> 当前版本判定以[当前版本定档验收台账](../../00-总览/09-当前版本定档验收台账.md)为准。
...
当前存在一条明确反例：`internal/collection-server/transport/rest/router.go` 将 `/api/v1/testees/:id` GET/PUT 和 `/:id/care-context` 直接交给 Testee handler；
handler/service 随后仅以客户端给出的裸 TesteeID 调用 Actor gRPC。该链路没有使用已有 `application/testeeaccess.Authorizer`，PUT 虽检查“已认证”，
却未证明该 User 对目标 Testee 有 ProfileLink/关系权限。apiserver 的 Actor gRPC service 也只按 TesteeID 读写；生产 gRPC ACL 证明的是 collection-server 服务身份，不是最终用户资源归属。
因此，已认证用户可能按可猜 ID 读取、修改或查看他人的 Testee 照护上下文。

关闭门槛：在 collection transport/application 边界统一接入 User -> ProfileLink -> Testee 的 ownership gate，GET、PUT、care-context 均 fail closed；
补同用户、跨用户、跨机构、inactive link、IAM 不可用和 cache hit/miss/unavailable 负向测试；验证 apiserver gRPC 服务 ACL 不能被误当作用户授权，并增加防止新 Testee ID 路由绕过 Authorizer 的架构守卫。
...
角色管理已收敛为 IAM 原子替换直接角色，qs-server 分开保存直接/有效角色、policy version 与 pending 状态；IAM 已提交但投影延迟不会被伪装成失败。仍需在生产观察中验证 pending 能由请求期同步与 IAM 版本事件收敛。
部署只接受 IAM AuthZ v4 真值，禁止请求级切换或恢复本地角色真值。

#### AR-R010/AR-R011：重点关注

当前已使用 durable event、projection ledger、event_id dedup、失败重试、`manual_required` 和 Artifact fact reconcile 恢复漏标。
生产缺口必须先按报告与受试者当前状态做跨存储只读分类，再经显式批准 apply，归零后关闭临时 fact reconcile。若要自动解除，先把 bool 升级为带 source/reason/status/opened/closed/operator 的 AttentionCase；
业务未决前保持自动只标记。

#### AR-R012/AR-R013：Entry 目标身份

先扫描非空 target_version 和消费者，禁止新入口指定版本，完成旧入口兼容后再删字段。目标类型改为 questionnaire 与 assessment_model；
具体 scale/typology/behavioral/cognitive 由 ModelCatalog 快照路由，Actor 不枚举模型族。
...
门店第一阶段的领域、应用与隔离 MySQL 测试覆盖首次配置、调店、重复请求、事务回滚和竞争写入；新库完整迁移链包含 000073。线上配置、旧码拒绝与新码可用仍需独立验收，不能由这些测试结果推定。
