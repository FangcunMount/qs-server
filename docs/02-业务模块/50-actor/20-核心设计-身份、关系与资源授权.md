...
- 授权投影：`roles` 保存直接角色，`effective_roles` 保存包含继承的有效角色，并记录实际 IAM policy version；Actor 不保留本地角色真值或禁用 IAM 后的回退路径。

员工角色编辑只原子替换调用方可管理的直接角色，不能把继承角色当 Assignment 撤销。访问服务优先读取请求上下文中的授权快照，否则调用 AuthzSnapshotReader；不能因为任何 `staff` 投影恰好包含角色名就跳过 IAM Snapshot/Check。
