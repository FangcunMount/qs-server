...
项目早期曾在 qs-server 内解析微信小程序身份并组织 User。IAM 统一身份系统出现后，账号、认证凭据、Profile 和 ProfileLink 迁出；
但 IAM 不应该知道“某个 Profile 在 A 机构是不是受试者”“某位医生当前负责哪些患者”，所以 Actor 仍然必要。
...
- 运营人员可以只有 Operator；
- 医生使用 Clinician 业务档案，由总部维护门店配置，不开通或绑定 Operator 后台身份。
...
- 授权投影：`roles` 与 `effective_roles` 均保存直接角色，并记录实际 IAM policy version；`inherited_roles` 兼容输出恒为空。Actor 不保留本地角色真值或禁用 IAM 后的回退路径。

运营人员角色编辑只原子替换调用方可管理的直接角色。访问服务优先读取请求上下文中的授权快照，否则调用 AuthzSnapshotReader；不能因为任何 `operators` 投影恰好包含角色名就跳过 IAM Snapshot/Check。
...
医生后台入口与 Operator—Clinician 绑定已退役。后台先验证有效 Operator，再检查 IAM 动作权限和既有业务范围。当前没有已配置业务范围的普通 Operator 继续拒绝受试者业务读取，不因删除医生身份推导全公司范围；Scope 为后续独立阶段。参与者与监护人通过原有 ProfileLink 自服务路径访问。
...
4. 通过 IAM Assignment API 同步 roles；
5. 从 IAM AuthZ v4 快照刷新本地投影；
6. IAM 写入或快照刷新失败时，尝试回滚新建 Operator。

这是一套补偿式跨系统流程，不是分布式原子事务。进程若在步骤之间退出，仍可能留下 IAM 账号、本地投影或角色不一致。

停用 Operator 会清理本地角色投影并禁止继续分配；角色分配和撤销必须经过 IAM，不会直接改写本地 Operator。删除本地 Operator 不等于删除 IAM User。姓名、邮箱和电话只是本地缓存资料。
...
上述模型是授权契约，不等于所有入口均已接线。当前 collection-server 的 `/api/v1/testees/:id` GET/PUT 和 `/:id/care-context` 直接把路径中的 TesteeID 交给 Actor gRPC；
它们没有调用已有的 `application/testeeaccess.Authorizer`。PUT 只验证请求已认证，没有证明当前 User 拥有目标 Testee 的 active ProfileLink 或其它授权关系；
生产 gRPC ACL 也只证明调用方是 collection-server 服务。因此 Actor 安全维度仍为 blocked，精确证据和关闭门槛见 [AR-R002](./90-设计问题与验证清单.md#ar-r002资源-ownership-必须继续验证)。
...
| operators | `(org_id, user_id)` 唯一 | 退出任务阻断复活及重新赋权 |
| clinician | org/employee_code 唯一；StoreID 可空 | 不再包含后台身份绑定 |
...
原有身份和关系表有软删除、审计字段和 version，但看到 version 字段不等于乐观锁已生效。每个 repository Update 必须使用 `WHERE version = old`、原子递增，并把零行更新转为 concurrency conflict 才算完整保护。
...
Testee 注册的 IAM Profile 校验是外部只读；Operator 注册包含 IAM 写入。本地 MySQL 事务不能回滚 IAM。
Operator provisioning 应有 operation_id、阶段记录、幂等重试、补偿和周期对账，而不能只依赖函数返回时的即时 rollback。
...
Store 是 Actor 模块下的独立聚合根。OrgID 表示公司，StoreID 表示服务门店；医生当前最多关联一个同公司门店，门店可关联多位医生。未配置不代表总部或全公司范围。

本阶段建设总部配置能力，不迁移 Testee 归属，不改变医生与受试者关系、测评、计划和 IAM 授权事实，也不实现 Scope。医生无需拥有运营后台账号。

### 8.2 分层与不变量

| 层 | 责任 | 实现入口 |
| --- | --- | --- |
| 领域 | 门店编号规范、不可变公司与编号、版本和启停规则 | `internal/apiserver/domain/actor/store/store.go` |
| 应用 | 总部能力检查、公司范围、事务编排、幂等和配置历史 | `internal/apiserver/application/actor/store/service.go` |
| 端口 | 事务内锁、查询、保存、历史与入口失效能力 | `internal/apiserver/port/actorstore/repository.go` |
| 基础设施 | PO 映射、SQL、行锁和比较更新 | `internal/apiserver/infra/mysql/actor/store/repository.go` |
| 装配 | 使用既有 Actor 模块与 MySQL 事务 Runner | `internal/apiserver/container/modules/actor/assemble.go` |
| 接口 | 已认证公司上下文、请求解析与响应转换 | `internal/apiserver/transport/rest/handler/store.go` |

领域对象不携带 JSON、GORM 标签或表名。数据库实现不依赖应用层 DTO。

公司内门店编号去空白并统一为大写，创建后不可修改。门店只能启停，不能删除；停用前检查所有当前关联医生，包括已停用医生。编辑和状态变更携带预期版本，过期版本返回冲突。

### 8.3 配置与永久失效

- 首次配置：空门店变为启用的同公司门店，保留所有旧入口。
- 调店：A 变为 B，在同一事务中更新医生、使已有入口永久失效并追加历史；不恢复旧二维码，不自动生成新码。
- 重复请求：同一请求标识和原参数返回原历史；参数不同拒绝。同门店提交不新增历史。
- 普通资料更新显式限制可写字段，不能覆盖门店归属或入口永久失效状态。

锁顺序为目标门店、医生、入口。入口创建与调店通过医生锁串行；解析、入组和重新启用在锁定医生后对入口执行当前读，避免 MySQL 可重复读快照再次使用旧入口状态。公开解析只是预览，入组事务仍重新检查入口。

永久失效与普通停用分别存储。因调店失效的入口保留 Token 和历史内容，返回失效原因，无法重新启用。医生转回原门店也不会恢复旧码。

### 8.4 API 与总部操作

门店接口位于 `/api/v1/stores`，提供列表、详情、创建、编辑、activate 和 deactivate。`/api/v1/stores/configuration-progress` 分别返回全部医生和有效医生的已配置、未配置数量。

医生配置通过 `PUT /api/v1/clinicians/:id/store`，历史通过 `GET /api/v1/clinicians/:id/store-history`。医生列表支持 store_id 或 unconfigured 筛选，两者不能同时启用；过滤在分页与计数前执行。

共享应用入口要求 QS 管理通配能力和有效公司、用户上下文，不根据角色显示名称放行。现有后台路由的公司解析和管理员中间件继续生效。

### 8.5 迁移与发布限制

新增迁移 000073：actor_stores、clinician_store_history、医生可空 store_id，以及入口 invalidated_at / invalidation_reason。历史迁移不修改，既有医生仍未配置，既有二维码不因结构升级失效。

发布先更新 QS，再更新运营端。开放调店前必须确认所有医生和入口写入实例已更新。新增结构和历史数据保留；发生过调店后，禁止回退到忽略永久失效字段的版本。配置错误通过新的受审计调店修正。

### 8.6 验证证据边界

领域与应用测试覆盖编号、版本、停用限制、首次配置、调店、幂等和权限拒绝。MySQL 专项覆盖实际迁移、事务回滚、并发重复提交、停用与配置竞争以及可重复读快照中的入口重新检查。

专项入口：`internal/apiserver/infra/mysql/actor/store/repository_mysql_test.go`。CI 的 Actor Store MySQL Transactions 使用独立 MySQL 8.0；QS_STORE_REQUIRE_MYSQL=true 时缺少测试连接配置直接失败。测试创建独立临时数据库并清理，不使用生产数据库。

代码检查、部署版本和线上业务验收分别记录。本文件不将本地测试通过视为生产发布或验收完成。
