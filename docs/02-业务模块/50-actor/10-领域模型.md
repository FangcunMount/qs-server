...
这是报告成功后的后置投影：先按 event_id 幂等落 `interpretation_attention_projections`，再调用 Actor；失败进入 `failed`，由常规 reconciler 重试，超过预算进入 `manual_required`，但不回滚 Report。Artifact fact reconciler 还会从显式切点扫描 high/severe 报告，发现从未建立 ledger 的缺口；生产默认 dry-run，apply 需显式批准。low/moderate 不会自动取消重点关注，防止覆盖人工判断。
