...
当前可靠链路是：durable `interpretation.report.generated` -> 幂等 projector -> retry/manual_required -> Artifact fact reconcile -> 可对账工作台状态。当前生产仍有 33 个历史 ledger 缺口，属于数据闭环而不是代码能力缺失。
...
| AR-R010 | P1 | 高风险重点关注可靠投影与历史缺口恢复 | 代码已完成，生产数据闭环中 |
