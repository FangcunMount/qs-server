...
Actor 同时拥有公司下的 Store 聚合根，Clinician 通过可空 StoreID 引用当前服务门店；本阶段不为 Testee 增加门店归属。规则与分层见[门店与医生配置](./20-核心设计-身份、关系与资源授权.md#8-门店与医生配置)。
...
classDiagram
    ...
    Store "0..1" <-- "0..*" Clinician : current service store
...
权限真值只来自 IAM AuthZ v4；本地 `roles` 与 `effective_roles` 均表示直接角色投影（兼容字段保留；`inherited_roles` 恒为空），不提供 IAM 不可用时的本地授权回退。角色编辑通过 IAM 原子替换直接角色；
已提交但投影尚未追上时保留旧投影并标记 pending。

当前支持的角色包括 `qs:admin`、`qs:content_manager`、`qs:assessment_operator`、`qs:evaluation_plan_manager` 和 `qs:result_reviewer`。
...
当前 ClinicianType 包括 doctor、counselor、therapist、other。Clinician 不再绑定 Operator；医生只保留业务档案，由总部通过 Store 配置服务门店。医生档案不能作为后台登录资格。
...
- 活动存储及接口统一为 `operators`；旧 `/staff` 返回 410。历史迁移和填写人序列化值 `staff` 保留用于解释旧数据；
