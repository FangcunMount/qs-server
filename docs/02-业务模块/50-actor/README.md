...
> 状态：**当前文档结构已收口**。模块边界、领域模型、核心设计、关键链路和风险验证已经按当前源码拆分复核。Actor 的角色不是“再实现一套用户系统”，而是把 IAM 身份转换为 qs-server 可使用的业务参与者、照护关系和访问范围。
...
| Clinician | 聚合根 | `org_id + clinician_id`，可选 `store_id` | 业务从业者身份与后台账号解耦 |
...
Actor 按问题边界维护四篇 canonical 文档，文件数量少于核心模块，但最低结构一致：

| 文档 | 负责回答 |
| --- | --- |
| [领域模型](./10-领域模型.md) | Testee、Operator、Clinician、Relation、AssessmentEntry 的聚合、不变量和模块契约 |
| [身份、关系与资源授权](./20-核心设计-身份、关系与资源授权.md) | IAM/Profile 投影、填写人语义、公开入口、授权与数据一致性 |
| [身份进入测评与报告访问](./30-关键链路-身份进入测评与报告访问.md) | 建档、分配/转诊、门诊入口和重点关注投影四条责任链 |
| [设计问题与验证清单](./90-设计问题与验证清单.md) | AR-Rxxx、测试矩阵、关闭门槛、事实源和验证命令 |
