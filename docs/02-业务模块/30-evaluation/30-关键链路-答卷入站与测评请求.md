...
本文说明一份已持久化 AnswerSheet 如何进入 Evaluation：collection-server 的同步 `EnsureAssessment` 与 `answersheet.submitted` Worker 重放如何汇聚到同一 Journey，Journey 如何解析模型 binding 和 Plan，以及 Assessment 在什么条件下发出 `evaluation.requested`。
...
AnswerSheet durable commit
  ...
  -> EnsureAssessment(answerSheetID)       同步提交路径
  -> EnsureAssessment(answerSheetID)       Worker 事件重放路径
...
两个入口会并发或先后到达，但都调用 `application/journey/assessmentintake.Service.Ensure`。Redis lease 只对 Worker 重复降噪；最终正确性由 `assessment.answer_sheet_id` 唯一约束和 duplicate-then-read 保证。

## 3. 参与者与边界

| 参与者 | 责任 |
| --- | --- |
| collection `SubmissionService` | 持久化答卷后同步调用 Assessment Intake，成功响应必须同时有 AnswerSheet ID 和 Assessment ID |
...
sequenceDiagram
    ...
    C->>S: Save AnswerSheet
    S->>O: answersheet.submitted (same transaction)
    S-->>C: AnswerSheet ID
    C->>G: EnsureAssessment
    O-->>W: answersheet.submitted
    W->>G: EnsureAssessment (replay / recovery)
    G->>J: Ensure(command)
...
上图中同步和事件入口不代表每次都会按固定顺序到达。Outbox 可能在 collection 的同步 gRPC 前后被发布，所以 Journey 必须在数据库约束下可重入。

## 5. 阶段一：双入口收敛

### 5.1 collection 同步路径

`SubmissionService.submitSync` 先完成 Survey 作答提交，然后调用 `AssessmentIntakeService.EnsureAssessment`。如果 intake 失败，AnswerSheet 和其 Outbox 已经成立，collection 本次请求返回失败，后续可由 `answersheet.submitted` 重放补齐 Assessment。

SubmitQueue 的 `done` 状态要求同时保存 AnswerSheet ID 和 Assessment ID。幂等 key 已完成时，collection 还会通过 AnswerSheet ID 解析 Assessment ID，缺失时不返回伪成功。
...
| 同步 intake 失败但 AnswerSheet 已落库 | collection 请求失败，Survey 事实仍存在 | `answersheet.submitted` Worker 可重放 Ensure |
