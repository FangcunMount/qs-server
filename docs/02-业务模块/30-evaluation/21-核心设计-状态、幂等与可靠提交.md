...
Outcome 审计查询只选择 `assessment_id`、`id`、`evaluation_run_id`、`model_kind`，不会加载 `report_input_json` 或 `payload_json`。
审计覆盖 Assessment×Run×Outcome 六类基础组合，并继续检查 Projection、Outbox 和 Run reference 漂移；一次扫描可以报告多个 mismatch。
...
一个 leader lock 覆盖完整审计周期；批次水位、周期耗时、扫描量和最近成功时间均可观测。dev/prod 当前每 24 小时完成一轮，配置校验限制在 6～24 小时。
Evaluation 和 Interpretation 的过期执行租约由两条 10～30 秒短周期 runner 独立恢复，不再拖着全量审计高频运行。
