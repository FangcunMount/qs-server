...
Scheduler 按 Assessment ID 水位分批扫描 submitted、evaluated、failed，批量装配以下只读证据：

- Assessment 状态；
- 最新 Evaluation Run 状态与引用；
- Outcome 身份、Run 引用与 ModelKind；
- score projection 行数及 Outcome 关联；
- committed Outbox 行数、Outcome/Run 引用与状态。

Outcome 审计查询只选择 `assessment_id`、`id`、`evaluation_run_id`、`model_kind`，不会加载 `report_input_json` 或 `payload_json`。审计覆盖 Assessment×Run×Outcome 六类基础组合，并继续检查 Projection、Outbox 和 Run reference 漂移；一次扫描可以报告多个 mismatch。

检测结果记录 Prometheus 指标和建议动作，但 disposition 仍为 `deferred`，不会自动修改 Assessment、Outcome、Projection 或 Outbox。原因是单项事实不能证明成功闭环，自动补写可能把部分迁移状态伪装成成功。

一个 leader lock 覆盖完整审计周期；批次水位、周期耗时、扫描量和最近成功时间均可观测。dev/prod 当前每 24 小时完成一轮，配置校验限制在 6～24 小时。Evaluation 和 Interpretation 的过期执行租约由两条 10～30 秒短周期 runner 独立恢复，不再拖着全量审计高频运行。
...
| Retry Policy | [`internal/pkg/retrygovernance`](../../../internal/pkg/retrygovernance/) |
