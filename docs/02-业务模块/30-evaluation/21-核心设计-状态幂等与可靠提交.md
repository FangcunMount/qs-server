...
retryable 失败后，Run 在失败事务内形成 RetryDecision。attempt 小于 3 时，同一事务按资源与 expected_attempt 写入确定性 `evaluation.retry.requested`；Worker ACK 原消息。消费 retry event 时用 latest attempt、disposition、origin 与 request_id 做 CAS，重复或过期事件 noop 后 ACK。

### 7.2 操作者显式重试

System Governance 对组织、expected_attempt、confirm、reason 与 request_id 校验后，仅给 latest `manual_required` Run 一次普通 Retry 授权；Force Retry 仅允许 latest `terminal` Run。兼容 REST 接口只适配普通 Retry，不再把 Assessment 预先改回 submitted。
