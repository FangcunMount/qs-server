...
- 模型身份、AlgorithmFamily、DecisionKind、PayloadFormat 和 ExecutionPath 分别解决什么问题；
...
| `PayloadFormat` | 版本化 payload 格式 | 如何理解模型执行载荷 |
| `ExecutionPath` | scale_descriptor 等 | 组合根要物化哪套 Provider 和 Pipeline |
| `DescriptorKey` | family + decision + format | 运行时选择哪份 Descriptor |
...
当前 RuntimeResolver 优先从已经物化的 InputSnapshot 构造 ModelRoute。只有无法从输入取得路由时，才回退到 Assessment 中的模型引用。

优先使用 InputSnapshot 的原因是：发布模型物化结果可能携带更完整的 DecisionKind 和 payload 信息，而 Assessment 只保存稳定模型引用。
...
1. 优先根据 kind/subKind/algorithm 推导 AlgorithmFamily；
2. 若 identity 无法推导，再尝试从 DecisionKind 推导；
3. 已发布模型提供的 DecisionKind 必须与 AlgorithmFamily 相容；
4. 未提供或不相容时，使用该 family 的默认 DecisionKind；
5. 最终形成 `AlgorithmFamily + DecisionKind + PayloadFormat`。
...
默认值提供机制级兜底，但不能用来掩盖一个完全不受支持的模型身份。不受支持的路由必须失败，而不是静默落入某个通用 Evaluator。
...
1. 完整 DescriptorKey：family + decision + payload format；
2. family + payload format；
3. family 级 Descriptor。

当前默认配置为每个 AlgorithmFamily 注册一个 family-level Descriptor，再将对应 Pipeline 三件套装配进去。

这种分层为未来扩展保留了空间：如果同一个 family 下某种 PayloadFormat 需要独立适配，可以注册更精确的 Descriptor，而不复制整个执行服务。
...
| 发布 payload 结构新增字段 | ModelCatalog codec 和对应 InputProvider |
...
| 特定 PayloadFormat 需要独立 Pipeline | 注册更精确 RuntimeDescriptor |
...
| Descriptor 解析 | 没有注册对应 AlgorithmFamily/PayloadFormat | validation failure |
...
当前 ModelRoute 的 DecisionKind 主要从 typology 显式 runtime 中读取，PayloadFormat 仍通过模型身份推导；Assessment fallback 又只携带 kind/subKind/algorithm。

这意味着发布快照中的完整运行身份还没有通过一份统一契约贯穿 InputSnapshot、路由和 Outcome。未来可以考虑让已发布模型直接提供规范 RuntimeRoute，减少重复推导和 legacy fallback。

### 13.3 NormSubject 尚未完整进入主解析链路

InputSnapshot 已定义 NormSubject，但当前 RepositoryResolver 不会统一加载受试者年龄、性别等人口学事实。需要人口学常模的机制可能只能获得空值或由特定路径补充。

这不是统一执行模型的结构问题，但会影响常模模型能否严格按发布规则执行，后续需要明确数据来源、快照时点和缺失值策略。
