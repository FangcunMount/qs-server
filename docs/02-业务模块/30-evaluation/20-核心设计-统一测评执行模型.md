...
当前 Scope 增量由运营应用入口承担，不进入 Assessment 的评分领域逻辑：`application/evaluation/operator/authorization.go` 使用调用者的资源与动作，通过 Actor 校验有效 Operator、公司和受试者当前门店。不能用另一动作的范围替代本次动作范围。

| 路径 | 范围执行方式 | 保留的业务限制 |
|---|---|---|
| 完整测评与进度查询 | 分别匹配 read/list 与 read_progress/list_progress，范围进入共享查询入口 | 进度 DTO 不携带专业结果 |
| 报告及评分相关读取 | 按实际资源/动作校验，不能借用测评 read 权限 | 报告投影前后复核当前归属，范围变化不返回旧页面 |
| 重试及强制重试 | 分别校验 retry / force_retry 和当前受试者门店 | 原重试资格、状态、次数、幂等、事务及事件规则保留 |
| 统计与缓存 | 使用对应动作的公司/门店范围，按范围计算或验证缓存结果 | 未归属受试者不因全部门店范围而可见 |

多角色范围只在同一公司、资源和动作的匹配项之间合并。医生、参与者和监护人的独立入口继续使用各自业务身份与关系规则，不能通过后台 Operator 适配器推断医生权限。本次本地测试尚不证明生产转店后缓存、列表和历史报告已经完成交接。
...
| `DescriptorKey` | decision | 运行时选择哪份 Descriptor |
...
同一个产品通道未来可能包含不同机制；不同产品通道也可能复用相同的基础计分能力。因此 Provider 输入物化使用模型身份，descriptor 路由使用发布快照的 DecisionKind，都不使用产品导航分类。
...
RuntimeResolver 只从 `InputSnapshot.Model.DecisionKind` 构造 ModelRoute。DecisionKind 缺失时 fail closed，不回退 Assessment ModelRef，也不从 Kind/Algorithm 推导。
...
flowchart LR
    ...
    Key["DescriptorKey"]
    Family["Derived AlgorithmFamily"]

    Route --> Decision
    Decision --> Key
    Decision -.derive / validate.-> Family
```

路由规则是：

1. 读取发布快照冻结并进入 InputSnapshot 的 DecisionKind；
2. DecisionKind 必须非空且能映射到已知 AlgorithmFamily；
3. 直接以 DecisionKind 形成 DescriptorKey；
4. 缺失、未知、注册时 family 映射冲突或未注册时 validation failure，不使用默认值。

当前默认关系为：

| DecisionKind | 派生 AlgorithmFamily |
| --- | --- |
| `score_range` | `factor_scoring` |
| `pole_composition` / `trait_profile` / `nearest_pattern` / `dominant_factor` | `factor_classification` |
| `norm_lookup` | `factor_norm` |
| `ability_level` | `task_performance` |
...
Registry 只按完整 `DecisionKind` 查找。不存在格式级或 family 级 fallback；当前七种 DecisionKind 均显式注册并装配 Pipeline 三件套。
注册时会额外验证 descriptor.AlgorithmFamily 与 DecisionKind 的派生 family 一致。
...
| ModelRoute 推导 | InputSnapshot 缺少冻结 DecisionKind，或该值无法映射到已知机制 | validation failure |
| Descriptor 解析 | 没有注册对应 DecisionKind | validation failure |
...
目标上应让 CalculationInput 显式承载计算所需的中性输入，Calculator 不再从 context 获取业务 DTO。
具体重构边界见 [EV-R013](./90-设计问题与重构清单.md#ev-r013让-inputassembler-真正组装-calculationinput)，本文只记录事实和方向。

### 13.2 冻结路由已经收口

发布快照中的 DecisionKind 通过 InputSnapshot、ModelRoute、DescriptorKey 和 Outcome RuntimeIdentity 原样贯穿。
AlgorithmFamily 由 DecisionKind 在进程内派生并用于校验 descriptor；Registry 只做 DecisionKind 精确查找，不再存在 identity、format、family 或 Assessment ModelRef fallback。
