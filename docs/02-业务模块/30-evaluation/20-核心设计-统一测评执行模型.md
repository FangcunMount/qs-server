...
Registry 只按完整 `DecisionKind` 查找。不存在格式级或 family 级 fallback；当前七种 DecisionKind 均显式注册并装配 Pipeline 三件套。注册时会额外验证 descriptor.AlgorithmFamily 与 DecisionKind 的派生 family 一致。
...
发布快照中的 DecisionKind 通过 InputSnapshot、ModelRoute、DescriptorKey 和 Outcome RuntimeIdentity 原样贯穿。AlgorithmFamily 由 DecisionKind 在进程内派生并用于校验 descriptor；Registry 只做 DecisionKind 精确查找，不再存在 identity、format、family 或 Assessment ModelRef fallback。
