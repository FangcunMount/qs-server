...
EV-R001、EV-R003、EV-R004、EV-R005、EV-R007、EV-R008、EV-R009、EV-R012、EV-R013、EV-R014、EV-R017 已于 2026-07-23 完成技术验收；维护者确认对应实现已经部署，台账状态更新为“已关闭”。

当前未关闭工作收敛为四类后续治理：

1. **整改完成但待生产复验**：EV-R002、EV-R006、EV-R011 分别等待 7 天、24 小时和全量扫描 + 24 小时证据；
2. **生产窗口证据待回填**：EV-R010 等最近 7 天 Lease 预算证据；EV-R015 等 `legacy_incomplete` 连续 14 天归零后再移除兼容分支；
3. **运维观察仍需收敛**：EV-R016 保持 ACK/requeue 语义不变，running 且无 lease 的历史脏数据另开治理项；
...
| EV-R008 | P1 | Evaluation 仍对 RuntimeIdentity 和 ModelRoute 做兼容推导 | 已关闭（2026-07-23，技术验收通过且已部署） | 精确版本执行、新 family 准入 |
...
| EV-R008 | RuntimeResolver 只消费冻结 RuntimeIdentity，缺失/非法路由 fail-closed，禁止回退；维护者确认已部署 | 新 family 仍须先满足 descriptor 与发布能力门禁 |
...
- 新增只读 [`audit_evaluation_r002_unbound.sql`](../../../scripts/oneoff/audit_evaluation_r002_unbound.sql)，按状态盘点历史 model identity 不完整 Assessment，不执行自动修复。
...
- 单元测试：`TestEnsureUnboundAnswerSheetEndsWithoutCreatingAssessment`、`TestEnsureIndependentReplayReusesLegacyAssessmentWithoutSubmit`、`TestEnsureLegacyMissingBindingResolverFailsClosed`、`TestEnsureLegacyWithoutLiveBindingIsUnclassified`
...
- `AuditOnce` 扫描 submitted/evaluated/failed，并强制装配 Assessment、Run、Outcome、Projection、committed Outbox 全部只读依赖；
...
- 测试：`TestClassifyDriftMatrix`、`TestConsistencyReadModelReadsProjectionOutcomeLink`、`TestConsistencyReadModelReadsCommittedOutboxReferences`

**剩余生产门槛**

- 部署后成功完成一次全量循环扫描，reader error 与未分类错误为零；
- mismatch 均能从 `sum(increase(qs_evaluation_consistency_mismatch_total[24h])) by (kind)`、`sum(increase(qs_evaluation_consistency_disposition_total[24h])) by (kind, disposition)` 和结构化日志定位；
