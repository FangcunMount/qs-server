...
3. **精确执行已收敛**：NormSubject 按 SubmittedAt 物化，InputSnapshotRef 为 `isn:v2`，运行时只消费冻结的 family+decision+DefinitionV2；
...
| EV-R012 | P1 | Outcome v0/v1 与缺失 ReportInput 的退出路径未完成 | 已实现待验收（Outcome schema 2 / ReportInput schema 3 only） | 历史查询、Interpretation 输入 |
