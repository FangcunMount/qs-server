...
| EV-R010 | P1 | EvaluationRun 默认 2 分钟 Lease 没有通用 heartbeat | 证据已回填（决策：保留 2min Lease，不引入 heartbeat） | 长时认知任务、重复计算 |
...
- AgeMonths 按 `Assessment.SubmittedAt` 计算；
- Calculation 侧 parametric Band 已与 direct lookup 共用严格人口学匹配（见 `MC-R002`）。

**剩余缺口**

- 缺失必需人口学时尚未形成稳定的可重试 FailureKind（当前多为 lookup miss / 缺少派生分）；
- NormSubject 尚未进入独立可验证 InputSnapshotRef；
- Import → Publish → Outcome 端到端 golden 仍待补。
