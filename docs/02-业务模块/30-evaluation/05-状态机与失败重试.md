...
4. `assessment.evaluated` 已与结果提交可靠衔接，可以触发 Interpretation。
...
当前代码在报告失败时仍可能调用 `Assessment.MarkAsFailed` 并清除总分、风险与摘要；这是待迁移行为，不能作为目标语义继续扩展。
...
- Evaluation 完成：`assessment.evaluated`。
