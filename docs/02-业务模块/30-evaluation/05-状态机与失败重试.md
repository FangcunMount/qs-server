...
结果可靠提交失败按当前 EvaluationRun 的执行失败处理：`EvaluationCommitter` 只在隔离副本上准备 `Assessment=evaluated` 和 `Run=succeeded`，只有 Outcome、score projection、Assessment、Run 与 outbox 全部提交成功后才发布终态。任一提交节点失败时，调用者持有的对象仍保持 `Assessment=submitted + Run=running`，随后由执行服务在同一失败事务中写入 `Assessment=failed + Run=failed(retryable, internal) + evaluation.failed`。

失败事务也在 Assessment/Run 隔离副本上准备终态，事务失败不污染调用者对象。当最新 Run 是 retryable failed，或 running Run 的 lease 已过期时，同一 `evaluation.requested` 消息重投会先 claim 下一 attempt/过期 attempt，再无新事件地恢复 Assessment 供当前 Worker 执行。

恢复时先通过后台恢复用例把 Assessment 从 `failed` 推回 `submitted`，再为同一 Assessment 创建 `attempt + 1` 的新 EvaluationRun；失败 attempt 保留审计，不复用、不覆盖。Assessment、Run、Outcome 与 `evaluation.outcome.committed` 使用同一个 `evaluated_at` 时间值。
