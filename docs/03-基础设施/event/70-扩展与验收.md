...
- 禁止在 publisher 回调中承载业务投影；独立失败域必须使用显式 consumer/channel。

### Step 9：同步文档和观测

- 更新[Event 契约与演进](./20-事件契约与演进.md)事件矩阵/附加消费者矩阵。
...
8. 更新[Signal 一次性信令](./60-Signal一次性信令.md)拓扑和清单同步测试。
...
- decode-invalid NACK；运输预算耗尽后进入 provider terminal handoff，recorder 成功后才保存死信。
- unknown ACK。
- handler error NACK。
- handled/duplicate ACK。
- automatic retry 暂停时 hold 成功 ACK；hold 持久化失败 NACK。
- NSQ handoff publish、handoff consumer 或 MySQL recorder 失败会 requeue，并能与业务 channel 故障区分。
...
go test -count=1 -race ./internal/pkg/eventing/runtime \
  ./internal/apiserver/eventing/subsystem
...
带 `//go:build integration` 的 NSQ + MySQL terminal handoff 测试不包含在上述命令中，需要 `MESSAGING_INTEGRATION=1` 和隔离服务。2026-09-25 的 M5-04 候选已在本地真实 NSQ／一次性 MySQL 运行，证明新物理投递失败能单独落库；进入目标环境验收前仍须按发布提交单独执行并保留 broker、MySQL 与 exact-SHA 证据。

最终 review 还应扫描：业务模块不得构造 relay/Store，独立投影不得绕过显式 consumer/channel，terminal handoff 与人工 replay 的证据边界不得被简化。
