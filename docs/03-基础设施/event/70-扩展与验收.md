...
带 `//go:build integration` 的 NSQ + MySQL terminal handoff 测试不包含在上述命令中，需要 `MESSAGING_INTEGRATION=1` 和隔离服务。本轮未运行该测试；进入目标环境验收前必须单独执行并保留 broker、MySQL 与 exact-SHA 证据。
