...
从工程化状态看，当前 event 的“投递机制”已基本完整，但“契约有效视图”和“运行时所有权”尚未完全收口。Mongo relay 已经以 `MongoDomainEventRelay` 身份由 process 启动，但仍由 Survey 构造；`outboxruntime.Policy` 已表达 immediate/priority，但 priority 仍存在多个装配入口。后续目标见 [10-事件工程化设计.md](10-事件工程化设计.md)。
...
| 当前是否有完整 Effective Event Registry | 没有；当前事实分布在 YAML、代码策略和契约矩阵 | `eventcatalog`、`outboxruntime`、`outboxpriority` |
| Mongo relay 是否已完全脱离 Survey | 没有；命名与启动责任已提升，构造权仍在 Survey | `container/modules/survey`、`container/compose_host.go` |
...
当前 relay 已重命名为 `MongoDomainEventRelay`，Container 持有私有引用，process 也以 Mongo domain-event relay 身份启动它。但它仍由 Survey/AnswerSheet 组装，再由 Container 从 Survey Module 复制引用；因此完成的是“运行时命名和启动责任提升”，不是“构造权和 store 所有权完全提升”。

另外，AnswerSheet Repository 内部代理 Mongo Outbox store，Interpretation 又构造了指向同一 collection 的 store。它们当前可以共同工作，但说明该 runtime 尚未成为组合根所有的单一 profile。

这是当前必须保持的运行时事实：在 EventSubsystem 接管之前，不能仅从 Survey 删除 relay 字段而没有新的 Mongo domain-event runtime 接管。
...
当前 event 系统已有清晰主链，但完整运行行为分布在多个位置：
...
| immediate event types | `container/internal/outboxruntime/policy.go` |
| P0/P1/P2 | `container/internal/outboxruntime/policy.go`、`internal/pkg/outboxpriority`、Mongo/MySQL Store 装配 |
...
这些位置不都适合合并成一个文件，但新增事件或修改可靠性策略时，必须按下一节的清单同步检查。
...
- Mongo relay 已以 `MongoDomainEventRelay` 身份由 process 启动。
- immediate 和 priority 已开始收口到显式 `outboxruntime.Policy`。

### 16.2 部分完成

- Mongo relay 的命名与启动责任已提升，但构造权、store 所有权和业务模块导出还未收口。
- `outboxruntime.Policy` 已包含 immediate 和 priority，但 Store claim 与 ready-index bucket 仍可从其他全局默认值派生。
- event status 已能统计 catalog 和 Outbox，但尚不能输出 owner、profile、immediate、priority、settlement 和 idempotency 的有效视图。

### 16.3 待补证据

- 验证所有外部通知副作用在重复消费下是否符合业务预期。
- 确认 modelcatalog hot-rank 投影是否应该成为 `answersheet.submitted` 发布的前置条件；当前 Redis 错误会阻止该 durable event 发布。
...
- 建立 `EventSpec + EffectiveRegistry`，将 owner、Outbox profile、immediate、priority、settlement 和 idempotency reference 变成可校验的有效契约。
- 建立 apiserver `EventSubsystem`，将 Mongo/MySQL Outbox runtime、publisher、status 和 lifecycle 从业务模块收口到组合根。
- 将 Mongo relay 的构造权完全移出 Survey，而不只是继续重命名字段。
- 去除可变 P0/P1/bucket 全局切片，让 Store claim 和 ready-index claim 共用同一有效策略。
- 将 unknown ACK 与普通 handler success ACK 分开观测，但不改变当前 ACK/NACK 语义。
- 为确实需要跨 handler 通用去重的外部副作用评估 inbox/processed-event record。
- 如果出现 payload 不兼容升级需求，再引入明确的 schema version 和兼容策略。
- 根据实际重试故障模式设计 DLQ/replay 运维闭环，不在没有操作流程时只增加一个名为 DLQ 的状态。

完整目标结构、风险和分批路线见 [10-事件工程化设计.md](10-事件工程化设计.md)。
