...
行为旅程不再通过 Event/Outbox/MQ 投影。`behavior_journey_scan` 直接从入口解析日志、接入日志、答卷、测评和报告事实表轮询重建 `behavior_footprint`、episode 与日聚合；`configs/events.yaml`、worker handler 与 Outbox 中均没有 `footprint.*` 事件。主看护关系转移也不再产生实时行为事件。
...
| Assessment Outbox | MySQL | Evaluation requested/outcome committed/failed、care relationship transferred | `assessment-mysql-outbox` |
...
当前 immediate event type 是代码策略，不是 `events.yaml` 字段。
...
尽管这个 relay 由 Survey/AnswerSheet 组装并以 `SubmittedEventRelay` 字段暴露，它的实际责任是“排空全局 Mongo domain-event Outbox”，不只是发布 `answersheet.submitted`。

这是当前必须保持的运行时事实：如果未来拆分或重命名 Survey 组件，不能在没有新的 Mongo domain-event relay 接管时删除它。
...
| Behavior projection | event ID 随投影请求传递，由投影端保护重复 |
...
`configs/signals.yaml` 当前没有被进程加载，它是说明性清单，不是像 `events.yaml` 一样的可执行契约。实际是否启用 Signal、Redis prefix 和 buffer size，由 apiserver、worker 和 collection-server 各自的 signaling options 决定。
...
| immediate event types | `application/eventing/immediate.go` |
| P0/P1/P2 | `internal/pkg/outboxpriority` |
| 行为旅程投影 | apiserver `behavior_journey_scan` 配置 + `application/statistics/behavior_scan.go` |
...
- 按每个 event type 维护 producer、store、handler、幂等保障和错误安置矩阵。
- 验证所有外部通知副作用在重复消费下是否符合业务预期。
- 将 Signal 名称、发布方和订阅方与当前代码自动校验。

### 16.3 规划改造

以下是值得讨论的改造方向，不是当前能力：

- 将 Mongo relay 从 Survey/AnswerSheet 所有权提升为 platform/eventing 级的 `MongoDomainEventRelay`。
- 将 immediate、priority 和 staging override 建模为显式 runtime policy，降低多事实源造成的理解成本。
- 将 `signals.yaml` 升级为可加载、可校验的契约，或明确改为由代码生成的说明文档。
