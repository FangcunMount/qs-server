...
| 输入结果 | MQ 动作 | 为什么 |
| --- | --- | --- |
| envelope/event type 无法解析 | NACK | 先保留有界重投与死信证据，不能静默丢 poison payload |
...
当前 MQ transport delivery 默认/硬上限为 8；30 秒起步、5 分钟封顶、20% jitter。耗尽后写 `event_delivery_dead_letter(manual_required)`，停止该 channel 的无限重投。
...
- decode-invalid 可能来自临时 metadata 缺失、非标准 producer 或 schema 破坏；先用有界 NACK 保留运输证据，耗尽后落 dead letter 供人工判断。
...
| MQ transport delivery attempt | 某 channel 再调用一次 handler | 8 | `event_delivery_dead_letter` |

不能说“总共重试 71 次”。四个 attempt 的 owner、动作、退避、幂等边界和终态完全不同。

一个业务 run 失败后可能产生 retry event；该 event 的 Outbox 可能 publish 重试；到 MQ 后 handler 又可能 transport redelivery。如果 automatic business retry 被暂停，worker 将 retry event 写入 durable hold 后 ACK，让 hold replayer 接管，而不是继续消耗 transport budget。
...
- 每条 channel 各自消耗 transport attempt，并可能形成自己的 dead letter；
...
安全的 replay 需要：组织范围、capability、二次确认、稳定 request ID、expected attempt/disposition、状态冲突检查、terminal audit。先修复 producer/handler/依赖，再重放；否则 replay 只是更快地再次失败。
...
| D | bounded delivery + dead letter | poison/永久错误不再无限重投 | 需要持久死信、诊断和人工恢复 |
...
- unknown/decode dead letter 长期出现，说明契约演进策略不足；
