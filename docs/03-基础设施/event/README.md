...
- 已持久化的 transport dead letter 由高风险治理动作 `events.replay_delivery` 做组织范围、带状态冲突检查的一次性人工重放；发布或完成结果未知时保留占用并列为待核对候选，不自动再次发送。旧 `task.opened` 会直接触发无逐收件回执的外部提醒，通用重放在整批预检和逐项占用时均拒绝该事件，交由人工核对任务与收件状态。
