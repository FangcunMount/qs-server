...
它横切 [基础设施统一模型](../04-统一模型与推理方法.md) 的每一层：日志还原分支，指标描述窗口趋势，runtime snapshot 说明当前装配，durable state/audit 决定最终事实。观测层缩小搜索范围，但不接管业务裁决。

建议先用本页掌握当前指标、端点和诊断链，再阅读 [核心设计决策与替代方案](./15-核心设计决策与替代方案.md)，理解为什么要分开日志、指标、快照、持久证据以及 liveness/readiness/deep check。
...
| Runtime snapshot | 某进程此刻如何装配、Redis family/韧性能力是否降级 | `/readyz`、`/serve-readyz`、`/governance/redis`、`/governance/resilience` | DB/MQ 全链健康、过去发生过什么 |
...
| Reliable submit | `qs_collection_answersheet_submit_total`、`qs_collection_answersheet_submit_stage_duration_seconds`、`qs_collection_answersheet_submit_coalescer_total`、`qs_collection_answersheet_submit_coalescer_wait_seconds`、`qs_collection_answersheet_submit_coalescer_redis_seconds`、`qs_apiserver_answersheet_durable_submit_total`、`qs_apiserver_answersheet_durable_operation_total` |
...
collection CD/LB 与周期 runner 使用 `/serve-readyz`；可靠提交与 K6 前置检查使用
严格 `/readyz`。前者避免运行期 Redis 降级导致实例整体摘流，后者保证压测只在
依赖完整 ready 时验收。
...
3. 区分 cache miss 增加、rate limiter degraded-open、SubmitCoalescer fallback 与数据库真实错误。
