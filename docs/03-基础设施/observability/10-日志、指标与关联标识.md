...
Observability 层解决的不是“有没有日志和 Prometheus”，而是当一次请求变慢、一个事件不再推进或一个治理动作执行失败时，能否用多种独立证据还原发生了什么、影响了谁、事实停在哪一层，以及下一步操作是否安全。探针、governance 与 durable evidence 的唯一维护入口见[健康探针、治理与持久证据](./20-健康探针、治理与持久证据.md)；本页保留日志、指标和关联诊断模型。
...
collection 和 worker component 支持 Docker DNS fan-out：聚合响应保留兼容
...
数据互相覆盖。期望实例数、治理 DNS 别名和发现结果属于本次 effective config/部署证据；
CD 与巡检应从 apiserver 容器反向验证 DNS 地址、治理端点和唯一 `instance_id` 数。
应用侧隔离不会删除容器网络控制面中的陈旧记录。
