...
3. **Redis 建议成对阅读**：先看 [07-Redis代码总览（源码审计版）](./07-Redis代码总览（源码审计版）.md) 理解“当前代码是什么”，再看 [08-Redis分层重构设计](./08-Redis分层重构设计.md) 理解“后续应该长成什么样”；旧文 [06-Redis使用情况.md](./06-Redis使用情况.md) 仍可作补充，但以源码审计版为准。  
...
| **03-基础设施（本文）** | 事件拓扑、存储分布、缓存限流、Redis、IAM、YAML→Options 等**机制与锚点**；Redis 现状与目标分层分别见 [07](./07-Redis代码总览（源码审计版）.md) / [08](./08-Redis分层重构设计.md) |
...
4. [07-Redis代码总览（源码审计版）.md](./07-Redis代码总览（源码审计版）.md) — Redis 在当前源码里的**真实使用面、family、TTL、key 与进程边界**  
5. [08-Redis分层重构设计.md](./08-Redis分层重构设计.md) — Redis 基础层 / Cache / Distributed Lock / Governance 的**目标分层模型**  
6. [09-Redis跨仓重构路线.md](./09-Redis跨仓重构路线.md) — 带着设计稿回看 `qs-server` / `component-base` / `iam-contracts` 后的**跨仓落地路线**  
7. [06-Redis使用情况.md](./06-Redis使用情况.md) — 历史汇总文档，阅读时以 07 审计版为准  
