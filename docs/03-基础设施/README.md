...
3. **Redis 细节单独成文**：项目内 Redis 使用现状、key family 与运行时边界以 [06-Redis使用情况.md](./06-Redis使用情况.md) 为准；统计模块内查询口径仍可对照 [06-statistics.md](../02-业务模块/06-statistics.md)。  
...
| **03-基础设施（本文）** | 事件拓扑、存储分布、缓存限流、Redis、IAM、YAML→Options 等**机制与锚点**；Redis 总表见 [06-Redis使用情况.md](./06-Redis使用情况.md) |
...
4. [06-Redis使用情况.md](./06-Redis使用情况.md) — Redis 在缓存、统计、锁、微信 SDK、工具链中的**当前用法总表**  
5. [04-IAM与认证.md](./04-IAM与认证.md) — 与 [01-运行时/05-IAM认证与身份链路.md](../01-运行时/05-IAM认证与身份链路.md) 互补  
6. [05-配置体系.md](./05-配置体系.md) — `configs/*.yaml`、`Options`、**`events.yaml` 的特殊性**
