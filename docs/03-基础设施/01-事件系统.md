...
| 契约模型 | `internal/pkg/eventcatalog` 是 `events.yaml` 的代码真值层；`internal/pkg/eventruntime` 是发布/订阅运行时；`eventconfig` 只保留兼容 facade |
...
- 兼容 facade：[`internal/pkg/eventconfig/`](../../internal/pkg/eventconfig/)
