...
本文档按 [CONTRIBUTING-DOCS.md](../CONTRIBUTING-DOCS.md) 中 **Why / What / Where / Verify** 组织；讲解维度说明见该文，正文不重复贴标签。与业务事件语义、各模块发布点对照见 [02-业务模块](../02-业务模块/) 及 [05-专题分析/02-异步评估链路](../05-专题分析/02-异步评估链路：从答卷提交到报告生成.md)。
...
事件系统**不是**独立进程，而是由 **`qs-apiserver`（发布）+ `configs/events.yaml`（拓扑）+ `qs-worker`（订阅与分发）** 组成的基础设施：领域产生事件 → 按配置路由到 Topic → MQ → worker 按 `event_type` 调 handler。消息抽象支持 **NSQ / RabbitMQ**，仓库默认路径以 **NSQ** 为主。
...
| **关联** | 异步链路叙事 [05-专题/02](../05-专题分析/02-异步评估链路：从答卷提交到报告生成.md)；各模块发布/消费 [02-业务模块](../02-业务模块/) |
...
- **MQ 实现选型（连接真实 Broker）**：[`internal/pkg/options/messaging_options.go`](../../internal/pkg/options/messaging_options.go)（`MessagingOptions.NewPublisher`）；worker 侧 [`createSubscriber`](../../internal/worker/server.go)（`messaging.Subscriber`）。
...
| Broker 选型与连接参数 | [`MessagingOptions`](../../internal/pkg/options/messaging_options.go)、worker `MessagingConfig` | `enabled`、`provider`、`nsq-addr`、`rabbitmq-url` 等（与 [05-配置体系](./05-配置体系.md) 对照） |
...
`apiserver` 创建容器时按环境与 `messaging` 初始化结果选择（见 [container.go](../../internal/apiserver/container/container.go) 相关逻辑）。**底层 Broker** 由 [`MessagingOptions`](../../internal/pkg/options/messaging_options.go) 与 worker [`createSubscriber`](../../internal/worker/server.go) 选择 **component-base** 的 NSQ / RabbitMQ 实现；默认运维与 Topic 预创建多围绕 **NSQ**。
...
- **并非所有 handler 都已「强业务实现」**；部分仍为日志或 TODO（与 [02](../02-业务模块/) 各模块边界一致）。  
- 已定义事件（如 `report.exported`、`assessment.failed`）**不等于**主链路同等权重；见各模块文与 `events.yaml` consumers。  
