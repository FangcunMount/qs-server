...
事件系统**不是**独立进程，而是由 **`qs-apiserver`（发布 / outbox relay）+ `configs/events.yaml`（拓扑）+ `qs-worker`（订阅与分发）** 组成的基础设施：领域产生事件 → 持久化为 outbox 或直接 publish → 按配置路由到 Topic → MQ → worker 按 `event_type` 调 handler。消息抽象支持 **NSQ / RabbitMQ**，仓库默认路径以 **NSQ** 为主。本文只覆盖 **QS 业务事件总线**；`IAM -> QS` 的 **`iam.authz.version`** 控制面通知不在本文范围内。
...
本轮之后，核心事件的可靠性边界已经分成三类：

- `answersheet.submitted`：**Mongo durable idempotency + Mongo outbox**
- `assessment.submitted`、`assessment.failed`：**MySQL outbox**
- `assessment.interpreted`、`report.generated`：**Mongo 报告落库 + Mongo outbox**

其余事件仍保持当前 best-effort direct publish。
...
sequenceDiagram
    ...
    participant C as apiserver Container / Relay
    participant P as RoutingPublisher
    participant Y as Registry / yaml
    participant MQ as MQ

    Dom->>C: Publish(event) 或写入 outbox
    C->>P: Publish(ctx, evt)
...
**投递语义**：至少一次；其中核心评估主链已经不是单一可靠性等级：

- `answersheet.submitted`：**答卷写库 + 幂等记录 + Mongo outbox 同事务**
- `assessment.submitted` / `assessment.failed`：**Assessment MySQL 状态更新 + MySQL outbox 同事务**
- `assessment.interpreted` / `report.generated`：**报告保存成功 + Mongo outbox 同事务**
...
- **主业务状态**先落库再出站；但不是所有事件都 outbox 化。当前只有：
  - `answersheet.submitted`
  - `assessment.submitted`
  - `assessment.failed`
  - `assessment.interpreted`
  - `report.generated`
  进入可靠出站链路。其余事件仍保持保存后 best-effort publish。  
...
- 旧的计划级生命周期事件与报告导出事件已经从运行时契约删除；不要再把它们讲成当前系统的一部分。  
