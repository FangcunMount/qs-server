...
**投递语义**：至少一次；其中 `answersheet.submitted` 当前已经改成 **答卷写库 + 幂等记录 + outbox 同事务**，再由 apiserver relay 补发。业务幂等仍需下游自建，例如：
...
- **主业务状态**先落库再出站；`answersheet.submitted` 通过 outbox relay 发布，其它事件当前大多仍是保存后 best-effort publish。  
