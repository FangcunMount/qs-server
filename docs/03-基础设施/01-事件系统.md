...
| YAML schema | 只有 `version`、`topics`、`events` 三层；`events.*.delivery` 声明交付等级，`events.*.handler` 用于 worker 侧处理器绑定 |
...
| 可靠性分层 | `events.*.delivery` 只有 `best_effort` / `durable_outbox` 两类；outbox store 会拒绝 staging `best_effort` 事件 |
...
1. 有哪些 Topic
2. 每个事件属于哪个 Topic
3. 每个事件的交付等级：`best_effort` 或 `durable_outbox`
4. 每个事件在 worker 侧绑定哪个 handler 名
...
| 事件 | delivery | 说明 | handler |
| ---- | -------- | ---- | ------- |
| `questionnaire.changed` | `best_effort` | 问卷生命周期变化 | `questionnaire_changed_handler` |
| `scale.changed` | `best_effort` | 量表生命周期变化 | `scale_changed_handler` |

#### `qs.evaluation.lifecycle`

| 事件 | delivery | 说明 | handler |
| ---- | -------- | ---- | ------- |
| `answersheet.submitted` | `durable_outbox` | 答卷已提交 | `answersheet_submitted_handler` |
| `assessment.submitted` | `durable_outbox` | 测评已提交 | `assessment_submitted_handler` |
| `assessment.interpreted` | `durable_outbox` | 测评已解读 | `assessment_interpreted_handler` |
| `assessment.failed` | `durable_outbox` | 测评失败 | `assessment_failed_handler` |
| `report.generated` | `durable_outbox` | 报告已生成 | `report_generated_handler` |

#### `qs.analytics.behavior`

| 事件 | delivery | 说明 | handler |
| ---- | -------- | ---- | ------- |
| `footprint.entry_opened` | `durable_outbox` | 用户成功打开入口 | `behavior_projector_handler` |
| `footprint.intake_confirmed` | `durable_outbox` | 用户完成一次接入 | `behavior_projector_handler` |
| `footprint.testee_profile_created` | `durable_outbox` | 新建受试者档案 | `behavior_projector_handler` |
| `footprint.care_relationship_established` | `durable_outbox` | 建立看护/服务关系 | `behavior_projector_handler` |
| `footprint.care_relationship_transferred` | `durable_outbox` | 转移看护/服务关系 | `behavior_projector_handler` |
| `footprint.answersheet_submitted` | `durable_outbox` | 提交答卷 | `behavior_projector_handler` |
| `footprint.assessment_created` | `durable_outbox` | 形成测评 | `behavior_projector_handler` |
| `footprint.report_generated` | `durable_outbox` | 产出报告 | `behavior_projector_handler` |

#### `qs.plan.task`

| 事件 | delivery | 说明 | handler |
| ---- | -------- | ---- | ------- |
| `task.opened` | `best_effort` | 任务已开放 | `task_opened_handler` |
| `task.completed` | `best_effort` | 任务已完成 | `task_completed_handler` |
| `task.expired` | `best_effort` | 任务已过期 | `task_expired_handler` |
| `task.canceled` | `best_effort` | 任务已取消 | `task_canceled_handler` |
...
2. 通过显式 `handlers.NewRegistry()` 组装 handler factory catalog
3. 按 topic 生成订阅列表
...
当前事件系统不是“全量统一 outbox”，而是按 `events.yaml` 中的 `delivery` 合同分层。`durable_outbox` 表示该事件必须先进入 MySQL/Mongo outbox，再由 relay 发布；`best_effort` 表示持久化后直接发布，发布失败只按当前 publisher 降级策略处理。`outboxcore.BuildRecords` 会在 resolver 能提供 delivery class 时拒绝把 `best_effort` 事件写入 outbox。
