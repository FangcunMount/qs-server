...
- worker 有双重 POSIX signal listener；版本化 production logging 已收敛为 `warn`/JSON，但部署 effective override 与 Debug 级原始 flag 日志仍需单独验收。

这些是代码事实，不表示接受为终局设计；具体证据与改造方向分别记录在 Security、Observability 和 Runtime 文档。

## 7. 阅读导航

- 跨模块概念、失败窗口和方案推导：[统一模型与推理方法](./04-统一模型与推理方法.md)
...
- 事务、repository 与存储所有权：[Data Access](./data-access/README.md)
- schema、dirty、one-off 与恢复：[Migration & Recovery](./migration-recovery/README.md)
- 用户/服务身份、组织和资源授权：[Security](./security/README.md)
- 指标、状态、审计和排障：[Observability](./observability/README.md)
- 三进程装配和关闭：[Runtime](./runtime/README.md)
- 配置、Secret、镜像、CD 和网络：[Config & Deployment](./config-deployment/README.md)
