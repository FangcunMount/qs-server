...
flowchart LR
    ...
    App --> Guard["Redis SubmitCoalescer<br/>建议性租约"]
...
| SubmitCoalescer | 同一提交意图的跨实例 owner 与 contender 有界等待 | Redis 可跨 collection 实例 | owner durable submit；contender 等待后 durable readback；Redis 故障 degraded-open |
...
3. [可靠提交：SubmitCoalescer 与幂等](./30-可靠提交-跨实例合并与幂等.md)：回答重复请求、409、202 与 Mongo 最终真相。
...
2. 为什么 SubmitCoalescer 可以失效，而 Mongo 唯一键不能缺失？
