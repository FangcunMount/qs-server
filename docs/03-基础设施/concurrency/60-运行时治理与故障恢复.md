...
版本化 production YAML 的 action flag 全部关闭；目标环境是否采用该 effective config 仍需部署证据。动态限流和 leader 让权具备代码路径，不等于已开放生产操作。没有持久业务队列或进程内 SubmitQueue，旧 queue control 协议已经删除。
...
`internal/pkg/resilience/control` 只定义 transport-neutral contract，不实现具体 RateLimit、Backpressure 或 Lease。真正的数据面仍归进程自己的 subsystem。

## 3. 三个进程的职责

| 进程 | control 行为 | data plane |
| --- | --- | --- |
| collection-server | 首次 control store 可用性确认、每秒 rate state reconcile、心跳 | Redis/local budgets、HTTP/gRPC Gates、SubmitCoalescer/LockLease |
...
生产 apiserver 对 collection governance 使用 `discovery=dns`，解析稳定别名
`qs-collection-server` 的全部 IPv4，并发读取最多 16 个 target。当前
`minimum_instances=2`：成功一个但不足两个时组件仍 `available=true`，同时
`partial=true`；全部失败或 DNS 失败才是 unavailable。兼容 `snapshot` 取成功
实例中 `instance_id` 字典序最小者，新的 backpressure/capability/cache
投影则遍历 `instances`，不能再用兼容快照代表整个组件。worker 本轮仍为
`discovery=single`。
...
- 首次同步确认 control state store 与 ops Redis family 可用；
- 成功后 `ControlSynchronized=true`；
- `/readyz` 在同步前返回 503 `synchronizing`。
- `/serve-readyz` 同样在同步前返回 503；首次同步后即使 Redis family degraded，
  也返回 200 `degraded` 并显式标出 `dependency_ready=false`。
...
| `已实现` | collection submit 的 Redis rate degraded-open 使用每实例保守本地预算；恢复后直接回到分布式决策。 |
| `已实现` | `run-submit-redis-degraded.sh` 自动验证 serving/strict readiness、逐实例 fallback 指标和 429 `Retry-After`；Redis 隔离与恢复仍由受控运维流程执行。 |
| `已实现` | MySQL action claim + Redis terminal fallback + recovery runner。 |
| `已实现但生产未启用` | tune rate limit、release leader。 |
| `规划改造` | 连续失败、半开与渐进恢复状态机若落地，应复用 control state/version/audit，而不是藏在单实例内存里。 |
