...
sequenceDiagram
    ...
    CS->>API: gRPC durable submit / readback
    API->>M: 查询 writer + key
    alt 已存在且指纹相同
...
`SubmissionService.AcceptDurably` 的总 deadline 生产基线为 2 秒。它先完成已发布问卷与答案结构 preflight，再进入 `SubmitCoalescer`。生产配置的 contender 等待预算为 500ms；代码会在请求预算接近耗尽时进一步缩短等待，为必须执行的 durable readback 保留时间。
...
存在、陈旧或内容异常，contender 仍会调用 `SaveAnswerSheet`；apiserver 的
early durable lookup 负责返回相同 AnswerSheet 或 fingerprint conflict。
...
但不改变 202、409 或 Mongo durable correctness。
