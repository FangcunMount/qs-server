...
| 当前变化 | 只读模型已进入 HTTP / gRPC / service-auth 运行时投影，不改变鉴权行为 |
...
| permission | capability middleware 消费 `authz.DecideCapability` / `DecideAnyCapability` | 用 `CapabilityDecision` 表达结果和原因 |
| service auth | apiserver / collection wrapper 复用 bearer metadata helper | 用 `ServiceIdentity` 表达服务身份 |
...
| Anti-Corruption Layer | IAM SDK wrapper、service auth helper | 业务层不直接理解 IAM SDK 细节 | service auth 只收口 bearer metadata，不改变 token 生命周期 |
| Value Object | `Principal`、`TenantScope`、`ServiceIdentity` | 让文档、测试和 runtime seam 使用共同语言 | 模型只读，不承接鉴权策略 |
...
| 运行时投影 | [`internal/pkg/securityprojection`](../../../internal/pkg/securityprojection) |
...
| apiserver authz snapshot | [`internal/apiserver/transport/rest/middleware/authz_snapshot_middleware.go`](../../../internal/apiserver/transport/rest/middleware/authz_snapshot_middleware.go) |
...
```bash
GOTOOLCHAIN=local /Users/yangshujie/.gvm/gos/go1.25.9/bin/go test ./internal/pkg/securityplane ./internal/pkg/securityprojection ./internal/pkg/grpc ./internal/apiserver/transport/rest/middleware ./internal/apiserver/transport/grpc
```
