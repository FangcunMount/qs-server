...
P0 不改运行时代码，只用 tests 证明 HTTP 和 gRPC 都能投影成同一类 `Principal` / `TenantScope` 视图。

## 设计取舍

| 选择 | 原因 | 代价 |
| ---- | ---- | ---- |
| `Principal` 是只读值对象 | 不让业务层依赖 transport context 细节 | 当前还需要 adapter 手工投影 |
...
```bash
GOTOOLCHAIN=local /Users/yangshujie/.gvm/gos/go1.25.9/bin/go test ./internal/pkg/securityplane ./internal/pkg/middleware ./internal/pkg/httpauth ./internal/pkg/grpc
```
