...
| capability 判断 | [`SnapshotSatisfiesCapability`](../../../internal/apiserver/application/authz/capability.go) |
| REST 保护 | `RequireCapabilityMiddleware` / `RequireAnyCapabilityMiddleware` |
| JWT roles | 只作为身份声明和投影输入，不作为 capability 真值 |
| P0 模型 | `securityplane.CapabilityDecision` 只读表达，不替换现有判断 |
...
sequenceDiagram
    ...
    C->>A: SnapshotSatisfiesCapability(snapshot, capability)
    A-->>C: allowed / denied
```
...
P0 的 `CapabilityDecision` 只是“可解释结果”的模型。后续阶段可以让 middleware 返回这个 decision，但当前不改 `SnapshotSatisfiesCapability`。
...
- middleware 返回 `ErrPermissionDenied` envelope，P0 不改错误格式。
...
| REST middleware | [`internal/apiserver/interface/restful/middleware/capability_middleware.go`](../../../internal/apiserver/interface/restful/middleware/capability_middleware.go) |
| route usage | [`internal/apiserver/transport/rest`](../../../internal/apiserver/transport/rest) |
| tests | [`internal/apiserver/interface/restful/middleware/capability_middleware_test.go`](../../../internal/apiserver/interface/restful/middleware/capability_middleware_test.go)、[`internal/pkg/securityplane/architecture_test.go`](../../../internal/pkg/securityplane/architecture_test.go) |

## Verify

```bash
GOTOOLCHAIN=local /Users/yangshujie/.gvm/gos/go1.25.9/bin/go test ./internal/apiserver/application/authz ./internal/apiserver/interface/restful/middleware ./internal/pkg/securityplane
```
