...
它表达 service ID 和 target audience，不是用户委托。
...
当前 JWT middleware 的 Debug 日志会记录 verifier result、raw claims 和 mapped claims；生产即使通常使用较高日志级别，这仍是需要明确治理的敏感日志风险。
安全做法是只记录 request ID、user/token ID 的不可逆或最小必要标识、issuer/audience 判定结果、错误分类和 Extra key names。
