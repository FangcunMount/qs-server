...
建议先核对当前 HTTP/gRPC 安全链，再阅读本页第 12 节，集中理解 授权与业务组织分工、snapshot、资源级授权、mTLS/service/delegation 和 fail-closed 的取舍。

## 1. 先看结论

- AuthN 只回答 token/证书是谁签发、是否有效；AuthZ 才回答该主体能否执行某个动作。
- IAM v5 使用单一授权空间；QS `org_id` 继续表示业务组织范围，由本地权威事实解析。
...
2. 本地/远程验证都要求 issuer、audience，并校验 clock skew、`sub`、`exp`、`user_id` 和过期时间。
...
- `iam.authz.version.v2` 消息推进 全局 authz version 水位并剔除旧快照。
- 拉取失败时 Authz middleware 返回 503，不继续扩大权限。

snapshot 包含 roles、带 `UNCONDITIONAL`/`OBJECT_CHECK_REQUIRED` 模式的 permissions、实际加载 authz version、app。
apiserver capability 映射把业务能力翻译为稳定的 IAM resource/action；通用 capability 只认可无条件权限，对象级 retry 在加载 Assessment 后调用 IAM AuthZ v4 Check。

AuthZ v4 的 snapshot 与 Check RPC 都携带服务身份。apiserver 使用 `qs-apiserver.svc`；只有该受信身份可以为 Assessment 提交从领域对象读取的 `object.origin_type`。
...
IAM v4 删除服务 JWT 和续期 helper。客户端通过 mTLS 调用，证书身份由服务端从经过验证的 TLS 连接提取。进程内任务使用 internal 来源，不伪装为网络证书身份。
...
- 用户 bearer token、cookie、session token 和 delegated subject 原文。
...
7. 若是 gRPC，检查 TLS/mTLS、用户 JWT、delegated subject 与 ACL，而不是只看其中一层。
...
### 12.2 为什么业务组织范围仍由 QS 解析

IAM v5 已移除授权分区；QS `org_id` 仍由本地 active operator membership 确定。直接信任 JWT org 虽然省去一次本地解析，却无法表达 membership 停用、多组织选择和两套 ID 生命周期差异。
...
snapshot `(resource, action)` 比 JWT role 更贴近动作权限，比每次在线 IAM 判定降低延迟和故障耦合；TTL + `iam.authz.version.v2` 同时提供最终刷新和快速失效。替代方案各有代价：
