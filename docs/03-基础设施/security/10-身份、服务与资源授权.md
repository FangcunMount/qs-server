...
snapshot 包含直接 roles、UNCONDITIONAL permissions、实际策略版本与 app。旧条件模式不作为已授权动作。retry / force_retry 使用有效快照中的实际动作权限，业务关系与状态由 QS 检查。

快照与 Check RPC 继续携带 qs-apiserver.svc 服务身份，但不提交对象属性；错误不得回退为本地角色授权。

`RequireCapabilityMiddleware` 只看 snapshot；snapshot 缺失、能力未知或 permission 不满足都 deny。仓库已删除角色名授权中间件，
任何动作级授权都不得信任 JWT role 或本地 `operators.roles` 代替 IAM 权限快照。
