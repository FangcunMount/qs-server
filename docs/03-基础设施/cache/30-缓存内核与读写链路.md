...
启用 singleflight 时必须注入 `loadguard.Coalescer`。nil miss 不写入普通 L1，也不会作为共享结果长期驻留。collection questionnaire/published-model/typology L1 使用该机制；published-model detail/list/options 各自使用最多 64 entry 的 FIFO bucket，hot rank 不缓存。
...
- `statistics.query`：机构级 Generation + Redis query payload，并保留最多 72 小时的进程内 last-known-good，用于 Redis 故障时返回可解释 stale；
...
| statistics | Query family `query:version:statistics:v2:org:<org_id>` + `query:data:statistics:v2:org:<org_id>:g:<generation>:<hash>` | publish 后切换机构 Generation；旧 data key 由 26h TTL 回收 |

payload schema 变化必须升级 key/version 或提供兼容 decoder。目录或 package 重构不能顺带改变 key 字节。

旧 `assessment:detail:<id>` 保存的是 domain aggregate，与新 participant outcome DTO 不兼容，因此本次使用 versioned 新 key，不尝试双读。旧 key 不由应用发布自动删除，在旧实例退出后按原 1h TTL 与 jitter 自然过期。

退役 assessment-list 后，DB Ops 仍以 `retired_assessment_list_data_candidate` 和 `retired_assessment_list_version_candidate` 只读统计旧前缀。DB4 data key 等待 TTL 自然过期；DB7 无 TTL version key 只有在旧 apiserver 全部退出并稳定至少 24 小时后，才允许通过独立审批的一次性操作清理，应用发布不自动删除。

statistics 的 generation 与 data 均由 Query Builder 构造；`statistics:v2` 隔离迁移前无 namespace 的 raw key。旧 data 最长 26 小时 TTL 后自然回收，旧 generation token 没有 TTL；生产清理同样必须等待新版本部署、旧实例退出与最大 data TTL 窗口结束，并单独审批。
