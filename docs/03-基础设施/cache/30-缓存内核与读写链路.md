...
核心约束是：一次普通 object/query read-through 开始时解析一次 Policy；该次 L2 get/load/negative/set 使用同一份解析结果。这不是所有 L1+L2 组合 adapter 的原子性保证：published-model 外层 `cacheEnabled()`、L2 read-through 与 L1 Set 的 `TTLProvider` 会多次 Resolve，并发 reload 可让同一顶层读取跨越两个快照。
...
启用 singleflight 时必须注入 `loadguard.Coalescer`。nil miss 不写入普通 L1，也不会作为共享结果长期驻留。collection questionnaire/published-model/typology L1 使用该机制；published-model detail/list/options 的代码默认容量和仓库 production policy 意图当前均为每个 FIFO bucket 64 entry，运行实例的 effective 值仍以其 Policy Source、SHA256 和 Registry snapshot 为准。hot rank 不缓存。
...
本章出现的 TTL 必须按四层读取：代码默认值、仓库 `configs/cache/*.prod.yaml` 的版本化配置意图、进程加载文件并叠加环境变量/显式 flag 后的 effective runtime，以及带 exact SHA、实例和观测时间的生产证据。前两层不能单独证明后两层；entry 的实际 expiry 由写入当时的 effective Policy 决定。
...
- `statistics.query`：机构级 Generation + Redis query payload；代码将进程内 last-known-good 固定保留最多 72 小时，用于 Redis 故障时返回可解释 stale；该 72 小时不是 production policy 文件中的可 reload TTL；
- version token/hotset 元数据走 `meta_hotset` family，query payload 走 `query_result` family。

历史 `evaluation.assessment_list` version-token/local-hot adapter 未接入实际读取路径，已从 capability、装配与 key builder 退役；assessment list 继续直接读取 read model。通用 query kernel 保留给 statistics 等真实消费者。
...
任何生产清理都必须先证明旧实例全部退出，再依据目标环境的历史 effective Policy、实际 Redis `PTTL`/无 TTL 盘点和观察窗口确定范围。当前仓库 production YAML 的 1h/26h 等值不是历史写入的安全上界，不能据此直接计算删除时间。无 TTL version/generation token 还必须单独审批并在 apply 时重新确认目标；清理逻辑不得进入应用启动或发布流程。

## 8. Negative cache

Negative cache 只适合“明确不存在且短期内重复查询”的对象读取。当前 questionnaire 与 testee adapter 显式声明 `CacheNegative: true`，仍需 effective Policy 同时启用才会写 sentinel；published-model 的 by-questionnaire、catalog-list 与 algorithms 三条 read-through 均显式声明 `CacheNegative: false`，Policy 打开也不会使它们写 negative sentinel。assessment/plan/query 同样不应因为共享 family 默认值就自动获得 negative-cache 语义。
...
一次 object read-through 在开头解析 Policy 后，get、load、negative 判断和写回继续使用同一份值。已有 Redis entry 不扫描、不重写，旧 expiry 保持不变。statistics L1 last-known-good 的 72 小时代码常量不属于 capability reload；published-model L1 在每次写入时通过 `TTLProvider` 取得当前 effective TTL，但 `TTLJitterRatio` 在 L1 构造时固定，reload 后仍使用启动时 jitter。
