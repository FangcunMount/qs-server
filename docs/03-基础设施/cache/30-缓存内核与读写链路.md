...
| assessment list | Meta family `query:version:assessment:list:v2:<user>` + Query family `query:assessment:list:v2:<user>:vN:<hash>` | bump version；旧 data key 由 TTL 回收 |
...
assessment list 的 version token 与 data 分别由 Meta Builder、Query Builder 构造；`v2` 同时隔离旧 token 与旧 v0 data，避免 namespace 迁移后从 0 起步时误命中旧缓存。旧 `cache:query:query:version:assessment:list:*` token 没有 TTL，只有在新版本完成部署、旧实例全部退出且超过 assessment-list 最大 data TTL 后，才允许单独审批清理。
