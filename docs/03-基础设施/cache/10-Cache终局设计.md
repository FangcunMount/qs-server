...
- `cache.governance` 管理 warmup、hotset；policy snapshot/reload 由 Effective Registry 与 system-governance action 管理；
- domain 不依赖 cache；
...
| TTL/policy 配置放哪里 | `cache.defaults` + `cache.capabilities.<module>.<capability>` | 已实现 |
...
type PolicyProvider interface {
    Resolve(cache.Capability) (cache.EffectiveCapability, bool)
    Version() uint64
    All() []cache.EffectiveCapability
}

type RegistrySnapshot struct {
    Version      uint64
    GeneratedAt  time.Time
    Capabilities []cache.EffectiveCapability
}
```
...
apiserver 动态 policy 已实现：现有 system-governance action 使用 expected-version CAS 发布完整 immutable snapshot；collection-server 保持启动时静态 snapshot。动态范围不包含 `enabled/family/layer/governance/report_status`。
...
| `wait_report.status_ttl_seconds` 与 `report_status.ttl_seconds` | 分别控制 DB fallback write 与 reporter write，可独立漂移 | 合并为 `report_status` operational-state capability 的唯一 TTL |
  ...
  capabilities:
    ...
    evaluation:
      ...
      assessment_list:
        ...
        ttl: 10m
    actor:
      testee: {enabled: true, ttl: 30m}
    plan:
      detail: {enabled: true, ttl: 12h}
    statistics:
      query: {enabled: true, ttl: 15m}
    report_status:
      ttl_seconds: 172800
...
- 仓库内配置已原子切换到新 schema，不提供 legacy/new 双读；
...
`report_status` 的 key、payload、状态优先级和 TTL 是 apiserver、collection-server、worker 的跨进程合同。三端可以各自装配，但 TTL 必须来自同一个 capability contract，并由生产配置合同测试验证一致。
...
scale/typology warmup 已通过 `modelcatalog.published_model` latest-by-code entry 闭环：source load、Set、Exists 和 read-back 必须全部成功才可报告 `ok`；Redis unavailable 或 capability disabled 报告 `skipped`。

latest-by-code key 固定为：

```text
<static namespace>:assessment_model:published:latest:<kind>:<lowercase-code>
```

upsert 后精确删除该 entry，并继续删除已登记的 catalog list 与 algorithms entry。
...
- 锁定 `report_status` 三进程 TTL/key/payload 合同；
...
- 建立 catalog、`internal/adapterkit` 与业务模块 adapter；
...
- scale/typology warmup 真正填充 latest-by-code entry；
- apiserver 七个普通 capability 的动态 policy snapshot/reload；
- Effective Registry status view 与 reload 指标。

后续独立处理：

- collection-server 动态 reload 与动态 enabled；
- invalidation 可靠事件；
...
13. 文档、配置和 status 使用 canonical capability 名称；metrics 暂时使用 Registry 中的 legacy label；
...
19. `report_status` 的 key/payload/TTL 跨 apiserver、collection-server、worker 合同测试通过；
...
- `cache.capabilities.<module>.<capability>` 是主缓存行为的配置 Source of Truth；
- adapter 只消费 typed effective registry，不直接读取 process Options；
- 同一行为只允许一个 effective 配置值，不保留 legacy schema 双读；
...
- IAM cache 接入深度；
- collection-server reload 与 capability dynamic enabled；
- capability policy reload 的集中配置分发后端；
