...
| 问题 | 终局答案 | 状态 |
| --- | --- | --- |
| 共享能力放哪里 | `internal/pkg/cache` | 规划改造 |
| 业务 capability 放哪里 | 各进程 `cache/catalog + adapter + bootstrap` | 规划改造 |
| Cache-Aside 放哪里 | application query service 或 infrastructure decorator | 已锁定 |
| Redis family/profile 放哪里 | `internal/pkg/redisruntime` + `redis_runtime` 配置 | 规划改造 |
| TTL/policy 配置放哪里 | `cache.capabilities.<id>` | 规划改造 |
| warmup/hotset/reload 放哪里 | `cache.governance` | 规划改造 |
| IAM/JWKS 私有缓存放哪里 | 保留在 IAM integration 配置，登记统一观测 | 已锁定 |
| 如何知道最终生效值 | Effective Cache Config Registry + status view | 规划改造 |
...
var (
    ...
    ErrCorrupt     = errors.New("cache payload corrupt")
)
```
...
- `ErrMiss` 是正常控制流；
- `ErrUnavailable` 默认允许 cache-aside 回源，但必须记录 degraded；
- `ErrCorrupt` 默认删除坏 entry 后回源，不把反序列化错误当作业务数据；
