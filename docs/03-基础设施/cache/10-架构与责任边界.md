...
collection-server 的 questionnaire/published-model/typology L1 缓存最终 BFF DTO，并通过 gRPC 回源 apiserver。`catalog.published_model` 只覆盖 detail/list/options，hot rank 保持实时回源。collection L1 与 apiserver cache 是两个独立 capability 和生命周期，不应包装成一个跨进程 `MultiLevelCache`。apiserver 的 `modelcatalog.published_model` 另外只为不可变 exact-version 运行快照提供进程内 L1，避免 Redis 命中后重复解码 `DefinitionV2`；可变目录桶仍是 L2，Active admission 仍直读 Mongo。
