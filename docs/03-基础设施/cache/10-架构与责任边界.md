...
Cache 当前不是一个全局 `CacheManager`，而是“共享技术内核 + 进程级 capability module + 业务侧 Cache-Aside + 独立 Redis runtime”。这种结构把两类问题分开：

- `internal/pkg/cache` 回答“怎样缓存”；
- apiserver/collection-server 的 cache module 回答“缓存什么、谁负责、何时失效、何时预热”。

因此，按 `survey / modelcatalog / evaluation / actor / plan / statistics` 划分 apiserver adapter 是必要的，
但共享 TTL、payload、singleflight、query version 等机制不应复制到业务包。

本章只建立当前结构与责任边界；事实、identity、穿透/击穿/雪崩、singleflight、失效、新鲜度和动态 reload 的推理主轴见 [从事实到派生读取：一致性与容量模型](./15-从事实到派生读取-一致性与容量模型.md)。

## 2. 为什么需要这层保护

qs-server 的读热点并不相同：已发布问卷和模型属于低频变更的静态目录；assessment、testee、plan 是按 ID 读取的对象；statistics 是计算成本较高的查询结果；collection-server 还要为小程序请求提供进程内目录 L1。
assessment list 当前直接读取 read model，不再声明一个未接入读路径的缓存 capability。
...
| [`internal/collection-server/cache`](../../../internal/collection-server/cache) | questionnaire/published-model/typology L1、signal watcher、startup warmup、静态 Registry、Start/Close | apiserver cache、动态 reload、业务数据持久化 |
...
当前 testee、plan、questionnaire 和 published model 采用这一形态。assessment 已拆为 application-owned 的 `assessment_access` 与
`assessment_detail`：Repository decorator 只负责写后同时失效两个 key，不再把 domain aggregate 伪装成 detail cache。
...
statistics 属于这一类。application 决定查询 identity、何时切换 generation、超时和 stale 语义；adapter 决定 key、payload 与 Redis 操作。
历史 `evaluation.assessment_list` adapter 因未接入读取路径而退役，不能据其旧文件或 Redis key 推断当前列表已缓存。

collection-server 的 questionnaire/published-model/typology L1 缓存最终 BFF DTO，并通过 gRPC 回源 apiserver。
`catalog.published_model` 只覆盖 detail/list/options，hot rank 保持实时回源。
collection L1 与 apiserver cache 是两个独立 capability 和生命周期，不应包装成一个跨进程 `MultiLevelCache`。
apiserver 的 `modelcatalog.published_model` 另外为不可变 exact-version 运行快照，
以及带全局 catalog version 的 list/by-questionnaire 读取提供进程内 L1，避免 Redis 命中后重复解码 `DefinitionV2`；
latest-by-code、algorithms 等非版本化可变目录桶仍是 L2，Active admission 仍直读 Mongo。

evaluation 同样按意图拆分：`assessment_access` 只缓存 testee/assessment ownership，`assessment_detail` 只缓存 participant outcome。
collection L1 key 同时包含 testee ID 与 assessment ID；apiserver L2 access payload 只包含 owner，detail payload 只接纳 `evaluated`。
User→Testee 的 transport 权限检查仍在每次请求执行。
...
| `query_result` | statistics 查询结果 | 是 |
...
| Query cache | miss 后按用例回源，可使用 loadguard | 按 statistics 用例的 timeout、stale 或 error 处理 |
| collection L1 | 直接调用 gRPC | 返回 gRPC error |
| warmup | 返回 `skipped` 或 `error`，不冒充成功 | 记录 target error，不污染在线读 |

失效失败不会回滚已经提交的业务事实。当前系统依赖写后删除或 version bump、best-effort signal 和 TTL 兜底收敛；可靠失效/outbox 尚未实现。
