...
| [`internal/collection-server/cache`](../../../internal/collection-server/cache) | questionnaire/published-model/typology L1、signal watcher、startup warmup、静态 Registry、Start/Close | apiserver L2、动态 reload、业务数据持久化 |
...
collection-server 的 questionnaire/published-model/typology L1 缓存最终 BFF DTO，并通过 gRPC 回源 apiserver。`catalog.published_model` 只覆盖 detail/list/options，hot rank 保持实时回源。collection L1 与 apiserver L2 是两个独立 capability 和生命周期，不应包装成一个跨进程 `MultiLevelCache`。
