...
Runtime Infrastructure 决定“一份代码最终以什么进程、什么依赖和什么故障语义运行”。它负责读取配置、构造基础设施、装配业务模块、启动 transport/consumer/scheduler，并在进程退出时按明确顺序停止接流量和释放资源。配置、Secret 和部署文件的唯一维护入口见[配置与部署](../config-deployment/README.md)；本页只在生命周期上下文中引用有效配置。
...
`configmask` 会按 password/secret/token/key 等字段名脱敏 Viper/Options 输出；但 `cliflag.PrintFlags`
在 Debug 级别逐个输出原始 flag value，没有调用 mask。版本化 worker production YAML 当前声明
`warn`/JSON、禁色且 `development=false`，但部署覆盖仍可能改变日志级别；一旦降到 Debug，通过 CLI
flag 传入的 secret 仍存在泄露风险。
...
- gRPC ACL 在启用时从配置文件读取；文件缺失、为空、含未知字段、非 default-deny 或与 runtime
  policy 不一致都会使 gRPC server 构造失败，不会回退为 allow。
...
版本化 production YAML 还声明 gRPC mTLS 与 default-deny 方法 ACL；启用 ACL 后的加载错误会阻止
transport 构造。这只关闭 workload/method 级准入，不会自动关闭业务 ownership：当前 public Intake
未证明调用者对 Profile 的代表权，collection Testee GET/PUT/care-context 也未接入已有
`testeeaccess.Authorizer`。这两项必须保持安全阻断，不能用 ACL 已装配替代。
...
worker 不启动业务 HTTP API。`metrics.enable` 时启动独立 HTTP server；版本化 production YAML
当前声明监听 `0.0.0.0:9092`，提供 metrics、health/readiness 与 governance。实际监听和网络暴露仍取决于
部署 effective config、容器端口映射与网络策略。
...
该顺序保证在途 HTTP/gRPC 完成业务提交时，Container 与 DatabaseManager 仍然可用。尚待补强的是在 shutdown callback 前显式 readiness=false/摘流量，以及逐阶段耗时指标；容器层为 apiserver 和 worker 提供 8 分 30 秒 stop grace，覆盖 Prompt 评测的 8 分钟 attempt lease 与 7 分钟 worker gRPC deadline。
