...
| --- | --- | --- |
| 同一 AnswerSheet 意图重复提交 | Mongo 唯一键 + fingerprint + transaction | SubmitGuard 降低重复工作 |
...
这也是“Redis SubmitGuard 不是正确性机制”的根本原因：只要 Redis 故障、过期或两个请求绕开同一 guard，正确性仍必须由持久化约束成立；否则系统只是把竞态隐藏在正常路径里。
