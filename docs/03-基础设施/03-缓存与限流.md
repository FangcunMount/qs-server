...
**本文回答**：`qs-server` 的保护层到底有哪些层、分别落在哪个进程、应先看哪些代码和配置；这篇文档只保留“保护层总览”和排障入口，不再重复 Redis family、缓存业务清单、预热细节等内容。
...
- 启动注入： [internal/apiserver/server.go](../../internal/apiserver/server.go)
...
| 场景 | 先看哪里 |
| ---- | -------- |
| 限流命中 | [internal/pkg/middleware/limit.go](../../internal/pkg/middleware/limit.go) + 进程 `routers.go` |
| collection 分布式限流 | [internal/collection-server/routers.go](../../internal/collection-server/routers.go) + `redisplane.NewDistributedLimiter` |
