...
**本文回答**：这篇文档解释 `qs-server` 的保护层到底分几层，限流、提交队列、缓存、统计读模型和背压分别落在哪个进程、如何配合，以及排查“慢、挤、打爆下游”这类问题时应先从哪些配置和代码入口入手。
...
| **不负责（摘要）** | 业务模块内 API 列表；Redis 使用总表与 key family 清单（见 [06-Redis使用情况](./06-Redis使用情况.md)） |
...
| 热点读缓存 | **apiserver** | `infra/cache`：问卷、量表、测评详情、计划、受试者、“我的测评列表”等；命名空间、TTL 抖动、可选压缩 | 读侧缓存 |
| 统计相关 Redis | **apiserver + worker** | apiserver 写 **`stats:query:*`**；worker 写 **`stats:daily:*` / `event:processed:*`**（总表见 [06-Redis使用情况](./06-Redis使用情况.md)） | 读模型 + 预聚合 |
...
**统计**：Redis 当前主要承担问卷 daily 中转、事件幂等、**查询结果短 TTL**；Redis 总表见 [06-Redis使用情况](./06-Redis使用情况.md)，统计查询口径与同步逻辑见 [06-statistics](../02-业务模块/06-statistics.md)。
