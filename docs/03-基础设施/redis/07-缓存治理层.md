...
| Interpretation Model Warmup | 新增 MBTI / BigFive 等解释模型时，通过新的 `WarmupKind`、`WarmupTarget`、list cache 和 family 绑定接入治理层 |
...
| Interpretation model warmup | 解释模型发布、归档或目录变化后预热模型详情、发布态模型列表和相关 Context cache |
...
flowchart TB
    ...
    subgraph Triggers["Triggers"]
        ...
        modelPublish["Interpretation Model Published / Changed"]
    ...
    subgraph WarmFuncs["Warm callbacks"]
        ...
        warmModel["WarmInterpretationModel"]
        warmModelList["WarmInterpretationModelList"]
        warmMBTIList["WarmMBTIModelList"]
    ...
    registry --> warmModel
    registry --> warmModelList
    registry --> warmMBTIList
...
- WarmQuestionnaire。
- WarmScaleList。
- ListPublishedInterpretationModelRefs。
- ListPublishedMBTIModelRefs。
- WarmInterpretationModel。
...
| `static.interpretation_model` | parse model scope -> `WarmInterpretationModel(ctx, modelRef)` |
| `static.interpretation_model_list` | `WarmInterpretationModelList(ctx)` |
| `static.mbti_model_list` | `WarmMBTIModelList(ctx)` |
...
### 5.3 新解释模型 WarmupKind 设计

新增解释模型时，建议优先把 warmup target 设计成稳定的 `kind + scope`。

示例：

```text
static.interpretation_model:scale:ADHD_PARENT:1.0.0
static.interpretation_model:mbti:MBTI_STANDARD:1.0.0
static.interpretation_model_list:published
static.mbti_model_list:published
```

scope 应使用 canonical 形式，不应在业务代码中手写 Redis key。
...
治理层只负责解析 target 并调用 warm callback。具体规则如何加载，应由具体模型 repository、ObjectCache 或 Provider Context cache 承接。
...
4. 如果 `ListPublishedInterpretationModelRefs != nil` -> `static.interpretation_model:{modelType}:{modelCode}:{modelVersion}`。
5. 如果 `WarmInterpretationModelList != nil` -> `static.interpretation_model_list:published`。
6. 如果 `WarmMBTIModelList != nil` -> `static.mbti_model_list:published`。
...
### 8.2.1 HandleInterpretationModelChanged

解释模型发布、归档或目录变化后，可以触发解释模型列表和具体模型详情预热。
...
修改 EvaluationResult；
修改 InterpretReport；
把完整 TypeProfile 复制进 Redis StaticList；
修复模型规则数据。
```
...
新增 MBTI / BigFive 等解释模型时，推荐治理策略如下：

| 场景 | WarmupTarget | 说明 |
| ---- | ------------ | ---- |
| 模型发布 | `static.interpretation_model:{type}:{code}:{version}` | 预热单个模型详情或 Context |
| 模型归档 | `static.interpretation_model_list:published` | 重建发布态模型列表 |
| MBTI 模型发布 | `static.mbti_model_list:published` | 重建 MBTI 已发布模型列表 |
| 解释模型目录变化 | `static.interpretation_model_list:published` | 重建跨模型目录 |
| 后台模型选择页高频访问 | hotset + manual warmup | 对发布态列表执行受控预热 |

核心原则：

```text
具体模型模块保存规则事实；
Redis 缓存发布态列表和可回源 Context；
Warmup 只让缓存变热，不修业务数据；
规则变化事件不默认触发历史测评重算。
```
...
解释模型列表也可以通过 manual warmup 触发，例如：

```json
{
  "targets": [
    {
      "kind": "static.interpretation_model_list",
      "scope": "published"
    },
    {
      "kind": "static.mbti_model_list",
      "scope": "published"
    }
  ]
}
...
### 19.7 “解释模型变化后应该由 governance 重算历史测评”

错误。Governance 只做缓存预热和状态观察。

解释模型变化后，治理层可以预热模型详情、模型列表和 Context cache；历史 Assessment 是否重算必须由显式 ReEvaluationJob 或业务补偿任务决定。
...
### 20.4.1 解释模型列表 warmup 没执行

检查：

1. 是否注册 `static.interpretation_model_list` 或 `static.mbti_model_list` 的 executor。
2. `WarmInterpretationModelList` / `WarmMBTIModelList` callback 是否绑定。
3. `ListPublishedInterpretationModelRefs` / `ListPublishedMBTIModelRefs` 是否返回数据。
4. 对应 Redis family 是否 allow warmup。
5. 解释模型发布、归档或 `interpretation-model.changed` 事件是否触发了治理入口。
6. run snapshot 中 target 是否 skipped / error。
7. 是否把完整模型规则塞进 StaticList，导致 payload 过大或序列化失败。
...
如果新增的是解释模型相关 `WarmupKind`，还需要额外确认：

```text
ModelType / ModelCode / ModelVersion 是否进入 canonical scope；
具体模型 repository 或 read model 是否提供 published summaries；
StaticList 是否只保存列表摘要；
Context cache 是否可回源；
规则变化事件是否只触发失效和预热，不触发历史重算。
```
...
| 缓存模块入口 | [../cache/README.md](../cache/README.md) |
