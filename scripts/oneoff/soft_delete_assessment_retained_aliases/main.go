// soft_delete_assessment_retained_aliases soft-deletes Assessment rows whose
// evaluation_model_algorithm is a retained-read alias (mbti/sbti/bigfive/
// behavioral_rating_default). Default dry-run.
//
// Also removes evaluation_outcome rows with the same retained algorithms
// (outcome table has no deleted_at).
//
// Destructive. Apply requires --confirm=DELETE_RETAINED_ALIAS_ASSESSMENTS.
package main
...
const confirmToken = "DELETE_RETAINED_ALIAS_ASSESSMENTS"
...
func main() {
	cfg := parseFlags()
	if cfg.mysqlDSN == "" {
		fmt.Fprintln(os.Stderr, "soft_delete assessment retained aliases failed: --mysql-dsn is required (or set MYSQL_DSN)")
		os.Exit(1)
	}
...
func parseFlags() config {
	...
	flag.IntVar(&cfg.limit, "limit", 0, "max Assessment rows to soft-delete (0 = all); Outcomes always all retained-alias rows")
...
func run(ctx context.Context, db *sql.DB, cfg config) (*report, error) {
	...
	aBuckets, aTotal, err := inventory(ctx, db, `
SELECT COALESCE(evaluation_model_kind, ''), COALESCE(evaluation_model_algorithm, ''), COUNT(*)
FROM assessment
WHERE deleted_at IS NULL
  AND evaluation_model_algorithm IN ('mbti','sbti','bigfive','behavioral_rating_default')
GROUP BY evaluation_model_kind, evaluation_model_algorithm`)
	...
	oBuckets, oTotal, err := inventory(ctx, db, `
