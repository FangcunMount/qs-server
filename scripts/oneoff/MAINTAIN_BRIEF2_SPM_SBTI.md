...
检查输出中的每一项变化。脚本会修复普通结果的 Pattern、`HHHH`/`DRUNK` 的特殊 Trigger，并将废弃的 `sbti` outcome/report adapter 迁移为 `personality_type`；dry-run 不写入草稿。

如果此前 Pattern/Trigger 已经保存但 adapter 校验失败，更新脚本后的期望输出是：

```text
Planned changes: patterns=0 special_flags=0 triggers=0 adapters=2 total=2
  DefinitionV2.OutcomeMapping.DetailAdapterKey: "sbti" -> "personality_type"
  DefinitionV2.ReportMap.Sections[0].AdapterKey: "sbti" -> "personality_type"
```
...
`factor_graph...scoring_mode` 的“旧版隐式计分规则”属于兼容 warning，可以保留；`validation error` 必须为零，脚本最终应输出 `Repair saved and server validation passed`。
...
### `outcome detail adapter sbti 已废弃` 或 `report adapter sbti 已废弃`

说明服务器仍在运行旧版修复脚本。更新 `repair_sbti_profiles` 后重新 dry-run；新版只会把这两个 adapter 定点迁移为 `personality_type`，不会改写题目贡献或结果文案。
