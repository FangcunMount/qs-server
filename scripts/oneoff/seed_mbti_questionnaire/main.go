...
//go:embed mbti_questionnaire.json
...
func main() {
	...
	if err := run(cfg); err != nil {
		log.Fatalf("seed mbti questionnaire failed: %v", err)
	}
}
...
func parseFlags() config {
	...
	flag.BoolVar(&cfg.force, "force", false, "replace existing published MBTI_OEJTS snapshot")
	flag.Parse()
	return cfg
}
...
func run(cfg config) error {
	...
	if existing != nil && !cfg.force {
		fmt.Printf("dry-run: MBTI_OEJTS@%s already exists (%d questions), skip\n", seed.Version, existing.QuestionCount())
		if !cfg.apply {
	...
	fmt.Printf("seeded MBTI_OEJTS@%s with %d questions\n", seed.Version, len(seed.Questions))
	return nil
}
