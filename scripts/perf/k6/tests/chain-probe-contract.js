...
import { chainProbePollDelaySeconds, classifyAssessmentReadiness, classifyReportStatus } from '../scenarios/chain-probe.js';
...
export default function () {
  ...
  const reportProcessing = classifyReportStatus({ status: 'processing', next_poll_after_ms: 3000 });
  const reportInterpreted = classifyReportStatus({ status: 'interpreted' });

  check({ cases, ready, pending, noAssessment, failed, backoff, serverHint, cappedHint, reportProcessing, reportInterpreted }, {
    ...
    'processing report status preserves the server polling hint': (state) =>
      !state.reportProcessing.terminal && state.reportProcessing.status === 'processing' && state.reportProcessing.nextPollAfterMS === 3000,
    'interpreted report status is terminal': (state) =>
      state.reportInterpreted.terminal && state.reportInterpreted.status === 'interpreted',
  });
}
