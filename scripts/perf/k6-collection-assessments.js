import http from 'k6/http';
import { check } from 'k6';

// collection-server 我的测评列表压测脚本。
const BASE_URL = __ENV.COLLECTION_BASE_URL || __ENV.BASE_URL || 'http://127.0.0.1:18083';
const TOKEN = __ENV.COLLECTION_TOKEN || __ENV.TOKEN || '';
const TESTEE_ID = __ENV.TESTEE_ID || '';
const STATUS = __ENV.STATUS || ''; // 可选：processing/done 等

const RATE = Number(__ENV.RPS || 100);
const DURATION = __ENV.DURATION || '2m';
const VUS = Number(__ENV.VUS || 60);
const MAX_VUS = Number(__ENV.MAX_VUS || 200);

const DEFAULT_QUERY = {
  testee_id: TESTEE_ID,
  status: STATUS,
  page: 1,
  page_size: 20,
};

export const options = {
  scenarios: {
    steady: {
      executor: 'constant-arrival-rate',
      rate: RATE,
      timeUnit: '1s',
      duration: DURATION,
      preAllocatedVUs: VUS,
      maxVUs: MAX_VUS,
    },
  },
  thresholds: {
    http_req_failed: ['rate<0.01'],
    http_req_duration: ['p(95)<800', 'p(99)<1500'],
  },
};

export default function () {
  const headers = TOKEN ? { Authorization: `Bearer ${TOKEN}` } : {};
  const res = http.get(`${BASE_URL}/api/v1/assessments?${buildQuery(DEFAULT_QUERY)}`, { headers });

  check(res, {
    'status 200': (r) => r.status === 200,
  });
}

function buildQuery(params) {
  return Object.entries(params)
    .filter(([, v]) => v !== undefined && v !== null && v !== '')
    .map(([k, v]) => `${encodeURIComponent(k)}=${encodeURIComponent(v)}`)
    .join('&');
}
