...
| `ceiling-120` | 110 → 恢复门 → 120 QPS，各 2min | 只执行仓库定义的 110/120 档复验；到 120 自动停止，不进入更高档 |
...
80～280 不保存独立 profile。编排器固定链路探针为 1 QPS，其余流量按 300 配比使用最大余数法整数缩放，确保总量精确。VU 按到达率、典型耗时、超时和 headroom 计算，并为 HTTP 到达率场景预留 `startupBuffer` 个冷启动 VU；环境变量仍可显式覆盖。完整 `admission` 中的 80/100/110 三档用于定位 60 到 120 QPS 之间的容量拐点，不允许用跳档方式掩盖首个失败阶段。只有当本次复验已绑定低档证据、exact deployed SHA 和有效环境边界时，才可使用 `ceiling-120`；该计划只执行 110 和 120，且不会继续升到 200 QPS。`ceiling-120` 是版本化负载计划，不代表当前任一硬件或环境的容量结论。
...
终端阶段摘要和 `report.md` 使用相同的三个一级维度，不再把重试拆成第四个维度。终端中的吞吐、时延、正确性与重试使用带边框的定宽表格，操作列左对齐、数值列右对齐；`report.md` 使用对应的 Markdown 表格，便于复制、评审和逐行比较：
