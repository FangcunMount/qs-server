...
| `statistics-stale-run-settlement.yml` | 经负责人明确授权后，仅结算六条固定 ID 的陈旧 Statistics Run；执行前复核 Redis 租约与重建进程，事务内 fail-close 并写后对账；本次维护完成后删除 | `workflow_dispatch` + 固定确认词 |
