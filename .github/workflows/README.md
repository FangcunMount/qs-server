...
| `compatibility-observation.yml` | 只读查询生产 Prometheus，分类 `/practitioners`、Statistics `validate_only` 和 `GenerateReportFromAssessment` 的活跃命中、完整零窗口或历史覆盖不足；零命中仍须调用方确认后才能删除 | 每日 / `workflow_dispatch` |
