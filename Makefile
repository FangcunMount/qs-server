...
docs-rest: docs-swagger ## 从 swagger 生成 api/rest 的 OAS 3.1 摘要
	...
	python scripts/generate_rest_from_swagger.py --swagger internal/apiserver/docs/swagger.json --output api/rest/apiserver.yaml --server http://localhost:18082 --server https://qs.fangcunmount.cn --public-operation GET:/health --public-operation GET:/readyz --public-operation GET:/ping --public-operation GET:/governance/redis --public-operation GET:/governance/cache --public-operation GET:/api/v1/public/info --public-operation GET:/api/v1/public/assessment-entries/{token} --public-operation POST:/api/v1/public/assessment-entries/{token}/intake --public-operation GET:/api/v1/qrcodes/{filename} --public-operation GET:/api/v1/assessment-assets/typology/{model}/{outcome}/{filename}
	python scripts/generate_rest_from_swagger.py --swagger internal/collection-server/docs/swagger.json --output api/rest/collection.yaml --server http://localhost:18083 --server https://collect.fangcunmount.cn --root-security BearerAuth --public-operation GET:/health --public-operation GET:/readyz --public-operation GET:/serve-readyz --public-operation GET:/ping --public-operation GET:/governance/redis --public-operation GET:/governance/cache --public-operation GET:/governance/resilience --public-operation GET:/api/v1/public/info --public-operation GET:/api/v1/assessment-models --public-operation GET:/api/v1/assessment-models/hot --public-operation GET:/api/v1/assessment-models/options --public-operation GET:/api/v1/typology-models --public-operation GET:/api/v1/typology-models/categories

docs-hygiene: ## 检查仓库现行 Markdown 的链接、锚点、章节编号与 docs/ 空白/表格/伪标题格式（排除 docs/_archive）
...
perf-verify: perf-check-k6 ## 校验统一编排器、报告契约与 k6 场景
	...
	PERF_RAW_SUMMARY_FILE="$$summary_file" k6 run --quiet $(PERF_SCRIPT_DIR)/k6/tests/structured-summary.js; \
	jq -e '.metrics.checks.value == 1 and .metrics.checks.passes == 1 and .metrics.checks.thresholds["rate==1"] == false and .state.testRunDurationMs > 0 and .scenarios.structured_summary.executor == "shared-iterations"' "$$summary_file" >/dev/null
	@summary_file=$$(mktemp); \
	trap 'rm -f "$$summary_file"' EXIT; \
	PERF_RAW_SUMMARY_FILE="$$summary_file" k6 run --quiet $(PERF_SCRIPT_DIR)/k6/tests/report-ws-model-metrics.js; \
	jq -e '.metrics.medical_report_ws_subscribe_to_first_message_latency["p(95)"] == 101 and .metrics.behavior_report_ws_subscribe_to_first_message_latency["p(95)"] == 102 and .metrics.personality_report_ws_subscribe_to_first_message_latency["p(95)"] == 103' "$$summary_file" >/dev/null
	...
	k6 run --quiet -e QUERY_RPS=0 -e SUBMIT_RPS=0 -e REPORT_RPS=0 -e STATS_RPS=0 -e CHAIN_PROBE_RPS=0 $(PERF_SCRIPT_DIR)/k6/tests/report-ws-allocation-and-failure.js
	k6 run --quiet -e QUERY_RPS=0 -e SUBMIT_RPS=0 -e REPORT_RPS=0 -e STATS_RPS=0 -e CHAIN_PROBE_RPS=0 $(PERF_SCRIPT_DIR)/k6/tests/chain-probe-contract.js
	...
	k6 inspect -e PERF_CONFIG_FILE="$(CURDIR)/$(PERF_SCRIPT_DIR)/qs-perf.config.example.json" -e PERF_ROOT_DIR="$(CURDIR)" -e QPS_PROFILE=experience_60 $(PERF_K6_SCRIPT) | jq -e '.thresholds.medical_model_query_duration == ["p(95)<300", "p(99)<500"] and .thresholds.medical_answer_submit_duration == ["p(95)<300", "p(99)<800"] and .thresholds.report_ws_first_message_latency == ["p(95)<300", "p(99)<800"] and .scenarios.medical_model_query.preAllocatedVUs == 9 and .scenarios.personality_model_query.preAllocatedVUs == 5 and .scenarios.personality_questionnaire_query.preAllocatedVUs == 5' >/dev/null
	k6 inspect -e PERF_CONFIG_FILE="$(CURDIR)/$(PERF_SCRIPT_DIR)/qs-perf.config.example.json" -e PERF_ROOT_DIR="$(CURDIR)" -e QPS_PROFILE=admission_300 $(PERF_K6_SCRIPT) | jq -e '.scenarios.behavior_report_ws_query.rate == 10 and .thresholds.medical_model_query_duration == ["p(95)<500", "p(99)<1200"] and .thresholds.statistics_overview_duration == ["p(95)<1000", "p(99)<2000"] and .thresholds.http_timeout_rate == ["rate<0.001"] and .thresholds.dropped_iterations == ["count==0"] and .thresholds["chain_probe_timeout{stage:assessment_readiness}"] == ["count==0"] and .thresholds["chain_probe_failed{reason:assessment_no_assessment_required}"] == ["count==0"]' >/dev/null
...
cd-validate: ## 校验 CD 服务元数据和脚本入口 (SERVICE=apiserver|collection|worker)
	...
	@test -x "$(CD_SCRIPT_DIR)/wait-worker-readiness.sh"
	@test -x "$(CD_SCRIPT_DIR)/verify-worker-dependencies.sh"
	@test -x "$(CD_SCRIPT_DIR)/verify-collection-nginx.sh"
	@test -x "$(CD_SCRIPT_DIR)/verify-observability-nginx.sh"
	@test -x "$(CD_SCRIPT_DIR)/verify-worker-governance.sh"
	@"$(CD_SCRIPT_DIR)/verify-collection-nginx.sh" --verify-config-policy configs/nginx/conf.d/collect.fangcunmount.cn.conf
